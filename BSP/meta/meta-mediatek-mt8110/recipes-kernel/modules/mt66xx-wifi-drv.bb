...
inherit workonsrc
WORKONSRC = "${MT66xx_SRC}/wlan_driver/gen4m_mt66xx"
...
do_compile() {
	if test "${COMBO_CHIP_ID}" = "mt6631"; then
		...
		cd ${S} && make -f Makefile.ce TOPDIR=${TOPDIR} MODULE_NAME=${MODULE_NAME} DRIVER_DIR=${S} LINUX_SRC=${LINUX_SRC} PACKAGE_ARCH=${PACKAGE_ARCH} TARGET_OS=${TARGET_OS} TARGET_VENDOR=${TARGET_VENDOR} TARGET_PLATFORM=${TARGET_PLATFORM} ARCH=arm64 CROSS_COMPILE=aarch64-poky-linux-
		else
		cd ${S} && make -f Makefile.ce TOPDIR=${TOPDIR} MODULE_NAME=${MODULE_NAME} DRIVER_DIR=${S} LINUX_SRC=${LINUX_SRC} PACKAGE_ARCH=${PACKAGE_ARCH} TARGET_OS=${TARGET_OS} TARGET_VENDOR=${TARGET_VENDOR} TARGET_PLATFORM=${TARGET_PLATFORM} ARCH=arm CROSS_COMPILE=arm-poky-linux-gnueabi-
		fi
