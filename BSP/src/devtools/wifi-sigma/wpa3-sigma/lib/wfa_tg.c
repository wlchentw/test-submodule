...
int wfaSendBitrateDataForVoE(int mySockfd, int streamId, BYTE *pRespBuf, int *pRespLen)
{
    tgProfile_t           *theProf = findTGProfile(streamId);
    tgStream_t            *myStream = findStreamProfile(streamId);
    int                   ret = WFA_SUCCESS;
    ...
    //int throttledRate = 0;
    struct timeval        before, after, stime, laststime;
    int usecPerPkt = 0;
    int usecUsedPerPkt = 0;

    DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE entering\n");
    /* error check section  */
    if ((mySockfd <= 0) || (streamId < 0) || (pRespBuf == NULL)
            || (pRespLen == NULL))
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE pass-in parameter err mySockfd=%i streamId=%i pRespBuf=0x%x pRespLen=0x%x\n",
                    mySockfd, streamId, pRespBuf, pRespLen);
        ret = WFA_FAILURE;
        goto errcleanup;
    }
    ...
    if (theProf == NULL || myStream == NULL)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE parameter err in NULL pt theProf=%l myStream=%l\n",
                    theProf, myStream);
        ret = WFA_FAILURE;
        goto errcleanup;
    }
    if (theProf->rate == 0 || theProf->duration == 0)
    {  /*  rate == 0 means unlimited to push data out which is not this routine to do */
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE usage error, bitrate=%i or duration=%i\n",
                    theProf->rate, theProf->duration);
        ret = WFA_FAILURE;
        goto errcleanup;
    }
    ...
    if ((theProf->rate > 3500) || (theProf->pksize > 1000))
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE Warning, must set total streams rate<=10000 and payload<=1000; stream bitrate may not meet\n");
    }

    /* calculate bitrate asked */
    if ((rate = theProf->pksize * theProf->rate * 8) > WFA_SEND_FIX_BITRATE_MAX)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE over birate can do in the routine, req bitrate=%l\n", rate);
        ret = WFA_FAILURE;
        goto errcleanup;
    }
    /* alloc sending buff  */
    ...
    if (packBuf == NULL)
    {
        DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE malloc err\n");
        ret = WFA_FAILURE;
        goto errcleanup;
    }
    memset(packBuf, 0, packLen + 4);
    ...
    usecPerPkt = 1000000 / theProf->rate;
    runLoop = 1; /* global defined share with thread routine, should remove it later  */
    while (runLoop)
    {
        iSleep = 1;
        ...
        {
           counter++;
           DPRINT_INFO(WFA_OUT, "counter %d, duration %d\n", counter, nDuration);
           //iSleep++;
           /* fill in the counter */
           int2BuffBigEndian(counter, &((tgHeader_t *)packBuf)->hdr[8]);
           /*
           ...
           wGETTIMEOFDAY(&stime, NULL);
           if (i == 0) {
               laststime.tv_sec = stime.tv_sec;
               laststime.tv_usec = stime.tv_usec;
           }
           if (stime.tv_sec == laststime.tv_sec)
               usecUsedPerPkt = stime.tv_usec - laststime.tv_usec;
           else
               usecUsedPerPkt = 1000000 + stime.tv_usec - laststime.tv_usec;

           usecUsedPerPkt = usecPerPkt - usecUsedPerPkt;
           if (usecUsedPerPkt > 0) {
               usleep(usecUsedPerPkt);
               stime.tv_usec += usecUsedPerPkt;
           ...
           else
           {
               ...
               sleepTotal = sleepTotal + (unsigned long long int) 1;
               DPRINT_INFO(WFA_OUT, "wfaSendBitrateDataForVoE wfaTrafficSendTo call ERR %d counter=%i i=%i; send busy, sleep %i MilSec then send\n", errno, counter, i, sleepTimePerSent);
               i--;
           }
        }// for loop per second sending
        //iSleep = 0;
    ...
    *pRespLen = WFA_TLV_HDR_LEN + sizeof(dutCmdResponse_t);
    ...
    extraTimeSpendOnSending = extraTimeSpendOnSending / 1000;
    DPRINT_INFO(WFA_OUT, "*** wfg_tg.cpp wfaSendBitrateDataForVoE Count=%i txFrames=%i totalByteSent=%i sleepTotal=%llu milSec extraTimeSpendOnSending=%llu nOverTimeCount=%d nOverSend=%i rate=%d nDuration=%d iSleep=%d ***\n",
        counter, (myStream->stats.txFrames), (unsigned int)(myStream->stats.txPayloadBytes), sleepTotal, extraTimeSpendOnSending, nOverTimeCount, nOverSend, theProf->rate, nDuration, iSleep);
    wfaSleepMilsec(1000);
    return ret;
