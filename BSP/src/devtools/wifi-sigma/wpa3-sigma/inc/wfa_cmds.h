...
typedef struct ca_sta_set_security
{
    ...
    char encpType[9];
    int  pmf;
    char intf[16];               /* WFA_IF_NAME_LEN */
    char certType[16];           /* WFA_CERT_TYPE_LEN */
    char ecGroupID[16];          /* WFA_EC_GROUP_LEN */
    char pairwiseCipher[32];     /* WFA_PARIWISE_CIPHER_LEN */
    char groupCipher[32];        /* WFA_PARIWISE_CIPHER_LEN */
    char groupMgmtCipher[32];    /* WFA_PARIWISE_CIPHER_LEN */
    char TLSCipher[64];          /* WFA_TLS_CIPHER_LEN */
    char SAEInvalidElement[64];  /* WFA_SAE_INVALID_ELEMENT_LEN */
    union _security
    {
...
typedef struct ca_ap_set_11n_wireless
{
	...
	int sgi20;/*enable, disable*/
} caApSet11nWireless_t;

typedef struct ca_ap_set_pmf
{
	char dutName[WFA_IF_NAME_LEN];
	int pmf;/*disalbed, optional, required*/
} caApSetPMF_t;
...
typedef struct ca_ap_set_wireless
{
   ...
   int fragement;
   int beaconInterval;
   int ampdu;/*enable, disable*/
   int amsdu;/*enable, disable*/
   int offset;/*above, below*/
   int addbaReject;/*enable, disable*/
   char spatialTxStream[WFA_SPATIAL_TX_STREAM_LEN];
   char spatialRxStream[WFA_SPATIAL_RX_STREAM_LEN];
   int greenfield;
   int mcs32;/*enable, disable*/
   int mcsMandatory;
   int mcsSupported;
   int stbcTx;
   int dtim;/*1, 2, 3 ...*/
...
typedef struct ca_ap_set_11d
{
   char dutName[WFA_IF_NAME_LEN];
   char ssid[WFA_SSID_NAME_LEN];
   int regulatorMode;/*11d*/
   int CountryCode;
} caApSet11d_t;

typedef struct ca_ap_set_11h
{
   char dutName[WFA_IF_NAME_LEN];
   char ssid[WFA_SSID_NAME_LEN];
   int regulatorMode;/*11h*/
   int dfsMode;/*enable/disable*/
   int dfsChannel;
} caApSet11h_t;

typedef struct ca_ap_commit
{
   char dutName[WFA_IF_NAME_LEN];
   int commit;
} caApCommit_t;
...
typedef struct ca_ap_set_apqos
{
    char dutName[WFA_IF_NAME_LEN];
    int cwMin[eApAcNum];
    int cwMax[eApAcNum];
...
typedef struct ca_ap_send_addba_req
{
    char dutName[WFA_IF_NAME_LEN];
    int tid;
    char staMacAddr[WFA_MAC_ADDR_STR_LEN];
} caApSendAddbaReq_t;
#endif
...
typedef struct mbo_non_pref_channel
{
    BYTE chPrefNum;
    BYTE chPref;
    BYTE chOpClass;
    BYTE chReasonCode;
} mbo_non_pref_channel_t;
...
#ifdef MTK_WFD_SIGMA
#define MTK_DTV_QUICK_CONN_CHECK_FILENAME   "/tmp/mtksigma_quitconn"
#endif /* MTK_WFD_SIGMA */
