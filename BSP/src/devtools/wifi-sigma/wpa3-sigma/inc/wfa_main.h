...
#define WFA_BUF_STR_SZ      512

#define MTK_IWPRIV_CMD(iw_cmd, ... )                                    \
    sprintf(gCmdStr, "iwpriv "iw_cmd, ##__VA_ARGS__);               \
    puts("\n\t");                                                   \
    puts(gCmdStr);                                                  \
    if (system(gCmdStr)) printf("\t\tSYSYTEM MTK_IW_CMD FAIL: \n\t\t\t%s \n", gCmdStr);

#define WFA_KEY_MGMT_LEN            16
#define WFA_CERT_TYPE_LEN           16
#define WFA_EC_GROUP_LEN            16
#define WFA_PARIWISE_CIPHER_LEN     32
#define WFA_TLS_CIPHER_LEN          64
#define WFA_SAE_INVALID_ELEMENT_LEN 64
