...
#ifdef MTK_WFD_SIGMA
extern int wfaStaCheckWfdConnection(unsigned char bAutoGO);
extern int wfdConnPeerIsInitiator;
extern int wfdConnIamAutoGo;
extern int wfdReinvokePeerIsInitiator;
#endif
            ...
            else
            {
               ...
               if(gxcSockfd != -1)
               {
                 if((ret = wfaCtrlSend(gxcSockfd, (BYTE *)respBuf, respLen)) != respLen)
                 {
                    ...
                    fprintf(stderr, "wfaCtrlSend success!\n");
                
                if ((xcCmdTag == WFA_STA_START_WFD_CONNECTION_TLV && wfdConnPeerIsInitiator) ||
					(xcCmdTag == WFA_STA_WPS_READ_PIN_TLV && wfdConnIamAutoGo) ||
					(xcCmdTag == WFA_STA_WPS_ENTER_PIN_TLV && wfdConnIamAutoGo) ||
					(xcCmdTag == WFA_STA_WPS_SETWPS_PBC_TLV && wfdConnIamAutoGo))
                {
                	if (xcCmdTag == WFA_STA_START_WFD_CONNECTION_TLV)
	                {
	                	printf("[SIGMA] Why DHCP will be called twice, Never tell you......\n");
	                //wfaStaCheckWfdConnection(0);
                	}
					else
					{
						printf("[SIGMA] For Sigma Owner:If you found No P2P is working ,because this API is marked \n");
						//wfaStaCheckWfdConnection(1);
					}
                }
                else if (xcCmdTag == WFA_STA_REINVOKE_WFD_SESSION_TLV && wfdReinvokePeerIsInitiator)
                {
                    printf("[SIGMA]  REINVOKE and ACCEPT case we should start WFD here! \n");
                    wfaStaCheckWfdConnection(0);
                }
#endif /* MTK_WFD_SIGMA */
