				     ...
				     unsigned int *pu4CheckSum)
{
	...
	DIGIT_T *y_2048 = malloc(64 * sizeof(DIGIT_T));
	DIGIT_T *t1_4096 = malloc(128 * sizeof(DIGIT_T));
	DIGIT_T *t2_4096 = malloc(128 * sizeof(DIGIT_T));
	DIGIT_T *t3_4096 = malloc(128 * sizeof(DIGIT_T));
	int i = 0;
...
unsigned char *rsa_encryptdecrypt(const unsigned char *sig, const unsigned char *e, const unsigned char *n)
{
	...
	unsigned char excepted_e[E_LEN] = {0x00, 0x01, 0x00, 0x01};

	if (memcmp(&e[RSA_LEN-E_LEN], excepted_e, E_LEN))	{
		printf("e value check fail,only support e value as 65537!\n");
		return NULL;
	}

	out_t = malloc(RSA_LEN);
	sig_t = malloc(RSA_LEN);
	out = malloc(RSA_LEN);
	key_tmp = malloc(RSA_LEN);

	for (i = 0; i < RSA_LEN; i++)
		sig_t[i] = sig[RSA_LEN - 1 - i];
	for (i = 0; i < RSA_LEN; i++)
		key_tmp[i] = n[RSA_LEN - 1 - i];
	RSADecryption65537((unsigned int *) sig_t, (unsigned int *) key_tmp, (unsigned int *) out_t);
	for (i = 0; i < RSA_LEN; i++)
