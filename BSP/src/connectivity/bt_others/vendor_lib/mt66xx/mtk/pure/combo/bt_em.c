...
typedef int (*RELAY)(bool enable);

INIT    bt_init = NULL;
DEINIT  bt_restore = NULL;
WRITE   bt_send_data = NULL;
READ    bt_receive_data = NULL;
RELAY   bt_set_relay = NULL;
/**************************************************************************
 *                          F U N C T I O N S                             *
***************************************************************************/

EXPORT_SYMBOL BOOL EM_BT_init(void)
{
    const char *errstr;
    ...
    bt_set_relay = dlsym(glib_handle, "bt_set_relay");
...
EXPORT_SYMBOL void EM_BT_deinit(void)
{
    TRC();
...
EXPORT_SYMBOL BOOL EM_BT_write(unsigned char *buf, unsigned int len)
{
    UINT32 i;
...
EXPORT_SYMBOL BOOL EM_BT_read(unsigned char *buf, unsigned int len, unsigned int *pu4ResultLen)
{
    UINT8 ucHeader = 0;
...
EXPORT_SYMBOL int EM_BT_set_relay(bool enable)
{
    return bt_set_relay(enable);
}

static void thread_exit(int signo)
{
#if BT_EM_DEBUG /* for unused warning */
...
EXPORT_SYMBOL void EM_BT_polling_start(void)
{
...
EXPORT_SYMBOL void EM_BT_polling_stop(void)
{
...
EXPORT_SYMBOL void EM_BT_getChipInfo(BT_CHIP_ID *chip_id, BT_HW_ECO *eco_num)
{
    return;
}

EXPORT_SYMBOL void EM_BT_getPatchInfo(char *patch_id, unsigned int *patch_len)
{
    return;
}
