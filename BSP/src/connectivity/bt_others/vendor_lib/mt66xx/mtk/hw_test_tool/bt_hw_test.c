...
EXPORT_SYMBOL BOOL HW_TEST_BT_init(void)
{
    const char *errstr;
...
EXPORT_SYMBOL void HW_TEST_BT_deinit(void)
{
    TRC();
...
EXPORT_SYMBOL BOOL HW_TEST_BT_reset(void)
{
...
EXPORT_SYMBOL BOOL HW_TEST_BT_TxOnlyTest_start(
    UINT8 tx_pattern,
    UINT8 hopping,
    int channel,
    UINT8 packet_type,
...
EXPORT_SYMBOL BOOL HW_TEST_BT_TxOnlyTest_end(void)
{
    //return HW_TEST_BT_reset();
    return TRUE;
}

EXPORT_SYMBOL BOOL HW_TEST_BT_NonSignalRx_start(
    UINT8 rx_pattern,
    int channel,
    UINT8 packet_type,
    UINT32 tester_addr
...
EXPORT_SYMBOL BOOL HW_TEST_BT_NonSignalRx_end(
...
EXPORT_SYMBOL BOOL HW_TEST_BT_TestMode_enter(int power)
{
...
EXPORT_SYMBOL BOOL HW_TEST_BT_TestMode_exit(void)
{
    fgKillThread = TRUE;
...
static BOOL BT_Inquiry(void)
{
    ...
    if (bt_send_data(bt_fd, INQ_MODE, sizeof(INQ_MODE)) < 0) {
        ERR("Send inquiry mode command fails errno %d\n", errno);
        return FALSE;
    }

    DBG("write:\n");
    for (i = 0; i < sizeof(INQ_MODE); i++) {
        DBG("%02x\n", INQ_MODE[i]);
    }
...
EXPORT_SYMBOL BOOL HW_TEST_BT_Inquiry(FUNC_CB info_cb)
{
    if (BT_Inquiry() == TRUE) {
        /*
...
EXPORT_SYMBOL BOOL HW_TEST_BT_LE_Tx_start(UINT8 tx_pattern, int channel)
{
...
EXPORT_SYMBOL BOOL HW_TEST_BT_LE_Tx_end(void)
{
...
EXPORT_SYMBOL BOOL HW_TEST_BT_LE_Rx_start(int channel)
{
...
EXPORT_SYMBOL BOOL HW_TEST_BT_LE_Rx_end(UINT16 *pu2RxPktCount)
{
    UINT8 HCI_LE_TEST_END[] = {0x01, 0x1F, 0x20, 0x00};
    UINT8 ucAckEvent[9];
...
EXPORT_SYMBOL BOOL HW_TEST_TX_POWER_OFFSET_op(int wr_flag, char *group_offset, char *group_flag)
{
    const char *errstr;
    ...
    bt_op_tx_power_offset = dlsym(glib_handle, "bt_op_tx_power_offset");
    ...
    bt_op_tx_power_offset(wr_flag, group_offset, group_flag);


    DBG("set/get tx power offset success\n");
