...
static VOID bt_app_spp_cbk_wrapper(BT_SPP_CBK_STRUCT *pt_spp_struct, VOID *pv_tag)
{
    RPC_DECL_VOID(2);
    RPC_CB_NFY_TAG_T  *pt_nfy_tag = (RPC_CB_NFY_TAG_T*)pv_tag;
    RPC_CHECK(bt_rpc_add_ref_desc(RPC_DEFAULT_ID, pt_spp_struct,
        RPC_DESC_BT_SPP_CBK_STRUCT, NULL));
    RPC_ARG_IO(ARG_TYPE_REF_DESC, pt_spp_struct);
    RPC_ARG_INP(ARG_TYPE_REF_VOID, pt_nfy_tag->pv_tag);

    RPC_DO_CB(pt_nfy_tag->t_id, "bt_app_spp_cbk",
                         ...
                         ARG_DESC_T*  pt_return)
{
    ...
    {
        apv_cb_addr[0] = (VOID*)p_bt_spp_app_cb_func;
        pt_nfy_tag  = ri_create_cb_tag(t_rpc_id, apv_cb_addr, 1,
            pt_args[1].u.pv_arg);

        BT_RH_LOG("pv_func=%p, pv_arg=%p, pt_nfy_tag=%p",
            p_bt_spp_app_cb_func, pt_args[1].u.pv_arg, pt_nfy_tag);

        pt_return->e_type   = ARG_TYPE_INT32;
        pt_return->u.i4_arg = x_mtkapi_spp_register_callback(bt_app_spp_cbk_wrapper, pt_nfy_tag);
    ...
    {
        pt_return->u.i4_arg = x_mtkapi_spp_register_callback(NULL, NULL);
    }
    return RPCR_OK;
                         ...
                         ARG_DESC_T*  pt_return)
{
    ...
    BT_RH_LOG("[_hndlr_]spp_send_data, arg_1 = %s, arg_2 = %s,\n\n arg_4=%d",
        pt_args[0].u.ps_str, pt_args[1].u.ps_str, pt_args[3].u.i4_arg);

    for (j=0; j < pt_args[3].u.i4_arg; j++)
    {
        BT_RH_LOG("[_hndlr_]spp_send_data, str[%d]=0x%02x", j, pt_args[2].u.pc_arg[j]);
    }
    ...
    pt_return->e_type   = ARG_TYPE_INT32;
    pt_return->u.i4_arg = x_mtkapi_spp_send_data(pt_args[0].u.ps_str, pt_args[1].u.ps_str, pt_args[2].u.pc_arg, pt_args[3].u.i4_arg);
                         ...
                         ARG_DESC_T*  pt_return)
{
    ...
    pt_return->e_type   = ARG_TYPE_INT32;
    pt_return->u.i4_arg = x_mtkapi_spp_start_server(pt_args[0].u.ps_str, pt_args[1].u.ps_str);
                         ...
                         ARG_DESC_T*  pt_return)
{
    ...
    pt_return->e_type   = ARG_TYPE_INT32;
    pt_return->u.i4_arg = x_mtkapi_spp_stop_server(pt_args[0].u.ps_str);
