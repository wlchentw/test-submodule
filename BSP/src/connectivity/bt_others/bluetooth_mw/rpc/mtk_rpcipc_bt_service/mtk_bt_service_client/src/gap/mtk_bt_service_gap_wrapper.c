    ...
    ARG_DESC_T*  pt_return)
{
    ...
    ((mtkrpcapi_BtAppGapInquiryResponseCbk)pv_cb_addr)((tBTMW_GAP_DEVICE_INFO *)pt_args[0].u.pv_desc, pt_args[1].u.pv_arg);
    return RPCR_OK;
}
    ...
    ARG_DESC_T*  pt_return)
{
    ...
    ((mtkrpcapi_BtAppGapEventCbk)pv_cb_addr)((tBTMW_GAP_STATE*)pt_args[0].u.pv_desc, pt_args[1].u.pv_arg);
    return RPCR_OK;
}
...
EXPORT_SYMBOL INT32 a_mtkapi_bt_bluetooth_factory_reset(VOID)
{
...
EXPORT_SYMBOL INT32 a_mtkapi_bt_mw_log_setStackLevel(const char *trc_name, int level)
{
    //Adding a invalid value for passing IPC/RPC, no other use
    RPC_CLIENT_DECL(2, INT32);
    RPC_CLIENT_ARG_INP(ARG_TYPE_REF_STR, trc_name);
    RPC_CLIENT_ARG_INP(ARG_TYPE_INT32, level);
    RPC_BT_SERVICE_CLIENT_DO_OP("x_mtkapi_bt_mw_log_setStackLevel");
    RPC_CLIENT_RETURN(ARG_TYPE_INT32, -1);
}
...
EXPORT_SYMBOL INT32 a_mtkapi_bt_gap_get_dev_info(BLUETOOTH_DEVICE* dev_info, CHAR* bd_addr)
{
    RPC_CLIENT_DECL(2, INT32);
    RPC_CLIENT_CHECK(bt_rpc_add_ref_desc(RPC_DEFAULT_ID,
                    dev_info,
...
EXPORT_SYMBOL INT32 a_mtkapi_bt_gap_start_inquiry_scan(UINT32 ui4_filter_type)
{
    RPC_CLIENT_DECL(1, INT32);
...
/* FUNCTION NAME: a_mtkapi_bt_gap_set_lhdc_key_data
 * PURPOSE:
 *      it's used to set the HWA/LHDC license key and license name
 * INPUT:
 *      name  -- the name for the license key, max length: 25(not include '\0'
 *      data  -- license key, max length: 128
 *      data_len -- the license key length.
 * OUTPUT:
 *      None
 * RETURN:
 *      BT_SUCCESS -- set success.
 *      others -- set fail.
 * NOTES:
 *      this function should be called after base init.
 */
EXPORT_SYMBOL INT32 a_mtkapi_bt_gap_set_lhdc_key_data(CHAR *name, UINT8 *data, INT32 data_len)
{
    RPC_CLIENT_DECL(3, INT32);
    RPC_CLIENT_ARG_INP(ARG_TYPE_REF_STR, name);
    RPC_CLIENT_CHECK(bt_rpc_add_ref_buff(RPC_DEFAULT_ID, data, data_len));
    RPC_CLIENT_ARG_INP(ARG_TYPE_REF_DESC, data);
    RPC_CLIENT_ARG_INP(ARG_TYPE_INT32, data_len);
    RPC_BT_SERVICE_CLIENT_DO_OP("x_mtkapi_bt_gap_set_lhdc_key_data");
    RPC_CLIENT_RETURN(ARG_TYPE_INT32, -1);
}


/* FUNCTION NAME: a_mtkapi_bt_gap_get_bond_state
 * PURPOSE:
 *      get the specified device bond state.
 * INPUT:
 *      bd_addr -- checked device address
 * OUTPUT:
 *      N/A
 * RETURN:
 *      BT_SUCCESS         -- is bonded
 *      BT_ERR_STATUS_FAIL -- is not bonded device.
 *      BT_ERR_STATUS_BUSY -- it's pairing or connecting.
 *      BT_ERR_STATUS_NOT_READY -- bluetooth is power off.
 * NOTES:
 *      N/A
 */
EXPORT_SYMBOL INT32 a_mtkapi_bt_gap_get_bond_state(CHAR* bd_addr)
{
    RPC_CLIENT_DECL(1, INT32);
    RPC_CLIENT_ARG_INP(ARG_TYPE_REF_STR, bd_addr);
