...
/* For Android, move all definition and type define structure to /hardward/bt_mesh.h
...
#define MESHIF_NET_KEY_RECORD_NUMBER              10  /**< The maximum number of network keys stored in flash.*/
...
#define MESHIF_MODEL_PUBLICATION_RECORD_NUMBER    10  /**< The maximum number of model publication records stored in flash.*/
#define MESHIF_MODEL_SUBSCRIPTION_RECORD_NUMBER   10  /**< The maximum number of model subscription records stored in flash.*/
...
typedef struct {
    ...
    const uint8_t *virtual_uuid;    /**< virtual uuid value, must be NULL unless address type is #MESHIF_ADDRESS_TYPE_VIRTUAL */
} meshif_address_t;
...
typedef struct {
    ...
    const meshif_access_opcode_handler_t *opcode_handlers;    /**< The access message handler for this model. */
...
typedef struct {
    uint16_t                          *model_handle;
    const meshif_model_add_params_t *model_params;
