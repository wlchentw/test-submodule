...
static unsigned char g_btmw_rpc_test_log_lv = (BTMW_RPC_TEST_LOG_LV_VBS | BTMW_RPC_TEST_LOG_LV_INF | BTMW_RPC_TEST_LOG_LV_DBG | BTMW_RPC_TEST_LOG_LV_WRN | BTMW_RPC_TEST_LOG_LV_ERR);

// Default enable log color
static unsigned char g_btmw_rpc_test_log_color = 0;

// Default disable time stamp in log
static unsigned char g_btmw_rpc_test_log_timestamp = 0;

// Default tag
static char *g_btmw_rpc_test_log_tag = "btcli";
...
static void BTMW_RPC_TEST_Log_Impl(unsigned char lv, const char *fmt, va_list args, unsigned char color)
{
    ...
    openlog(g_btmw_rpc_test_log_tag, 0, 0);
    va_copy(tmp_args, args);
    vsyslog(LOG_DEBUG, fmt, tmp_args);
    closelog();

    if ((lv & g_btmw_rpc_test_log_lv))
    {
        buf_ptr = fmt_buf;
        buf_remain = FMT_BUF_SIZE - 1;

        if (g_btmw_rpc_test_log_timestamp)
        {
            _get_timestamp(&ts);
        ...
        if (g_btmw_rpc_test_log_color)
        {
            // Apply color prefix.
            switch (lv)
            {
                case BTMW_RPC_TEST_LOG_LV_ERR:
                    snprintf(buf_ptr, buf_remain, "%s", CAM_LIGHT_RED);
                    break;
                case BTMW_RPC_TEST_LOG_LV_WRN:
                    snprintf(buf_ptr, buf_remain, "%s", CAM_YELLOW);
                    break;
                case BTMW_RPC_TEST_LOG_LV_DBG:
                    snprintf(buf_ptr, buf_remain, "%s", CAM_LIGHT_GREEN);
                    break;
                case BTMW_RPC_TEST_LOG_LV_INF:
...
void BTMW_RPC_TEST_Log(unsigned char lv, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    BTMW_RPC_TEST_Log_Impl(lv, fmt, args, 1);
    va_end(args);
}

void BTMW_RPC_TEST_Log_SetFlag(unsigned short flags)
{
    g_btmw_rpc_test_log_lv = flags & BTMW_RPC_TEST_LOG_MASK;

    g_btmw_rpc_test_log_color = (flags & BTMW_RPC_TEST_LOG_FLAG_COLOR) ? 1 : 0;
    g_btmw_rpc_test_log_timestamp= (flags & BTMW_RPC_TEST_LOG_FLAG_TIMESTAMP) ? 1 : 0;
}
