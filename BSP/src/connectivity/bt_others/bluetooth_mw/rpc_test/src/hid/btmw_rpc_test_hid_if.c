...
#define BTMW_RPC_HID_CONNECTED 1
#define BTMW_RPC_HID_DISCONNECTED 0

#define BTMW_RPC_HID_HOST_MAX_DEVICES 7

BTMW_HID_CONNECTED_DEVICE hid_connected_devices[BTMW_RPC_HID_HOST_MAX_DEVICES];
int hid_connected_devices_num = 0;
...
static int btmw_rpc_test_hid_get_connection_status(int argc, char *argv[])
{
    BTMW_RPC_TEST_Logi("[HID] %s()\n", __func__);
    CHAR ps_addr[MAX_BDADDR_LEN];
    int connected = BTMW_RPC_HID_DISCONNECTED;  //connected=1; disconnected=0

    if (argc != 1)
    {
        BTMW_RPC_TEST_Loge("[HID] Usage : get_connection_status ([addr])\n", __func__);
        return -1;
    }
    ...
    INT32 i4_ret = a_mtkapi_bt_gap_get_bond_state(ps_addr);
    if (i4_ret == BT_SUCCESS)
    {
        for (int i=0; i<BTMW_RPC_HID_HOST_MAX_DEVICES; i++)
        {
            if (!strcmp(ps_addr, hid_connected_devices[i].addr))
            {
                connected = hid_connected_devices[i].connected;
                break;
            }
        }
    }
    BTMW_RPC_TEST_Logd("get hid connection status:!\n");
    BTMW_RPC_TEST_Logi("bdAddr:      %s\n", ps_addr);
    BTMW_RPC_TEST_Logd("bonded:      %s\n", i4_ret == BT_SUCCESS?"yes":"no");
    BTMW_RPC_TEST_Logi("connected:   %s\n", connected == BTMW_RPC_HID_CONNECTED?"yes":"no");
    return connected;
}
...
static VOID btmw_rpc_test_hidh_app_cbk(BT_HIDH_CBK_STRUCT *param, VOID *pv_tag)
{
    ...
    switch (param->event)
    {
        case BT_HIDH_CONNECTED:
            if (hid_connected_devices_num < BTMW_RPC_HID_HOST_MAX_DEVICES) {
                strncpy(hid_connected_devices[hid_connected_devices_num].addr, param->addr, MAX_BDADDR_LEN);
                hid_connected_devices[hid_connected_devices_num].connected = BTMW_RPC_HID_CONNECTED;
                hid_connected_devices_num++;
            }
            break;
        case BT_HIDH_DISCONNECTED:
            for (int i=0; i<BTMW_RPC_HID_HOST_MAX_DEVICES; i++)
            {
                if (!strcmp(param->addr, hid_connected_devices[i].addr))
                {
                    hid_connected_devices[i].connected = BTMW_RPC_HID_DISCONNECTED;
                    break;
                }
            }
        default:
