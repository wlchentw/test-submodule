...
static BTMW_RPC_TEST_CLI btmw_rpc_test_tools_cli_commands[] =
{
    ...
    {"shell", shell_cmd_handler, "shell cmd handled by rpc test"},
    { NULL, NULL, NULL }
};
...
static int shell_cmd_handler(int argc, char *argv[])
{
    ...
    if (argc < 1)
    {
        BTMW_RPC_TEST_Logd("[Tools] shell_cmd_handler, no cmd\n");
        return 0;
    }
    ...
    for (int i = 0; i < argc; i++)
    {
        if (i == argc - 1)
        {
            snprintf(cmd+index, 128 - index, "%s", argv[i]);
            index += strlen(argv[i]);
        }
        else
        {
    ...
    BTMW_RPC_TEST_Logd("[Tools] shell_cmd_handler, cmd: %s\n", cmd);
