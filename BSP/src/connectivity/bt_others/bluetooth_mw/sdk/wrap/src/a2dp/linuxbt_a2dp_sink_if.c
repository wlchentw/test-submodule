...
INT32 linuxbt_a2dp_sink_active_src(CHAR *pbt_addr, BOOL active)
{
    INT32 status = BT_SUCCESS;
    bt_bdaddr_t bdaddr;
    BT_CHECK_POINTER(BT_DEBUG_A2DP, pbt_addr);
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
    if ((g_bt_a2dp_sink_ext_interface == NULL))
    {
        BT_DBG_ERROR(BT_DEBUG_A2DP, "[A2DP] Failed to get A2DP sink ex interface");
        return BT_ERR_STATUS_NOT_READY;
    }
    ...
    linuxbt_btaddr_stoh(pbt_addr, &bdaddr);
    BT_DBG_NORMAL(BT_DEBUG_A2DP, "A2DP active_src to %s, %d",pbt_addr, active);

    if (g_bt_a2dp_sink_ext_interface && g_bt_a2dp_sink_ext_interface->active_src)
    {
        g_bt_a2dp_sink_ext_interface->active_src(&bdaddr, active);
    }
    else
    {
...
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
    g_bt_a2dp_sink_ext_interface = (btav_sink_ext_interface_t *) linuxbt_gap_get_profile_interface(BT_PROFILE_ADVANCED_AUDIO_SINK_EXT_ID);
    if (NULL == g_bt_a2dp_sink_ext_interface)
    {
        BT_DBG_ERROR(BT_DEBUG_A2DP, "Failed to get A2DP SINK extended interface");
        return -1;
    }
    else
    {
        BT_DBG_NORMAL(BT_DEBUG_A2DP, "get A2DP SINK extended interface successfully");
    }
    // Init A2DP SRC ext interface
    ret = g_bt_a2dp_sink_ext_interface->init(&g_bt_a2dp_sink_ext_callbacks, sink_num);
    if (ret == BT_STATUS_SUCCESS)
    {
        BT_DBG_NORMAL(BT_DEBUG_A2DP, "init A2DP SINK extended interface successfully");
    }
    else
    {
        BT_DBG_NORMAL(BT_DEBUG_A2DP, "Fail to init A2DP SINK extended interface");
    }
#endif
