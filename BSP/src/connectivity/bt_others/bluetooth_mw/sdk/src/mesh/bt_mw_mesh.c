...
VOID bluetooth_mesh_register_profile(VOID)
{
    ...
    mesh_op.deinit = bluetooth_mesh_deinit;
    mesh_op.notify_acl_state = NULL;

    bt_mw_register_profile(BTWM_ID_MESH, &mesh_op);
}
...
INT32 bluetooth_mesh_register_cbk_fct(BT_APP_MESH_CB_FUNC_T *func)
{
    ...
    //register mesh profile operator
    bluetooth_mesh_register_profile();
    return ret;
}
...
{
    return linuxbt_mesh_enable(init_params);
}
...
{
    return linuxbt_mesh_access_model_reply(model_handle, msg, reply);
}
...
{
    return linuxbt_mesh_bearer_adv_set_params(adv_params, scan_params);
}
