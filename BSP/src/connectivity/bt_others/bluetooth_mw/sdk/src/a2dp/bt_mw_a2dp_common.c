...
#if ENABLE_A2DP_SRC
static INT32 bt_mw_a2dp_enable_src_stack(BOOL src_enable, INT32 src_num);
#endif
...
INT32 bt_mw_a2dp_connect(CHAR *addr, BT_MW_A2DP_ROLE local_role)
{
    ...
    idx = bt_mw_a2dp_alloc_dev_index(addr, local_role);
    if (-1 == idx)
    {
...
INT32 bt_mw_a2dp_sink_active_src(CHAR *addr, BOOL active)
{
    INT32 ret = BT_SUCCESS;
    INT32 idx = 0;
    ...
    if (BT_MW_A2DP_CONNECT_STATUS_CONNECTED
        != g_bt_mw_a2dp_cb.devices[idx].conn_status)
    {
        BT_DBG_ERROR(BT_DEBUG_A2DP, "%s is not connected", addr,
            g_bt_mw_a2dp_cb.devices[idx].conn_status);
        BT_MW_A2DP_UNLOCK();
        return BT_ERR_STATUS_NOT_READY;
    }
    ...
    ret = linuxbt_a2dp_sink_active_src(addr, active);
...
INT32 bt_mw_a2dp_src_active_sink(CHAR *addr, BOOL active)
{
    INT32 ret = BT_SUCCESS;
    INT32 idx = 0;
   ...
   // bt_mw_a2dp_sink_select_player(addr, g_bt_mw_a2dp_cb.devices[idx].config.codec_type);
    ...
    ret = linuxbt_a2dp_src_active_sink(addr, active);
...
INT32 bt_mw_a2dp_set_link_num(INT32 src_num, INT32 sink_num)
{
    INT32 ret = BT_SUCCESS;

    BT_MW_FUNC_ENTER(BT_DEBUG_A2DP, "src_link_num=%d, sink_link_num=%d",
        src_num, sink_num);

    if (src_num > BT_MW_A2DP_MAX_DEVICES)
    {
        src_num = BT_MW_A2DP_MAX_DEVICES;
    }
    ...
    g_bt_mw_a2dp_cb.config.link_num[BT_MW_A2DP_ROLE_SRC] = src_num;
    BT_MW_A2DP_UNLOCK();
    return ret;
}
...
#if defined(MTK_A2DP_MULTI_AVRCP) && (MTK_A2DP_MULTI_AVRCP == TRUE)
        BT_DBG_ERROR(BT_DEBUG_A2DP, "multi-a2dp skip check");
#else
        return -1;
#endif
    }
    ...
    UINT32 freq, UINT8 channel)
{
    ...
    if (-1 == idx)
    {
        BT_DBG_WARNING(BT_DEBUG_A2DP, "not found  %s, for bt_mw_a2dp_config_cb", addr);
...
static INT32 bt_mw_a2dp_enable_src_stack(BOOL src_enable, INT32 src_num)
{
    BT_DBG_NORMAL(BT_DEBUG_A2DP, "src_enable=%d, src_num=%d", src_enable, src_num);
    if (TRUE == src_enable)
    {
        linuxbt_a2dp_src_init(src_num);
        //usleep(500*1000);
