 ...
 *      N/A
 */
EXPORT_SYMBOL INT32 c_btm_a2dp_sink_active_src(CHAR *addr, BOOL active)
{
    bt_mw_a2dp_sink_active_src(addr, active);
    return BT_SUCCESS;
}

/* FUNCTION NAME: c_btm_a2dp_sink_player_load
 * PURPOSE:
 *      when user want to select player, it can be used to load player so
 * INPUT:
 *      player so path
 * OUTPUT:
 *      N/A
 ...
 *
 */
EXPORT_SYMBOL INT32 c_btm_a2dp_sink_player_load(CHAR* player_so_path)
{
   bt_mw_a2dp_sink_player_load(player_so_path);
   return BT_SUCCESS;
}

/* FUNCTION NAME: c_btm_a2dp_sink_player_unload
 * PURPOSE:
 *      when user want to remove player, it can be used to unregister player
 * INPUT:
 *      want to remove player name
 * OUTPUT:
 *      N/A
 * RETURN:
 *      BT_SUCCESS  -- unregister player successfully
 *      others      -- fail
 * NOTES:
 *
 */
EXPORT_SYMBOL INT32 c_btm_a2dp_sink_player_unload(CHAR *player_name)
{
   bt_mw_a2dp_sink_player_unload(player_name);
   return BT_SUCCESS;
}
