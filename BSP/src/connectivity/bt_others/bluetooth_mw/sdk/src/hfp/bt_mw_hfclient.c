...
INT32 bluetooth_hfclient_cpbs_cb(BT_HFCLIENT_CPBS_CB_DATA_T *cpbs_cb)
{
    ...
    FUNC_ENTRY
    if ((NULL == cpbs_cb) || (NULL == cpbs_cb->storage_lookup))
    {
        return -1;
    }
...
INT32 bluetooth_hfclient_init(VOID)
{
    ...
    hfclient_op.init = bluetooth_hfclient_init_profile;
    hfclient_op.deinit = bluetooth_hfclient_deinit_profile;
    hfclient_op.notify_acl_state = NULL;
    hfclient_op.facotry_reset = NULL;

    bt_mw_register_profile(BTWM_ID_HFP, &hfclient_op);
    return BT_SUCCESS;
...
{
    FUNC_ENTRY
    bt_mw_register_profile(BTWM_ID_HFP, NULL);
