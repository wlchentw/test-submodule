...
#define BT_DBG_DUMP(tag, fmt, args...)           {if (btmw_trc_map[tag].trace_level >= BT_MW_TRACE_LEVEL_API)        log_write("%s<N>%s@%d " fmt "\n", btmw_trc_map[tag].trc_name, __FUNCTION__, __LINE__, ## args); bt_mw_dump_info_write("%s@%d " fmt "\n", __FUNCTION__, __LINE__, ## args);}
#define BT_DBG_DUMP_2_FILE(to_file, tag, fmt, args...)           {if (btmw_trc_map[tag].trace_level >= BT_MW_TRACE_LEVEL_API)        log_write("%s<N>%s@%d " fmt "\n", btmw_trc_map[tag].trc_name, __FUNCTION__, __LINE__, ## args); if(to_file)bt_mw_dump_info_write("%s@%d " fmt "\n", __FUNCTION__, __LINE__, ## args);}
