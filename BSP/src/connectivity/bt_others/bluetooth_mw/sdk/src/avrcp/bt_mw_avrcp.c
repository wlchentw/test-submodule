...
typedef struct
{
    ...
    BT_AVRCP_FOLDER_TYPE folder_type;
...
static VOID bt_mw_avrcp_handle_get_folder_items_req(CHAR *addr,

    BT_AVRCP_GET_FOLDER_ITEMS_REQ *req);
...
static VOID bt_mw_avrcp_handle_feature_cb(CHAR *addr, BT_MW_AVRCP_ROLE role, UINT32 feature)
{
    ...
    if (BT_MW_AVRCP_ROLE_CT == role)
    {
        ...
        if ((g_bt_mw_avrcp_cb.devices[idx].ct_feature & BTRC_FEAT_ABSOLUTE_VOLUME))
        {
            param.data.feature.feature = TRUE;
        }
        else
        {
            param.data.feature.feature = FALSE;
        }
    }
    else
    {
...
static VOID bt_mw_avrcp_handle_get_play_status_req(CHAR *addr)
{
    ...
    BT_MW_AVRCP_LOCK();
    BT_DBG_NORMAL(BT_DEBUG_AVRCP,"player_status pos=%d, len=%d, status=%s",
            g_bt_mw_avrcp_cb.player_status.song_pos,
            g_bt_mw_avrcp_cb.player_status.song_len,
            bt_mw_avrcp_get_play_status_str(g_bt_mw_avrcp_cb.player_status.play_status));
...

{
    ...
    BT_MW_FUNC_ENTER(BT_DEBUG_AVRCP, "Yuming test  bt_mw_avrcp_handle_get_folder_items_req----1096 ");

    linuxbt_rc_tg_get_folder_items_rsp(addr, req,

        &g_bt_mw_avrcp_cb.folder_type);
...

{
    ...
    BT_MW_AVRCP_LOCK();
         ...
         //strcpy((char*)g_bt_mw_avrcp_cb.player_media_info.player_name,"kobo_test");

		BT_DBG_NORMAL(BT_DEBUG_AVRCP, "player_name = %s",g_bt_mw_avrcp_cb.player_media_info.player_name);


    BT_MW_FUNC_ENTER(BT_DEBUG_AVRCP, "Yuming test bt_mw_avrcp_handle_get_folder_items_cb----1112 ");

    //linuxbt_rc_tg_get_folder_items_rsp(req,&g_bt_mw_avrcp_cb.player_media_info.media_info);

    linuxbt_rc_tg_get_folder_items_rsp(addr, req,
        &g_bt_mw_avrcp_cb.player_media_info);
//        &g_bt_mw_avrcp_cb.folder_type);
...
static VOID bt_mw_avrcp_msg_handle(tBTMW_MSG *p_msg)
{
    ...
    switch(p_msg->hdr.event)
    {
        ...
        case BTMW_AVRCP_GET_ELEMENT_ATTR_REQ:
            ...
            break;
        case BTMW_AVRCP_GET_FOLDER_ITEMS_REQ:
            bt_mw_avrcp_handle_get_folder_items_cb(avrcp_msg->addr,
                &avrcp_msg->data.get_folder_items_req);
            break;
...
static CHAR* bt_mw_avrcp_get_event_str(UINT32 event)
{
    switch((int)event)
    {
        ...
        BT_MW_AVRCP_CASE_RETURN_STR(BTMW_AVRCP_GET_FOLDER_ITEMS_REQ, "get_folder_items_req");
...
//        BT_MW_AVRCP_CASE_RETURN_STR(BT_AVRCP_EVENT_GET_FOLDER_ITEMS_REQ, "get_folder_items_req");
