...
const tBTA_AV_ACTION bta_av_action[] =
{
    ...
    bta_av_rc_msg,
    bta_av_rc_close,
#if defined(MTK_AVRCP_TG_15_BROWSE)&&(MTK_AVRCP_TG_15_BROWSE == FALSE)
    NULL
#endif
};
...
static void bta_av_api_active_src(tBTA_AV_DATA *p_data);
static void bta_av_api_active_sink(tBTA_AV_DATA *p_data);
static void bta_av_api_stream_check(tBTA_AV_DATA *p_data);
#endif
...
const tBTA_AV_NSM_ACT bta_av_nsm_act[] =
{
    ...
    bta_av_codec_enable,    /* BTA_AV_API_CODEC_ENABLE_EVT */
#endif
    bta_av_api_to_ssm,      /* BTA_AV_API_START_EVT */
    bta_av_api_to_ssm,      /* BTA_AV_API_STOP_EVT */
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    bta_av_api_stream_check, /* BTA_AV_API_STREAM_CHECK_EVT */
    bta_av_api_active_src,  /* BTA_AV_API_ACTIVE_SRC_EVT */
    bta_av_api_active_sink,  /* BTA_AV_API_ACTIVE_SINK_EVT */
#endif
};
...
#define BTA_AV_PAUSE_TIMEOUT_MS (8 * 1000)         /* 8 seconds */
extern fixed_queue_t *btu_bta_alarm_queue;

static void bta_av_api_active_src(tBTA_AV_DATA *p_data)
{
    BTA_AV_PRINT_BDA(&p_data->api_active_src.addr, "active");

    for(int xx = 0; xx < BTA_AV_NUM_STRS; xx++)
    {
        if (bta_av_cb.p_scb[xx])
        {
            APPL_TRACE_WARNING(" xx: %d, active:%d.",
                xx, bta_av_cb.p_scb[xx]->active);

            if (memcmp(bta_av_cb.p_scb[xx]->peer_addr, p_data->api_active_src.addr, BD_ADDR_LEN) == 0)
            {
                bta_av_cb.p_scb[xx]->active = TRUE;
                APPL_TRACE_WARNING(" xx: %d, avdt_handle: %x, set active:%d.",
                    xx, bta_av_cb.p_scb[xx]->hndl, bta_av_cb.p_scb[xx]->active);
            }
            else
            {
                bta_av_cb.p_scb[xx]->active = FALSE;
                APPL_TRACE_WARNING(" xx: %d, set deactive:%d.", xx, bta_av_cb.p_scb[xx]->active);
            }
        }
        else
        {
            APPL_TRACE_WARNING(" xx: %d is NULL.", xx);
        }
    }
}
...
static void bta_av_api_active_sink(tBTA_AV_DATA *p_data)
{
    BTA_AV_PRINT_BDA_WARNING(&p_data->api_active_sink.addr, "active");

    for(int xx = 0; xx < BTA_AV_NUM_STRS; xx++)
    {
        APPL_TRACE_WARNING(" xx: %d.", xx);
        if (bta_av_cb.p_scb[xx])
        {
            if (memcmp(bta_av_cb.p_scb[xx]->peer_addr, p_data->api_active_sink.addr, BD_ADDR_LEN) == 0)
            {
                bta_av_cb.p_scb[xx]->active = TRUE;
            }
            else
...
void bta_av_stop_stream_check_timer_cback(tBTA_AV_SCB *p_scb)
{
    if (p_scb)
    {
        APPL_TRACE_ERROR("bta_av_stop_stream_check_timer_cback, num=%d.",
                          p_scb->zero_stream_num);
        alarm_cancel(p_scb->pause_timer);
        p_scb->zero_stream_num = 0;
    }
    else
        APPL_TRACE_ERROR("bta_av_stop_stream_check_timer_cback scb null, num=%d.",
                          p_scb->zero_stream_num);
}

void bta_av_start_stream_check_timer_cback(void *data)
{
    tBTA_AV_SCB *p_scb = (tBTA_AV_SCB*)data;

    APPL_TRACE_ERROR("bta_av_start_stream_check_timer_cback, num=%d.", p_scb->zero_stream_num);
    p_scb->zero_stream_num = 0;
}

static void bta_av_api_stream_check(tBTA_AV_DATA *p_data)
{
    BTA_AV_PRINT_BDA_WARNING(&p_data->stream_check.addr, "active");
    APPL_TRACE_ERROR("%s@%d start=%d", __FUNCTION__, __LINE__,
                       p_data->stream_check.start);

    for(int xx = 0; xx < BTA_AV_NUM_STRS; xx++)
    {
        if (bta_av_cb.p_scb[xx] &&
            (memcmp(bta_av_cb.p_scb[xx]->peer_addr, p_data->stream_check.addr, BD_ADDR_LEN) == 0))
        {
            APPL_TRACE_WARNING(" xx: %d.", xx);
            if (p_data->stream_check.start)
            {
                APPL_TRACE_DEBUG("%s@%d start timer.", __FUNCTION__, __LINE__);
                bta_av_cb.p_scb[xx]->zero_stream_num = 0;
                alarm_set_on_queue(bta_av_cb.p_scb[xx]->pause_timer,
                        BTA_AV_PAUSE_TIMEOUT_MS,
                        bta_av_start_stream_check_timer_cback,
                        bta_av_cb.p_scb[xx],
                        btu_bta_alarm_queue);
            }
            else
            {
                APPL_TRACE_DEBUG("%s@%d cancel timer.", __FUNCTION__, __LINE__);
                alarm_cancel(bta_av_cb.p_scb[xx]->pause_timer);
                bta_av_cb.p_scb[xx]->zero_stream_num = 0;
            }
    ...
    APPL_TRACE_ERROR("%s@%d scb not found.", __FUNCTION__, __LINE__);
}
...
static void bta_av_api_enable(tBTA_AV_DATA *p_data)
{
    ...
    if (bta_av_cb.features == p_data->api_enable.features)
    {
        return;
    }
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
int BTA_AvObtainPeerChannelIndex(BD_ADDR bd_addr)
{
    APPL_TRACE_DEBUG("%s,%02x:%02x:%02x:%02x:%02x:%02x", __func__,
                         bd_addr[0], bd_addr[1], bd_addr[2],
                         bd_addr[3], bd_addr[4], bd_addr[5]);
    int index = 0;
    // Find the entry for the peer (if exists)
    tBTA_AV_SCB* p_scb = bta_av_addr_to_scb(bd_addr);
    if (p_scb != NULL)
    {
        APPL_TRACE_DEBUG("%s,scb.hdi:%d.", __func__, p_scb->hdi);
        return p_scb->hdi;
    }

    APPL_TRACE_ERROR("%s not found, allocated.", __func__);
    ...
    for (index = 0; index < BTA_AV_NUM_STRS; index++)
    {
        ...
        {
            APPL_TRACE_WARNING("%s,scb[%d].hdi:%d.", __func__, index, p_scb->hdi);
            return p_scb->hdi;
        }
    }
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    APPL_TRACE_DEBUG("bta_av_hndl_to_scb:(idx-1): 0x%x",(idx-1));
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
                p_ret->pause_timer = alarm_new("bta_av.pause_timer");
                p_ret->zero_stream_num = 0;
                APPL_TRACE_DEBUG("init scb, num:%d, hndl:%d, hdi:%d", xx, p_ret->hndl, p_ret->hdi);
#endif
                break;
            }
        }
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
static tBTA_AV_SCB * bta_av_find_scb(tBTA_AV_CHNL chnl, UINT8 chn_id)
#else
static tBTA_AV_SCB * bta_av_find_scb(tBTA_AV_CHNL chnl)
#endif
{
    int xx = 0;
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    APPL_TRACE_DEBUG("bta_av_find_scb:channel:%d, chn_id:%d", chnl, chn_id);
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    for(xx=chn_id; xx<BTA_AV_NUM_STRS; xx++)
    {
        if((bta_av_cb.p_scb[xx] != NULL)
            && (chnl == bta_av_cb.p_scb[xx]->chnl)
            && (chn_id == bta_av_cb.p_scb[xx]->chn_id))
        {
            APPL_TRACE_DEBUG("bta_av_find_scb: p_scb[%d] found", xx);
    ...
    for(xx=0; xx<BTA_AV_NUM_STRS; xx++)
    {
        if((bta_av_cb.p_scb[xx] != NULL) && (chnl == bta_av_cb.p_scb[xx]->chnl))
        {
            APPL_TRACE_DEBUG("bta_av_find_scb: p_scb[%d] found", xx);
...
#endif
    APPL_TRACE_DEBUG("bta_av_find_scb nout found");
    return NULL;
}
#endif
...
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    memset(&registr,0,sizeof(registr));
#endif // defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)

    registr.status = BTA_AV_FAIL_RESOURCES;
    registr.app_id = p_data->api_reg.app_id;
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    registr.chnl   = (tBTA_AV_CHNL)p_data->hdr.layer_specific & BTA_AV_CHNL_MSK;
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
    chn_id = (tBTA_AV_CHNL)p_data->hdr.layer_specific & ~BTA_AV_CHNL_MSK;
#endif
#else
...
#endif // defined(MTK_AVRCP_TG_15_BROWSE)&&(MTK_AVRCP_TG_15_BROWSE == TRUE)

    do
    {
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
        p_scb = bta_av_find_scb(registr.chnl, chn_id);
#else
        p_scb = bta_av_find_scb(registr.chnl);
#endif
        if (NULL == p_scb)
        {
        ...
        if((bta_av_cb.reg_audio + bta_av_cb.reg_video) == 0)
        {
            ...
            if (bta_av_cb.features & (BTA_AV_FEAT_RCTG))
            {
                ...
                {
#if defined(MTK_AVRCP_ABS_VOL_DISABLE) && (MTK_AVRCP_ABS_VOL_DISABLE == TRUE)
#else
                    // Initialize AVRCP1.4 to provide Absolute Volume control.
                    bta_ar_reg_avrc(UUID_SERVCLASS_AV_REM_CTRL_TARGET, "AV Remote Control Target",
                                   NULL, p_bta_av_cfg->avrc_tg_cat, BTA_ID_AVK,
                                   (bta_av_cb.features & BTA_AV_FEAT_BROWSE), AVRC_REV_1_4);
#endif
                }
#endif
        ...
        if (bta_av_cb.features & (BTA_AV_FEAT_RCTG))
        {
            ...
            if (profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE)
            {
                UINT16 AVRC_version = 0;
#if !defined(MTK_AVRCP_ABS_VOL_DISABLE) || (MTK_AVRCP_ABS_VOL_DISABLE == FALSE)
                AVRC_version = AVRC_REV_1_5;
#else
                AVRC_version = AVRC_REV_1_3;
#endif
                bta_ar_reg_avrc(UUID_SERVCLASS_AV_REM_CTRL_TARGET, "AV Remote Control Target",
                               NULL, p_bta_av_cfg->avrc_tg_cat, BTA_ID_AV,
                               (bta_av_cb.features & BTA_AV_FEAT_BROWSE), AVRC_version);
            }
            else if (profile_initialized == UUID_SERVCLASS_AUDIO_SINK)
            {
#if defined(MTK_AVRCP_ABS_VOL_DISABLE) && (MTK_AVRCP_ABS_VOL_DISABLE == TRUE)
        ...
        */
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE) //tt PTS test -- AVDTP/SNK/ACP/SIG/SMG/BV-14-C
        //remove reconfig on nonsupported list.
        cs.nsc_mask = ((bta_av_cb.features & BTA_AV_FEAT_PROTECT) ? 0 : AVDT_NSC_SECURITY);
#else
        ...
        if(registr.chnl == BTA_AV_CHNL_AUDIO)
        {
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                bta_av_a2d_cos.get_sep_info(AVDT_TSEP_SRC, &index, &max_sep_idx);
#endif
            }
            else if (profile_initialized == UUID_SERVCLASS_AUDIO_SINK)
            {
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                bta_av_a2d_cos.get_sep_info(AVDT_TSEP_SNK, &index, &max_sep_idx);
#endif
            }
#if defined(MTK_A2DP_SRC_SINK_BOTH ) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
            for (int xx=index; xx < max_sep_idx; xx++)
            {
...
#endif
            {
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
                if(AVDT_CreateStream(&p_scb->seps[index].av_handle, &cs, p_scb->hdi) == AVDT_SUCCESS)
#else
...
#endif
                {
                    ...
                    memcpy(p_scb->seps[index].codec_info, cs.cfg.codec_info, AVDT_CODEC_SIZE);
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
                    APPL_TRACE_DEBUG("scb_idx=%d audio[%d] av_handle: %d codec_type: %d, tsep=%d",
                        p_scb->hdi, index, p_scb->seps[index].av_handle,
                        p_scb->seps[index].codec_type, p_scb->seps[index].tsep);
#else
...
#endif
            {
...
#endif
                {
                    ...
                    && 0 == bta_av_cb.sdp_a2d_snk_handle)
#else
                else if (profile_initialized == UUID_SERVCLASS_AUDIO_SINK)
#endif
                {
...
#endif
                        bta_ar_reg_avrc(UUID_SERVCLASS_AV_REM_CTRL_TARGET, "AV Remote Control Target", NULL,
                            p_bta_av_cfg->avrc_ct_cat, profile_initialized == UUID_SERVCLASS_AUDIO_SOURCE?BTA_ID_AV:BTA_ID_AVK,
                            (bta_av_cb.features & BTA_AV_FEAT_BROWSE), AVRC_version);
#else
                        bta_ar_reg_avrc(UUID_SERVCLASS_AV_REMOTE_CONTROL, NULL, NULL,
                        p_bta_av_cfg->avrc_ct_cat, BTA_ID_AV,
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
#else
            if (bta_av_cb.reg_audio & BTA_AV_HNDL_TO_MSK(p_scb->hdi))
            {
...
static void bta_av_api_to_ssm(tBTA_AV_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_av_api_to_ssm handle:%d.", handle);
    ...
    {
        APPL_TRACE_ERROR("bta_av_api_to_ssm not found:%d.", handle);
    }
#else
    for(xx=0; xx<BTA_AV_NUM_STRS; xx++)
    {
...
BOOLEAN bta_av_hdl_event(BT_HDR *p_msg)
{
    ...
    {
#if (defined(BTA_AV_DEBUG) && BTA_AV_DEBUG == TRUE)
        APPL_TRACE_DEBUG("AV nsm event=0x%x(%s)", event, bta_av_evt_code(event));
#else
        APPL_TRACE_DEBUG("AV nsm event=0x%x", event);
#endif
    ...
    {
#if (defined(BTA_AV_DEBUG) && BTA_AV_DEBUG == TRUE)
        APPL_TRACE_DEBUG("AV sm event=0x%x(%s)", event, bta_av_evt_code(event));
#else
        APPL_TRACE_DEBUG("AV sm event=0x%x", event);
#endif
    ...
    {
        APPL_TRACE_DEBUG("handle=0x%x", p_msg->layer_specific);
...
char *bta_av_evt_code(UINT16 evt_code)
{
    ...
    case BTA_AV_API_SET_ACL_PRIOR_EVT: return "API_SET_ACL_PRIORITY";
#endif
...
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
    case BTA_AV_API_CODEC_ENABLE_EVT: return "API_CODEC_ENABLE_EVT";
#endif
