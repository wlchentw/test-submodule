...
{
    tBTA_AV_API_ACTIVE_SRC *p_buf =
        (tBTA_AV_API_ACTIVE_SRC *)osi_malloc(sizeof(tBTA_AV_API_ACTIVE_SRC));

    p_buf->hdr.event = BTA_AV_API_ACTIVE_SRC_EVT;
    memcpy(p_buf->addr, bdaddr, sizeof(BD_ADDR));
...
{
    tBTA_AV_API_ACTIVE_SINK *p_buf =
        (tBTA_AV_API_ACTIVE_SINK *)osi_malloc(sizeof(tBTA_AV_API_ACTIVE_SINK));

    p_buf->hdr.event = BTA_AV_API_ACTIVE_SINK_EVT;
    memcpy(p_buf->addr, bdaddr, sizeof(BD_ADDR));
...
{
    tBTA_AV_API_STREAM_CHECK *p_buf =
        (tBTA_AV_API_STREAM_CHECK *)osi_malloc(sizeof(tBTA_AV_API_STREAM_CHECK));

    p_buf->hdr.event = BTA_AV_API_STREAM_CHECK_EVT;
    memcpy(p_buf->addr, bdaddr, sizeof(BD_ADDR));
    p_buf->start = start;
...
{
    tBTA_AV_API_SET_ACL_PRIORITY  *p_buf;
    if ((p_buf = (tBTA_AV_API_SET_ACL_PRIORITY *)osi_malloc(
            (UINT16) (sizeof(tBTA_AV_API_SET_ACL_PRIORITY)))) != NULL)
    {
        p_buf->hdr.event = BTA_AV_API_SET_ACL_PRIOR_EVT;
        p_buf->hdr.layer_specific   = hndl;
...
{
    tBTA_AV_API_CODEC_ENABLE *p_buf =
        (tBTA_AV_API_CODEC_ENABLE *)osi_malloc(sizeof(tBTA_AV_API_CODEC_ENABLE));

    p_buf->hdr.event = BTA_AV_API_CODEC_ENABLE_EVT;
