...
// MTK_A2DP_SILENT_STREAM_PRIORITY end
    ...
    ,BTA_AV_API_STREAM_CHECK_EVT
    ,BTA_AV_API_ACTIVE_SRC_EVT
    ,BTA_AV_API_ACTIVE_SINK_EVT
#endif
};
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
#define BTA_AV_LAST_EVT             BTA_AV_API_ACTIVE_SINK_EVT
#else
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
#define BTA_AV_PRINT_BDA(pbda, s, ...) \
    do { char buf[18]; bdaddr_to_string(pbda, buf, sizeof(buf));\
        APPL_TRACE_DEBUG("%s@%d addr=%s "s, __FUNCTION__, __LINE__, buf, ##__VA_ARGS__); \
    }while(0)

#define BTA_AV_PRINT_BDA_WARNING(pbda, s, ...) \
        do { char buf[18]; bdaddr_to_string(pbda, buf, sizeof(buf));\
            APPL_TRACE_ERROR("%s@%d addr=%s "s, __FUNCTION__, __LINE__, buf, ##__VA_ARGS__); \
        }while(0)
...
typedef void (*tBTA_AV_CO_GET_SEP_INFO)(UINT8 tsep, UINT8 *start_idx, UINT8 *max_sep_idx);
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    tBTA_AV_CO_GET_SEP_INFO get_sep_info;
#endif
} tBTA_AV_CO_FUNCTS;

#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
/* data type for BTA_AV_API_ACTIVE_SRC_EVT */
typedef struct
{
    BT_HDR              hdr;
    BD_ADDR             addr;
} tBTA_AV_API_ACTIVE_SRC;

typedef struct
{
    BT_HDR              hdr;
    BD_ADDR             addr;
} tBTA_AV_API_ACTIVE_SINK;
...
typedef struct
{
    ...
    BOOLEAN             incoming; /* peer launch connection */
#endif
} tBTA_AV_API_OPEN;
...
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
/* data type for BTA_AV_API_CODEC_ENABLE_EVT */
typedef struct
{
    BT_HDR              hdr;
    UINT8               codec_type;     /* codec_type*/
    UINT8               enable;         /* enable or disable */
} tBTA_AV_API_CODEC_ENABLE;
#endif
...
typedef struct
{
    ...
    UINT8               codec_info[AVDT_CODEC_SIZE]; /* Codec info */
    tBTA_AV_DATA_CBACK  *p_app_data_cback; /* Application callback for media packets */
} tBTA_AV_SEP;

// MTK_A2DP_SILENT_STREAM_PRIORITY begin
#ifdef MTK_A2DP_SILENT_STREAM_PRIORITY
/* data type for BTA_AV_API_SET_ACL_PRIOR_EVT */
typedef struct
{
    BT_HDR              hdr;
    UINT8              priority;
} tBTA_AV_API_SET_ACL_PRIORITY;
#endif
// MTK_A2DP_SILENT_STREAM_PRIORITY end
...
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
    tBTA_AV_API_CODEC_ENABLE  api_codec_enable;
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    tBTA_AV_API_ACTIVE_SRC      api_active_src;
    tBTA_AV_API_ACTIVE_SINK     api_active_sink;
    tBTA_AV_API_STREAM_CHECK    stream_check;
#endif
// MTK_A2DP_SILENT_STREAM_PRIORITY begin
#ifdef MTK_A2DP_SILENT_STREAM_PRIORITY
    tBTA_AV_API_SET_ACL_PRIORITY api_set_priority;
#endif
// MTK_A2DP_SILENT_STREAM_PRIORITY end
} tBTA_AV_DATA;
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    alarm_t             *pause_timer;   /* When send pause cmd, start timer to statistics zero stream data */
    UINT16              zero_stream_num;/* number to statistics zero stream data */
    BOOLEAN             active;
#endif
#if defined(MTK_A2DP_SRC_SINK_BOTH) && (MTK_A2DP_SRC_SINK_BOTH == TRUE)
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
...
extern UINT8  bta_av_get_scb_sep_type ( tBTA_AV_SCB *p_scb, UINT8 tavdt_handle);
#endif
...
extern void bta_av_rc_disc_ar(UINT8 disc, BOOLEAN get_rc_version);
#endif
...
#if defined(MTK_LINUX_A2DP_PLUS) && (MTK_LINUX_A2DP_PLUS == TRUE)
extern void bta_av_codec_enable(tBTA_AV_DATA* p_data);
#endif
// MTK_A2DP_SILENT_STREAM_PRIORITY begin
#ifdef MTK_A2DP_SILENT_STREAM_PRIORITY
extern void bta_av_set_acl_priority(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data);
#endif
// MTK_A2DP_SILENT_STREAM_PRIORITY end
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
extern void bta_av_switch_stream_device(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data);
extern void bta_av_stop_stream_check_timer_cback(tBTA_AV_SCB *p_scb);
#endif
