...
UINT8 bta_av_ste_cfg_matches_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    UINT8           status = 0;
    tA2D_SBC_CIE    cfg_cie;

    /* parse configuration */
    if ((status = A2D_ParsSteInfo(&cfg_cie, p_cfg, TRUE)) != 0)
    {
...
UINT8 bta_av_ste_cfg_in_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    UINT8           status = 0;
    tA2D_SBC_CIE    cfg_cie;

    /* parse configuration */
    if ((status = A2D_ParsSteInfo(&cfg_cie, p_cfg, FALSE)) != 0)
    {
        return status;
    }
...
#endif  //MTK_A2DP_SNK_STE_CODEC = TRUE ||  MTK_A2DP_SRC_STE_CODEC = TRUE
