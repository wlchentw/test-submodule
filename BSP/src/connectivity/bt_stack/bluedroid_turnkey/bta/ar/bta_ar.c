                     ...
                     UINT16 profile_version)
{
    ...
    APPL_TRACE_API("%s uuid=0x%x, categories=%d, mask=0x%x",
        __FUNCTION__, service_uuid, categories, mask);
#endif
 ...
 *
 *****************************************************************************/
void bta_ar_update_avrc_version(UINT16 peer_rc_version, tBTA_SYS_ID sys_id, UINT16 categories)
{
    UINT8 mask = bta_ar_id(sys_id);

    if (mask) {
        AVRC_Update_Version(bta_ar_cb.sdp_tg_handle, peer_rc_version, categories);
    }
