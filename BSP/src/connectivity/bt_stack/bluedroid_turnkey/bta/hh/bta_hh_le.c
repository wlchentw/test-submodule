...
void bta_hh_le_open_cmpl(tBTA_HH_DEV_CB *p_cb)
{
    if ( p_cb->disc_active == BTA_HH_LE_DISC_NONE)
    {
        ...
        {
            L2CA_EnableUpdateBleConnParamsOnly(p_cb->addr, TRUE);
...
void bta_hh_le_notify_enc_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    /* when key missing error, remove bond */
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    if(p_cb->status == BTA_HH_ERR_SEC && p_cb->reason == BTM_ERR_KEY_MISSING)
    {
#if defined(PIN_KEY_MISSING_HANDLE_UNPAIR)&&(PIN_KEY_MISSING_HANDLE_UNPAIR == TRUE)
        tBTA_DM_SEC param;
        bdcpy(param.delete_key_RC_to_unpair.bd_addr, p_cb->addr);
...
void process_included(tBTA_HH_DEV_CB *p_dev_cb, tBTA_GATTC_SERVICE *service) {
    ...
    for (list_node_t *isn = list_begin(service->included_svc);
        ...
        if (incl_svc->uuid.uu.uuid16 == UUID_SERVCLASS_BATTERY &&
            ...
            if (!incl_svc->characteristics || list_is_empty(incl_svc->characteristics)) {
                continue;
            }
...
static void bta_hh_le_search_hid_chars(tBTA_HH_DEV_CB *p_dev_cb, tBTA_GATTC_SERVICE *service)
{
    ...
    if (!service->characteristics || list_is_empty(service->characteristics)) {
        return;
    }
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            APPL_TRACE_DEBUG("BTA_GATTC_ENC_CMPL_CB_EVT  status : %d", p_data->status);
            p_dev_rec = btm_find_dev(p_data->enc_cmpl.remote_bda);

            p_dev_cb = bta_hh_le_find_dev_cb_by_bda(p_data->enc_cmpl.remote_bda);
            if (p_dev_cb) {
               if (p_dev_rec)
               {
                   APPL_TRACE_DEBUG("p_dev_rec->sec_status : %d", p_dev_rec->sec_status);
                   if (p_dev_rec->sec_status == HCI_ERR_KEY_MISSING)
                   {
                       p_dev_cb->status = BTA_HH_ERR_SEC;
                       p_dev_cb->reason = BTM_ERR_KEY_MISSING;
                   }
               }
