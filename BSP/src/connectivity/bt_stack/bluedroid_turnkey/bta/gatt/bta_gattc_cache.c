...
static void service_free(void *ptr) {
  ...
  srvc->characteristics= NULL;
  list_free(srvc->included_svc);
  srvc->included_svc = NULL;
  osi_free(srvc);
}
                                       ...
                                       int *count)
{
    ...
    if (!p_srvc_cb || !count || !db)
    {
        APPL_TRACE_WARNING("%s p_srvc_cb=0x%p count=0x%p db=0x%p", __func__, p_srvc_cb, count, db);
        *count = 0;
        *db = NULL;
        return;
    }
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    //flush cache file before close file
    fflush(fd);
#endif
