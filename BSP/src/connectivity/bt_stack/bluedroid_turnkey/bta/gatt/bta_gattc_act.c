...
static void bta_gattc_phy_update_cback (tGATT_IF gatt_if, BD_ADDR bd_addr, uint8_t tx_phy, uint8_t rx_phy, uint8_t status);
#endif
...
void bta_gattc_register(tBTA_GATTC_CB *p_cb, tBTA_GATTC_DATA *p_data)
{
    ...
    for (i = 0; i < BTA_GATTC_CL_MAX; i ++)
    {
        if (!p_cb->cl_rcb[i].in_use)
        {
            ...
            if ((p_app_uuid == NULL) || (p_cb->cl_rcb[i].client_if = GATT_Register(p_app_uuid, &bta_gattc_cl_cback, GATT_CLINET_FLAG)) == 0)
#else
...
void bta_gattc_conn(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (p_data != NULL)
    {
        ...
        if (!GATT_GetConnectionInfor(p_data->hdr.layer_specific,
                                     &gatt_if, p_clcb->bda, &p_clcb->transport))
        {
            APPL_TRACE_ERROR("bta_gattc_conn ligical link information is not found");
        }
    }
...
void bta_gattc_start_discover(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (p_clcb == NULL)
    {
        APPL_TRACE_ERROR("bta_gattc_start_discover p_clcb is null");
        return;
    }
...
{
    if (p_clcb == NULL) {
        APPL_TRACE_ERROR("bta_gattc_disc_cmpl p_clcb is null");
        return;
    }
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
extern UINT8 bta_gattc_read_phy_callback(BD_ADDR bd_addr, uint8_t tx_phy, uint8_t rx_phy, uint8_t status);

void bta_gattc_set_preferred_phy(tBTA_GATTC_DATA * p_msg)
{
    BTM_BleSetPhy(p_msg->api_set_preferred_phy.bd_addr,
                  p_msg->api_set_preferred_phy.tx_phy,
                  p_msg->api_set_preferred_phy.rx_phy,
                  p_msg->api_set_preferred_phy.phy_options);
}

void bta_gattc_read_phy(tBTA_GATTC_DATA * p_msg)
{
    BTM_BleReadPhy(p_msg->api_read_phy.bd_addr, bta_gattc_read_phy_callback);
}
...
static void bta_gattc_phy_update_cback(tGATT_IF gatt_if, BD_ADDR bd_addr, uint8_t tx_phy, uint8_t rx_phy, uint8_t status)
{
    ...
    APPL_TRACE_DEBUG("%s client_if=%d", __func__, gatt_if);
    if (!p_clreg || !p_clreg->p_cback)
    {
        APPL_TRACE_WARNING("%s client_if=%d not found!", __func__, gatt_if);
        return;
    }

    tBTA_GATTC cb_data;
    bdcpy(cb_data.phy_update.bd_addr, bd_addr);
