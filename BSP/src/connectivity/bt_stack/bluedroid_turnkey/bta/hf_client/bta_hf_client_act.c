...
/*******************************************************************************
**
** Function         bta_hf_client_cpbs
**
** Description      Send  to application.
**
**
** Returns          void
**
*******************************************************************************/
void bta_hf_client_cpbs()
{
    tBTA_HF_CLIENT evt;

    memset(&evt, 0, sizeof(evt));

    memcpy(evt.cpbs.storage_lookup, bta_hf_client_cb.scb.at_cb.storage_lookup, BT_HFCLIENT_PB_STORAGE_COUNT*sizeof(UINT8));

    (*bta_hf_client_cb.p_cback)(BTA_HF_CLIENT_CPBS_EVT, &evt);
}


/*******************************************************************************
**
** Function         bta_hf_client_cpbr_count
**
** Description      Send phonebook entries count to application.
**
**
** Returns          void
**
*******************************************************************************/
void bta_hf_client_cpbr_count(UINT16 idx_max)
{
    tBTA_HF_CLIENT evt;

    memset(&evt, 0, sizeof(evt));

    evt.cpbr.count.idx_max = idx_max;

    (*bta_hf_client_cb.p_cback)(BTA_HF_CLIENT_CPBR_COUNT_EVT, &evt);
}


/*******************************************************************************
**
** Function         bta_hf_client_cpbr_entry
**
** Description      Send phonebook entries to application.
**
**
** Returns          void
**
*******************************************************************************/
void bta_hf_client_cpbr_entry(UINT16 idx, char *number, UINT16 type, char *name)
{
    tBTA_HF_CLIENT evt;

    memset(&evt, 0, sizeof(evt));

    evt.cpbr.entry.idx = idx;
    evt.cpbr.entry.type = type;

    if (number && (0 != strlen(number)))
    {
        strlcpy(evt.cpbr.entry.number, number, BTA_HF_CLIENT_CPBR_NUMBER_LEN + 1);
        evt.cpbr.entry.number[BTA_HF_CLIENT_CPBR_NUMBER_LEN] = '\0';
    }

    if (name && (0 != strlen(name)))
    {
        strlcpy(evt.cpbr.entry.name, name, BTA_HF_CLIENT_CPBR_NAME_LEN + 1);
        evt.cpbr.entry.name[BTA_HF_CLIENT_CPBR_NAME_LEN] = '\0';
    }
    (*bta_hf_client_cb.p_cback)(BTA_HF_CLIENT_CPBR_ENTRY_EVT, &evt);
}
#endif
