...
static const char* bta_hf_client_storages[BT_HFCLIENT_PB_STORAGE_COUNT] =
{
    "DC",
    "EN",
    "FD",
...
#define BTA_HF_CLIENT_MAX_CPBR_COUNT 32767
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
static void bta_hf_client_handle_cpbr_complete();
#endif
...
static void bta_hf_client_handle_ok()
{
    ...
    switch(bta_hf_client_cb.scb.at_cb.current_cmd)
    {
        ...
        case BTA_HF_CLIENT_AT_NONE:
            ...
            break;
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
        case BTA_HF_CLIENT_AT_CPBR_ENT:
            bta_hf_client_handle_cpbr_complete();
            break;
#endif
        default:
...
static void bta_hf_client_handle_vgs(UINT32 value)
{
    ...
    if(value <= BTA_HF_CLIENT_VGS_MAX && value > BTA_HF_CLIENT_VGS_MIN)
    {
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
static void bta_hf_client_handle_cpbs()
{
    APPL_TRACE_DEBUG("%s ", __FUNCTION__);
    bta_hf_client_cpbs();
...
static void bta_hf_client_handle_cpbr_count(UINT16 idx_max)
{
    APPL_TRACE_DEBUG("%s %d", __FUNCTION__, idx_max);
    bta_hf_client_cpbr_count(idx_max);
}

static void bta_hf_client_handle_cpbr_entry(UINT16 idx, char *number, UINT16 type, char *name)
{
    bta_hf_client_cpbr_entry(idx, number, type, name);
}

static void bta_hf_client_handle_cpbr_complete()
{
    APPL_TRACE_DEBUG("%s ", __FUNCTION__);
    bta_hf_client_evt_val(BTA_HF_CLIENT_CPBR_COMPLETE_EVT,0);
}
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
static char *bta_hf_client_parse_cpbs(char *buffer)
{
    char storage_name[3] = {0};
    int res;
    int offset = 0;

    AT_CHECK_EVENT(buffer, "+CPBS:");
    APPL_TRACE_DEBUG("%s buffer:%s", __FUNCTION__, buffer);
    ...
    {
        res = sscanf(buffer, "\"%2[^\"]\"%n", storage_name, &offset);
        if (res < 0)
        {
            return NULL;
        }
        ...
        for (int i = 0; i < sizeof(bta_hf_client_storages)/sizeof(bta_hf_client_storages[0]); i++)
        {
            if (strncmp(storage_name, bta_hf_client_storages[i], sizeof(bta_hf_client_storages[i])) == 0)
            {
                bta_hf_client_cb.scb.at_cb.storage_lookup[i] = 1;
            }
        }
...
static char *bta_hf_client_parse_cpbr_count(char *buffer)
{
    APPL_TRACE_DEBUG("%s ", __FUNCTION__);
    INT16 idx_min = 0;
    INT16 idx_max = 0;
    ...
    res = sscanf(buffer, "(%hu-%hu),%hu,%hu%n", &idx_min, &idx_max, &nlength, &tlength, &offset);
    APPL_TRACE_DEBUG("idx_min=%d, idx_max=%d, nlength=%d, tlength=%d, res=%d",
                    idx_min, idx_max, nlength, tlength, res);
    if (res < 2)
    {
        APPL_TRACE_ERROR("%s: parse Error %s", __func__, buffer);
        return NULL;
    }
    /* Abort in case offset not set because of format error */
    if (offset == 0)
...
/******************************************************************************
**  Special case from AG: +CPBR:(0--1),50,50
*******************************************************************************/
    if (idx_min <= 0 ||
        idx_max <= 0 ||
        nlength <= 0 ||
        tlength <= 0)
    {
        APPL_TRACE_ERROR("%s: Parse Data Error %s", __func__, buffer);
        idx_max = 0;
    }
    else if (idx_max >= BTA_HF_CLIENT_MAX_CPBR_COUNT)
    {
        APPL_TRACE_ERROR("%s: Exceeds max count %s", __func__, buffer);
        idx_max = BTA_HF_CLIENT_MAX_CPBR_COUNT;
    }

    buffer += offset;
    AT_SKIP_REST(buffer);
    AT_CHECK_RN(buffer);
    bta_hf_client_handle_cpbr_count(idx_max);
    // check for OK Response in end
    AT_CHECK_EVENT(buffer, "OK");
    AT_CHECK_RN(buffer);
...
static char *bta_hf_client_parse_cpbr_entry(char *buffer)
{
    char number[BTA_HF_CLIENT_CPBR_NUMBER_LEN + 1] = {0};     /* spec forces 32 chars, plus one for \0*/
    UINT16 index = 0;
    UINT16 type = 0;
    char text[BTA_HF_CLIENT_CPBR_NAME_LEN + 1] = {0};
    int res;

    APPL_TRACE_DEBUG("%s, buffer:%s", __FUNCTION__, buffer);

    res = sscanf(buffer, "%hu,\"%50[^\"]\",%hu,\"%50[^\"]\"", &index, number, &type, text);
    /*Maybe len of number or text > 50 byte*/
    if (res < 3)
    {
        APPL_TRACE_ERROR("%s: Format Error: res < 3", __FUNCTION__);
        res = sscanf(buffer, "%hu,\"%50[^\"]%*[^\"]\",%hu,\"%50[^\"]%*[^\"]\"", &index, number, &type, text);
    }
    APPL_TRACE_DEBUG("%s index=%d, number=%s, type=%d, text=%s, res=%d",
                        __FUNCTION__, index, number, type, text, res);

    /* Skip any remain param,as they are not defined by BT HFP spec */
    AT_SKIP_REST(buffer);
    AT_CHECK_RN(buffer);

    bta_hf_client_handle_cpbr_entry(index, number, type, text);
...
static char *bta_hf_client_parse_cpbr(char *buffer)
{
    if (strncmp("\r\n+CPBR:", buffer,sizeof("\r\n+CPBR:") - 1) == 0)
    {
        buffer += sizeof("\r\n+CPBR:") - 1;
    ...
    else if (strncmp("+CPBR:", buffer,sizeof("+CPBR:") - 1) == 0)
    {
        buffer += sizeof("+CPBR:") - 1;
        while (*buffer == ' ') buffer++;
    }
/******************************************************************************
**  skip the case:(at the end of buffer)
**  0D0A +CPBR:idx,text,type,number 0D0A
**  0D0A
**  the case occurs in iphone some times.
*******************************************************************************/
    else if ((strncmp("\r\n", buffer, strlen(buffer)) == 0) &&
              (BTA_HF_CLIENT_AT_CPBR_ENT == bta_hf_client_cb.scb.at_cb.current_cmd))
    {
        buffer += sizeof("\r\n") - 1;
        return buffer;
    }
    ...
    if (*buffer == '(')
    {
        return bta_hf_client_parse_cpbr_count(buffer);
    }
    else
    {
        return bta_hf_client_parse_cpbr_entry(buffer);
    }
}
#endif
...
static char *bta_hf_client_skip_unknown(char *buffer)
{
    ...
    APPL_TRACE_DEBUG("%s begin buffer:%s", __FUNCTION__, buffer);
    ...
    APPL_TRACE_DEBUG("%s end buffer:%.*s", __FUNCTION__, buffer - start - 2, start);
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
    bta_hf_client_parse_cpbs,
    bta_hf_client_parse_cpbr,
#endif
    bta_hf_client_skip_unknown
};

/* calculate supported event list length */
static const UINT16 bta_hf_client_parser_cb_count =
...
static void bta_hf_client_at_parse_start(void)
{
    ...
    while(*buf != '\0')
    {
        ...
        for(i = 0; i < bta_hf_client_parser_cb_count; i++)
        {
            ...
            if (tmp == NULL)
            {
                APPL_TRACE_ERROR("HFPClient: AT event/reply parsing failed, skipping");
        ...
        if (tmp == NULL)
        {
            APPL_TRACE_ERROR("HFPClient: Could not skip unknown AT event, disconnecting");
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
void bta_hf_client_send_at_cpbs(UINT8 storage_idx)
{
    char buf[BTA_HF_CLIENT_AT_MAX_LEN] = {0};
    int at_len;

    if (storage_idx == BT_HFCLIENT_PB_STORAGE_COUNT)
    {
        at_len = snprintf(buf, sizeof(buf), "AT+CPBS=?\r");
        APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
        bta_hf_client_send_at(BTA_HF_CLIENT_AT_CPBS_SEL, buf, at_len);
    }
    else if (storage_idx < BT_HFCLIENT_PB_STORAGE_COUNT)
    {
        at_len = snprintf(buf, sizeof(buf), "AT+CPBS=\"%s\"\r", bta_hf_client_storages[storage_idx]);
        APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
        bta_hf_client_send_at(BTA_HF_CLIENT_AT_CPBS_SET, buf, at_len);
    }
}


void bta_hf_client_send_at_cpbr(UINT32 idx_min, UINT32 idx_max)
{
    char buf[BTA_HF_CLIENT_AT_MAX_LEN] = {0};
    int at_len;
    ...
    if (idx_min == idx_max)
    {
        if (idx_min == 0)
        {
            at_len = snprintf(buf, sizeof(buf), "AT+CPBR=?\r");
            APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
            bta_hf_client_send_at(BTA_HF_CLIENT_AT_CPBR_CNT, buf, at_len);
        }
        else
        {
            at_len = snprintf(buf, sizeof(buf), "AT+CPBR=%u\r", idx_min);
            APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
            bta_hf_client_send_at(BTA_HF_CLIENT_AT_CPBR_ENT, buf, at_len);
        }
    }
    else if (idx_max > idx_min)
    {
        at_len = snprintf(buf, sizeof(buf), "AT+CPBR=%u,%u\r", idx_min, idx_max);
        APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
        bta_hf_client_send_at(BTA_HF_CLIENT_AT_CPBR_ENT, buf, at_len);
    }
    else
    {
        APPL_TRACE_ERROR("%s Error, not send, idx_min:%d > idx_max:%d", __FUNCTION__, idx_min, idx_max);
    }
}


void bta_hf_client_send_at_cscs(const char* charset)
{
    char buf[BTA_HF_CLIENT_AT_MAX_LEN] = {0};
    int at_len;
    ...
    if (NULL != charset)
    {
        at_len = snprintf(buf, sizeof(buf), "AT+CSCS=\"%s\"\r", charset);
    }
    else
    {
        at_len = snprintf(buf, sizeof(buf), "AT+CSCS=\"UTF-8\"\r");
    }
    APPL_TRACE_DEBUG("%s %s", __FUNCTION__, buf);
    bta_hf_client_send_at(BTA_HF_CLIENT_AT_CSCS, buf, at_len);
}
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
    for (i = 0; i < BT_HFCLIENT_PB_STORAGE_COUNT; i++)
    {
        bta_hf_client_cb.scb.at_cb.storage_lookup[i] = 0;
    }
#endif
    bta_hf_client_cb.scb.at_cb.current_cmd = BTA_HF_CLIENT_AT_NONE;
}
