...
void bta_hf_client_send_at_cmd(tBTA_HF_CLIENT_DATA *p_data)
{
    ...
    switch(p_val->uint8_val)
    {
        ...
        case BTA_HF_CLIENT_AT_CMD_CPBS:
            bta_hf_client_send_at_cpbs(p_val->uint32_val1);
            break;
        case BTA_HF_CLIENT_AT_CMD_CPBR:
            bta_hf_client_send_at_cpbr(p_val->uint32_val1, p_val->uint32_val2);
            break;
        case BTA_HF_CLIENT_AT_CMD_CSCS:
            bta_hf_client_send_at_cscs(p_val->str);
            break;
#endif
        default:
