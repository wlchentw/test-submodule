...
#define BTA_HF_CLIENT_CPBS_EVT              22 /* AG Supported Phonebook memory storage*/
#define BTA_HF_CLIENT_CPBR_ENTRY_EVT        23 /* Read phonebook entries*/
#define BTA_HF_CLIENT_CPBR_COUNT_EVT        24 /* Read phonebook entry count*/
#define BTA_HF_CLIENT_CPBR_COMPLETE_EVT     25 /* Read phonebook entries complete*/
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
#define BTA_HF_CLIENT_AT_CMD_CPBS   16
#define BTA_HF_CLIENT_AT_CMD_CPBR   17
#define BTA_HF_CLIENT_AT_CMD_CSCS   18
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
/* data associated with BTA_HF_CLIENT_CPBR_ENTRY_EVT and
                        BTA_HF_CLIENT_CPBR_COUNT_EVT event */
#define BTA_HF_CLIENT_CPBR_NAME_LEN     50
#define BTA_HF_CLIENT_CPBR_NUMBER_LEN   50
typedef struct
{
    UINT16                     idx_max;
}tBTA_HF_CLIENT_CPBR_CNT;

typedef struct
{
    UINT16                     idx;
    char                       number[BTA_HF_CLIENT_CPBR_NUMBER_LEN + 1];
    UINT16                     type;
    char                       name[BTA_HF_CLIENT_CPBR_NAME_LEN + 1];
} tBTA_HF_CLIENT_CPBR_ENTRY;

typedef struct
{
    tBTA_HF_CLIENT_CPBR_CNT    count;
    tBTA_HF_CLIENT_CPBR_ENTRY  entry;
} tBTA_HF_CLIENT_CPBR;

/* data associated with supported phonebook memory storage */
#define BT_HFCLIENT_PB_STORAGE_COUNT  12
typedef struct
{
    UINT8                      storage_lookup[BT_HFCLIENT_PB_STORAGE_COUNT];
} tBTA_HF_CLIENT_CPBS;
#endif
...
#if defined(MTK_LINUX_HFP_PHONEBOOK) && (MTK_LINUX_HFP_PHONEBOOK == TRUE)
    tBTA_HF_CLIENT_CPBS             cpbs;
    tBTA_HF_CLIENT_CPBR             cpbr;
#endif
} tBTA_HF_CLIENT;
...
/*******************************************************************************
**
** Function         BTA_HfClientScoClose
**
** Description      Close the currently active SCO connection
**                       IOT handle for AG not closing SCO link
**
** Returns          void
**
*******************************************************************************/
void BTA_HfClientScoClose(UINT16 handle);
