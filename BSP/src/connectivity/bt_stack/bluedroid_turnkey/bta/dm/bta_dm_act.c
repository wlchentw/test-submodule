...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
static void bta_dm_remove_sec_dev_entry(BD_ADDR remote_bd_addr, uint8_t delete_device);
#else
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
static void bta_dm_rem_name_cback(const BD_ADDR bd_addr, UNUSED_ATTR DEV_CLASS dc, BD_NAME bd_name);
#endif
...
// applications.
const UINT16 kAdvertisingEventTypeLegacyConnectable = 0x0013;
const UINT16 kAdvertisingEventTypeLegacyScannable = 0x0012;
const UINT16 kAdvertisingEventTypeLegacyNonConnectable = 0x0010;

UINT16 btm_ble_map_adv_event_properties(tBTA_BLE_ADV_EVT adv_type)
{
    UINT16 properties;
    switch(adv_type)
    {
    case BTA_BLE_CONNECT_EVT:
        properties = kAdvertisingEventTypeLegacyConnectable;
        break;

    /* Scannable undirected advertising */
    case BTM_BLE_DISCOVER_EVT:
        properties = kAdvertisingEventTypeLegacyScannable;
        break;

    /* Non connectable undirected advertising */
    case BTM_BLE_NON_CONNECT_EVT:
        properties = kAdvertisingEventTypeLegacyNonConnectable;
        break;
...
void parseParams(tBTA_BLE_ADV_PARAMS* int_params, tBTM_BLE_ADV_PARAMS* out_params)
{
    out_params->adv_int_max = int_params->adv_int_max;
    out_params->adv_int_min = int_params->adv_int_min;
    out_params->adv_type = int_params->adv_type;
    out_params->channel_map = int_params->channel_map;
    out_params->adv_filter_policy = int_params->adv_filter_policy;
    out_params->tx_power = int_params->tx_power;
    if (int_params->advertising_event_properties & 0x0010)
    {
        out_params->advertising_event_properties = btm_ble_map_adv_event_properties(int_params->adv_type);
    }
    else
    {
        out_params->advertising_event_properties = int_params->advertising_event_properties;
    }
    out_params->primary_advertising_phy = int_params->primary_advertising_phy;
    out_params->secondary_advertising_phy = int_params->secondary_advertising_phy;
    out_params->scan_request_notification_enable = int_params->scan_request_notification_enable;
}

void parseAdvParams(tBTA_BLE_ADVERTISING_PARAMS* int_params, tBTM_BLE_ADVERTISING_PARAMS* out_params)
{
    out_params->adv_int_max = int_params->adv_int_max;
    out_params->adv_int_min = int_params->adv_int_min;
    out_params->adv_type = int_params->adv_type;
    ...
    out_params->adv_filter_policy = int_params->adv_filter_policy;
    out_params->tx_power = int_params->tx_power;
    if (int_params->advertising_event_properties & BLE_EXT_ADV_USE_LEGACY_PDU)
    {
        out_params->advertising_event_properties = btm_ble_map_adv_event_properties(int_params->adv_type);
    }
    else
...
void parsePeriParams(tBTA_BLE_ADVERTISING_SET* int_params, tBTM_BLE_ADVERTISING_SET* out_params)
{
    /*adv param*/
    out_params->adv_params.advertising_event_properties =
        int_params->adv_params.advertising_event_properties;
    out_params->adv_params.adv_int_max = int_params->adv_params.adv_int_max;
    out_params->adv_params.adv_int_min = int_params->adv_params.adv_int_min;
    out_params->adv_params.channel_map = int_params->adv_params.channel_map;
    out_params->adv_params.tx_power = int_params->adv_params.tx_power;
    out_params->adv_params.primary_advertising_phy =
        int_params->adv_params.primary_advertising_phy;
    out_params->adv_params.secondary_advertising_phy =
        int_params->adv_params.secondary_advertising_phy;
    out_params->adv_params.scan_request_notification_enable =
         int_params->adv_params.scan_request_notification_enable;

    /* adv data*/
    out_params->adv_data.adv_len = int_params->adv_data.adv_len;
    if (int_params->adv_data.adv_len != 0)
    {
        memcpy(out_params->adv_data.adv_data,
        int_params->adv_data.adv_data, int_params->adv_data.adv_len);
    }

    /*adv scan response data*/
    out_params->adv_scan_rsp_data.adv_len = int_params->adv_scan_rsp_data.adv_len;
    if (int_params->adv_scan_rsp_data.adv_len != 0)
    {
        memcpy(out_params->adv_scan_rsp_data.adv_data,
        int_params->adv_scan_rsp_data.adv_data, int_params->adv_scan_rsp_data.adv_len);
    }

    /*periodic param*/
    out_params->peri_params.enable = int_params->peri_params.enable;
    out_params->peri_params.max_interval = int_params->peri_params.max_interval;
    out_params->peri_params.min_interval = int_params->peri_params.min_interval;
    out_params->peri_params.periodic_advertising_properties =
        int_params->peri_params.periodic_advertising_properties;

     /*periodic adv data*/
    APPL_TRACE_DEBUG("%s:peri_adv_data.adv_len=%d", __func__, int_params->peri_adv_data.adv_len);
    out_params->peri_adv_data.adv_len = int_params->peri_adv_data.adv_len;
    if (int_params->peri_adv_data.adv_len != 0)
    {
        memcpy(out_params->peri_adv_data.adv_data,
        int_params->peri_adv_data.adv_data, int_params->peri_adv_data.adv_len);
    }
    out_params->duration = int_params->duration;
    out_params->maxExtAdvEvents = int_params->maxExtAdvEvents;
}
#endif
...
void bta_dm_remove_device(tBTA_DM_MSG *p_data)
{
    ...
    /* if remove an unpair device, it will report twice */
    if (continue_delete_dev)
    {
        bta_dm_process_remove_device(p_dev->bd_addr);
        /* if it is same, it will remove again, so reset the flag */
        if ((bdcmp(other_address, p_dev->bd_addr) == 0)
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            bta_dm_remove_sec_dev_entry(p_data->bond.bd_addr, TRUE);
#else
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        bta_dm_remove_sec_dev_entry(bd_addr, FALSE);
#else
...
**
*******************************************************************************/
static void bta_dm_rem_name_cback(const BD_ADDR bd_addr,
...
void bta_dm_encrypt_cback(BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data, tBTM_STATUS result)
{
    ...
    switch (result)
    {
        ...
        case BTM_ERR_KEY_MISSING:
            bta_status = BTM_ERR_KEY_MISSING;
            break;
#endif
        default:
...
;
    ...
    result.inq_res.dev_class[0] = p_inq->dev_class[0];
    result.inq_res.dev_class[1] = p_inq->dev_class[1];
    result.inq_res.dev_class[2] = p_inq->dev_class[2];
#endif
#endif
    result.inq_res.flag             = p_inq->flag;
    ...
    switch (event)
    {
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
#if (BTM_LOCAL_IO_CAPS_BLE != BTM_IO_CAP_NONE)
        ...
        case BTM_LE_COMPLT_EVT:
            ...
            if (p_data->complt.reason != 0)
            {
                ...
                /* delete this device entry from Sec Dev DB */
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                bta_dm_remove_sec_dev_entry (bda, FALSE);
#else
...
void bta_dm_ble_multi_adv_enb(tBTA_DM_MSG *p_data)
{
    ...
    if(BTM_BleMaxMultiAdvInstanceCount() > 0 && NULL != p_data->ble_multi_adv_enb.p_ref)
    {
        ...
        parseParams(p_data->ble_multi_adv_enb.p_params, &params);
        btm_status = BTM_BleEnableAdvInstance(&params,
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
        tBTM_BLE_ADV_PARAMS params;
        parseParams(p_data->ble_multi_adv_param.p_params, &params);
...
{
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void bta_dm_ble_ext_adv_data(tBTA_DM_MSG *p_data)
{
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
    btm_ble_extended_adv_set_data(p_data->ble_ext_adv_data.client_if,
                                  p_data->ble_ext_adv_data.is_scan_rsp,
                                  p_data->ble_ext_adv_data.data_mask,
                                  (tBTM_BLE_ADV_DATA*)&p_data->ble_ext_adv_data.data);

}

void bta_dm_ble_start_advertising_set(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    void *p_ref = NULL;
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
    bta_dm_cb.p_multi_adv_cback = p_data->ble_start_advertising_set.p_cback;
    if (BTM_BleMaxMultiAdvInstanceCount() > 0 && NULL != p_data->ble_start_advertising_set.p_ref)
    {
        tBTM_BLE_ADVERTISING_SET start_advertising_set;
        memset(&start_advertising_set, 0, sizeof(tBTM_BLE_ADVERTISING_SET));
        parsePeriParams(p_data->ble_start_advertising_set.p_adv_info, &start_advertising_set);
        btm_status = BTM_BleStartAdvertisingSet(&start_advertising_set,
                                            p_data->ble_multi_adv_enb.p_cback,
                                            p_data->ble_multi_adv_enb.p_ref);
    }
    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref =  p_data->ble_start_advertising_set.p_ref;
       bta_dm_cb.p_multi_adv_cback(BTA_BLE_ADV_START_ADVERTISING_SET_EVT,
                                      0xFF, p_ref, BTA_FAILURE);
    }
}

void bta_dm_ble_set_peri_param(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    UINT8 inst_id = 0;
    void *p_ref = NULL;
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
    bta_dm_cb.p_multi_adv_cback = p_data->ble_set_peri_param.p_cback;

    if (BTM_BleMaxMultiAdvInstanceCount() > 0 && NULL != p_data->ble_start_advertising_set.p_ref)
    {
        tBTM_BLE_PERIODIC_ADV_PARAMS adv_peri_param;
        memset(&adv_peri_param, 0, sizeof(tBTM_BLE_PERIODIC_ADV_PARAMS));
        inst_id = p_data->ble_set_peri_param.inst_id;
        memcpy(&adv_peri_param, p_data->ble_set_peri_param.p_peri_param, sizeof(tBTM_BLE_PERIODIC_ADV_PARAMS));
        btm_status = BTM_BlePeriodicAdvsetParams(inst_id, &adv_peri_param, 0);
    }
    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref = btm_ble_multi_adv_get_ref(p_data->ble_set_peri_param.inst_id);
       bta_dm_cb.p_multi_adv_cback(BTA_BLE_ADV_SET_PERI_PARAM_EVT,
                                   p_data->ble_set_peri_param.inst_id, p_ref, BTA_FAILURE);
    }
}

void bta_dm_ble_peri_adv_enable(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    UINT8 inst_id = 0;
    UINT8 enable = 0;
    void *p_ref = NULL;
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
    bta_dm_cb.p_multi_adv_cback = p_data->ble_peri_adv_enable.p_cback;

    inst_id = p_data->ble_peri_adv_enable.inst_id;
    enable = p_data->ble_peri_adv_enable.enable;

    btm_status = BTM_BlePeriodicAdvEnable(inst_id, enable);
    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref = btm_ble_multi_adv_get_ref(inst_id);
       bta_dm_cb.p_multi_adv_cback(BTA_BLE_ADV_PERI_ENABLE_EVT,
                                   inst_id, p_ref, BTA_FAILURE);
    }
}

void bta_dm_ble_set_advertising_data(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    UINT8 inst_id = 0;
    tBTA_BLE_ADV_TYPE adv_data_type = BTA_BLE_ADV_DATA_TYPE;
    void *p_ref = NULL;
    APPL_TRACE_DEBUG("%s:%d", __func__, __LINE__);
    bta_dm_cb.p_multi_adv_cback = p_data->ble_set_advertising_data.p_cback;

    if (BTM_BleMaxMultiAdvInstanceCount() > 0 && NULL != p_data->ble_set_advertising_data.p_ref)
    {
        tBTM_BLE_PERI_ADV_DATA advertising_data;
        memset(&advertising_data, 0, sizeof(tBTM_BLE_PERI_ADV_DATA));
        inst_id = p_data->ble_set_advertising_data.inst_id;
        adv_data_type = p_data->ble_set_advertising_data.adv_data_type;
        memcpy(&advertising_data, p_data->ble_set_advertising_data.p_adv_data, sizeof(tBTA_BLE_PERI_ADV_DATA));
        btm_status = BTM_BleExtOrMultiAdvSetdata(adv_data_type, inst_id,
                           &advertising_data.adv_data, advertising_data.adv_len);
    }
    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref = btm_ble_multi_adv_get_ref(p_data->ble_multi_adv_disable.inst_id);
       if (adv_data_type == BTA_BLE_ADV_DATA_PERIODIC_TYPE)
       {
           bta_dm_cb.p_multi_adv_cback(BTA_BLE_ADV_SET_PERI_DATA_EVT,
...
void bta_dm_ble_stop_advertising_set(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    void *p_ref = NULL;
    ...
    {
        btm_status = BTM_BleStopAdvertisingSet(p_data->ble_multi_adv_disable.inst_id);
    }

    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref = btm_ble_multi_adv_get_ref(p_data->ble_multi_adv_disable.inst_id);
       bta_dm_cb.p_multi_adv_cback(BTA_BLE_ADV_STOP_ADVERTISING_SET_EVT,
                                   p_data->ble_multi_adv_disable.inst_id, p_ref, BTA_FAILURE);
    }
}

void bta_dm_ble_advertising_set_param(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    void *p_ref = NULL;

    if (BTM_BleMaxMultiAdvInstanceCount() > 0 && p_data->ble_adv_set_param.inst_id > 0
        && p_data->ble_adv_set_param.inst_id < BTM_BleMaxMultiAdvInstanceCount())
    {
        tBTM_BLE_ADVERTISING_PARAMS params;
        parseAdvParams(p_data->ble_adv_set_param.p_params, &params);
        btm_status = BTM_BleAdvertisingSetParam(p_data->ble_adv_set_param.inst_id,
                                               &params);
    }
    if (BTM_CMD_STARTED != btm_status)
    {
       p_ref = btm_ble_multi_adv_get_ref(p_data->ble_adv_set_param.inst_id);
       bta_dm_cb.p_multi_adv_cback(BTA_BLE_MULTI_ADV_PARAM_EVT,
                                   p_data->ble_adv_set_param.inst_id, p_ref, BTA_FAILURE);
    }
}
#endif
/*******************************************************************************
