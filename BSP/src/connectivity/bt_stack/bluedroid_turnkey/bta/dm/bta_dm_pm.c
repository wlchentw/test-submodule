                               ...
                               tBTA_DM_PM_REQ pm_req )
{
    ...
    if ((pm_req != BTA_DM_PM_EXECUTE) && (timeout_ms > 0))
    {
        for (i=0; i<BTA_DM_NUM_PM_TIMER; i++)
        {
            ...
            {
                APPL_TRACE_DEBUG("%s dm_pm_timer:%d, %lu ms", __func__, i,
                                 timeout_ms);
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        if ((BTM_GetRole(p_peer_dev->peer_bdaddr, &role) == BTM_SUCCESS) &&
            (role == BTM_ROLE_SLAVE))
        {
            APPL_TRACE_DEBUG("%s: bta_dm_pm_sniff current role is %d", __func__, role);
            return TRUE;
        }
#endif
#endif
