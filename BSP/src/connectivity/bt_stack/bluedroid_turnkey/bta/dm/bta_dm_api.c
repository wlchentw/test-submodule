...
void BTA_BleStartAdvertisingSet (tBTA_BLE_ADVERTISING_SET *p_start_advertising_set,
                                  tBTA_BLE_MULTI_ADV_CBACK *p_cback,
                                  void *p_ref)
{
    const size_t len = sizeof(tBTA_BLE_ADVERTISING_SET) +
        sizeof(tBTA_DM_API_BLE_START_ADVERTISING_SET);
    tBTA_DM_API_BLE_START_ADVERTISING_SET *p_msg =
        (tBTA_DM_API_BLE_START_ADVERTISING_SET *)osi_calloc(len);

    APPL_TRACE_API("%s", __func__);

    p_msg->hdr.event = BTA_DM_API_BLE_START_ADVERTISING_SET_EVT;
    p_msg->p_cback = (void *)p_cback;
    if (p_start_advertising_set != NULL) {
        p_msg->p_adv_info = (void *)(p_msg + 1);
        memcpy(p_msg->p_adv_info, p_start_advertising_set, sizeof(tBTA_BLE_ADVERTISING_SET));
    }
    p_msg->p_ref = p_ref;
    bta_sys_sendmsg(p_msg);
}

void BTA_BleAdvSetPeriodicParam (UINT8 inst_id,
                                  tBTA_BLE_PERIODIC_ADV_PARAMS *p_peri_param,
                                  tBTA_BLE_MULTI_ADV_CBACK *p_cback,
                                  void *p_ref)
{
    const size_t len = sizeof(tBTA_BLE_PERIODIC_ADV_PARAMS) +
        sizeof(tBTA_DM_API_BLE_PERI_PARAM);
    tBTA_DM_API_BLE_PERI_PARAM *p_msg =
        (tBTA_DM_API_BLE_PERI_PARAM *)osi_calloc(len);
    APPL_TRACE_API("%s", __func__);

    p_msg->hdr.event = BTA_DM_API_BLE_ADV_SET_PERI_PARAM_EVT;
    p_msg->p_cback = (void *)p_cback;
    p_msg->inst_id = inst_id;
    if (p_peri_param != NULL) {
        p_msg->p_peri_param = (void *)(p_msg + 1);
        memcpy(p_msg->p_peri_param, p_peri_param, sizeof(tBTA_BLE_PERIODIC_ADV_PARAMS));
    }
    p_msg->p_ref = p_ref;
    bta_sys_sendmsg(p_msg);
}

void BTA_BleAdvPeriodicEnable (UINT8 inst_id, UINT8 enable,
                                  tBTA_BLE_MULTI_ADV_CBACK *p_cback,
                                  void *p_ref)
{
    tBTA_DM_API_BLE_PERI_ADV_ENABLE *p_msg =
        (tBTA_DM_API_BLE_PERI_ADV_ENABLE *)osi_calloc(sizeof(tBTA_DM_API_BLE_PERI_ADV_ENABLE));
    APPL_TRACE_API("%s", __func__);

    p_msg->hdr.event = BTA_DM_API_BLE_ADV_PERI_ENABLE_EVT;
    p_msg->p_cback = (void *)p_cback;
    p_msg->inst_id = inst_id;
    p_msg->enable = enable;
    p_msg->p_ref = p_ref;
...
void BTA_BleAdvSetAdvertisingData (UINT8 inst_id, UINT8 adv_data_type,
                                  tBTA_BLE_PERI_ADV_DATA *p_advertising_data,
                                  tBTA_BLE_MULTI_ADV_CBACK *p_cback,
                                  void *p_ref)
{
    const size_t len = sizeof(tBTA_BLE_PERI_ADV_DATA) +
        sizeof(tBTA_DM_API_BLE_ADVERTISING_DATA);
    tBTA_DM_API_BLE_ADVERTISING_DATA *p_msg =
        (tBTA_DM_API_BLE_ADVERTISING_DATA *)osi_calloc(len);

    APPL_TRACE_API("%s", __func__);

    p_msg->hdr.event = BTA_DM_API_BLE_ADV_SET_ADVERTISING_DATA_EVT;
    p_msg->p_cback = (void *)p_cback;
    p_msg->inst_id = inst_id;
    p_msg->adv_data_type = adv_data_type;
    if (p_advertising_data != NULL) {
        p_msg->p_adv_data = (void *)(p_msg + 1);
        memcpy(p_msg->p_adv_data, p_advertising_data, sizeof(tBTA_BLE_PERI_ADV_DATA));
    }
    p_msg->p_ref = p_ref;
    bta_sys_sendmsg(p_msg);
}

void BTA_BleStopAdvertisingSet(UINT8 inst_id)
{
    ...
    p_msg->hdr.event = BTA_DM_API_BLE_STOP_ADVERTISING_SET_EVT;
    p_msg->inst_id = inst_id;

    bta_sys_sendmsg(p_msg);
}

void BTA_BleAdvertisingSetParam (UINT8 inst_id, tBTA_BLE_ADVERTISING_PARAMS *p_params)
{
    const size_t len = sizeof(tBTA_BLE_ADVERTISING_PARAMS) +
        sizeof(tBTA_DM_API_BLE_ADVERTISING_PARAM);
    tBTA_DM_API_BLE_ADVERTISING_PARAM *p_msg =
        (tBTA_DM_API_BLE_ADVERTISING_PARAM *)osi_calloc(len);

    APPL_TRACE_API("%s", __func__);

    p_msg->hdr.event = BTA_DM_API_BLE_ADV_SET_PARAM_EVT;
    p_msg->inst_id = inst_id;
    p_msg->p_params = (void *)(p_msg + 1);
    memcpy(p_msg->p_params, p_params, sizeof(tBTA_BLE_ADVERTISING_PARAMS));
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void BTA_BleSetExtAdvData (UINT32 client_if, BOOLEAN is_scan_rsp,
                            tBTA_BLE_AD_MASK data_mask,
                            tBTA_BLE_ADV_DATA *p_data)
{
  tBTA_DM_API_BLE_EXT_ADV_DATA *p_msg = osi_calloc(sizeof(*p_msg));

  p_msg->hdr.event = BTA_DM_API_BLE_EXT_ADV_DATA_EVT;
  p_msg->client_if = client_if;
  p_msg->is_scan_rsp = is_scan_rsp;
  p_msg->data_mask = data_mask;
  memcpy(&p_msg->data, p_data, sizeof(p_msg->data));
