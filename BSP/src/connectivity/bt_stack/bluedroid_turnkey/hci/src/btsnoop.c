...
static void update_logging() {
  ...
  if (should_log) {
    ...
    int max_file_num = stack_config->get_btsnoop_max_file_num();

#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
    if (twrite_init(log_path, log_mode, &logfile_fd, max_file_num)) {
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
  if ((int)length != twrite_write(data, length, 0))
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
static void btsnoop_write_in_thread(const void *data, size_t length, int last_frag) {
  if ((int)length != twrite_write(data, length, last_frag))
    LOG_ERROR(LOG_TAG, "%s twrite_write data failed", __func__);
}
#endif
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
  length = length_he;
  twrite_write_packet(btsnoop_write_in_thread, type, packet, length_he, length, flags,
