...
static void transmit_fragment(BT_HDR *packet, bool send_transmit_finished) {
  ...
  if (event == MSG_STACK_TO_HC_HCI_CMD)
    bperf_notify_cmd(packet->data+packet->offset, packet->len);
  else if (event == MSG_STACK_TO_HC_HCI_ACL)
    bperf_notify_data(packet->data+packet->offset, packet->len);
#endif
...
static void command_timed_out(UNUSED_ATTR void *context) {
  ...
  BT_HDR* bt_hdr =
      (BT_HDR *)(osi_malloc(sizeof(BT_HDR) + HCIC_PREAMBLE_SIZE));
  bt_hdr->len = HCIC_PREAMBLE_SIZE;
  bt_hdr->event = MSG_STACK_TO_HC_HCI_CMD;
  bt_hdr->offset = 0;
  uint8_t* hci_packet = (uint8_t*)(bt_hdr + 1);
  ...
  LOG_ERROR(LOG_TAG, "%s send fw dump cmd end", __func__);
  usleep(500000); //500ms
...
static void hal_says_data_ready(serial_data_type_t type) {
  ...
  while (hal->read_data(type, &byte, 1) != 0) {
    switch (incoming->state) {
      ...
      case BODY:
        if (!incoming->buffer) {
            LOG_ERROR(LOG_TAG, "%s error getting buffer for incoming packet of type %d", __func__, type);
            break;
        }
...
#if defined(MTK_BT_PERFORMANCE_ANALYSIS) && (MTK_BT_PERFORMANCE_ANALYSIS == TRUE)
        bperf_notify_data(incoming->buffer->data, incoming->buffer->len);
#endif
...
#if defined(MTK_BT_PERFORMANCE_ANALYSIS) && (MTK_BT_PERFORMANCE_ANALYSIS == TRUE)
        bperf_notify_event(incoming->buffer->data, incoming->buffer->len);
#endif
