...
#define UHID_EVENT_SIZE_3_18    4380
        ...
        if (ret < (ssize_t)sizeof(ev)) {
            APPL_TRACE_ERROR("%s: Invalid size read from uhid-dev: %ld != %u",
...
#if defined(MTK_LINUX_HID_KEY_QUEUE) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        // Not set flag, avoid Key coming between UHID_START and UHID_OPEN,
        // then cause the key can not be cached.
#else
    ...
    case UHID_OPEN:
        ...
        pthread_mutex_lock(&p_dev->lock);

        APPL_TRACE_DEBUG("UHID_OPEN before wait.\n");

        p_dev->ready_for_data = TRUE;
        while (!fixed_queue_is_empty(p_dev->keys_queue))
        {
            uinput_type *uinput = fixed_queue_dequeue(p_dev->keys_queue);
            APPL_TRACE_WARNING("%s: dequeue RC keys", __func__);
            char *ptr = (char *)uinput->data;
            int ret_val = 0, i = 0;

            for (i = 0; i < uinput->len; ++i)
            {
                APPL_TRACE_WARNING("[%d] = %x", i, *(ptr + i));
            }

            ret_val = bta_hh_co_write(uinput->fd, uinput->data, uinput->len);
            if (ret_val < 0)
            {
                APPL_TRACE_ERROR("%s: key is still failed to inject", __func__);
            }
            else
            {
                APPL_TRACE_WARNING("%s: key is injected successfully", __func__);
            }

            osi_free(uinput->data);
            osi_free(uinput);
        }

        APPL_TRACE_DEBUG("UHID_OPEN before unlock.\n");
        pthread_mutex_unlock(&p_dev->lock);
        APPL_TRACE_DEBUG("UHID_OPEN unlock.\n");
#else
        p_dev->ready_for_data = TRUE;
#endif /* defined(MTK_LINUX_HID_KEY_QUEUE) && defined(MTK_COMMON) && (MTK_COMMON == TRUE) */
...
#if defined(MTK_LINUX_HID_KEY_QUEUE) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                p_dev->keys_queue = fixed_queue_new(SIZE_MAX);
                pthread_mutex_init(&p_dev->lock, NULL);
                if (p_dev->keys_queue == NULL) {
                    BTIF_TRACE_ERROR("%s unable to create queue for RC keys", __func__);
                }
#endif /* defined(MTK_LINUX_HID_KEY_QUEUE) && defined(MTK_COMMON) && (MTK_COMMON == TRUE) */
                    ...
                    UINT8 sub_class, UINT8 ctry_code, BD_ADDR peer_addr, UINT8 app_id)
{
    ...
    if (p_dev->fd >= 0) {
        ...
        /*
             * Sometimes UHID_INPUT is earlier than UHID_OPEN for RC, we should buffer keys in either case:
             * 1. ready_for_data is true
             */
        pthread_mutex_lock(&p_dev->lock);
        uinput_type *uinput;
        ...
        APPL_TRACE_WARNING("%s: enqueue RC keys", __FUNCTION__);
        if ((fixed_queue_length(p_dev->keys_queue) <= 9))
        {
            uinput = (uinput_type *)osi_calloc(sizeof(uinput_type));
            uinput->data = (UINT8 *)osi_calloc(len);
            uinput->fd = p_dev->fd;
            uinput->len = len;
            memcpy(uinput->data, p_rpt, len);
            fixed_queue_enqueue(p_dev->keys_queue, (void *)uinput);
        }
        else
        {
            APPL_TRACE_ERROR("%s: enqueue too much keys, number:%d.",
                __FUNCTION__, fixed_queue_length(p_dev->keys_queue));
        }
                                                   ...
                                                   UINT8 *p_num_rpt, UINT8 app_id)
{
    ...
    {
        BTIF_TRACE_WARNING("%s p_num_rpt=0x%p, len=%d", __func__, p_num_rpt, len);
        return NULL;
    }
