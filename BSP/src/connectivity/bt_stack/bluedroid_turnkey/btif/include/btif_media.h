...
#define BTIF_AV_PRINT_BDA(pbda, s, ...) \
    do { char buf[18]; bdaddr_to_string(pbda, buf, sizeof(buf));\
        BTIF_TRACE_DEBUG("%s@%d addr=%s "s, __FUNCTION__, __LINE__, buf, ##__VA_ARGS__); \
    }while(0)

#define BTIF_AV_PRINT_BDA_WARNING(pbda, s, ...) \
    do { char buf[18]; bdaddr_to_string(pbda, buf, sizeof(buf));\
        BTIF_TRACE_WARNING("%s@%d addr=%s "s, __FUNCTION__, __LINE__, buf, ##__VA_ARGS__); \
    }while(0)
#endif
