...
extern alarm_t *bt_gap_scan_timer;
#endif
...
typedef void (tBTIF_CBACK_BY_BDA) (UINT16 event, char *p_param, bt_bdaddr_t* bd_addr, UINT32 handle);
#endif
...
{
    BT_HDR               hdr;
    tBTIF_CBACK_BY_BDA*  p_cb;    /* context switch callback */
    bt_bdaddr_t          bd_addr;
    UINT32               handle;
    ...
    UINT16               event;   /* message event id */
    char __attribute__ ((aligned)) p_param[]; /* parameter area needs to be last */
} tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK;
#endif
...
bt_status_t btif_transfer_context_by_bda (tBTIF_CBACK_BY_BDA *p_cback, UINT16 event,
    char* p_params, int param_len, tBTIF_COPY_CBACK *p_copy_cback, bt_bdaddr_t* bd_addr, UINT32 handle);
#endif
