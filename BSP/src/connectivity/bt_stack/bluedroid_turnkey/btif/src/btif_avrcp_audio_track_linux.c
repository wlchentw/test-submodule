...
EXPORT_SYMBOL int BtifAvrcpAudioTrackInit(BtifAvrcpAudioTrack *pt_track_cb)
{
    ...
    if(NULL == pt_track_cb)
    {
        BTIF_TRACE_ERROR( "%s invalid callback func pointer", __func__);
        return -1;
    }
...
{
    BTIF_TRACE_API( "%s btCreateTrack freq %d  channel %d bitdepth %d",
                     __func__, trackFreq, channelType&0xFFFF, (channelType>>16));
    ...
    {
        BTIF_TRACE_WARNING( "null %s", __func__);
    }
    return (void *)NULL;
}
...
void BtifAvrcpAudioTrackStart(void *handle)
{
    ...
    BTIF_TRACE_API( "%s btStartTrack", __func__);
...
void BtifAvrcpAudioTrackStop(void *handle)
{
    if (handle == NULL)
    {
        BTIF_TRACE_WARNING( "%s handle is null.", __func__);
        return;
    }
...
int BtifAvrcpAudioTrackWriteData(void *handle, void *audioBuffer, int bufferlen)
{
    ...
    if (trackHolder && trackHolder->linux_bt_pb_push_data)
    {
        ...
        //BTIF_TRACE_VERBOSE( "%s btWriteData len = %d ret = %d",
