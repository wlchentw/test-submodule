...
btgatt_multi_adv_common_data *btif_obtain_multi_adv_data_cb()
{
    ...
    if (NULL == p_multi_adv_com_data_cb)
    {
        ...
        memset(p_multi_adv_com_data_cb->inst_cb, 0, (max_adv_inst + 1) * sizeof(btgatt_multi_adv_inst_cb));
#endif
    }
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
BOOLEAN btif_gattc_copy_ext_datacb(const btif_adv_data_t *p_adv_data,
                                   btgatt_ext_adv_data_t *p_ext_adv_data)
{
    BTIF_TRACE_DEBUG("%s", __func__);
    memset(p_ext_adv_data, 0, sizeof(btgatt_ext_adv_data_t));

    if (!p_adv_data->set_scan_rsp)
    {
         p_ext_adv_data->mask = BTM_BLE_AD_BIT_FLAGS;
         p_ext_adv_data->data.flag = ADV_FLAGS_GENERAL;
         if (p_ext_adv_data->data.flag == 0)
            p_ext_adv_data->mask = 0;
    }

    if (p_adv_data->include_name)
        p_ext_adv_data->mask |= BTM_BLE_AD_BIT_DEV_NAME;

    if (p_adv_data->include_txpower)
        p_ext_adv_data->mask |= BTM_BLE_AD_BIT_TX_PWR;

    if (p_adv_data->appearance != 0)
    {
        p_ext_adv_data->mask |= BTM_BLE_AD_BIT_APPEARANCE;
        p_ext_adv_data->data.appearance = p_adv_data->appearance;
    }
    ...
    {
      p_ext_adv_data->mask |= BTM_BLE_AD_BIT_MANU;
      p_ext_adv_data->data.manu.len =
          p_adv_data->manufacturer_len;
      memcpy(&p_ext_adv_data->data.manu.val,
        ...
        p_adv_data->service_data_len < MAX_SIZE_PROPRIETARY_ELEMENT)
    {
      ...
      tBTA_BLE_PROPRIETARY *p_prop = &p_ext_adv_data->data.proprietary;
      p_prop->num_elem = 1;
      ...
      p_ext_adv_data->mask |= BTM_BLE_AD_BIT_PROPRIETARY;
    }
    ...
    if (p_adv_data->service_uuid_len && p_adv_data->p_service_uuid)
    {
        ...
        for (int position = 0; position < p_adv_data->service_uuid_len; position += LEN_UUID_128)
        {
             ...
             switch(bt_uuid.len)
             {
                case (LEN_UUID_16):
                {
                  if (p_ext_adv_data->data.services.num_service == 0)
                  {
                      p_ext_adv_data->data.services.list_cmpl = FALSE;
                      p_uuid_out16 = p_ext_adv_data->data.services.uuid;
                  }

                  if (p_ext_adv_data->data.services.num_service < MAX_16BIT_SERVICES)
                  {
                     BTIF_TRACE_DEBUG("%s - In 16-UUID_data", __func__);
                     p_ext_adv_data->mask |= BTM_BLE_AD_BIT_SERVICE;
                     ++p_ext_adv_data->data.services.num_service;
                ...
                case (LEN_UUID_32):
                {
                   if (p_ext_adv_data->data.service_32b.num_service == 0)
                   {
                      p_ext_adv_data->data.service_32b.list_cmpl = FALSE;
                      p_uuid_out32 = p_ext_adv_data->data.service_32b.uuid;
                   }

                   if (p_ext_adv_data->data.service_32b.num_service < MAX_32BIT_SERVICES)
                   {
                      BTIF_TRACE_DEBUG("%s - In 32-UUID_data", __func__);
                      p_ext_adv_data->mask |= BTM_BLE_AD_BIT_SERVICE_32;
                      ++p_ext_adv_data->data.service_32b.num_service;
                ...
                case (LEN_UUID_128):
                {
                   ...
                   if (p_ext_adv_data->data.services_128b.num_service == 0)
                   {
                     BTIF_TRACE_DEBUG("%s - In 128-UUID_data", __FUNCTION__);
                     p_ext_adv_data->mask |=
                         BTM_BLE_AD_BIT_SERVICE_128;
                     memcpy(p_ext_adv_data->data.services_128b.uuid128,
                     ...
                     ++p_ext_adv_data->data.services_128b.num_service;
                     p_ext_adv_data->data.services_128b.list_cmpl = TRUE;
                   }
                   break;
                }
