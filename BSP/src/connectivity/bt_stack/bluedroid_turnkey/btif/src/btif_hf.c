                                            ...
                                            const char *number, bthf_call_addrtype_t type)
{
    ...
    if (interop_mtk_match_addr_name(INTEROP_MTK_HFP_DEALY_OPEN_SCO, (const bt_bdaddr_t *)&(btif_hf_cb[idx].connected_bda))) {
        if ((num_active == 0 && num_held == 0)
            ...
            if (0x3 == btif_sm_get_state(btif_av_get_sm_handle_by_bda((const bt_bdaddr_t *)&(btif_hf_cb[idx].connected_bda)))) {
#else
...
/*******************************************************************************
**
** Function         btif_hf_connect_disconnect
**
** Description      Connect/Disconnect HFP
**
** Returns          bt_status_t
**
*******************************************************************************/
#include "osi/include/properties.h"
bt_status_t btif_hf_connect_disconnect()
{
    char prop_value_char[PROPERTY_VALUE_MAX];;
    bt_bdaddr_t addr;

    if (osi_property_get("bt.hfp.conn", prop_value_char, ""))
    {
        ...
        else if (strlen(prop_value_char) == 17)
        {
            ...
            {
                BTIF_TRACE_DEBUG("%s, start HFP connect : %s", __FUNCTION__, prop_value_char);
                osi_property_set("bt.hfp.conn", "");
                connect(&addr);
                return BT_STATUS_SUCCESS;
            }
            else
    ...
    if (osi_property_get("bt.hfp.disc", prop_value_char, ""))
    {
        if (strlen(prop_value_char) == 0)
        {
            // Do nothing
        ...
        else if (strlen(prop_value_char) == 17)
        {
            if ( string_to_bdaddr(prop_value_char, &addr) )
            {
                BTIF_TRACE_DEBUG("%s, start HFP disconnect : %s", __FUNCTION__, prop_value_char);
                osi_property_set("bt.hfp.disc", "");
                disconnect(&addr);
                return BT_STATUS_SUCCESS;
            }
            else
    ...
    if (osi_property_get("bt.hfp.audio.conn", prop_value_char, ""))
    {
        if (strlen(prop_value_char) == 0)
        {
            // Do nothing
        ...
        else if (strlen(prop_value_char) == 17)
        {
            if ( string_to_bdaddr(prop_value_char, &addr) )
            {
                BTIF_TRACE_DEBUG("%s, start HFP_Audio connect : %s", __FUNCTION__, prop_value_char);
                osi_property_set("bt.hfp.audio.conn", "");
                connect_audio(&addr);
                return BT_STATUS_SUCCESS;
            }
            else
    ...
    if (osi_property_get("bt.hfp.audio.disc", prop_value_char, ""))
    {
        if (strlen(prop_value_char) == 0)
        {
            // Do nothing
        ...
        else if (strlen(prop_value_char) == 17)
        {
            if ( string_to_bdaddr(prop_value_char, &addr) )
            {
                BTIF_TRACE_DEBUG("%s, start HFP_AUDIO disconnect : %s", __FUNCTION__, prop_value_char);
                osi_property_set("bt.hfp.audio.disc", "");
                disconnect_audio(&addr);
                return BT_STATUS_SUCCESS;
            }
            else
                goto conn_disconn_error;
...
conn_disconn_error:
    BTIF_TRACE_DEBUG("%s, Usage (start HFP connect) : setprop bt.hfp.conn XX:XX:XX:XX:XX:XX", __FUNCTION__);
    BTIF_TRACE_DEBUG("%s, Usage (start HFP disconnect) : setprop bt.hfp.disc XX:XX:XX:XX:XX:XX", __FUNCTION__);
    BTIF_TRACE_DEBUG("%s, Usage (start HFP_Audio connect) : setprop bt.hfp.audio.conn XX:XX:XX:XX:XX:XX", __FUNCTION__);
    BTIF_TRACE_DEBUG("%s, Usage (start HFP_Audio disconnect) : setprop bt.hfp.audio.disc XX:XX:XX:XX:XX:XX", __FUNCTION__);
    osi_property_set("bt.hfp.conn", "");
    osi_property_set("bt.hfp.disc", "");
    osi_property_set("bt.hfp.audio.conn", "");
    osi_property_set("bt.hfp.audio.disc", "");
