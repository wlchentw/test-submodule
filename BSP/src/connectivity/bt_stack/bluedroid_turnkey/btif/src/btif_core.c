...
static void btif_context_switched_by_bda(void *p_msg)
{

    BTIF_TRACE_VERBOSE("btif_context_switched");

    tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK *p = (tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK *) p_msg;

    /* each callback knows how to parse the data */
    if (p->p_cb)
        p->p_cb(p->event, p->p_param, &p->bd_addr, p->handle);
}
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
bt_status_t btif_transfer_context_by_bda (tBTIF_CBACK_BY_BDA *p_cback, UINT16 event,
    char* p_params, int param_len, tBTIF_COPY_CBACK *p_copy_cback,
    bt_bdaddr_t* bd_addr, UINT32 handle)
{
    tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK *p_msg =
        (tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK *)osi_malloc(sizeof(tBTIF_CONTEXT_SWITCH_BY_BDA_CBACK) + param_len);

    BTIF_TRACE_VERBOSE("btif_transfer_context event %d, len %d", event, param_len);

    /* allocate and send message that will be executed in btif context */
    p_msg->hdr.event = BT_EVT_CONTEXT_SWITCH_BY_BDA_EVT; /* internal event */
    p_msg->p_cb = p_cback;
    if (NULL != bd_addr)
    {
        bdaddr_copy(&p_msg->bd_addr, bd_addr);
    }
    else
    {
        memset(&p_msg->bd_addr, 0, sizeof(p_msg->bd_addr));
    }
    p_msg->handle = handle;
...
static void bt_jni_msg_ready(void *context) {
  ...
  switch (p_msg->event) {
    case BT_EVT_CONTEXT_SWITCH_EVT:
      ...
      break;
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    case BT_EVT_CONTEXT_SWITCH_BY_BDA_EVT:
        btif_context_switched_by_bda(p_msg);
        break;
#endif
    default:
...
void btif_enable_bluetooth_evt(tBTA_STATUS status)
{
    ...
    if (status == BTA_SUCCESS)
    {
        ...
        const char *did_conf_path = BTE_DID_CONF_FILE;
        if((did_conf_path == NULL) || access(did_conf_path, R_OK)) {
            if(!access("/etc/bluetooth/bt_did.conf", R_OK)) {
                did_conf_path = "/etc/bluetooth/bt_did.conf";
            }
        }
        bte_load_did_conf(did_conf_path);
#else
        /* load did configuration */
...
static void btif_jni_associate(UNUSED_ATTR uint16_t event, UNUSED_ATTR char *p_param) {
  ...
  if (bt_hal_cbacks && bt_hal_cbacks->thread_evt_cb)
  {
