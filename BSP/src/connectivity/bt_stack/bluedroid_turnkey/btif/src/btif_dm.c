...
static void btif_dm_update_rmt_device_name(const BD_ADDR bd_addr, BD_NAME bd_name);
#endif
/******************************************************************************
**  Constants & Macros
******************************************************************************/
                                ...
                                UINT8 *p_remote_name, UINT8 *p_remote_name_len)
{
        ...
        &remote_bdaddr, &prop_name) == BT_STATUS_SUCCESS)
    {
        if (p_remote_name && p_remote_name_len)
        {
            strncpy((char *)p_remote_name, (char *)bdname.name, sizeof(bdname.name));
            p_remote_name[sizeof(bdname.name) - 1] = '\0';
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) && (BTM_LOCAL_IO_CAPS == BTM_IO_CAP_NONE)
          bond_state_changed(BT_STATUS_SUCCESS, &bd_addr, BT_BOND_STATE_BONDING);
          btif_storage_set_remote_addr_type(&bd_addr, p_auth_cmpl->addr_type);
          btif_update_remote_properties(p_auth_cmpl->bd_addr, p_auth_cmpl->bd_name, NULL, p_auth_cmpl->dev_type);
          bond_state_changed(BT_STATUS_SUCCESS, &bd_addr, BT_BOND_STATE_BONDED);
#endif
          return;
        }
    ...
    else
    {
        ...
        switch(p_auth_cmpl->fail_reason)
        {
            ...
            case HCI_ERR_UNSPECIFIED:
                if ((p_auth_cmpl->fail_reason == HCI_ERR_AUTH_FAILURE) ||
                    (p_auth_cmpl->fail_reason == HCI_ERR_KEY_MISSING))
                {
                    btif_storage_remove_bonded_device(&bd_addr);
                }
    ...
    switch (event)
    {
        ...
        case BTA_DM_INQ_RES_EVT:
        {
...

            {
                ...
                /* DEV_CLASS */
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                /* Check COD again from storage */
                bt_status_t status_cod;
                UINT32 cod_check;
                cod_check = devclass2uint (p_search_data->inq_res.dev_class);
                BTIF_TRACE_DEBUG("%s cod in inquiry result is 0x%06x", __FUNCTION__, cod_check);
                if (cod_check == 0) {
                    /* Try to retrieve cod from storage */
                    BTIF_TRACE_DEBUG("%s cod in inquiry result is 0, checking cod from storage", __FUNCTION__);
                    BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                   BT_PROPERTY_CLASS_OF_DEVICE, sizeof(uint32_t), &cod_check);
                    status_cod = btif_storage_get_remote_device_property(&bdaddr, &properties[num_properties]);
                    BTIF_TRACE_DEBUG("%s cod retrieved from storage is 0x%06x", __FUNCTION__, cod_check);
                    if (cod_check == 0) {
                        BTIF_TRACE_DEBUG("%s cod in storage is 0, set as unclassified", __FUNCTION__);
                        /*nothing changed,cod will be still COD_UNCLASSIFIED */
                        cod = COD_UNCLASSIFIED;
                    } else {
                        /*If cod in storage is COD_UNCLASSIFIED,now it is the same.*/
                        cod = cod_check;
                    }
                }
#endif
                ...
                /* Service */
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                    BT_PROPERTY_LOCAL_LE_FEATURES+1, sizeof(services),
                                    &services);
                num_properties++;
#endif
...
static void btif_dm_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_DM_LE_FEATURES_READ:
        {
            ...
            btgap_ext_local_le_features_t ext_local_le_features;
           ...
           /* LE features are not stored in storage. Should be retrived from stack */
            ...
            prop.len = sizeof (btgap_ext_local_le_features_t);
            ...
            ext_local_le_features.local_le_features = local_le_features;
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
            ext_local_le_features.le_maximum_advertising_data_length =
                controller_get_interface()->get_ble_maxium_advertising_data_length();
#else
            ext_local_le_features.le_maximum_advertising_data_length = 31;
#endif
            memcpy(prop.val, &ext_local_le_features, prop.len);
        ...
        case BTA_DM_REM_NAME_EVT:
          ...
          btif_dm_update_rmt_device_name(p_data->rem_name_evt.bd_addr, p_data->rem_name_evt.bd_name);
          break;
#endif
...
{
    BTIF_TRACE_DEBUG("energy_info_cb-Status:%u, state=%u, tx_t=%u, rx_t=%u, idle_time=%u, used=%u",
...
bt_status_t btif_dm_start_discovery(void)
{
    ...
    char prop_inq[PROPERTY_VALUE_MAX];
    osi_property_get("service.inq_mode", prop_inq, "1");
    ...
    if (prop_inq[0] == '2') {
        inq_params.mode = BTA_DM_LIMITED_INQUIRY|BTA_BLE_LIMITED_INQUIRY;
    }
#if (defined(BTA_HOST_INTERLEAVE_SEARCH) && BTA_HOST_INTERLEAVE_SEARCH == TRUE)
    ...
    if (prop_inq[0] == '2') {
        inq_params.mode = BTA_DM_LIMITED_INQUIRY;
    }
#endif
...
bt_status_t btif_dm_create_bond(const bt_bdaddr_t *bd_addr, int transport)
{
    ...
    bdstr_t bdstr;
    BTIF_TRACE_EVENT("%s: bd_addr=%s, transport=%d, state=%d, sdp_attempts=%d",
        __FUNCTION__, bdaddr_to_string(bd_addr, bdstr, sizeof(bdstr)), transport,
        pairing_cb.state, pairing_cb.sdp_attempts);
    if ((pairing_cb.state != BT_BOND_STATE_NONE) || (pairing_cb.sdp_attempts != 0)) return BT_STATUS_BUSY;
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            /* Write BLE device link key to file */
            btif_config_flush();
#endif
...
static void btif_dm_read_rssi_cb (tBTM_RSSI_RESULTS *p_result)
{
    ...
    BTIF_TRACE_DEBUG("%s: get rssi cb,addr is 0x%02x:%02x:%02x:%02x:%02x:%02x:", __FUNCTION__,
              btif_cb.bd_addr.address[0], btif_cb.bd_addr.address[1],
              btif_cb.bd_addr.address[2], btif_cb.bd_addr.address[3],
              btif_cb.bd_addr.address[4], btif_cb.bd_addr.address[5]);
    BTIF_TRACE_DEBUG("%s: get rssi cb,status is %d", __FUNCTION__,btif_cb.status);
...
** Function         btif_dm_update_rmt_device_name
**
...
**
*******************************************************************************/
static void btif_dm_update_rmt_device_name(const BD_ADDR bd_addr,
                                    BD_NAME bd_name) {
...
static void btif_stats_add_bond_event(const bt_bdaddr_t *bd_addr,
    ...
    uint64_t ts = event->timestamp.tv_sec * 1000LL +
...
/*******************************************************************************
**
** Function         btif_dm_read_vendor_rssi_cb
**
** Description      read rssi callback
**
** Returns          bt_status_t
**
*******************************************************************************/
static void btif_dm_read_vendor_rssi_cb (tBTM_RSSI_RESULTS *p_result)
{
    BTIF_TRACE_DEBUG("%s", __FUNCTION__);
}
...
/*******************************************************************************
**
** Function         btif_dm_get_vendor_rssi
**
** Description      get remote device's rssi raw data
**
** Returns          bt_status_t
**
*******************************************************************************/
bt_status_t btif_dm_get_vendor_rssi(const uint8_t index)
{
    BTM_ReadVndRSSI(index, (tBTM_CMPL_CB *)btif_dm_read_vendor_rssi_cb);
    return BT_STATUS_SUCCESS;
}

/*******************************************************************************
**
** Function         btif_dm_read_vendor_per
**
** Description      read packet error rate callback
**
** Returns          bt_status_t
**
*******************************************************************************/
static void btif_dm_read_vendor_per_cb(tBTM_RSSI_RESULTS *p_result)
{
    BTIF_TRACE_DEBUG("%s", __FUNCTION__);
}
...
/*******************************************************************************
**
** Function         btif_dm_get_vendor_per
**
** Description      get packet error rate
**
** Returns          bt_status_t
**
*******************************************************************************/
bt_status_t btif_dm_get_vendor_per(void)
{
    BTM_ReadVndPER((tBTM_CMPL_CB *)btif_dm_read_vendor_per_cb);
    return BT_STATUS_SUCCESS;
}

/*******************************************************************************
**
** Function         btif_dm_start_inquiry
**
** Description      Start device inquiry
**
** Returns          bt_status_t
**
*******************************************************************************/
bt_status_t btif_dm_start_inquiry(void)
{
    tBTA_DM_INQ inq_params;
    tBTA_SERVICE_MASK services = 0;
    ...
    /* Set inquiry params and call API */
    inq_params.mode = BTA_DM_GENERAL_INQUIRY | BTA_BLE_INQUIRY_NONE;

    /* Use Max inquiry length = 0x30 * 1.28sec = 61.44 sec */
    inq_params.duration = 0x30;
    ...
    if (osi_property_get("persist.bt.inquiry", prop_value_char, ""))
    {
        if (strncmp(prop_value_char, "started", 7) == 0)
        {
            if (!btif_dm_inquiry_in_progress)
                BTA_DmSearch(&inq_params, services, bte_search_devices_evt);
        }
        else if (strncmp(prop_value_char, "start", 5) == 0)
        {
            BTIF_TRACE_EVENT("%s, start BT Inquiry", __FUNCTION__);

            /* Will be enabled to TRUE once inquiry busy level has been received */
            btif_dm_inquiry_in_progress = FALSE;
            /* find nearby devices */
            BTA_DmSearch(&inq_params, services, bte_search_devices_evt);
            osi_property_set("persist.bt.inquiry", "started");
        }
        else if (strncmp(prop_value_char, "stopped", 7) == 0)
        {
            // Do nothing
        }
        else if (strncmp(prop_value_char, "stop", 4) == 0)
        {
            BTIF_TRACE_EVENT("%s, stop BT Inquiry", __FUNCTION__);
            btif_dm_cancel_discovery();
            osi_property_set("persist.bt.inquiry", "stopped");
        }
        else
        {
            // Do nothing
