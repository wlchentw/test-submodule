...
const char* dump_hf_client_event(UINT16 event)
{
    switch(event)
    {
        ...
        CASE_RETURN_STR(BTA_HF_CLIENT_CPBS_EVT)
        CASE_RETURN_STR(BTA_HF_CLIENT_CPBR_ENTRY_EVT)
        CASE_RETURN_STR(BTA_HF_CLIENT_CPBR_COUNT_EVT)
        CASE_RETURN_STR(BTA_HF_CLIENT_CPBR_COMPLETE_EVT)
#endif
        default:
            return "UNKNOWN MSG ID";
     }
