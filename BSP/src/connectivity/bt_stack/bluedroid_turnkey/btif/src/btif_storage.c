       ...
       case BT_PROPERTY_REMOTE_DEVICE_TIMESTAMP:
        ...
        case BT_PROPERTY_UUIDS:
        {
            ...
            for (i=0; i < (prop->len)/sizeof(bt_uuid_t); i++)
            {
                ...
                if (sizeof(value) >= strlen(value) + 1)
                    strncat(value, buf, sizeof(value) - strlen(value) - 1);
                //strcat(value, ";");
                if (sizeof(value) >= strlen(value) + 1)
    ...
    for (const btif_config_section_iter_t *iter = btif_config_section_begin(); iter != btif_config_section_end(); iter = btif_config_section_next(iter)) {
        ...
        if (btif_config_get_bin(name, "LinkKey", link_key, &size)) {
            ...
            if (btif_config_get_int(name, "LinkKeyType", &linkkey_type)) {
                ...
                bt_linkkey_file_found = TRUE;
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                /*bug fix: avoid device num larger  than BTM_SEC_MAX_DEVICE_RECORDS */
                if (BTM_SEC_MAX_DEVICE_RECORDS <= p_bonded_devices->num_devices)
                {
                    BTIF_TRACE_WARNING("!!! num_devices too large:%d,%s, %d", p_bonded_devices->num_devices, __FUNCTION__, __LINE__);
                    bt_bdaddr_t temp_bd_addr;
                    memcpy(&temp_bd_addr, &p_bonded_devices->devices[0], sizeof(bt_bdaddr_t));
                    for (int i = 0; i < BTM_SEC_MAX_DEVICE_RECORDS - 1; i++)
                    {
                        memcpy(&p_bonded_devices->devices[i], &p_bonded_devices->devices[i + 1], sizeof(bt_bdaddr_t));
                    }
                    p_bonded_devices->num_devices = BTM_SEC_MAX_DEVICE_RECORDS - 1;
                    btif_storage_remove_bonded_device(&temp_bd_addr);
                }
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    //bugfix for when profile just connected, AC off/on suddently, then hid info will not save correctly
    btif_config_flush();
    BTIF_TRACE_DEBUG("btif_storage_add_hid_device_info: sync after config flush");
#else
    btif_config_save();
#endif
    return BT_STATUS_SUCCESS;
