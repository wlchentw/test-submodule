...
static void btif_gatt_set_encryption_cb (BD_ADDR bd_addr, tBTA_TRANSPORT transport, tBTA_STATUS result)
{
    ...
    if (result != BTA_SUCCESS && result != BTA_BUSY)
    {
        ...
        if (result == BTA_ERR_KEY_MISSING)
        {
            ...
            HAL_CBACK(bt_hal_cbacks, bond_state_changed_cb, BT_STATUS_SUCCESS, &bda, BT_BOND_STATE_BONDING);
...
#else
#if defined(MTK_LINUX_GATTC_NOT_SUPPORT_SMP) && (MTK_LINUX_GATTC_NOT_SUPPORT_SMP == TRUE)
#else
        BTA_DmSetEncryption(bd_addr,transport_link,
