...
void queue_free_connect_node(void *ptr)
{
    connect_node_t *p_node = (connect_node_t *)ptr;
    /* if HFP disable, the a2dp connect req will be released.
     * then no event reported to upper layer.
     * so when release it and it did not request before it, then report
     * DISCONNECTED.
     */
    if ((p_node->uuid == UUID_SERVCLASS_AUDIO_SOURCE
        || p_node->uuid == UUID_SERVCLASS_AUDIO_SINK)
        && (false == p_node->busy))
    {
        p_node->connect_cb(&p_node->bda, 0);
    }
...
static void queue_int_add(connect_node_t *p_param) {
    if (!connect_queue) {
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        connect_queue = list_new(queue_free_connect_node);
#else
    ...
    for (const list_node_t *node = list_begin(connect_queue); node != list_end(connect_queue); node = list_next(node)) {
        if (((connect_node_t *)list_node(node))->uuid == p_param->uuid) {
            p_param->connect_cb(&p_param->bda, ~(p_param->uuid));
