...
static void btu_exec_tap_fd_read(void *p_param) {
    ...
    for (int i = 0; i < PAN_BUF_MAX && btif_is_enabled() && btpan_cb.flow; i++) {
        ...
        memcpy(packet, btpan_cb.congest_packet, MIN(sizeof(btpan_cb.congest_packet), MIN(btpan_cb.congest_packet_size, buffer->len)));
        buffer->len = MIN(sizeof(btpan_cb.congest_packet), MIN(btpan_cb.congest_packet_size, buffer->len));
