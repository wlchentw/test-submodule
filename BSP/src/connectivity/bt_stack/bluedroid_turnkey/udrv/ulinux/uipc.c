...
static void uipc_flush_ch_locked(tUIPC_CH_ID ch_id)
{
    ...
    while (1)
    {
        ...
        OSI_NO_INTR(ret = read(pfd.fd, &buf, UIPC_FLUSH_BUFFER_SIZE));
        if (ret <= 0) {
            BTIF_TRACE_ERROR("%s() - read result%d\n", __FUNCTION__, ret);
            return;
        }
    }
}
...
static void uipc_read_task(void *arg)
{
    ...
    while (uipc_main.running)
    {
        ...
        tv.tv_sec = 10;
        tv.tv_usec = 0;
        result = select(uipc_main.max_fd+1, &uipc_main.read_set, NULL, NULL, &tv);
        ...
        result = uipc_check_fd_locked(UIPC_CH_ID_AV_AUDIO) < 0;

        /* check for other connections */
        for (ch_id = 0; ch_id < UIPC_CH_NUM; ch_id++)
        {
            if (ch_id != UIPC_CH_ID_AV_AUDIO)
                result |= uipc_check_fd_locked(ch_id) < 0;
        }
...
{
    BTIF_TRACE_DEBUG("UIPC_Open : ch_id %d, p_cback 0X%p", ch_id, p_cback);
