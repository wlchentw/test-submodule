...
//Sets the value |data| indexed by |key| into the |hash_map|.
//If |key| is duplicated, |data| also be set, not removed.
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
bool hash_map_set_duplicate(hash_map_t *hash_map, const void *key, void *data);
#endif
