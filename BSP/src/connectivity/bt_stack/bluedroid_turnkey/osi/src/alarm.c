...
struct alarm_t {
  ...
  bool in_callback;  // Set to true if we're in the callback currently.
  bool is_deleted;   // Set to true if the object was deleted.
...
void alarm_free(alarm_t *alarm) {
  ...
  {
      // Don't delete the alarm while we're currently in the callback. This is
      // undefined behavior because the alarm is used after the callback
      // completes. Mark the object as deleted so the callback can delete it after
      // completion.
      pthread_mutex_lock(&alarm->callback_lock);
      if (alarm->in_callback) {
...
static void alarm_queue_ready(fixed_queue_t *queue,
  ...
  if (alarm->is_periodic) {
    ...
    /*  when power off, this alarm is not in alarm list and it's not in
       *   the alarm queue. when power on, free alarm or cancel alarm will access
       *   the invalid queue.
       */
    alarm-> queue = NULL;
#endif
  ...
  period_ms_t t0 = now();
  alarm->in_callback = true;
  callback(data);
  alarm->in_callback = false;
  period_ms_t t1 = now();

  need_delete = alarm->is_deleted;
