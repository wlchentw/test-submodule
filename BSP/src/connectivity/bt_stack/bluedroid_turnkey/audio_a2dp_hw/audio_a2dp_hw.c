...
bool enable_lowlatency_feature = false;
thread_t*start_thread = NULL;
static bool isThreadCreated = false;
struct timespec pst; /*received the first packet data timestamp*/
long long latency_sum = 0;
bool cond_has_send = false;
#endif
/*****************************************************************************
**  Constants & Macros
******************************************************************************/
...
#if (defined(MTK_LINUX_MW_STACK_LOG2FILE) && (MTK_LINUX_MW_STACK_LOG2FILE == TRUE)) || (defined(MTK_LINUX_STACK_LOG2FILE) && (MTK_LINUX_STACK_LOG2FILE == TRUE))
#include <syslog.h>

EXPORT_SYMBOL int log_save_to_syslog = 1;
#define printf(format, args...)                \
do{                                            \
    if (1 == log_save_to_syslog)               \
    {                                          \
...
/* Define trace levels */
#define AUDIO_BT_TRACE_LEVEL_ERROR   0          /* Error condition trace messages       */
#define AUDIO_BT_TRACE_LEVEL_WARNING 1          /* Warning condition trace messages     */
#define AUDIO_BT_TRACE_LEVEL_INFO    2          /* API traces                           */
#define AUDIO_BT_TRACE_LEVEL_DEBUG   3          /* Full debug messages                  */
#define AUDIO_BT_TRACE_LEVEL_VERBOSE 4          /* Verbose debug messages               */

#define AUDIO_MAX_TRACE_LEVEL        6
EXPORT_SYMBOL uint8_t audio_a2dp_trace_level = AUDIO_BT_TRACE_LEVEL_WARNING;
#define AUDIOLOGWRAPPER(tag, fmt, args...) printf("%s: " fmt "\n", tag, ## args)

#define AUDIO_LOG_DEBUG(...) AUDIOLOGWRAPPER(__VA_ARGS__)

#define FNLOG()             {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_VERBOSE) AUDIO_LOG_DEBUG(LOG_TAG, "%s", __FUNCTION__);}
...
#define INFO(fmt, ...)      {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_INFO) AUDIO_LOG_DEBUG(LOG_TAG, "%s: " fmt,__FUNCTION__, ## __VA_ARGS__);}
...
static int skt_connect(char *path, size_t buffer_sz)
{
    ...
    if (skt_fd < 0)
    {
        ERROR("skt_fd < 0 (%s)", strerror(errno));
        return -1;
    }
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
void start_send_start_cmd_thread_handler(void* context) {

  /*start cmd to peer then receive response time spent*/
  LOG_INFO(LOG_TAG, "begin to send start cmd to peer");

  struct a2dp_stream_common* common = (struct a2dp_stream_common*)context;
  ...
  /*let send data thread wake up immediately*/
  cond_has_send = true;
  pthread_cond_signal(&common->cond);
  LOG_INFO(LOG_TAG, "cond signal has send");
...
static void drop_data_sending_start_cmd(struct a2dp_stream_common *common, int bytes)
{
  /*start cmd is sending to peer, so drop received data*/
    LOG_INFO(LOG_TAG, "%s: sending start cmd to peer", __func__);

    /*current packet data timestamp*/
    struct timespec pre;
    memset(&pre, 0, sizeof(pre));
    clock_gettime(CLOCK_MONOTONIC, &pre);

    /* the time difference of current packet data and the first packet data received */
    long long delay_sub  = (long long)((pre.tv_sec - pst.tv_sec) * 1000000LL + (pre.tv_nsec - pst.tv_nsec)/1000);
    LOG_INFO(LOG_TAG, "time difference (%lld us)", delay_sub);

    /*then substract the time difference of current packet data and  the second last packet data received */
    int sleep_delay = delay_sub - latency_sum;
    LOG_INFO(LOG_TAG, "sleep delay (%d us)", sleep_delay);

    /*caculate current data need to sleep time*/
    int us_delay = calc_audiotime(common->cfg, bytes);
    latency_sum += us_delay;

    int sleeptime = us_delay - sleep_delay;
    LOG_INFO(LOG_TAG, "sleep time(%d us)", sleeptime);
    ...
    if (!cond_has_send)
    {
      pthread_cond_timedwait(&common->cond, &common->lock, &sleep_time);
    }
    return;
}

static int thread_send_start_cmd(struct a2dp_stream_common *common)
{
    /* Schedule the rest of the startup operations */
    if (!isThreadCreated)
      {
        /*the first packet data trigger start time*/
        clock_gettime(CLOCK_MONOTONIC, &pst);
        isThreadCreated = true;
        cond_has_send = false;
        thread_post(start_thread, start_send_start_cmd_thread_handler, (void*)(common));
      }
    return 0;
}

static int socket_connect(struct a2dp_stream_common *common)
{
     /*audio write the first data time after receiving start response */
    if (common->audio_fd == AUDIO_SKT_DISCONNECTED) {
        LOG_INFO(LOG_TAG, "%s: socket connect", __func__);
        common->audio_fd = skt_connect(A2DP_DATA_PATH, common->buffer_sz);
        if (common->audio_fd < 0) {
          ERROR("Audiopath start failed - error opening data socket");
          return -1;
        }
        LOG_INFO(LOG_TAG, "skt write the first data to stack");
      }
    return 0;
}
#endif //MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE
                         ...
                         size_t bytes)
{
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
        (out->common.state == AUDIO_A2DP_STATE_STARTING && (enable_lowlatency_feature == true)) ||
#endif
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
        if (enable_lowlatency_feature == true)
        {
            /* New a thread to send start cmd to peer sink */
            LOG_WARN(LOG_TAG, "%s:thread post send start handle", __func__);
            if (thread_send_start_cmd(&out->common) < 0)
            {
               goto error;
            }

            drop_data_sending_start_cmd(&out->common, bytes);
    ...
    if (enable_lowlatency_feature == true)
    {
      /*audio write the first data time after receiving start response, run encoder timer in advance  */
      if (socket_connect(&out->common) < 0)
      {
        goto error;
      }
    }
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
    /*enabel low latency feature*/
    keyval = (char *)hash_map_get(params, "LowLatency");
    if (keyval && strcmp(keyval, "true") == 0)
    {
      enable_lowlatency_feature = true;
    }
    else
    {
      enable_lowlatency_feature = false;
    }
#endif
    pthread_mutex_unlock(&out->common.lock);
...
#if defined(MTK_LINUX_AUDIO_LOG_DEFINE) && (MTK_LINUX_AUDIO_LOG_DEFINE == TRUE)
EXPORT_SYMBOL uint8_t audio_get_trace_level()
{
...
EXPORT_SYMBOL uint8_t audio_set_trace_level(uint8_t new_level)
{
    ...
    if (new_level > AUDIO_BT_TRACE_LEVEL_VERBOSE)
    {
        audio_a2dp_trace_level = AUDIO_BT_TRACE_LEVEL_VERBOSE;
    }
    else
    {
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
     /* New a thread to send start cmd to peer sink */
    if ((flags & 0x80000000) || enable_lowlatency_feature)
      {
        if (start_thread == NULL) {
            start_thread = thread_new("start_send_start_cmd_thread");
            if (start_thread == NULL) {
                ERROR("%s: unable to start up start_send_start_cmd_thread", __func__);
                ret = -1;
                goto err_open;
          }
        }
      }
    /*init cond and condattr*/
    ret = pthread_condattr_init(&out->common.condattr);
    if (ret < 0) {
        ERROR("init condattr fail:%d\n", ret);
    }
    pthread_condattr_setclock(&out->common.condattr, CLOCK_MONOTONIC);
    ret = pthread_cond_init(&out->common.cond, &out->common.condattr);
    if (ret < 0) {
        ERROR("init cond fail:%d\n", ret);
      }
#endif
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    /* if audio write pcm data and close output stream, the data will be dropped
     * then the remote device output less data then expected.
     */
    latency_us = ((out->common.buffer_sz * 1000 ) /
                    audio_stream_out_frame_size(&out->stream) /
                    out->common.cfg.rate) * 1000;

    latency_us += BTIF_MEDIA_TIME_TICK * 1000;

    WARN("closing output sleep %u us", latency_us);
    usleep(latency_us);
#endif
...
#if defined(MTK_LOW_LATENCY_FEATURE)&&(MTK_LOW_LATENCY_FEATURE == TRUE)
    /*destroy cond and condattr*/
    pthread_cond_destroy(&out->common.cond);
    pthread_condattr_destroy(&out->common.condattr);

    /*free the thread that sending start cmd to peer*/
    if (start_thread != NULL)
    {
      thread_free(start_thread);
      start_thread = NULL;
    }
#endif
...
#endif
{
    ...
    *stream_in = &in->stream;
    ...
    if (in->common.ctrl_fd == AUDIO_SKT_DISCONNECTED)
    {
        ERROR("ctrl socket failed to connect (%s)", strerror(errno));
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        /* audio module cannot handle this error */
#else
        ret = -1;
        goto err_open;
#endif
...
#endif
{
      ...
      {
          printf("output all audio_a2dp_hw log to terminal!!\n");
          log_save_to_syslog = 0;
      }
#endif
#endif
