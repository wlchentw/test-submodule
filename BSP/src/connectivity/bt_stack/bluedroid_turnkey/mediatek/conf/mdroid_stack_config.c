...
bool parse_override_cfg(config_t * config) {
  ...
  if (str_ovconf_fpath[0] != '\0') {
    ...
    if (!strcmp(str_ovconf_fpath, "bt_stack.conf.sqc") ||
      ...
      {
        if (sizeof(prefix_fpath1) >= strlen(prefix_fpath1) + 1)
        {
            strncat(prefix_fpath1, str_ovconf_fpath, sizeof(prefix_fpath1) - strlen(prefix_fpath1) - 1);
        }
        prefix_fpath1[sizeof(prefix_fpath1) - 1] = '\0';
      }
      else {
