...
typedef enum {
  ...
  // save in multi files when flash is too small.
  // every file size is limited to BtSnoopMaxLogFileSize
  BTSNOOP_LOG_MODE_MULTI,
} BTSNOOP_LOG_MODE;

typedef void (*SNOOP_WRITE)(const void *data, size_t length);

typedef void (*SNOOP_WRITE_IN_THREAD)(const void *data, size_t length, int last_frag);
