...
typedef enum {
    ...
    BTRC_STS_BAD_RANGE      = 0x0b,    /* Range Out of Bounds - The start of range provided is not valid  GetFolderItems*/
...
/** Callback for list player application attributes (Shuffle, Repeat,...) */
typedef void (* btrc_list_player_app_values_callback)(bt_bdaddr_t *bd_addr, btrc_player_attr_t attr_id);
...
typedef void (* btrc_get_player_app_value_callback) (bt_bdaddr_t *bd_addr, uint8_t num_attr, btrc_player_attr_t *p_attrs);
...
typedef void (* btrc_get_player_app_attrs_text_callback) (bt_bdaddr_t *bd_addr, uint8_t num_attr, btrc_player_attr_t *p_attrs);
...
typedef void (* btrc_get_player_app_values_text_callback) (bt_bdaddr_t *bd_addr, uint8_t attr_id, uint8_t num_val, uint8_t *p_vals);

/** Callback for setting the player application settings values */
typedef void (* btrc_set_player_app_value_callback) (bt_bdaddr_t *bd_addr, btrc_player_settings_t *p_vals);
...
typedef void (* btrc_get_element_attr_callback) (bt_bdaddr_t *bd_addr, uint8_t num_attr, btrc_media_attr_t *p_attrs);
...
typedef void (* btrc_register_notification_callback) (bt_bdaddr_t *bd_addr, btrc_event_id_t event_id, uint32_t param);

/* AVRCP 1.4 Enhancements *//** Callback for volume change on CT**  volume: Current volume setting on the CT (0-127)*/
typedef void (* btrc_volume_change_callback) (bt_bdaddr_t *bd_addr, uint8_t volume, uint8_t ctype);

/** Callback for passthrough commands */
typedef void (* btrc_passthrough_cmd_callback) (bt_bdaddr_t *bd_addr, int id, int key_state);

/** Callback for set addressed player response on TG **/
typedef void (* btrc_set_addressed_player_callback) (bt_bdaddr_t *bd_addr, uint16_t player_id);

/** Callback for get folder items on TG**  num_attr: specifies the number of attributes requested in p_attr_ids*/
typedef void (* btrc_get_folder_items_callback) (bt_bdaddr_t *bd_addr, uint8_t scope, uint32_t start_item, uint32_t end_item, uint8_t num_attr, uint32_t *p_attr_ids);
...
typedef void (* btrc_tg_connection_state_callback) (bool state, bt_bdaddr_t *bd_addr);

#if defined(MTK_A2DP_MULTI_AVRCP) && (MTK_A2DP_MULTI_AVRCP == TRUE)
typedef void (* btrc_tg_passthrough_rsp_callback) (bt_bdaddr_t *bd_addr, int id, int key_state);
#else
typedef void (* btrc_tg_passthrough_rsp_callback) (int id, int key_state);
#endif
...
/** BT-RC Target ext callback structure. */
typedef struct {
    /** set to sizeof(btrc_ctrl_ext_callbacks_t) */
    size_t      size;

    btrc_tg_connection_state_callback           connection_state_cb;
    btrc_tg_passthrough_rsp_callback            passthrough_rsp_cb;
} btrc_ext_callbacks_t;
...
typedef struct {
    ...
    */
    bt_status_t (*register_notification_rsp)(bt_bdaddr_t *bd_addr,
    ...
    */
	bt_status_t (*set_volume)(bt_bdaddr_t *bd_addr, uint8_t volume);
#else
    /** Respose to GetPlayStatus request. Contains the current
    **  1. Play status
    **  2. Song duration/length
...
//    bt_status_t (*set_addressed_player_rsp)(btrc_status_t rsp_status);
    ...
    bt_status_t (*set_addressed_player_rsp)(btrc_status_t rsp_status);
    bt_status_t (*set_browsed_player_rsp)(btrc_status_t rsp_status,
                                             uint32_t num_items,
                                             uint16_t charset_id,
                                             uint8_t folder_depth,
                                             btrc_br_folder_name_t* p_folders);
    bt_status_t (*get_folder_items_list_rsp)(btrc_status_t rsp_status,uint16_t uid_counter,uint8_t num_items,btrc_folder_items_t* p_items);
    bt_status_t (*change_path_rsp)(btrc_status_t rsp_status,uint32_t num_items);
    bt_status_t (*get_item_attr_rsp)(btrc_status_t rsp_status, uint8_t num_attr, btrc_element_attr_val_t* p_attrs);
    bt_status_t (*play_item_rsp)(btrc_status_t rsp_status);
    bt_status_t (*get_total_num_of_items_rsp)(btrc_status_t rsp_status,
                                              uint32_t uid_counter,
                                              uint32_t num_items);
    bt_status_t (*search_rsp)(btrc_status_t rsp_status,
                              uint32_t uid_counter, uint32_t num_items);
    bt_status_t (*add_to_now_playing_rsp)(btrc_status_t rsp_status);
    ...
    /** Resupose to get folder item command
      ...
      **/
    //bt_status_t (*get_folder_items_rsp)(btrc_status_t rsp_status,uint16_t uid_counter,uint8_t num_items,btrc_folder_items_t* p_items);
...
typedef struct {
    ...
    /** set to sizeof(btrc_ex_interface_t) */
    size_t          size;

    /**
     * Register the BtRc ext callbacks
     */
    bt_status_t (*init)( btrc_ext_callbacks_t* ext_callbacks );
...
/*    btrc_ctrl_set_browsed_player_callback                       set_browsed_player_cb;
    btrc_ctrl_change_path_cb_callback                           change_path_cb;
    btrc_ctrl_search_cb_callback                            	search_cb;
    btrc_ctrl_get_item_attr_callback                            get_item_attr_cb;
    btrc_ctrl_add_to_now_playing_callback                       add_to_now_playing_cb;
    btrc_ctrl_play_item_callback                            	play_item_cb;			//CT
    btrc_ctrl_set_addressed_player_callback                     set_addressed_player_cb;
    btrc_ctrl_get_folder_items_callback                         get_folder_items_cb;
    btrc_ctrl_change_folder_path_callback                       change_folder_path_cb;
*/
...
typedef struct {
    ...
    bt_status_t (*play_item_cmd)(bt_bdaddr_t* bd_addr, uint8_t scope,
                                 uint8_t* uid, uint16_t uid_counter);

    bt_status_t (*get_playback_state_cmd)(bt_bdaddr_t* bd_addr);
    bt_status_t (*get_now_playing_list_cmd)(bt_bdaddr_t* bd_addr,
                                            uint8_t start_item,
                                            uint8_t num_items);

    bt_status_t (*get_folder_list_cmd)(bt_bdaddr_t* bd_addr, uint8_t start_item,
                                       uint8_t num_items);

    bt_status_t (*get_player_list_cmd)(bt_bdaddr_t* bd_addr, uint8_t start_item,
                                       uint8_t num_items);

    bt_status_t (*change_folder_path_cmd)(bt_bdaddr_t* bd_addr,
                                          uint8_t direction, uint8_t* uid);

    bt_status_t (*set_browsed_player_cmd)(bt_bdaddr_t* bd_addr, uint16_t id);
    bt_status_t (*set_addressed_player_cmd)(bt_bdaddr_t* bd_addr, uint16_t id);
...
bt_status_t (*volume_change_notification_rsp) (bt_bdaddr_t *bd_addr, btrc_notification_type_t rsp_type,
            uint8_t abs_vol, uint8_t label);
...
#if defined(MTK_A2DP_MULTI_AVRCP) && (MTK_A2DP_MULTI_AVRCP == TRUE)
    void  (*send_get_playstatus_cmd)( bt_bdaddr_t *bd_addr );
#else
