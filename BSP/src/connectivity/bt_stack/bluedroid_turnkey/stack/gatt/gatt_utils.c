...
BOOLEAN gatt_find_app_hold_link(tGATT_TCB *p_tcb, UINT8 start_idx, UINT8 *p_found_idx, tGATT_IF *p_gatt_if)
{
    ...
    UINT8 gatt_max;
    gatt_max = (GATT_MAX_APPS < GATT_CL_MAX_LCB) ? GATT_MAX_APPS : GATT_CL_MAX_LCB;

    for (i = start_idx; i < gatt_max; i ++)
#else
...
void gatt_end_operation(tGATT_CLCB *p_clcb, tGATT_STATUS status, void *p_data)
{
    ...
    if (p_cmpl_cb != NULL && p_clcb->operation != 0)
    {
        ...
        if (NULL != p_clcb->p_tcb && p_clcb->operation == GATTC_OPTYPE_CONFIG)
    ...
    if ((p_tcb = gatt_find_tcb_by_addr(bda, transport)) != NULL)
    {
        ...
        for (i = 0; i < GATT_MAX_APPS; i ++)
        {
            ...
            if (p_reg->in_use && p_reg->app_cb.p_conn_cb)
            {
                ...
                {
                    GATT_TRACE_ERROR ("disconnect - gattx_flag unknown, p_tcb->gattx_flag: 0x%x, p_reg->gattx_flag: 0x%x",
