...
EXPORT_SYMBOL void gatt_mesh_set_conn_status(BD_ADDR bd_addr, UINT16 handle)
{
    L2CA_MeshSetConnStatus(bd_addr, handle);
}
#endif
...
{
    /* Add judgement for p_tcb->app_hold_link[i] to avoid adding the same app(gatt_if) */
#if (defined(MTK_COMMON) && MTK_COMMON == TRUE)
                          ...
                          tBT_TRANSPORT transport, BOOLEAN opportunistic)
{
    ...
    else
    {
        if ((p_tcb = gatt_allocate_tcb_by_bdaddr(bd_addr, transport)) != NULL)
        {
            ...
            /* add the gattx_flag */
            p_tcb->gattx_flag = p_reg->gattx_flag;
#endif
            if (!gatt_connect(bd_addr,  p_tcb, transport))
            {
                                   ...
                                   UINT16 reason, tBT_TRANSPORT transport)
{
    ...
    if (connected)
    {
        ...
        else
        {
            ...
            {
#if defined(MTK_GATT_CLIENT_AND_SERVER) && (MTK_GATT_CLIENT_AND_SERVER == TRUE)
                /* if we are connected with no p_tcb, then the gattx_flag is server flag*/
                p_tcb->gattx_flag = GATT_SERVER_FLAG;
#endif
                p_tcb->att_lcid = L2CAP_ATT_CID;
...
static void gatt_channel_congestion(tGATT_TCB *p_tcb, BOOLEAN congested)
{
    ...
    if (NULL == p_tcb)
    {
        GATT_TRACE_WARNING ("%s p_tcb is null and return", __func__);
        return;
    }
    if (congested == FALSE)
    {
                                  ...
                                  uint8_t rx_phy)
{
    ...
    for (int i = 0; i < GATT_MAX_APPS; i++)
    {
        ...
        if (p_reg->in_use && p_reg->app_cb.p_phy_update_cb)
        {
            ...
            (*p_reg->app_cb.p_phy_update_cb)(p_reg->gatt_if, p_tcb->peer_bda, tx_phy,
                                            rx_phy, status);
        }
    }
}
...
#if defined(MTK_GATT_CLIENT_AND_SERVER) && (MTK_GATT_CLIENT_AND_SERVER == TRUE)
            if (!p_tcb->gattx_flag || !p_reg->gattx_flag || p_tcb->gattx_flag > 2 || p_reg->gattx_flag >2)
            {
                GATT_TRACE_ERROR ("connect - gattx_flag unknown, p_tcb->gattx_flag: 0x%x, p_reg->gattx_flag: 0x%x",
                p_tcb->gattx_flag, p_reg->gattx_flag);
            }

            memset (app_name, 0, MAX_UUID_SIZE + 1);
            strncpy(app_name, "BTA HH OVER LE", MAX_UUID_SIZE);

            //Do not check gattx flag when HH UUID
            if ((p_tcb->gattx_flag == p_reg->gattx_flag) || !memcmp((void *)p_reg->app_uuid128.uu.uuid128, (void *)app_name, MAX_UUID_SIZE))
            {
#endif
