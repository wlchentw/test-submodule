...
BOOLEAN smp_calculate_link_key_from_long_term_key(tSMP_CB *p_cb)
{
    ...
    else
    {
        ...
        /* Fix for link key reverse order wrong: */
        //UINT8 *p;
        //BT_OCTET16 notif_link_key;
        //p = notif_link_key;
        //ARRAY16_TO_STREAM(p, link_key);

        btm_sec_link_key_notification (bda_for_lk, link_key, link_key_type);
...
EXPORT_SYMBOL void smp_generate_key(UINT8 pub_key[BT_OCTET64_LEN], UINT8 pri_key[BT_OCTET32_LEN])
{
    Point       public_key;
    BT_OCTET32  private_key;
    ...
    smp_debug_print_nbyte_little_endian (pri_key, (const UINT8 *)"private",
                                         BT_OCTET32_LEN);
    smp_debug_print_nbyte_little_endian (public_key.x, (const UINT8 *)"local public(x)",
                                         BT_OCTET32_LEN);
    smp_debug_print_nbyte_little_endian (public_key.y, (const UINT8 *)"local public(y)",
                                         BT_OCTET32_LEN);
}

EXPORT_SYMBOL void smp_calculate_dhkey (const UINT8 public_key[BT_OCTET64_LEN], const UINT8 private_key[BT_OCTET32_LEN], UINT8 secret[BT_OCTET32_LEN])
{
    Point       peer_publ_key, new_publ_key;

    memcpy(peer_publ_key.x, public_key, BT_OCTET32_LEN);
    memcpy(peer_publ_key.y, &public_key[BT_OCTET32_LEN], BT_OCTET32_LEN);

    ECC_PointMult(&new_publ_key, &peer_publ_key, (DWORD*) private_key, KEY_LENGTH_DWORDS_P256);

    memcpy(secret, new_publ_key.x, BT_OCTET32_LEN);

    smp_debug_print_nbyte_little_endian (secret, (const UINT8 *)"Old DHKey",
                                         BT_OCTET32_LEN);

    smp_debug_print_nbyte_little_endian (private_key, (const UINT8 *)"private",
                                         BT_OCTET32_LEN);
    smp_debug_print_nbyte_little_endian (public_key, (const UINT8 *)"rem public(x)",
                                         BT_OCTET32_LEN);
    smp_debug_print_nbyte_little_endian (&public_key[32], (const UINT8 *)"rem public(y)",
                                         BT_OCTET32_LEN);
    smp_debug_print_nbyte_little_endian (secret, (const UINT8 *)"Reverted DHKey",
                                         BT_OCTET32_LEN);
}
#endif
#endif
