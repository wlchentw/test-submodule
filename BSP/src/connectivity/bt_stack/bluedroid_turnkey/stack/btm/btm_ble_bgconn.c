...
#if defined(PATCH_FOR_RC_ADDR_TYPE_ERROR) && (PATCH_FOR_RC_ADDR_TYPE_ERROR == TRUE)
...
void btm_update_scanner_filter_policy(tBTM_BLE_SFP scan_policy)
{
    ...
    {
        BTM_TRACE_EVENT("%s : mesh scan is active, %d, %d", __func__, p_inq->mesh_scan_interval, p_inq->mesh_scan_window);
        scan_interval = !p_inq->mesh_scan_interval ? BTM_BLE_GAP_DISC_SCAN_INT : p_inq->mesh_scan_interval;
        scan_window = !p_inq->mesh_scan_window ? BTM_BLE_GAP_DISC_SCAN_WIN : p_inq->mesh_scan_window;
    }
#endif
...
#if defined(PATCH_FOR_RC_ADDR_TYPE_ERROR) && (PATCH_FOR_RC_ADDR_TYPE_ERROR == TRUE)
        /* unknown device,find it in inqDb or bt_config.conf to check the addr_type */
        UINT8 addr_type = BLE_ADDR_PUBLIC;
        tBTM_INQ_INFO *p_inq_info = BTM_InqDbRead(bd_addr);
        bt_bdaddr_t bdaddr;
        bdcpy(bdaddr.address, bd_addr);
        int addr_type_temp = BLE_ADDR_PUBLIC;
        if(p_inq_info != NULL)
        {
            addr_type = p_inq_info->results.ble_addr_type;
            BTM_TRACE_DEBUG("%s, known device find it in inqDB, addr_type = %d ", __func__, addr_type);
        }
        else    /*maybe it was a bonded device before platform power off,after power on ,it will not be in inqDb.*/
        {
            if (btif_storage_get_remote_addr_type(&bdaddr, &addr_type_temp) != BT_STATUS_SUCCESS)
            {
                addr_type = BTM_IS_PUBLIC_BDA(bd_addr) ? BLE_ADDR_PUBLIC : BLE_ADDR_RANDOM;
                BTM_TRACE_DEBUG("%s, not find it in bt_config.conf, addr_type = %d", __func__, addr_type);
            }
            else
            {
                addr_type = addr_type_temp;
                BTM_TRACE_DEBUG("%s, find it in bt_config.conf, addr_type = %d ", __func__, addr_type);
            }
        }
...
BOOLEAN btm_ble_start_auto_conn(BOOLEAN start)
{
    ...
    if (start)
    {
            ...
            && btm_ble_topology_check(BTM_BLE_STATE_INIT))
        {
            ...
            if (!btm_send_hci_create_connection(scan_int,  /* UINT16 scan_int      */
