...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
static tBTM_READ_PHY_CALLBACK *btm_read_phy_cb = NULL;
/*******************************************************************************
 *
...
void BTM_BleReadPhy(BD_ADDR bd_addr, tBTM_READ_PHY_CALLBACK *cb)
{
  ...
  btm_read_phy_cb = cb;
  btsnd_hcic_ble_read_phy(p_acl->hci_handle);
  return;
}
...
tBTM_STATUS BTM_BleSetDefaultPhy(uint8_t all_phys, uint8_t tx_phys,
  ...
  btsnd_hcic_ble_set_default_phy(all_phys, tx_phys, rx_phys);
  return BTM_SUCCESS;
}
 ...
 *
 ******************************************************************************/
void BTM_BleSetPhy(BD_ADDR bd_addr, uint8_t tx_phys, uint8_t rx_phys,
  ...
  btsnd_hcic_ble_set_phy(p_acl->hci_handle, all_phys, tx_phys, rx_phys, phy_options);
}

void btm_ble_read_phy_complete(UINT8 *p, UINT16 evt_len)
{
  ...
  if (status == HCI_SUCCESS)
  {
    ...
    BTM_TRACE_DEBUG("%s tx_phy: %d rx_phy: %d", __func__, tx_phy, rx_phy);
    if (btm_read_phy_cb) {
      xx = btm_handle_to_acl_index (handle);
      if (xx >= MAX_L2CAP_LINKS)
        return;
      con = &btm_cb.acl_db[xx];
      btm_read_phy_cb(con->remote_addr,tx_phy,rx_phy,status);
    }
  }
  btm_read_phy_cb = NULL;
}
                            ...
                            tBTM_SEC_CALLBACK *p_callback, void *p_ref_data)
{
    ...
    {
       BTM_TRACE_WARNING ("%s p_lcb = NULL", __func__);
    }
...
void btm_ble_conn_complete(UINT8 *p, UINT16 evt_len, BOOLEAN enhanced)
{
    ...
    BD_ADDR     local_rpa, peer_rpa;
#endif /* (defined(MTK_COMMON) && MTK_COMMON == TRUE) && (BLE_PRIVACY_SPT == FALSE) */
        ...
        switch (event)
        {
            ...
            case SMP_COMPLT_EVT:
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                   if ((event == SMP_COMPLT_EVT) && (p_data->cmplt.reason != 0))
                       delete_device = TRUE;
#endif
                }
                ...
                if (event == SMP_COMPLT_EVT)
                {
                    ...
                    if (delete_device)
                    {
                        /* if call this api will free p_dev_rec, but  btm_ble_link_encrypted use the p_dev_rec  so may crash */
                        /* BTM_SecDeleteDevice(p_dev_rec->bd_addr); */
                        BTM_TRACE_DEBUG ("encryption failed - delete linkkey use btm_ble_link_encrypted trigger");
                    }
#endif
