...
tBTM_SEC_DEV_REC* btm_sec_allocate_dev_rec(void)
{
    ...
    p_dev_rec->timestamp = btm_cb.dev_rec_count++;
 #if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    /* Fix WCNCR00203043: disable smp on br since IOT issue, dut may send smp_pair_req
      just at the time that remote device doing role switch, and then cause smp pair failed. */
    p_dev_rec->no_smp_on_br = true;
 #endif
    return p_dev_rec;
}
