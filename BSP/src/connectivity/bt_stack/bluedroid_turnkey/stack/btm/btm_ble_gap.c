...
#define BTM_BLE_MESH_SCAN_WIN                32 //32*0.625ms = 20ms
#endif

#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
extern UINT16 g_adv_data_len;
#endif
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
static void btm_ble_process_adv_pkt_cont(BD_ADDR bda, UINT8 addr_type, UINT16 evt_type, UINT8 *p);
#else
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
const uint8_t BLE_EVT_CONNECTABLE_BIT = 0;
const uint8_t BLE_EVT_SCANNABLE_BIT = 1;
const uint8_t BLE_EVT_DIRECTED_BIT = 2;
const uint8_t BLE_EVT_SCAN_RESPONSE_BIT = 3;
const uint8_t BLE_EVT_LEGACY_BIT = 4;
...
uint16_t ble_evt_type_legacy2ext(uint8_t legacy_evt_type)
{
    uint16_t event_type = 0x0013;
                           ...
                           tBTM_INQ_RESULTS_CB *p_results_cb, tBTM_CMPL_CB *p_cmpl_cb)
{
    ...
    if (start)
    {
        ...
        /* scan is not started */
#ifdef MTK_MESH_SUPPORT
        if (!BTM_BLE_IS_SCAN_ACTIVE(btm_cb.ble_ctr_cb.scan_activity) && !btm_cb.ble_ctr_cb.mesh_enabled)
#else
...
#ifdef MTK_MESH_SUPPORT
//for mesh scan
EXPORT_SYMBOL tBTM_STATUS BTM_MeshScan(BOOLEAN start, tBTM_INQ_RESULTS_CB *p_results_cb)
{
    tBTM_BLE_INQ_CB *p_inq = &btm_cb.ble_ctr_cb.inq_var;
    tBTM_STATUS status = BTM_WRONG_MODE;

    UINT32 scan_interval = !p_inq->mesh_scan_interval ? BTM_BLE_GAP_DISC_SCAN_INT : p_inq->mesh_scan_interval;
    UINT32 scan_window = !p_inq->mesh_scan_window ? BTM_BLE_GAP_DISC_SCAN_WIN : p_inq->mesh_scan_window;
    ...
    {
        /* shared inquiry database, do not allow observe if any inquiry is active */
        if (btm_cb.ble_ctr_cb.mesh_enabled)
        {
            BTM_TRACE_ERROR("%s Mesh Scan Already Active", __func__);
            return status;
        }

        btm_cb.ble_ctr_cb.p_mesh_results_cb = p_results_cb;
        status = BTM_CMD_STARTED;
        ...
        /* allow config of scan type */
        p_inq->mesh_scan_type = (p_inq->mesh_scan_type == BTM_BLE_SCAN_MODE_NONE) ?
                                                BTM_BLE_SCAN_MODE_ACTI: p_inq->mesh_scan_type;
        /* assume observe always not using white list */
        #if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == TRUE)
            /* enable resolving list */
            btm_ble_enable_resolving_list_for_platform(BTM_BLE_RL_SCAN);
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
        btm_send_hci_set_scan_params(
            p_inq->mesh_scan_type, (uint16_t)scan_interval, (uint16_t)scan_window,
            btm_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type, BTM_BLE_DEFAULT_SFP);
#else
        if (cmn_ble_vsc_cb.extended_scan_support == 0)
        {
            btsnd_hcic_ble_set_scan_params(p_inq->mesh_scan_type, (UINT16)scan_interval,
                                           (UINT16)scan_window,
                                           btm_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type,
                                           BTM_BLE_DEFAULT_SFP);
        }
        ...
        {
            btm_ble_send_extended_scan_params(p_inq->mesh_scan_type, scan_interval, scan_window,
                                              btm_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type,
                                              BTM_BLE_DEFAULT_SFP);
        }
#endif
        ...
        if (status == BTM_CMD_STARTED)
        {
            btm_cb.ble_ctr_cb.mesh_enabled = TRUE;
        }

    }
    else if (btm_cb.ble_ctr_cb.mesh_enabled)
    {
        status = BTM_CMD_STARTED;
        btm_cb.ble_ctr_cb.p_mesh_results_cb = NULL;
        if (!BTM_BLE_IS_SCAN_ACTIVE(btm_cb.ble_ctr_cb.scan_activity))
        {
            btm_ble_stop_scan();
        }
        if (status == BTM_CMD_STARTED)
        {
            btm_cb.ble_ctr_cb.mesh_enabled = FALSE;
        }
    }
    else
    {
        BTM_TRACE_ERROR("%s Mesh Scan not active", __func__);
    }
...
EXPORT_SYMBOL void BTM_MeshSetScanParams(UINT32 scan_interval, UINT32 scan_window, tBLE_SCAN_MODE scan_mode)
{
    ...
    {
        p_cb->mesh_scan_type = scan_mode;
        p_cb->mesh_scan_interval = scan_interval; //normal mode,interval=90ms
        p_cb->mesh_scan_window = scan_window;//normal mode,window=20ms
        p_cb->scan_type = scan_mode;
        p_cb->scan_interval = BTM_BLE_GAP_DISC_SCAN_INT;
        p_cb->scan_window = BTM_BLE_MESH_SCAN_WIN;
    }
    else
    {
...
EXPORT_SYMBOL void BTM_MeshRegisterCb(bt_hci_evt_callback_func_t callback)
{
    btm_cb.ble_ctr_cb.hci_evt_cb = callback;
    BTM_TRACE_ERROR ("BTM_MeshRegisterCb, btm_cb.ble_ctr_cb.hci_evt_cb= %p", btm_cb.ble_ctr_cb.hci_evt_cb);
}
#endif
#endif
...
{
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
    if (btm_cb.extended_adv_support)
    {
        return btm_cb.adv_set_max;
    }
#endif
...
static void btm_ble_resolve_random_addr_on_adv(void * p_rec, void *p)
{
    ...
    BTM_TRACE_VERBOSE("btm_ble_resolve_random_addr_on_adv ");
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
    btm_ble_process_adv_pkt_cont(bda, addr_type, ble_evt_type_legacy2ext(evt_type), pp);
#else
...
*******************************************************************************/
#ifdef MTK_MESH_SUPPORT
EXPORT_SYMBOL UINT8 *BTM_CheckAdvData( UINT8 *p_adv, UINT8 type, UINT8 *p_length)
#else
...
#endif
{
    ...
    UINT8 length;
    UINT8 adv_type;
    BTM_TRACE_VERBOSE("%s: type=0x%02x", __func__, type);
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
    g_adv_data_len = BTM_BLE_EXT_ADV_DATA_LEN - len;
    BTM_TRACE_DEBUG("adv data len=%d", g_adv_data_len);
#endif
...
tBTM_STATUS btm_ble_set_connectability(UINT16 combined_mode)
{
    ...
    BTM_TRACE_EVENT ("%s mode=0x%0x combined_mode=0x%x", __FUNCTION__, mode, combined_mode);
    /*after set value, mode must be between 0 and 256.*/
...
BOOLEAN btm_send_hci_create_connection(
  ...
  if (controller_get_interface()->supports_ble_extended_advertising()) {
    ...
    if (phy_cnt > 3)
    {
      BTM_TRACE_ERROR("%s More than three phys provided", __func__);
      return (FALSE);
    }
    ...
    return btsnd_hcic_ble_ext_create_conn(init_filter_policy, addr_type_own,
                                   addr_type_peer, bda_peer, initiating_phys,
                                   phy_cfg);
  } else {
    return btsnd_hcic_ble_create_ll_conn(scan_int, scan_win, init_filter_policy,
...
tBTM_STATUS btm_ble_start_inquiry (UINT8 mode, UINT8   duration)
{
    ...
    if (btm_cb.ble_ctr_cb.mesh_enabled)
    {
        BTM_TRACE_EVENT("%s : mesh scan is active", __func__);
    ...
    if (!BTM_BLE_IS_SCAN_ACTIVE(p_ble_cb->scan_activity))
    {
        ...
        btm_send_hci_set_scan_params(BTM_BLE_SCAN_MODE_ACTI,
                                     BTM_BLE_LOW_LATENCY_SCAN_INT,
                                     BTM_BLE_LOW_LATENCY_SCAN_WIN,
                                     btm_cb.ble_ctr_cb.addr_mgnt_cb.own_addr_type,
                                     SP_ADV_ALL);
...
*******************************************************************************/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void btm_ble_cache_adv_data(tBTM_INQ_RESULTS *p_cur, UINT8 data_len, UINT8 *p, UINT16 evt_type)
#else
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
    if (!ble_evt_type_is_scan_resp(evt_type))
#else
...
#if (defined MTK_BLE_SCAN_CHECK_ADDR_AND_MULTI_REPORT && MTK_BLE_SCAN_CHECK_ADDR_AND_MULTI_REPORT == TRUE)
        memcpy(p_le_inq_cb->adv_data_cache_addr, p_cur->remote_bd_addr, BD_ADDR_LEN);
#endif
    }
...
#endif
    {
        if (memcmp (p_le_inq_cb->adv_data_cache_addr, p_cur->remote_bd_addr, BD_ADDR_LEN))
        {
            BTM_TRACE_DEBUG("czw check cache:bda= %0x:%0x:%0x:%0x:%0x:%0x",
                                                p_le_inq_cb->adv_data_cache_addr[0],p_le_inq_cb->adv_data_cache_addr[1],
                                                p_le_inq_cb->adv_data_cache_addr[2],p_le_inq_cb->adv_data_cache_addr[3],
                                                p_le_inq_cb->adv_data_cache_addr[4],p_le_inq_cb->adv_data_cache_addr[5]);
            BTM_TRACE_DEBUG("czw check cur:bda= %0x:%0x:%0x:%0x:%0x:%0x",
                                                p_cur->remote_bd_addr[0],p_cur->remote_bd_addr[1],
                                                p_cur->remote_bd_addr[2],p_cur->remote_bd_addr[3],
                                                p_cur->remote_bd_addr[4],p_cur->remote_bd_addr[5]);
            p_le_inq_cb->adv_len = 0;
            memset(p_le_inq_cb->adv_data_cache, 0, BTM_BLE_CACHE_ADV_DATA_MAX);
            memset(p_le_inq_cb->adv_data_cache_addr, 0, BD_ADDR_LEN);
        }
    }
    if (data_len > 0)
    {
...
*******************************************************************************/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
UINT8 btm_ble_is_discoverable(BD_ADDR bda, UINT16 evt_type, UINT8 *p)
#else
...
#endif
{
    ...
    if (BTM_BLE_IS_SEL_CONN_ACTIVE(btm_cb.ble_ctr_cb.scan_activity) &&
        ble_evt_type_is_connectable(evt_type))
#else
    if (BTM_BLE_IS_SEL_CONN_ACTIVE(btm_cb.ble_ctr_cb.scan_activity) &&
...
*******************************************************************************/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
BOOLEAN btm_ble_update_inq_result(tINQ_DB_ENT *p_i, UINT8 addr_type, UINT16 evt_type, UINT8 *p)
#else
...
#endif
{
...
#endif
    {
        ...
        {
            BTM_TRACE_VERBOSE("BR/EDR NOT support bit not set, treat as DUMO");
    ...
    {
        BTM_TRACE_VERBOSE("BR/EDR NOT SUPPORT bit set, LE only device");
    }
...
*******************************************************************************/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void btm_send_sel_conn_callback(BD_ADDR remote_bda, UINT16 evt_type, UINT8 *p_data, UINT8 addr_type)
#else
...
/*******************************************************************************
**
** Function         btm_ble_resolve_random_addr_on_ext_adv
**
** Description      resolve random address complete callback for ext adv.
**
** Returns          void
**
*******************************************************************************/
static void btm_ble_resolve_random_addr_on_ext_adv(void * p_rec, void *p)
{
    tBTM_SEC_DEV_REC    *match_rec = (tBTM_SEC_DEV_REC *) p_rec;
    UINT8       addr_type = BLE_ADDR_RANDOM;
    BD_ADDR     bda;
    ...
    btm_ble_process_adv_pkt_cont(bda, addr_type, evt_type, pp + 14);
    return;
}
#endif
 ...
 * entry is discarded.
 */
void btm_ble_process_ext_adv_pkt(uint8_t data_len, uint8_t* p_data) {
...
#ifdef MTK_MESH_SUPPORT
  /* Only process the results if the inquiry is still active or mesh_scan enabled */
  if (!BTM_BLE_IS_SCAN_ACTIVE(btm_cb.ble_ctr_cb.scan_activity) && !btm_cb.ble_ctr_cb.mesh_enabled)
#else
  ...
  while (num_reports--) {
    ...
    BTM_TRACE_DEBUG("btm_ble_process_ext_adv_pkt:bda= %0x:%0x:%0x:%0x:%0x:%0x",
                                 bda[0],bda[1],bda[2],bda[3],bda[4],bda[5]);
    /* always do RRA resolution on host */
    if (!match && BTM_BLE_IS_RESOLVE_BDA(bda))
    {
        btm_ble_resolve_random_addr(bda, btm_ble_resolve_random_addr_on_ext_adv, p_data);
    }
    else
#endif
    {
        btm_ble_process_adv_pkt_cont(bda, addr_type, event_type, pkt_data - 1);
    }
  }
}
#endif
...
void btm_ble_process_adv_pkt (UINT8 *p_data)
{
    ...
    while (num_reports--)
    {
        ...
        {
#if (defined MTK_BLE_SCAN_CHECK_ADDR_AND_MULTI_REPORT && MTK_BLE_SCAN_CHECK_ADDR_AND_MULTI_REPORT == TRUE)
            BTM_TRACE_DEBUG("czw p_report_start=%p", p_report_start);
            btm_ble_resolve_random_addr(bda, btm_ble_resolve_random_addr_on_adv, p_report_start);
#else
        ...
        {
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
            btm_ble_process_adv_pkt_cont(bda, addr_type, ble_evt_type_legacy2ext(evt_type), p);
#else
...
#endif
{
    ...
    BTM_TRACE_DEBUG("czw btm_ble_process_adv_pkt_cont:bda= %0x:%0x:%0x:%0x:%0x:%0x",
                                     bda[0],bda[1],bda[2],bda[3],bda[4],bda[5]);
    ...
    if (!btm_ble_update_inq_result(p_i, addr_type, evt_type, p))
        return;
#ifdef MTK_MESH_SUPPORT
    /* if mesh scan enable, no need to discarding advertising packet pkt */
    if (((result = btm_ble_is_discoverable(bda, evt_type, p)) == 0) && !btm_cb.ble_ctr_cb.mesh_enabled)
#else
...
#ifdef MTK_MESH_SUPPORT
        if (btm_cb.ble_ctr_cb.mesh_enabled && (btm_cb.ble_ctr_cb.p_mesh_results_cb))
        {
            (btm_cb.ble_ctr_cb.p_mesh_results_cb)((tBTM_INQ_RESULTS *) &p_i->inq_info.results, p_le_inq_cb->adv_data_cache);
        }
#endif
    }
}
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void btm_ble_process_phy_update_pkt(UINT8 len, UINT8* data)
{
    UINT8 status, tx_phy, rx_phy;
    UINT16 handle;
    if (len != 5)
    {
        BTM_TRACE_ERROR("%s, len = %d is error", __func__, len);
        return;
    }
    UINT8* p = data;
    STREAM_TO_UINT8(status, p);
...
#ifdef MTK_MESH_SUPPORT
    if (!BTM_BLE_IS_SCAN_ACTIVE(p_ble_cb->scan_activity) && !p_ble_cb->mesh_enabled)
#else
    if (!BTM_BLE_IS_SCAN_ACTIVE(p_ble_cb->scan_activity))
#endif
        btm_ble_stop_scan();
...
#if defined(MTK_PATCH_FOR_BT_OFF_ON_FAIL) && (MTK_PATCH_FOR_BT_OFF_ON_FAIL == TRUE)
/*******************************************************************************
**
