                                      ...
                                      void *p_ref_data)
{
    ...
    BTM_TRACE_DEBUG ("%s() is_originator:%d, %p", __func__, is_originator, p_ref_data);
#endif
...
void btm_sec_rmt_name_request_complete (UINT8 *p_bd_addr, UINT8 *p_bd_name, UINT8 status)
{
    ...
    */
    ...
    if (p_dev_rec)
    {
        BTM_TRACE_EVENT ("%s PairState: %s  RemName: %s  status: %d State:%d  p_dev_rec: %p ", __func__,
    ...
    {
        BTM_TRACE_EVENT ("%s() delayed pin now being requested flags:0x%x, (p_pin_callback=%p)",
...
void btm_io_capabilities_req (UINT8 *p)
{
    ...
    BTM_TRACE_EVENT("%s: State: %s  Flags: 0x%04x  p_cur_service: 0x%p",
...
void btm_sec_auth_complete (UINT16 handle, UINT8 status)
{
    ...
    if (BTM_INVALID_HCI_HANDLE != handle)
    {
        p_dev_rec = btm_find_dev_by_handle (handle);
    }
    else
...
// some devices still in bonding state after forgeting it without btservice restart.
// So remove the bonding checking for workaround.
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                /** M: callback auth status @{*/
                if ((btm_cb.api.p_auth_complete_callback) && (old_state != BTM_PAIR_STATE_IDLE))
                  (*btm_cb.api.p_auth_complete_callback)(p_dev_rec->bd_addr, p_dev_rec->dev_class,
                                                         p_dev_rec->sec_bd_name, status);
                /** @} */
#endif
...
void btm_sec_disconnected (UINT16 handle, UINT8 reason)
{
    ...
    BTM_TRACE_EVENT("%s end", __func__);
#endif
}
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                    BTM_SecDeleteDevice(p_cb->pairing_bda);
#endif
                }
                else
                {
...
static tBTM_STATUS btm_sec_execute_procedure (tBTM_SEC_DEV_REC *p_dev_rec)
{
        ...
        && (p_dev_rec->hci_handle != BTM_SEC_INVALID_HANDLE))
    {
        ...
        /* if the DUT do RNR and the remote device don't response
         * and it will ignore the RNR at security check.
         * But there is not any RNR and the security check will ignore.
         * then l2cap connect req pending will not accept.
         */
#if 0
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        /** M: add for not init second RNR @{*/
        if((p_inq->remname_active) &&
            !bdcmp(p_inq->remname_bda, p_dev_rec->bd_addr)) {
          BTM_TRACE_WARNING ("Security Manager:Other module started RNR already");
