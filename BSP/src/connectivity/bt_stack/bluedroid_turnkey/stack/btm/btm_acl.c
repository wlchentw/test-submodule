...
EXPORT_SYMBOL tBTM_STATUS BTM_SetPSM_State (BOOLEAN new_state)
#else
...
void btm_use_preferred_conn_params(BD_ADDR bda) {
    ...
    if (NULL == p_lcb)
    {
        BTM_TRACE_ERROR("%s: p_lcb NULL !", __func__);
        return;
    }
...
**
*******************************************************************************/
tBTM_STATUS BTM_ReadVndRSSI (uint8_t index, tBTM_CMPL_CB *p_cb)
{
    tACL_CONN   *p = &btm_cb.acl_db[0];
    uint16_t    xx;

    /* If someone already waiting on the version, do not allow another */
    if (btm_cb.devcb.p_vendor_rssi_cmpl_cb)
    {
        BTM_TRACE_WARNING("[btper] %s: BUSY! %p", __FUNCTION__, p_cb);
        return(BTM_BUSY);
    }

    for (xx = 0; xx < MAX_L2CAP_LINKS; xx++, p++)
    {
        if ( xx == index && (p->in_use) )
        {
            btm_cb.devcb.p_vendor_rssi_cmpl_cb = p_cb;
            btsnd_hcic_read_vendor_rssi(p->hci_handle);
            return(BTM_CMD_STARTED);
        }
    }
    return(BTM_UNKNOWN_ADDR);
...
/*******************************************************************************
**
** Function         BTM_ReadVndPER
**
** Description      This function is called to read the packet error rate.
**
** Returns          BTM_CMD_STARTED if successfully initiated or error code
**
*******************************************************************************/
tBTM_STATUS BTM_ReadVndPER (tBTM_CMPL_CB *p_cb)
{
    /* If someone already waiting on the version, do not allow another */
    if (btm_cb.devcb.p_vendor_per_cmpl_cb)
    {
        BTM_TRACE_WARNING("[btper] %s: BUSY!", __FUNCTION__);
        return(BTM_BUSY);
    }

    btm_cb.devcb.p_vendor_per_cmpl_cb = p_cb;
    btsnd_hcic_read_vendor_per();
...
/*******************************************************************************
**
** Function         btm_read_vendor_per_complete
**
** Description      This function is called when the command complete message
**                  is received from the HCI for the read per request.
**
** Returns          void
**
*******************************************************************************/
void btm_read_vendor_per_complete (uint8_t *p)
{
    #define PER_LINK_PER_MAX_LINK_NUM 6
    tBTM_CMPL_CB *p_cb = btm_cb.devcb.p_vendor_per_cmpl_cb;
    uint8_t status;

    btm_cb.devcb.p_vendor_per_cmpl_cb = NULL;
    STREAM_TO_UINT8(status, p);

    if (status == HCI_SUCCESS)
    {
        int i;
        uint32_t bt_proc_tx_count = 0;
        uint32_t bt_proc_rx_count = 0;
        uint32_t ble_proc_tx_count = 0;
        uint32_t ble_proc_rx_count = 0;
        uint8_t link_count = 0;
        uint8_t per_link_addr[PER_LINK_PER_MAX_LINK_NUM][6] ={{0}};
        uint8_t per_link_type[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_tx_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_tx_total_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_tx_error_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_rx_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_rx_total_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_rx_error_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_tx_per[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint32_t per_link_rx_per[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint8_t per_link_tx_power_lower_bound_index[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint8_t per_link_last_used_tx_power_index[PER_LINK_PER_MAX_LINK_NUM] = {0};
        uint8_t per_link_last_used_tx_power_dbm[PER_LINK_PER_MAX_LINK_NUM] = {0};

        STREAM_TO_UINT16(bt_proc_tx_count, p);
        STREAM_TO_UINT16(bt_proc_rx_count, p);
        STREAM_TO_UINT16(ble_proc_tx_count, p);
        STREAM_TO_UINT16(ble_proc_rx_count, p);
        BTM_TRACE_WARNING(NONECOLOR "[btper] BT Proc " LIGHT_CYAN "Tx:%d, Rx:%d" NONECOLOR ", BLE Proc " LIGHT_CYAN "Tx:%d, Rx:%d" NONECOLOR,
                                    bt_proc_tx_count, bt_proc_rx_count, ble_proc_tx_count, ble_proc_rx_count);

        STREAM_TO_UINT8(link_count, p);

        for ( i = 0 ; i < link_count && i < PER_LINK_PER_MAX_LINK_NUM ; i++ )
        {
            STREAM_TO_UINT8(per_link_addr[i][0], p);
            STREAM_TO_UINT8(per_link_addr[i][1], p);
            STREAM_TO_UINT8(per_link_addr[i][2], p);
            STREAM_TO_UINT8(per_link_addr[i][3], p);
            STREAM_TO_UINT8(per_link_addr[i][4], p);
            STREAM_TO_UINT8(per_link_addr[i][5], p);
            STREAM_TO_UINT8(per_link_type[i], p);
            STREAM_TO_UINT16(per_link_tx_count[i], p);
            STREAM_TO_UINT16(per_link_tx_total_count[i], p);
            STREAM_TO_UINT16(per_link_tx_error_count[i], p);
            STREAM_TO_UINT16(per_link_rx_count[i], p);
            STREAM_TO_UINT16(per_link_rx_total_count[i], p);
            STREAM_TO_UINT16(per_link_rx_error_count[i], p);
            STREAM_TO_UINT8(per_link_tx_power_lower_bound_index[i], p);
            STREAM_TO_UINT8(per_link_last_used_tx_power_index[i], p);
            STREAM_TO_UINT8(per_link_last_used_tx_power_dbm[i], p);

            if (per_link_tx_total_count[i])
                per_link_tx_per[i] = ((per_link_tx_error_count[i]*100)/per_link_tx_total_count[i]);
            if (per_link_rx_total_count[i])
                per_link_rx_per[i] = ((per_link_rx_error_count[i]*100)/per_link_rx_total_count[i]);

            BTM_TRACE_WARNING(CYAN "[btper][%d][I'm %s][%02X:%02X:%02X:%02X:%02X:%02X] Tx:%s%d%%" CYAN "(%d/%d) Rx:%s%d%%" CYAN "(%d/%d) Total=%s%d:%d Lower Power:%d TX Power:%d %ddBm " NONECOLOR,
                              i,
                              per_link_type[i]==(uint8_t)0?"BT Master":
                              per_link_type[i]==(uint8_t)1?"BT Slave":
                              per_link_type[i]==(uint8_t)2?"BLE Master":
                              per_link_type[i]==(uint8_t)3?"BLE SLave":"Unknown",
                              per_link_addr[i][5], per_link_addr[i][4], per_link_addr[i][3], per_link_addr[i][2], per_link_addr[i][1], per_link_addr[i][0],
                              per_link_tx_total_count[i]<=100?LIGHT_GREEN:per_link_tx_per[i]<20?LIGHT_GREEN:per_link_tx_per[i]>=20&&per_link_tx_per[i]<=55?YELLOW:LIGHT_RED,
                              per_link_tx_per[i],
                              per_link_tx_error_count[i],
                              per_link_tx_total_count[i],
                              per_link_rx_total_count[i]<=100?LIGHT_GREEN:per_link_rx_per[i]<20?LIGHT_GREEN:per_link_rx_per[i]>=20&&per_link_rx_per[i]<=55?YELLOW:LIGHT_RED,
                              per_link_rx_per[i],
                              per_link_rx_error_count[i],
                              per_link_rx_total_count[i],
                              LIGHT_GREEN,
                              per_link_tx_count[i],
                              per_link_rx_count[i],
                              per_link_tx_power_lower_bound_index[i],
                              per_link_last_used_tx_power_index[i],
                              per_link_last_used_tx_power_dbm[i]);
        }

        // average
        {
            static int average_total_time = 0;
            static uint32_t average_bt_proc_tx_count = 0;
            static uint32_t average_bt_proc_rx_count = 0;
            static uint32_t average_ble_proc_tx_count = 0;
            static uint32_t average_ble_proc_rx_count = 0;
            static uint8_t average_per_link_type[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_tx_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_tx_total_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_tx_error_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_rx_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_rx_total_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_rx_error_count[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_tx_per[PER_LINK_PER_MAX_LINK_NUM] = {0};
            static uint32_t average_per_link_rx_per[PER_LINK_PER_MAX_LINK_NUM] = {0};

            char prop_value_char[PROPERTY_VALUE_MAX];
            char prop_name_char[PROPERTY_VALUE_MAX];
            int prop_value_int = 0;
            if ( osi_property_get("persist.bluetooth.btper.average", prop_value_char, "0") > 0 )
            {
                sscanf(prop_value_char, "%d", &prop_value_int);
                if ( prop_value_int > 0 && prop_value_int < 100000 )
                {
                    average_total_time++;
                    prop_value_int--;
                    snprintf(prop_value_char, 5, "%d", prop_value_int);
                    osi_property_set("persist.bluetooth.btper.average", prop_value_char);

                    average_bt_proc_tx_count += bt_proc_tx_count;
                    average_bt_proc_rx_count += bt_proc_rx_count;
                    average_ble_proc_tx_count += ble_proc_tx_count;
                    average_ble_proc_rx_count += ble_proc_rx_count;
                    ...
                    for ( i = 0 ; i < link_count && i < PER_LINK_PER_MAX_LINK_NUM ; i++ )
                    {
                        ...
                        average_ble_proc_tx_count += ble_proc_tx_count;
                        average_ble_proc_rx_count += ble_proc_rx_count;
                        average_per_link_type[i] = per_link_type[i];
                        average_per_link_tx_count[i] += per_link_tx_count[i];
                        average_per_link_tx_total_count[i] += per_link_tx_total_count[i];
                        average_per_link_tx_error_count[i] += per_link_tx_error_count[i];
                        average_per_link_rx_count[i] += per_link_rx_count[i];
                        average_per_link_rx_total_count[i] += per_link_rx_total_count[i];
                        average_per_link_rx_error_count[i] += per_link_rx_error_count[i];
                        if (average_per_link_tx_total_count[i])
                            average_per_link_tx_per[i] = ((average_per_link_tx_error_count[i]*100)/average_per_link_tx_total_count[i]);
                        if (average_per_link_rx_total_count[i])
                            average_per_link_rx_per[i] = ((average_per_link_rx_error_count[i]*100)/average_per_link_rx_total_count[i]);
                    }

                    if ( prop_value_int == 0 )
                    {
                        if ( average_total_time )
                        {
                            BTM_TRACE_WARNING(YELLOW "[btper](average)(%ds) BT Proc " LIGHT_CYAN "Tx:%d, Rx:%d" YELLOW ", BLE Proc " LIGHT_CYAN "Tx:%d, Rx:%d" NONECOLOR,
                                        average_total_time,
                                        bt_proc_tx_count / average_total_time,
                                        bt_proc_rx_count / average_total_time,
                                        ble_proc_tx_count / average_total_time,
                                        ble_proc_rx_count / average_total_time);

                            snprintf(prop_value_char, 5, "%d", average_total_time);
                            osi_property_set("persist.bt.total_time", prop_value_char);
                            snprintf(prop_value_char, 5, "%d", bt_proc_tx_count);
                            osi_property_set("persist.bt.bt_proc_tx_num", prop_value_char);
                            snprintf(prop_value_char, 5, "%d", bt_proc_rx_count);
                            osi_property_set("persist.bt.bt_proc_rx_num", prop_value_char);
                            snprintf(prop_value_char, 5, "%d", ble_proc_tx_count);
                            osi_property_set("persist.bt.ble_proc_tx_num", prop_value_char);
                            snprintf(prop_value_char, 5, "%d", ble_proc_rx_count);
                            osi_property_set("persist.bt.ble_proc_rx_num", prop_value_char);
                            snprintf(prop_value_char, 5, "%d", link_count);
                            osi_property_set("persist.bt.link_count", prop_value_char);
                        }

                        for ( i = 0 ; i < link_count && i < PER_LINK_PER_MAX_LINK_NUM ; i++ )
                        {
                            BTM_TRACE_WARNING(YELLOW "[btper](average)(%ds)[%d][I'm %s][%02X:%02X:%02X:%02X:%02X:%02X] Tx:%s%d%%" YELLOW "(%d/%d) Rx:%s%d%%" YELLOW "(%d/%d) Total=%s%d:%d" NONECOLOR,
                                              average_total_time,
                                              i,
                                              average_per_link_type[i]==(uint8_t)0?"BT Master":
                                              average_per_link_type[i]==(uint8_t)1?"BT Slave":
                                              average_per_link_type[i]==(uint8_t)2?"BLE Master":
                                              average_per_link_type[i]==(uint8_t)3?"BLE SLave":"Unknown",
                                              per_link_addr[i][5], per_link_addr[i][4], per_link_addr[i][3], per_link_addr[i][2], per_link_addr[i][1], per_link_addr[i][0],
                                              LIGHT_CYAN,
                                              average_per_link_tx_per[i], average_per_link_tx_error_count[i]/average_total_time, average_per_link_tx_total_count[i]/average_total_time,
                                              LIGHT_CYAN,
                                              average_per_link_rx_per[i], average_per_link_rx_error_count[i]/average_total_time, average_per_link_rx_total_count[i]/average_total_time,
                                              LIGHT_CYAN,
                                              average_per_link_tx_count[i]/average_total_time,
                                              average_per_link_rx_count[i]/average_total_time);

                            sprintf(prop_name_char, "persist.bt.per.link%d.link_type", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_type[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.tx", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_tx_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.tx_total", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_tx_total_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.tx_err", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_tx_error_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.rx", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_rx_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.rx_total", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_rx_total_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);

                            sprintf(prop_name_char, "persist.bt.per.link%d.rx_err", i);
                            snprintf(prop_value_char, 5, "%d", average_per_link_rx_error_count[i]);
                            osi_property_set(prop_name_char, prop_value_char);
                        }
                        osi_property_set("persist.bluetooth.perf.average", "0");
                    }
                }
                else
                {
                    average_total_time = 0;
                    average_bt_proc_tx_count = 0;
                    average_bt_proc_rx_count = 0;
                    average_ble_proc_tx_count = 0;
                    average_ble_proc_rx_count = 0;
                    for ( i = 0 ; i < PER_LINK_PER_MAX_LINK_NUM ; i++ )
                    {
                        average_per_link_type[i]=0;
                        average_per_link_tx_count[i]=0;
                        average_per_link_tx_total_count[i]=0;
                        average_per_link_tx_error_count[i]=0;
                        average_per_link_rx_count[i]=0;
                        average_per_link_rx_total_count[i]=0;
                        average_per_link_rx_error_count[i]=0;
                    }

                    osi_property_get("persist.bluetooth.perf.average", prop_value_char, "0");
                    sscanf(prop_value_char, "%d", &prop_value_int);
                    if ( prop_value_int > 0 )
                    {
                        osi_property_set("persist.bluetooth.btper.average", prop_value_char);
    ...
    {
        BTM_TRACE_WARNING("[btper] %s: ERROR! (%d)", __FUNCTION__, status);
    }
...
/*******************************************************************************
**
** Function         btm_read_vendor_rssi_complete
**
** Description      This function is called when the command complete message
**                  is received from the HCI for the read rssi request.
**
...
**
*******************************************************************************/
void btm_read_vendor_rssi_complete (uint8_t *p)
{
    tBTM_CMPL_CB    *p_cb = btm_cb.devcb.p_vendor_rssi_cmpl_cb;
    uint8_t         status;
    int8_t          rssi;
    uint16_t        handle;
    uint16_t        index;
    tACL_CONN       *acl_p = &btm_cb.acl_db[0];
    char            prop_value_char[PROPERTY_VALUE_MAX];
    char            prop_name_char[PROPERTY_VALUE_MAX];
    int32_t         prop_value_int = 0;
    int32_t         btrssi_average_on = 0;
    static int32_t  btrssi_average[MAX_L2CAP_LINKS]={0};
    static int32_t  btrssi_average_total_time[MAX_L2CAP_LINKS] = {0};

    BTM_TRACE_DEBUG("%s", __func__);
    btm_cb.devcb.p_vendor_rssi_cmpl_cb = NULL;
    ...
    if (status == HCI_SUCCESS)
    {
        ...
        for (index = 0; index < MAX_L2CAP_LINKS; index++, acl_p++)
        {
            if ( acl_p->hci_handle == handle )
            {
                if ( osi_property_get("persist.bluetooth.rssi.average", prop_value_char, "0") > 0 )
                {
                    sscanf(prop_value_char, "%d", &prop_value_int);
                    if ( prop_value_int > 0 )
                    {
                        btrssi_average_total_time[index] += 1;
                        btrssi_average[index] += rssi;
                        btrssi_average_on = 1;
                    }
                }

                BTM_TRACE_WARNING ("[btrssi] RSSI : " BROWN "%d" NONECOLOR " (%02X:%02X:%02X:%02X:%02X:%02X)(%s)(%s)", rssi,
                                        acl_p->remote_addr[0],
                                        acl_p->remote_addr[1],
                                        acl_p->remote_addr[2],
                                        acl_p->remote_addr[3],
                                        ...
                                        acl_p->remote_addr[5],
                                        acl_p->remote_name,
                                        acl_p->transport==BT_TRANSPORT_LE?LIGHT_CYAN "BLE" NONECOLOR:CYAN "BT" NONECOLOR);
                break;
            }
        }
    }
    ...
    if ( btrssi_average_on == 1 )
    {
        osi_property_get("persist.bluetooth.rssi.average", prop_value_char, "0");
        sscanf(prop_value_char, "%d", &prop_value_int);

        for ( index = 0 ; index < MAX_L2CAP_LINKS ; index++ )
        {
            if ( btrssi_average_total_time[index] != 0 && btrssi_average_total_time[index] != prop_value_int )
                break;

            if ( index == (MAX_L2CAP_LINKS -1) )
            {
                int i;

                acl_p = &btm_cb.acl_db[0];
                for ( i = 0 ; i < MAX_L2CAP_LINKS ; i++, acl_p++)
                {
                    if ( btrssi_average[i] && btrssi_average_total_time[i] )
                    {
                        BTM_TRACE_WARNING (YELLOW "[btrssi] RSSI(average)(%ds) : " LIGHT_CYAN "%d" YELLOW " (%02X:%02X:%02X:%02X:%02X:%02X)(%s)(%s)" NONECOLOR,
                                            btrssi_average_total_time[i],
                                            btrssi_average[i]/btrssi_average_total_time[i],
                                            acl_p->remote_addr[0],
                                            acl_p->remote_addr[1],
                                            acl_p->remote_addr[2],
                                            acl_p->remote_addr[3],
                                            ...
                                            acl_p->remote_addr[5],
                                            acl_p->remote_name,
                                            acl_p->transport==BT_TRANSPORT_LE?"BLE":"BT");

                        sprintf(prop_name_char, "persist.bt.rssi.link%d.rssi", i);
                        snprintf(prop_value_char, 5, "%d", btrssi_average[i]/btrssi_average_total_time[i]);
                        osi_property_set(prop_name_char, prop_value_char);
                        sprintf(prop_name_char, "persist.bt.rssi.link%d.addr", i);
                        snprintf(prop_value_char, 15, "%02X:%02X:%02X:%02X:%02X:%02X",
                                    acl_p->remote_addr[0], acl_p->remote_addr[1], acl_p->remote_addr[2],
                                    acl_p->remote_addr[3], acl_p->remote_addr[4], acl_p->remote_addr[5]);
                        osi_property_set(prop_name_char, prop_value_char);

                        btrssi_average[i] = 0;
                        btrssi_average_total_time[i] = 0;
                    }
                }

                btrssi_average_on = 0;
                osi_property_set("persist.bluetooth.rssi.average", "0");
            }
        }
    }
    else
    {
        ...
        if ( prop_value_int > 0 )
        {
            osi_property_set("persist.bluetooth.rssi.average", prop_value_char);

            for ( index = 0 ; index < MAX_L2CAP_LINKS ; index++ )
            {
                btrssi_average_total_time[index] = 0;
                btrssi_average_total_time[index] = 0;
            }
        }
    }
