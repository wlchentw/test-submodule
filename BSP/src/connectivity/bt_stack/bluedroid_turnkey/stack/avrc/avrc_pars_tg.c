...
tAVRC_STS AVRC_ParsCommand (tAVRC_MSG *p_msg, tAVRC_COMMAND *p_result, UINT8 *p_buf, UINT16 buf_len)
{
    ...
    if (p_msg && p_result)
    {
        ...
        case AVRC_OP_BROWSE:
            ...
            //titan add to suppor general reject for pts (AVRCP/TG/INV/BI-02-C)
            if (status != AVRC_STS_NO_ERROR)
            {
                AVRC_TRACE_DEBUG("[pts]%s, send General Reject test", __func__);
                p_result->pdu = (UINT8)AVRC_PDU_GENERAL_REJECT;
            }
        break;
#endif
