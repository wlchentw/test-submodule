                ...
                BOOLEAN browse_supported, UINT16 profile_version)
{
    ...
    if ((profile_version > AVRC_REV_1_3) && (browse_supported))
    {
        ...
        avrc_add_proto_desc_list.list_elem[1].params[0] = profile_version;
#else
 ...
 *
 ******************************************************************************/
BOOLEAN AVRC_Update_Version(UINT32 sdp_handle, UINT16 profile_version, UINT16 categories)
{
    UINT8       temp[8];
    UINT8       *p;
    /* update profile descriptor list*/
    BOOLEAN result = SDP_AddProfileDescriptorList(sdp_handle, UUID_SERVCLASS_AV_REMOTE_CONTROL, profile_version);
