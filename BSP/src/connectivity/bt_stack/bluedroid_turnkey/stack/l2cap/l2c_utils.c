...
static void acl_priority_vsc_cback( tBTM_VSC_CMPL *p );
#endif
...
void l2cu_process_fixed_disc_cback (tL2C_LCB *p_lcb)
{
    ...
    for (int xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx++)
    {
        ...
        else if (xx + L2CAP_FIRST_FIXED_CHNL > 31)
        {
            L2CAP_TRACE_WARNING ("%s() can not let left shifting by more than 31 bits", __func__);
        }
...
void l2cu_send_peer_ble_credit_based_conn_res (tL2C_CCB *p_ccb, UINT16 result)
{
    ...
    if (!(p_ccb->p_lcb))
    {
        L2CAP_TRACE_WARNING ("%s p_ccb->p_lcb is null, return", __func__);
        return;
    }
...
void l2cu_send_peer_ble_flow_control_credit(tL2C_CCB *p_ccb, UINT16 credit_value)
{
    ...
    if (!p_ccb)
    {
        L2CAP_TRACE_WARNING ("%s, p_ccb is null,return", __func__);
        return;
    }
    ...
    if (!p_lcb)
    {
        L2CAP_TRACE_WARNING ("%s, p_lcb is null,return", __func__);
        return;
    }
    /* Create an identifier for this packet */
    p_ccb->p_lcb->id++;
...
/*******************************************************************************
**
** Function         l2cu_set_acl_priority_by_tci
**
** Description      Sets the transmission priority for a channel by tci cmd.
**                  (For initial implementation only two values are valid.
**                  L2CAP_PRIORITY_NORMAL and L2CAP_PRIORITY_HIGH).
**
** Returns          TRUE if a valid channel, else FALSE
...
BOOLEAN l2cu_set_acl_priority_by_tci (BD_ADDR bd_addr, UINT8 priority)
{
    tL2C_LCB            *p_lcb;
    UINT8               *pp;
    UINT8                vs_param;
    UINT8                data_direction;
    UINT8                a2dp_priority_command[HCI_A2DP_ACL_PRIORITY_PARAM_SIZE];

    APPL_TRACE_DEBUG("%s SET ACL PRIORITY %d", __func__, priority);

    /* Find the link control block for the acl channel */
    if ((p_lcb = l2cu_find_lcb_by_bd_addr(bd_addr, BT_TRANSPORT_BR_EDR)) == NULL)
    {
        L2CAP_TRACE_ERROR ("L2CAP - no LCB for l2cu_set_acl_priority_by_tci");
        return (FALSE);
    }

    APPL_TRACE_DEBUG("%s pre-priority %d", __func__, p_lcb->tci_acl_priority);

    /* Called from above L2CAP through API; send tci cmd if changed */
    if (priority != p_lcb->tci_acl_priority)
    {
        p_lcb->tci_acl_priority = priority;
        pp = a2dp_priority_command;
        /* With A2DP Sink enabled, need to send MTK_A2DP_SET_ACL_PRIORITY */
        vs_param = (priority == L2CAP_PRIORITY_HIGH) ?
                    HCI_A2DP_ACL_PRIORITY_HIGH: HCI_A2DP_ACL_PRIORITY_LOW;
        data_direction = (p_lcb->is_acl_type_a2dp_sink) ?
                    HCI_A2DP_ACL_PRIORITY_TYPE_SNK: HCI_A2DP_ACL_PRIORITY_TYPE_SRC;

        UINT16_TO_STREAM (pp, p_lcb->handle);
        UINT8_TO_STREAM  (pp, vs_param);
        UINT8_TO_STREAM  (pp, data_direction);

        /* MTK: send tci cmd: opcode 0xFD18, connection handle, flag(0x01:off, 0x00:on),
                           Data Direction(0x00:Src, 0x01:Sink) */
        BTM_VendorSpecificCommand (MTK_A2DP_SET_ACL_PRIORITY,
                    HCI_A2DP_ACL_PRIORITY_PARAM_SIZE, a2dp_priority_command,
                    acl_priority_vsc_cback);
    }
    else
    {
        APPL_TRACE_WARNING("%s set priority %d,pre-priority %d.",
            __func__, priority, p_lcb->tci_acl_priority);
    }
...
/*
* Local callback function of BTM_VendorSpecificCommand() for ACL Priority
* Command.
*/
static void acl_priority_vsc_cback( tBTM_VSC_CMPL *p )
{
    UINT8 status = 0xFF, *p_buf;

    /* Check status of command complete event */
    if((p->opcode == MTK_A2DP_SET_ACL_PRIORITY) && (p->param_len > 0 ))
    {
        p_buf = p->p_param_buf;
        STREAM_TO_UINT8(status, p_buf);
    }

    /* If the VSC response is not HCI_SUCCESS, throw an Error */
    if(status != HCI_SUCCESS)
    {
        APPL_TRACE_ERROR("L2CAP - Failed to set A2dp Acl Priority: 0x%x, status %d.",
                MTK_A2DP_SET_ACL_PRIORITY, status);
    }
    else
    {
        APPL_TRACE_DEBUG("L2CAP - Success to set A2dp Acl Priority: 0x%x.",
                MTK_A2DP_SET_ACL_PRIORITY);
    }
}
#endif
// set_bt_a2dp_acl_priority end

//fosmod_bt_enhancement begin
void l2cu_link_copy_bda(bdstr_t *bdstr, BD_ADDR  bda)
{
    sprintf(*bdstr, "%02x:%02x:%02x:%02x:%02x:%02x",
        bda[0], bda[1], bda[2],
        bda[3], bda[4], bda[5]);
}
//fosmod_bt_enhancement end
