...
BOOLEAN l2c_link_hci_disc_comp (UINT16 handle, UINT8 reason)
{
    ...
    else
    {
...
#endif
       {
        ...
        L2CAP_TRACE_DEBUG("l2c_link_hci_disc_comp: mesh_gatt_link_conn_status %d, link_role %d, handle = 0x%04x, reason = %d",
                            p_lcb->mesh_gatt_link_conn_status, p_lcb->link_role, handle, reason);
        if (((1 == p_lcb->mesh_gatt_link_conn_status) && (HCI_ROLE_MASTER == p_lcb->link_role)) ||
            (HCI_ROLE_SLAVE == p_lcb->link_role))
        {
            INT8 p_disconn_data[5];
            UINT8 *p = p_disconn_data;
            UINT8_TO_STREAM (p, HCI_DISCONNECTION_COMP_EVT); //evt
            UINT8_TO_STREAM (p, 0);  //status
            UINT16_TO_STREAM (p, handle); //handle
            UINT8_TO_STREAM (p, reason); //reason
            if (btm_cb.ble_ctr_cb.mesh_enabled && (btm_cb.ble_ctr_cb.hci_evt_cb))
            {
              (btm_cb.ble_ctr_cb.hci_evt_cb)(p_disconn_data, 5);
            }
            p_lcb->mesh_gatt_link_conn_status = 0;
        }
#endif
