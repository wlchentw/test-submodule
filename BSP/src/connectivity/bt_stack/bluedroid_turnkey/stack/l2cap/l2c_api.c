...
void L2CA_DeregisterLECoc(UINT16 psm)
{
    ...
    if (p_rcb == NULL)
    {
        L2CAP_TRACE_WARNING("PSM: 0x%04x not found for deregistration", psm);
        return;
    }
...
/*******************************************************************************
**
** Function         L2CA_SetAclPriority_By_Tci
**
...
**
*******************************************************************************/
BOOLEAN L2CA_SetAclPriority_By_Tci (BD_ADDR bd_addr, UINT8 priority)
{
    L2CAP_TRACE_WARNING ("L2CA_SetAclPriority_By_Tci()  bdaddr: %02x%02x%02x%02x%04x, priority:%d",
                    bd_addr[0], bd_addr[1], bd_addr[2],
                    bd_addr[3], (bd_addr[4] << 8) + bd_addr[5], priority);

    return (l2cu_set_acl_priority_by_tci(bd_addr, priority));
}

/*******************************************************************************
**
** Function         L2CA_SetAclLinkType
**
** Description      Sets the link type for the channel. This is needed for scenarios where
**                  L2CAP needs to know the PSM type. For eg., need to identify
**                  A2DP_SRC and A2DP_SNK scenarios.
**
** Returns          TRUE if a valid channel, else FALSE
**
*******************************************************************************/
BOOLEAN L2CA_SetAclLinkType(BD_ADDR bd_addr, BOOLEAN is_a2dp_sink)
{
     tL2C_LCB            *p_lcb;
     L2CAP_TRACE_API("L2CA_SetAclLinkType   bdaddr: %02x%02x%02x%02x%04x, is_a2dp_sink:%s",
     bd_addr[0], bd_addr[1], bd_addr[2],
     bd_addr[3], (bd_addr[4] << 8) + bd_addr[5], ((is_a2dp_sink)?"true":"false"));

     /* First, find the link control block */
     if ((p_lcb = l2cu_find_lcb_by_bd_addr (bd_addr, BT_TRANSPORT_BR_EDR)) == NULL)
     {
         /* No link. Return false */
         L2CAP_TRACE_ERROR ("L2CAP - no LCB for L2CA_SetAclLinkType");
         return (FALSE);
     }
     p_lcb->is_acl_type_a2dp_sink = is_a2dp_sink;
     L2CAP_TRACE_DEBUG("L2CA_SetAclLinkType Setting acl_type_a2dp_sink to %s",
                      ((is_a2dp_sink)?"true":"false"));
     return (TRUE);
}
#endif
...
EXPORT_SYMBOL void L2CA_MeshSetConnStatus(BD_ADDR bd_addr, UINT16 handle)
{
    tL2C_LCB *p_lcb = NULL;
    if (0 == handle)
    {
    ...
    if (p_lcb)
    {
        p_lcb->mesh_gatt_link_conn_status = 1;
    }
}
#endif
