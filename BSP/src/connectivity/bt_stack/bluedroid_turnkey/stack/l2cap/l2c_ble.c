...
**
*******************************************************************************/
BOOLEAN L2CA_EnableUpdateBleConnParamsOnly (BD_ADDR rem_bda, BOOLEAN enable)
{
    if (stack_config_get_interface()->get_pts_conn_updates_disabled())
        return false;
...
void l2cble_notify_le_connection (BD_ADDR bda)
{
    ...
    if (p_lcb != NULL && p_acl != NULL && p_lcb->link_state != LST_CONNECTED)
    {
        ...
        L2CAP_TRACE_DEBUG("%s p_lcb->handle = 0x%04x, p_lcb->link_role = %d", __func__,
                  p_lcb->handle, p_lcb->link_role);
        if ((1 == p_lcb->mesh_gatt_link_conn_status) && (HCI_ROLE_MASTER == p_lcb->link_role))
        {
            UINT8 p_conn_data[19];
            UINT8 *p = p_conn_data;
            UINT8_TO_STREAM (p, HCI_BLE_CONN_COMPLETE_EVT);
            UINT8_TO_STREAM (p, 0);
            UINT16_TO_STREAM (p, p_lcb->handle);
            UINT8_TO_STREAM (p, p_lcb->link_role);
            UINT8_TO_STREAM (p, p_lcb->ble_addr_type);
            ARRAY_TO_STREAM (p, p_lcb->remote_bd_addr, BD_ADDR_LEN);
            UINT16_TO_STREAM (p, p_lcb->min_interval);
            UINT16_TO_STREAM (p, p_lcb->latency);
            UINT16_TO_STREAM (p, p_lcb->timeout);
            UINT8_TO_STREAM (p, 0);
            if (btm_cb.ble_ctr_cb.mesh_enabled && (btm_cb.ble_ctr_cb.hci_evt_cb))
            {
                (btm_cb.ble_ctr_cb.hci_evt_cb)(p_conn_data, 19);
            }
        }
#endif
    }
                      ...
                      UINT16 conn_interval, UINT16 conn_latency, UINT16 conn_timeout)
{
    ...
    else
    {
        ...
        if (!p_lcb)
        {
            L2CAP_TRACE_WARNING ("l2cble_conn_comp - p_lcb is NULL");
            return;
    }
...
static void l2cble_start_conn_update (tL2C_LCB *p_lcb)
{
    ...
    if (p_acl_cb == NULL)
    {
        BTM_TRACE_ERROR("l2cble_start_conn_update No connection exist!");
        return;
    }
...
void l2cble_credit_based_conn_req (tL2C_CCB *p_ccb)
{
    ...
    if (p_ccb->p_lcb == NULL)
    {
        L2CAP_TRACE_WARNING ("p_ccb->p_lcb doesn't exist");
        return;
    }
