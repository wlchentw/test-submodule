...
void btu_hcif_process_event (UNUSED_ATTR UINT8 controller_id, BT_HDR *p_msg)
{
    ...
    switch (hci_evt_code)
    {
        ...
        case HCI_BLE_EVENT:
            ...
            HCI_TRACE_VERBOSE("BLE HCI(id=%d) event = 0x%02x)", hci_evt_code,  ble_sub_code);
            ...
            switch (ble_sub_code)
            {
               ...
               case HCI_LE_PHY_UPDATE_COMPLETE_EVT:
                    btm_ble_process_phy_update_pkt(hci_evt_len - 1, p);
                    break;
               ...
               case HCI_LE_ADVERTISING_SET_TERMINATED_EVT:
                    btm_advertising_terminated_evt(p - 1, hci_evt_len);
                    break;
#endif
            }
            break;
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
void btu_hcif_send_extended_cmd (UNUSED_ATTR UINT8 controller_id, UINT16 opcode,
                                UINT8 param_len,UINT8 *p_param_buf, tBTM_EXT_CMPL_CB *p_cb)
{
    ...
    if (p_param_buf)
    {
        memcpy(pp, p_param_buf, param_len);
    }
...
EXPORT_SYMBOL void btu_hci_send_cmd(BT_HDR *p_buf, command_complete_cb complete_callback)
{
    if (!p_buf)
      return;
    ...
    uint16_t opcode;
    uint8_t *stream = p_buf->data + p_buf->offset;
    void * vsc_callback = *((void **)(p_buf + 1));;
...
/* BLE Commands sComplete*/
        ...
        case HCI_LE_REMOVE_ADVERTISING_SET:
            btm_extended_cmd_complete(p, opcode, evt_len, (tBTM_EXT_CMPL_CB *)p_cplt_cback);
            break;

        case HCI_BLE_READ_PHY:
            btm_ble_read_phy_complete(p, evt_len);
            break;
#endif
        ...
        case HCI_VENDOR_READ_PER:
            btm_read_vendor_per_complete(p);
            break;
        case HCI_VENDOR_READ_RSSI:
            btm_read_vendor_rssi_complete(p);
            break;
#endif
        default:
            if ((opcode & HCI_GRP_VENDOR_SPECIFIC) == HCI_GRP_VENDOR_SPECIFIC)
...
static void btu_ble_ll_conn_complete_evt ( UINT8 *p, UINT16 evt_len)
{
    ...
    if (status)
    {
        UINT8 *p_evt_data = p - 1;
        if (btm_cb.ble_ctr_cb.mesh_enabled && (btm_cb.ble_ctr_cb.hci_evt_cb))
        {
            (btm_cb.ble_ctr_cb.hci_evt_cb)(p_evt_data, *(p_evt_data - 1));
        }
    }
#endif
}
