                     ...
                     tGAP_CONN_CALLBACK *p_cb, tBT_TRANSPORT transport)
{
    ...
    if (transport == BT_TRANSPORT_LE)
    {
        ...
        if (!p_cfg)
        {
            GAP_TRACE_WARNING ("%s, p_cfg is null, set default mtu = %d", __func__, L2CAP_LE_DEFAULT_MTU);
            p_ccb->local_coc_cfg.mtu = L2CAP_LE_DEFAULT_MTU;
        }
        else
        {
...
static void gap_release_ccb (tGAP_CCB *p_ccb)
{
    ...
    if (GAP_MAX_CONNECTIONS <= xx) return;
