...
#define MAX_SIZE_MANUFACTURER_DATA 192
#else
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
#define MAX_SIZE_PROPRIETARY_ELEMENT 192
#else
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
#define BTM_BLE_EXT_ADV_ENB_EVT             5
#define BTM_BLE_EXT_ADV_DISABLE_EVT         6
#define BTM_BLE_EXT_ADV_DATA_EVT            7
#define BTM_BLE_ADV_SET_PERI_PARAM_EVT      8
#define BTM_BLE_ADV_SET_PERI_DATA_EVT       9
#define BTM_BLE_ADV_PERI_ENABLE_EVT         10
#define BTM_BLE_ADV_PERI_DISABLE_EVT        11
#define BTM_BLE_REMOVE_ADVERTISING_SET_EVT  12
#endif
...
typedef struct{
    ...
    tBTM_BLE_ADV_DATA adv_data;
    UINT16            adv_data_len;
}tBTM_BLE_EXT_ADV_DATA;

#define MAX_BTM_SIZE_ADV_DATA_LEN 1024
typedef struct
{
    UINT32 adv_len;
    UINT8 adv_data[MAX_BTM_SIZE_ADV_DATA_LEN];
}tBTM_BLE_PERI_ADV_DATA;

typedef struct {
  UINT8 enable;
  UINT16 min_interval;
  UINT16 max_interval;
  UINT16 periodic_advertising_properties;
...
typedef struct
{
    ...
    UINT8           secondary_advertising_phy;
    UINT8           scan_request_notification_enable;
}tBTM_BLE_ADVERTISING_PARAMS;

typedef struct
{
    tBTM_BLE_ADVERTISING_PARAMS   adv_params;
    tBTM_BLE_PERI_ADV_DATA        adv_data;
    tBTM_BLE_PERI_ADV_DATA        adv_scan_rsp_data;
    tBTM_BLE_PERIODIC_ADV_PARAMS  peri_params;
    tBTM_BLE_PERI_ADV_DATA        peri_adv_data;
    UINT16                        duration;
    UINT8                         maxExtAdvEvents;
}tBTM_BLE_ADVERTISING_SET;
#endif
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
    UINT16                      advertising_event_properties;
    UINT16                      duration;  // 1 unit is 10ms
    UINT8                       maxExtAdvEvents;
    INT8                        tx_power;
    BOOLEAN                     periodic_enabled;
    UINT32                      advertising_interval;  // 1 unit is 0.625 ms
    tBTM_BLE_EXT_ADV_DATA       adv_data_no_scan_rsp;
    tBTM_BLE_EXT_ADV_DATA       adv_data_scan_rsp;
#endif
 ...
 *
 ******************************************************************************/
extern void BTM_BleReadPhy(BD_ADDR bd_addr, tBTM_READ_PHY_CALLBACK *cb);
 ...
 *
 ******************************************************************************/
extern void BTM_BleSetPhy(BD_ADDR bd_addr, uint8_t tx_phys,
                          uint8_t rx_phys, uint16_t phy_options);

extern tBTM_STATUS BTM_BleStartAdvertisingSet (tBTM_BLE_ADVERTISING_SET *p_adv_info,
                                      tBTM_BLE_MULTI_ADV_CBACK *p_cback,
                                      void *p_ref);

extern tBTM_STATUS BTM_BlePeriodicAdvsetParams (UINT8 inst_id,
                                          tBTM_BLE_PERIODIC_ADV_PARAMS *p_params,
                                          UINT8 cb_evt);

extern tBTM_STATUS BTM_BlePeriodicAdvEnable (UINT8 inst_id, UINT8 enable);

extern tBTM_STATUS BTM_BleExtOrMultiAdvSetdata(BTM_BLE_ADV_DATA_TYPE adv_data_type, UINT8 inst_id,
                                                                  UINT8 *p_data, UINT32 len);

extern tBTM_STATUS BTM_BleStopAdvertisingSet (UINT8 inst_id);

extern tBTM_STATUS BTM_BleAdvertisingSetParam (UINT8 inst_id, tBTM_BLE_ADVERTISING_PARAMS *p_params);
#endif
