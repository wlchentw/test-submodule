...
typedef enum
{
    BTM_BLE_ADV_DATA,                    /*0 scan advertising data*/
    BTM_BLE_ADV_SCAN_RSP_DATA,           /*1 scan response data*/
    BTM_BLE_ADV_PERIODIC_DATA            /*2 periodic advertising data*/
}BTM_BLE_ADV_DATA_TYPE;
#endif
...
/* Structure returned with Extended Command complete callback */
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
typedef struct
{
    UINT16  opcode;
...
/* Extended callback function for notifying an application that a synchronous
** BTM function is complete. The pointer contains the address of any returned data.
*/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
typedef void (tBTM_EXT_CMPL_CB) (tBTM_EXT_CMPL *p1);
#endif
/*****************************************************************************
...
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
#define PHY_LE_NO_PACKET 0x00;
#define PHY_LE_1M  0x01;
#define PHY_LE_2M  0x02;
#define PHY_LE_CODED  0x03;
#endif
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
/* default security key distribution value, not open SMP_SEC_KEY_TYPE_LK */
/* SMP key types */
/* SMP_SEC_KEY_TYPE_ENC                  (1 << 0)    encryption key */
/* SMP_SEC_KEY_TYPE_ID                   (1 << 1)    identity key  */
/* SMP_SEC_KEY_TYPE_CSRK                 (1 << 2)    slave CSRK  */
/* SMP_SEC_KEY_TYPE_LK                   (1 << 3)    BR/EDR link key */
#define BTM_BLE_INITIATOR_KEY_SIZE 7
#define BTM_BLE_RESPONDER_KEY_SIZE 7
#else
...
typedef struct
{
    ...
    tBTM_AUTH_COMPLETE_CALLBACK *p_auth_complete_callback; /* bta_dm_authentication_complete_cback */
    tBTM_BOND_CANCEL_CMPL_CALLBACK *p_bond_cancel_cmpl_callback;
    tBTM_SP_CALLBACK            *p_sp_callback; /* bta_dm_sp_cback */
#if BLE_INCLUDED == TRUE
...
#endif
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
typedef UINT8 (tBTM_READ_PHY_CALLBACK) (BD_ADDR bd_addr, uint8_t tx_phy, uint8_t rx_phy, uint8_t status);
void BTM_BleReadPhy(BD_ADDR bd_addr, tBTM_READ_PHY_CALLBACK *cb);
tBTM_STATUS BTM_BleSetDefaultPhy(uint8_t all_phys, uint8_t tx_phys, uint8_t rx_phys);
void BTM_BleSetPhy(BD_ADDR bd_addr, uint8_t tx_phys, uint8_t rx_phys, uint16_t phy_options);
#endif
/*******************************************************************************
**
...
*******************************************************************************/
#if defined(MTK_BLE_BT_5_0) && (MTK_BLE_BT_5_0 == TRUE)
extern tBTM_STATUS BTM_ExtendedCommand(UINT16 opcode,
                                             UINT8 param_len,
                                             UINT8 *p_param_buf,
                                             tBTM_EXT_CMPL_CB *p_cb);
#endif
...
**
** Description      This function is called to read packet error rate.
**
** Returns          BTM_CMD_STARTED if command issued to controller.
**                  BTM_NO_RESOURCES if couldn't allocate memory to issue command
**                  BTM_UNKNOWN_ADDR if no active link with bd addr specified
...
**
*******************************************************************************/
extern tBTM_STATUS BTM_ReadVndPER (tBTM_CMPL_CB *p_cb);
...
**
*******************************************************************************/
extern tBTM_STATUS BTM_ReadVndRSSI (uint8_t index, tBTM_CMPL_CB *p_cb);
#endif
#ifdef __cplusplus
}
#endif
