...
void btsnd_hcic_ext_ble_rand (void *p_cmd_cplt_cback)
{
    btu_hcif_send_extended_cmd(LOCAL_BR_EDR_CONTROLLER_ID, HCI_BLE_RAND, HCIC_PARAM_SIZE_BLE_RAND,
                               NULL, p_cmd_cplt_cback);
}
...
void btsnd_hcic_ble_set_extended_scan_params(UINT8 own_address_type,
...
BOOLEAN btsnd_hcic_ble_read_phy (UINT16 handle)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_BLE_READ_PHY;
    p->offset = 0;
...
BOOLEAN btsnd_hcic_ble_set_default_phy (UINT8 all_phys, UINT8 tx_phys, UINT8 rx_phys)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_BLE_SET_DEFAULT_PHY;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_BLE_SET_DEFAULT_PHY);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_BLE_SET_DEFAULT_PHY);
...
BOOLEAN btsnd_hcic_ble_set_phy (UINT16 handle, UINT8 all_phys, UINT8 tx_phys, UINT8 rx_phys, UINT16 phy_options)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_BLE_SET_PHY;
    p->offset = 0;
...
BOOLEAN btsnd_hcic_ble_ext_create_conn(UINT8 init_filter_policy,
                                    UINT8 addr_type_own,
                                    UINT8 addr_type_peer,
                                    BD_ADDR bda_peer,
