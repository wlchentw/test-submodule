                               ...
                               UINT16 clock_offset, UINT8 allow_switch)
{
    ...
    HCI_TRACE_WARNING("%s, create connection to %02x:%02x:%02x:%02x:%02x:%02x", \
        __FUNCTION__, (UINT8)dest[0], (UINT8)dest[1], (UINT8)dest[2], \
        (UINT8)dest[3], (UINT8)dest[4], (UINT8)dest[5]);
...
BOOLEAN btsnd_hcic_accept_conn (BD_ADDR dest, UINT8 role)
{
    ...
    HCI_TRACE_WARNING("%s, accept connection from %02x:%02x:%02x:%02x:%02x:%02x", \
        __FUNCTION__, (UINT8)dest[0], (UINT8)dest[1], (UINT8)dest[2], \
        (UINT8)dest[3], (UINT8)dest[4], (UINT8)dest[5]);
...
bool btsnd_hcic_read_vendor_rssi (uint16_t handle)
{
    BT_HDR* p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    ...
    UINT16_TO_STREAM (pp, HCI_VENDOR_READ_RSSI);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_CMD_HANDLE);
...
bool btsnd_hcic_read_vendor_per (void)
{
    BT_HDR* p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    uint8_t* pp = (uint8_t *)(p + 1);
    ...
    UINT16_TO_STREAM (pp, HCI_VENDOR_READ_PER);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_READ_CMD);
