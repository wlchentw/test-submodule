...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
static void printCodecInfo(tAVDT_SCB *p_scb)
{
    tAVDT_CFG p_cfg;
    UINT8 i = 0;
    if (NULL == p_scb)
    {
        AVDT_TRACE_WARNING("printCodecInfo. p_scb is NULL");
    }
    else
    {
        p_cfg = p_scb->cs.cfg;
        AVDT_TRACE_DEBUG("printCodecInfo:", p_cfg.codec_info[i]);
        for (i = 0; i < AVDT_CODEC_SIZE; i++)
        {
            AVDT_TRACE_DEBUG("p_cfg.codec_info[%d] = %d ", i, p_cfg.codec_info[i]);
        }
    }
}
#endif
...
void avdt_ccb_hdl_discover_cmd(tAVDT_CCB *p_ccb, tAVDT_CCB_EVT *p_data)
{
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    tAVDT_SCB           *p_scb = &p_ccb->scb[0];
    tAVDT_SCB           *p_scb1 = NULL;
    tAVDT_CCB           *p_ccb1; // used to query current connect sep information
    int                 role = -1;
#else
    tAVDT_SCB           *p_scb = &avdt_cb.scb[0];
#endif
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    // ck: query all cb
    for (i = 0; i < AVDT_NUM_LINKS; i++) {
        p_ccb1 = &avdt_cb.ccb[i];
        if (p_ccb1) {
            for(int j = 0; j < AVDT_NUM_SEPS; j++) {
                p_scb1 = &p_ccb1->scb[j];
                if (p_scb1->in_use == 1) {
                    AVDT_TRACE_DEBUG("scb is in use, i:%d, j:%d, current link role:%d", i, j, p_scb1->cs.tsep);
                    role = p_scb1->cs.tsep;
                }
            }
        } else {
            AVDT_TRACE_DEBUG("ccb[%d] is null", i);
        }
    }

    AVDT_TRACE_DEBUG("role:%d", role);
#endif
    AVDT_TRACE_DEBUG("aac_enabled %d", aac_enabled);
    AVDT_TRACE_DEBUG("ste_enabled %d", ste_enabled);
    AVDT_TRACE_DEBUG("lhdc_enabled %d", lhdc_enabled);
    ...
    for (i = 0; i < AVDT_NUM_SEPS; i++, p_scb++)
    {
        AVDT_TRACE_DEBUG("ccb[%d].scb[%d] allocated %d", avdt_ccb_to_idx(p_ccb), i, p_scb->allocated);
        if (p_scb->allocated)
        {
#if (defined(MTK_A2DP_SRC_AAC_CODEC) && (MTK_A2DP_SRC_AAC_CODEC == TRUE) || defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE))
            APPL_TRACE_DEBUG("SEP codec_type:%d", p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX]);
            if (p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX] == A2D_MEDIA_CT_M24)
            {
                if (!aac_enabled)
                {
                    APPL_TRACE_DEBUG("AAC disabled ,don't expose AAC SEP to the peer device.");
                    continue;
                }
            }
#endif
            ...
            APPL_TRACE_DEBUG("SEP codec_type:%d", p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX]);
            if (p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX] == A2D_MEDIA_CT_STE)
            {
                if (!ste_enabled)
                {
                    APPL_TRACE_DEBUG("STE disabled ,don't expose STE SEP to the peer device.");
                    continue;
                }
            }
#endif
            ...
            APPL_TRACE_DEBUG("SEP codec_type:%d", p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX]);
            if (p_scb->cs.cfg.codec_info[AVDT_CODEC_TYPE_INDEX] == A2D_MEDIA_CT_NON)
            {
                if (!lhdc_enabled)
                {
                    APPL_TRACE_DEBUG("lhdc disabled ,don't expose lhdc SEP to the peer device.");
                    continue;
                }
            }
            ...
            /* copy sep info */
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
            if (role != -1 && p_scb->cs.tsep != role) {
                sep_info[p_data->msg.discover_rsp.num_seps].in_use = 1;
                AVDT_TRACE_DEBUG("set sep role to in use only by sep_info");
            } else {
                sep_info[p_data->msg.discover_rsp.num_seps].in_use = p_scb->in_use;
                AVDT_TRACE_DEBUG("set sep role to %d", sep_info[p_data->msg.discover_rsp.num_seps].in_use);
            }
#else
            sep_info[p_data->msg.discover_rsp.num_seps].in_use = p_scb->in_use;
#endif
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
            sep_info[p_data->msg.discover_rsp.num_seps].seid = p_scb->handle;
#else
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            AVDT_TRACE_DEBUG("avdt_ccb_hdl_discover_cmd.(in_use:%s, i:%d, seid:%d, media_type:%d, tsep:%d)",
                            p_scb->in_use ? "TRUE" : "FALSE", i,
                            sep_info[p_data->msg.discover_rsp.num_seps].seid,
                            p_scb->cs.media_type, p_scb->cs.tsep);
#endif
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    AVDT_TRACE_DEBUG("avdt_ccb_hdl_getcap_cmd. seid:%d", p_data->msg.single.seid);
    printCodecInfo(p_scb);
#endif
