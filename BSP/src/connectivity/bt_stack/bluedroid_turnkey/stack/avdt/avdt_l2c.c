...
void avdt_l2c_connect_ind_cback(BD_ADDR bd_addr, UINT16 lcid, UINT16 psm, UINT8 id)
{
    ...
    {
        /* no, allocate ccb */
        channel_index = BTA_AvObtainPeerChannelIndex(bd_addr);
        if (channel_index >= 0)
        {
            p_ccb = avdt_ccb_alloc_by_peer_id(bd_addr, channel_index);
        }
        if (p_ccb == NULL)
        {
            AVDT_TRACE_ERROR("%s find and alloc fail.", __func__);
