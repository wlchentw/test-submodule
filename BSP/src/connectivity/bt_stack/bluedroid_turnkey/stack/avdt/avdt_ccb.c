...
void avdt_ccb_event(tAVDT_CCB *p_ccb, UINT8 event, tAVDT_CCB_EVT *p_data)
{
    ...
    for (i = 0; i < AVDT_CCB_ACTIONS; i++)
    {
        ...
        {
#if AVDT_DEBUG == TRUE
            AVDT_TRACE_EVENT("CCB action=%s", avdt_ccb_action_str[action]);
#endif
...
void avdt_ccb_release(BD_ADDR bd_addr)
{
    tAVDT_CCB   *p_ccb = &avdt_cb.ccb[0];
    int         i;

    AVDT_TRACE_ERROR("release ccb for addr %02x-%02x-%02x-%02x-%02x-%02x",
            bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5]);
    ...
    if (i == AVDT_NUM_LINKS)
    {
        AVDT_TRACE_WARNING("Not release ccb for addr %02x-%02x-%02x-%02x-%02x-%02x",
            bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5]);
    }
    return;
}
...
tAVDT_CCB *avdt_ccb_alloc_by_peer_id(BD_ADDR bd_addr, UINT8 peer_id)
{
    tAVDT_CCB   *p_ccb = NULL;

    if (peer_id >= AVDT_NUM_LINKS) return NULL;

    if (avdt_cb.ccb[peer_id].allocated) return NULL;

    p_ccb = &avdt_cb.ccb[peer_id];
    ...
    p_ccb->ret_ccb_timer = alarm_new("avdt_ccb.ret_ccb_timer");
    p_ccb->rsp_ccb_timer = alarm_new("avdt_ccb.rsp_ccb_timer");
    AVDT_TRACE_DEBUG("avdt_ccb_alloc_by_peer_id %d", peer_id);
...
void avdt_ccb_dealloc(tAVDT_CCB *p_ccb, tAVDT_CCB_EVT *p_data)
{
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    {
        ...
        memcpy(scb, p_ccb->scb, sizeof(scb));
        memset(p_ccb, 0, sizeof(tAVDT_CCB));
        memcpy(p_ccb->peer_addr, bd_addr, BD_ADDR_LEN);
        memcpy(p_ccb->scb, scb, sizeof(scb));
    }
#else
    memset(p_ccb, 0, sizeof(tAVDT_CCB));
#endif
