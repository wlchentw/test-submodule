...
/* action function list */
const char* avdt_scb_action_str[] = {
    "avdt_scb_hdl_abort_cmd",
    "avdt_scb_hdl_abort_rsp",
    "avdt_scb_hdl_close_cmd",
    "avdt_scb_hdl_close_rsp",
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE) // tt pts test -- AVDTP/SNK/INT/SIG/SMG/BV-11-C
/* API_SETCONFIG_RSP_EVT */ {AVDT_SCB_SND_SETCONFIG_RSP,    AVDT_SCB_SND_GETCONFIG_REQ, AVDT_SCB_CONF_ST},
#else
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)// tt pts test for AVDTP/SNK/ACP/SIG/SMG/BV-14-C
/* API_RECONFIG_RSP_EVT */  {AVDT_SCB_SND_RECONFIG_RSP,     AVDT_SCB_IGNORE,            AVDT_SCB_OPENING_ST},
#else
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)// tt pts test for AVDTP/SNK/ACP/SIG/SMG/ESR05/BV-14-C
/* MSG_RECONFIG_CMD_EVT */  {AVDT_SCB_HDL_RECONFIG_CMD,     AVDT_SCB_IGNORE,            AVDT_SCB_OPENING_ST},
#else
...
void avdt_scb_event(tAVDT_SCB *p_scb, UINT8 event, tAVDT_SCB_EVT *p_data)
{
    ...
    for (i = 0; i < AVDT_SCB_ACTIONS; i++)
    {
        ...
        {
#if AVDT_DEBUG == TRUE
            AVDT_TRACE_EVENT("SCB action=%s", avdt_scb_action_str[action]);
#endif
...
tAVDT_SCB *avdt_scb_alloc(tAVDT_CS *p_cs, UINT8 peer_id)
#else
tAVDT_SCB *avdt_scb_alloc(tAVDT_CS *p_cs)
#endif
{
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    if (peer_id >= AVDT_NUM_LINKS || peer_id < 0)
    {
        return NULL;
    }
    tAVDT_SCB   *p_scb = &avdt_cb.ccb[peer_id].scb[0];
#else
    ...
    for (i = 0; i < AVDT_NUM_SEPS; i++, p_scb++)
    {
        ...
        {
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
            AVDT_TRACE_DEBUG("avdt_scb_alloc ccb[%d].scb[%d]", peer_id, i);
#endif
            memset(p_scb,0,sizeof(tAVDT_SCB));
            p_scb->allocated = TRUE;
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
            p_scb->handle = peer_id * AVDT_NUM_SEPS + i + 1;
#endif
            p_scb->p_ccb = NULL;
...
tAVDT_SCB *avdt_scb_by_hdl(UINT8 hdl)
{
    ...
    /* verify index */
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    if ((hdl > 0) && (hdl <= AVDT_NUM_SEPS * AVDT_NUM_LINKS))
#else
    if ((hdl > 0) && (hdl <= AVDT_NUM_SEPS))
#endif
    {
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
        p_scb = &avdt_cb.ccb[(hdl-1)/AVDT_NUM_SEPS].scb[(hdl-1)%AVDT_NUM_SEPS];
#else
        ...
        if (!p_scb->allocated)
        {
            p_scb = NULL;
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
            AVDT_TRACE_WARNING("avdt_cb.ccb[%d].scb[%d] hdl %d not allocated",
                (hdl-1)/AVDT_NUM_SEPS, (hdl-1)%AVDT_NUM_SEPS, hdl);
#else
    ...
    for (i = 0, *p_err_code = 0; (i < num_seid) && (*p_err_code == 0) && (i < AVDT_NUM_SEPS); i++)
    {
        ...
        switch (state) {
          ...
          case AVDT_VERIFY_START:
            if ((NULL != p_scb) && p_scb->state != AVDT_SCB_OPEN_ST && p_scb->state != AVDT_SCB_STREAM_ST)
              *p_err_code = AVDT_ERR_BAD_STATE;
            break;

          case AVDT_VERIFY_SUSPEND:
          case AVDT_VERIFY_STREAMING:
            if ((NULL != p_scb) && p_scb->state != AVDT_SCB_STREAM_ST)
              *p_err_code = AVDT_ERR_BAD_STATE;
            break;
        }
    }
