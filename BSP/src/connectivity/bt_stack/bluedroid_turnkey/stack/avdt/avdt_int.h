...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    struct tAVDT_CCB_STRUCT *p_ccb;         /* ccb associated with this scb */
#else
...
#endif
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
    UINT32          handle;         /* from 1 to max_link * max_sep */
#endif
} tAVDT_SCB;
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
/* channel control block type */
typedef struct tAVDT_CCB_STRUCT{
    BD_ADDR             peer_addr;      /* BD address of peer */
    ...
    BOOLEAN             reconn;         /* If TRUE, reinitiate connection after transitioning from CLOSING to IDLE state */
    UINT8               ret_count;      /* Command retransmission count */
/// M : add code to handle AVDTP connection collision-ALPS04725350 @{
...
#if defined(MTK_A2DP_DUAL_AUDIO) && (MTK_A2DP_DUAL_AUDIO == TRUE)
extern tAVDT_CCB *avdt_ccb_alloc_by_peer_id(BD_ADDR bd_addr, UINT8 peer_id);
#endif
