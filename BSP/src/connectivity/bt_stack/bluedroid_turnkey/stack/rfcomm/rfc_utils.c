...
void rfc_release_multiplexer_channel (tRFC_MCB *p_mcb)
{
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    /* this may be invoked in timer callback, so we cannot free it */
    mcb_timer = p_mcb->mcb_timer;
    cmd_q = p_mcb->cmd_q;
#else
    alarm_free(p_mcb->mcb_timer);
    ...
    memset (p_mcb, 0, sizeof (tRFC_MCB));
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    /* they will be released in allocate function */
    p_mcb->mcb_timer = mcb_timer;
    p_mcb->cmd_q = cmd_q;
#endif
