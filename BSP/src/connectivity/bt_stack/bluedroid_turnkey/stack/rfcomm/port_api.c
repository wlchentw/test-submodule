...
int PORT_SetDataCallback (UINT16 port_handle, tPORT_DATA_CALLBACK *p_port_cb)
{
    ...
    RFCOMM_TRACE_API ("PORT_SetDataCallback() handle:%d cb %p", port_handle, p_port_cb);
...
int PORT_SetDataCOCallback (UINT16 port_handle, tPORT_DATA_CO_CALLBACK *p_port_cb)
{
    ...
    RFCOMM_TRACE_API ("PORT_SetDataCOCallback() handle:%d cb 0X%p", port_handle, p_port_cb);
...
void RFCOMM_DeInit (void)
{
    ...
    {
        if (rfc_cb.port.rfc_mcb[i].mcb_timer != NULL)
        {
            alarm_free(rfc_cb.port.rfc_mcb[i].mcb_timer);
        }
        if (rfc_cb.port.rfc_mcb[i].cmd_q != NULL)
        {
            fixed_queue_free(rfc_cb.port.rfc_mcb[i].cmd_q, NULL);
        }
    }
}
#endif
