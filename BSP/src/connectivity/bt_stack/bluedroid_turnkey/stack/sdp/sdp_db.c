...
**
*******************************************************************************/
tSDP_RECORD *sdp_db_service_search (tSDP_RECORD *p_rec, tSDP_UUID_SEQ *p_seq, BOOLEAN only_service_class)
{
    UINT16          xx, yy;
    ...
    for ( ; p_rec < p_end; p_rec++)
    {
        for (yy = 0; yy < p_seq->num_uids; yy++)
        {
            ...
            {
                if (only_service_class && p_attr->id != ATTR_ID_SERVICE_CLASS_ID_LIST)
                {
                    continue;
                }
