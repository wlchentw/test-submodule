...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    sdp_cb.pending_q = fixed_queue_new(SIZE_MAX);
#endif
...
void sdp_deinit (void)
{
    if (sdp_cb.pending_q)
    {
        fixed_queue_free(sdp_cb.pending_q, NULL);
        sdp_cb.pending_q = NULL;
    }
}
#endif
...
*
**
** Function         sdp_connection_pending_request_queue
**
** Description      This function is called to queued up the sdp request.
**                  if we are the originator of the connection, we are the SDP
...
**
*******************************************************************************/
bool sdp_connection_pending_request_queue(uint8_t con_functon, uint8_t *p_bd_addr,
                                  tSDP_DISCOVERY_DB *p_db, tSDP_DISC_CMPL_CB *p_cb,
                                  tSDP_DISC_CMPL_CB2 *p_cb2, void * user_data)
{
    bool ret = fixed_queue_is_empty(sdp_cb.pending_q);
    tCONN_PCB *p_pcb = (tCONN_PCB *)osi_malloc(sizeof(tCONN_PCB));
    p_pcb->con_functon = con_functon;
    memcpy (&p_pcb->device_address[0], p_bd_addr, sizeof (BD_ADDR));
    p_pcb->p_db = p_db;
    p_pcb->p_cb = p_cb;
    p_pcb->p_cb2 = p_cb2;
    p_pcb->user_data = user_data;
    fixed_queue_enqueue(sdp_cb.pending_q, (void *)p_pcb);
    return !ret;
}
/******************************************************************************
*
**
** Function         sdp_pending_request_cback
**
** Description      This function is called to handle callback and pending
request.
**                  if we are the originator of the connection, we are the SDP
**                  client, and the received message is queued up for the
client.
...
**
*******************************************************************************/
void sdp_pending_request_cback(uint16_t result)
{
    sdp_pending_request_cback2(result, NULL);
}
/******************************************************************************
*
**
** Function         sdp_pending_request_cback2
**
** Description      This function is called to handle callback and pending
request.
**                  if we are the originator of the connection, we are the SDP
...
**
*******************************************************************************/
void sdp_pending_request_cback2(uint16_t result, void* user_data)
{
    tCONN_PCB   *p_pcb;
    tCONN_CB     *p_ccb;
    bool        is_empty;

    p_pcb = (tCONN_PCB *)fixed_queue_dequeue(sdp_cb.pending_q);
    is_empty = fixed_queue_is_empty(sdp_cb.pending_q);
    if (p_pcb->p_cb)
    {
        (*p_pcb->p_cb) (result);
    }
    ...
    /*if SDP connection pneding request exist, do it now*/
    p_pcb = (tCONN_PCB *)fixed_queue_try_peek_first(sdp_cb.pending_q);
    SDP_TRACE_DEBUG("sdp_pending_request_cback2 handle pending request");
    /* Specific BD address */
    p_ccb = sdp_conn_originate (p_pcb->device_address);
    ...
    if (!p_ccb)
    {
        ...
        if (p_pcb->p_cb)
        {
            sdp_pending_request_cback(SDP_CONN_FAILED);
        }
        else if (p_pcb->p_cb2)
        {
            sdp_pending_request_cback2 (SDP_CONN_FAILED, p_pcb->user_data);
        }
        return;
    }
    p_ccb->disc_state = SDP_DISC_WAIT_CONN;
    p_ccb->p_db = p_pcb->p_db;
    if (p_pcb->con_functon == SDP_SS_REQUEST)
    {
        p_ccb->p_cb = sdp_pending_request_cback;
    }
    else if (p_pcb->con_functon == SDP_SSA_REQUEST)
    {
        p_ccb->p_cb = sdp_pending_request_cback;
        p_ccb->is_attr_search = TRUE;
    }
    else if (p_pcb->con_functon == SDP_SSA_REQUEST2)
    {
        p_ccb->p_cb2 = sdp_pending_request_cback2;
        p_ccb->is_attr_search = TRUE;
        p_ccb->user_data = p_pcb->user_data;
    }
}
#endif
