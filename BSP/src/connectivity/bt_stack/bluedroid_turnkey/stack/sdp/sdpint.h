...
/* Define the SDP pending Connection Control Block */
typedef struct
{
#define SDP_SS_REQUEST           0
#define SDP_SSA_REQUEST          1
#define SDP_SSA_REQUEST2         2
    uint8_t           con_functon;
    BD_ADDR        device_address;
    tSDP_DISCOVERY_DB *p_db;                    /* Database to save info into   */
    tSDP_DISC_CMPL_CB *p_cb;                    /* Callback for discovery done  */
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
extern bool sdp_connection_pending_request_queue(uint8_t conn_functon, uint8_t *p_bd_addr,
                                  tSDP_DISCOVERY_DB *p_db, tSDP_DISC_CMPL_CB *p_cb,
                                  tSDP_DISC_CMPL_CB2 *p_cb2, void * user_data);
extern void sdp_pending_request_cback(uint16_t result);
extern void sdp_pending_request_cback2(uint16_t result, void* user_data);
#endif
...
*/
extern tSDP_RECORD    *sdp_db_service_search (tSDP_RECORD *p_rec, tSDP_UUID_SEQ *p_seq, BOOLEAN only_service_class);
