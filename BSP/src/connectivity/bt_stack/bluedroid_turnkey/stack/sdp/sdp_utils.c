...
tCONN_CB *sdpu_allocate_ccb (void)
{
    ...
    for (xx = 0, p_ccb = sdp_cb.ccb; xx < SDP_MAX_CONNECTIONS; xx++, p_ccb++)
    {
        if (p_ccb->con_state == SDP_STATE_IDLE)
        {
            ...
            {
                SDP_TRACE_DEBUG("before alarm_free");
                alarm_free(p_ccb->sdp_conn_timer);
            }
            else
            {
                SDP_TRACE_DEBUG("sdp_conn_timer is NULL");
            }
#endif
...
{
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
     /* here is run into the callback function of sdp_conn_timer timeout,
           should not free sdp_conn_timer;
           otherwise, crash issue will occur .
     */
...
UINT16 sdpu_get_list_len(tSDP_UUID_SEQ *uid_seq, tSDP_ATTR_SEQ *attr_seq)
{
    ...
    for (p_rec = sdp_db_service_search (NULL, uid_seq, FALSE); p_rec; p_rec = sdp_db_service_search (p_rec, uid_seq, FALSE))
    {
        len += 3;
