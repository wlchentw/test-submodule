...
void wifi_tx(void)
{
	...
	if(txGain != Default_TxGain){
		...
		retval = WIFI_TEST_SetTxPath(0x01);
		printf("(%s) Set Tx SetTxPath to 0x01\n", retval ? "success":"fail");
		if (retval == 0) return;
		retval = WIFI_TEST_GETPWROFF(&Txpower);
		printf("(%s) Get TxPower FromNvram is 0x%2x\n", retval ? "success":"fail" , Txpower);
		if (retval == 0) return;
		txGain = Txpower/2.0;
		retval = WIFI_TEST_TxGain(txGain);
		printf("(%s) Set Tx power gain to %.2f dBm\n", retval ? "success":"fail", txGain);
		if (retval == 0) return;
		txGain = Default_TxGain;
	}
