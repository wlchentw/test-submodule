...
int ntx_firmware_parse_fw_buf_ex( volatile NTX_FIRMWARE_HDR *I_ptFWHDR,unsigned long I_dwFWSize,
		NTX_FIRMWARE_PARSE_PROC_EX *I_pfnFWParseProc,void *IO_pvParam)
{
	...
	if(0!=strcmp(I_ptFWHDR->szSignatureS,gszNTXFW_parser_SignatureS)) {
		...
		printk(KERN_ERR"[NTX][%s][%s] !!\n",I_ptFWHDR->szSignatureS,gszNTXFW_parser_SignatureS);
		return -1;
	}
		...
		for (w=0;w<I_ptFWHDR->wFirmwareItems;w++)
		{
			...
			if(0==memcmp(L_ptFWItemHdr->bMagicA,gbNTXFW_parser_item_magicA,sizeof(gbNTXFW_parser_item_magicA))) {
				if(I_pfnFWParseProc(I_ptFWHDR,L_ptFWItemHdr,L_pvFWItemBin,(int)w,IO_pvParam)>=0) {
					...
					printk(KERN_ERR"[NTX][%s_%d] firmware item parsing failed  !!\n",__FUNCTION__,__LINE__);
					iRet = -2;break;
				}
			}
			else {
				// firmware magic error ! 
				printk(KERN_ERR"[NTX][%s_%d] firmware magic error !!\n",__FUNCTION__,__LINE__);
				printk(KERN_ERR"[NTX][%x_%x_%x_%x_%x_%x_%x_%x]",gbNTXFW_parser_item_magicA[0],gbNTXFW_parser_item_magicA[1],gbNTXFW_parser_item_magicA[2],gbNTXFW_parser_item_magicA[3],
				gbNTXFW_parser_item_magicA[4],gbNTXFW_parser_item_magicA[5],gbNTXFW_parser_item_magicA[6],gbNTXFW_parser_item_magicA[7]);
				printk(KERN_ERR"[NTX][%x_%x_%x_%x_%x_%x_%x_%x]",L_ptFWItemHdr->bMagicA[0],L_ptFWItemHdr->bMagicA[1],L_ptFWItemHdr->bMagicA[2],L_ptFWItemHdr->bMagicA[3],
				L_ptFWItemHdr->bMagicA[4],L_ptFWItemHdr->bMagicA[5],L_ptFWItemHdr->bMagicA[6],L_ptFWItemHdr->bMagicA[7]);
				iRet = -3;break;
			}
