...
#define NTX_FW_TYPE_TLC5947_24CH_TAB_HDR					8// frontlight current table header for tlc5947/ricoh for each combinations .
#define NTX_FW_TYPE_TLC5947_24CH_TAB					9// frontlight current table for tlc5947/ricoh for each combinations .
#define NTX_FW_TYPE_TLC5947_DUALFL_HDR					10// frontlight table header for tlc5947 for each 100% table which combine 2colors .
#define NTX_FW_TYPE_TLC5947_DUALFL_PERCENTTAB		11// frontlight percent table for tlc5947 for each combinations on 2colors models .
#define NTX_FW_TYPE_LM3630_DUALFL_X2_HDR				12// frontlight table header for lm3630 for each 100% table which combine 2colors .
#define NTX_FW_TYPE_LM3630_DUALFL_X2_PERCENTTAB		13 //
#define NTX_FW_TYPE_LM36922_DUALFL_X2_HDR			14
#define NTX_FW_TYPE_LM36922_DUALFL_X2_PERCENTTAB	15
#define NTX_FW_TYPE_AW99703_DUALFL_X2_HDR			16
#define NTX_FW_TYPE_AW99703_DUALFL_X2_PERCENTTAB	17
...
typedef struct tagNTX_FW_LM36922FL_DUALFL_percent_tab {
	char szColorName[32]; // 
	unsigned short bBrightnessA[100]; // brightness percentage table of Ch1 .
	unsigned short bBrightnessB[100]; // brightness percentage table of Ch2 .
	unsigned long dwCurrentA[100]; 	// current table in each combination .
	unsigned char bReservedA[8];
}NTX_FW_LM36922FL_DUALFL_percent_tab;

// firmware descriptions for type NTX_FW_TYPE_LM36922_DUALFL_HDR
typedef struct tagNTX_FW_LM36922FL_dualcolor_hdr {
	unsigned long dwTotalColors; // total color temperatures .
	unsigned char bReservedA[8]; // reserved .
}NTX_FW_LM36922FL_dualcolor_hdr;
...
typedef struct tagNTX_FW_AW99703FL_DUALFL_percent_tab {
	char szColorName[32]; // 
	...
	unsigned short bC1_BrightnessA[100]; // brightness percentage table of Ch1 .
	unsigned char bC1_CurrentA[100]; // power/current percentage table of Ch1 .
	unsigned short bC2_BrightnessA[100]; // brightness percentage table of Ch2 .
	unsigned char bC2_CurrentA[100]; // power/current percentage table of Ch2 .
	unsigned long dwCurrentA[100]; // current table in each combination .
	unsigned char bReservedA[8];
}NTX_FW_AW99703FL_DUALFL_percent_tab;

// firmware descriptions for type NTX_FW_TYPE_LM36922_DUALFL_HDR
typedef struct tagNTX_FW_AW99703FL_dualcolor_hdr {
	unsigned long dwTotalColors; // total color temperatures .
	...
	unsigned char bReservedA[8]; // reserved .
}NTX_FW_AW99703FL_dualcolor_hdr;
...
//
// firmware descriptions for type NTX_FW_TYPE_TLC5947_DUALFL_PERCENTTAB
//
typedef struct tagNTX_FW_TLC5947FL_dualcolor_percent_tab {
	char szColorName[32]; // 
	unsigned short wC0_ChValA[2][100]; // brightness percentage table control the ch val of color0 .
	unsigned short wC1_ChValA[2][100]; // brightness percentage table control the ch val of color1 .
	unsigned char bBoostLvlA[100]; // 
	unsigned long dwCurrentA[100]; // current table in each combination .
	unsigned char bReservedA[8];
}NTX_FW_TLC5947FL_dualcolor_percent_tab;


// firmware descriptions for type NTX_FW_TYPE_TLC5947_DUALFL_HDR
typedef struct tagNTX_FW_TLC5947FL_dualcolor_hdr {
	unsigned long dwTotalColors; // total color temperatures . 
	unsigned char bC0_ChNumA[2],bC1_ChNumA[2]; // channel number for FL color . 
	
}NTX_FW_TLC5947FL_dualcolor_hdr;
