...
static void _ntx_wifi_bt_power_ctrl (int power_status)
{
    ...
    unsigned int ret;
        ...
        fd = filp_open(CUST_COMBO_WIFI_DEV, O_RDWR | O_NOCTTY,0666);
	msleep(1000);
	if(IS_ERR(fd)){
		printk( "open wmtWifi error\n");
	}
	...
	if(power_status){
		...
		printk("enable wmtWifi \n");
		fd->f_op->write(fd, (char *)on, sizeof(on), &fd->f_pos);
	
	}else{
		fd->f_op->write(fd, (char *)off, sizeof(off), &fd->f_pos);
		printk("disable wmtWifi \n");
...
extern void gpio_keys_report_event(unsigned int type, unsigned int code, int value);

void ntx_report_event(unsigned int type, unsigned int code, int value)
{
	gpio_keys_report_event(type,code,value);
}
