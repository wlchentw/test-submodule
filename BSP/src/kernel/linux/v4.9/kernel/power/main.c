...
//EXPORT_SYMBOL_GPL(gSleep_Mode_Suspend);
				...
				char *buf)
{
	...
	pm_get_wakeup_count(&val, false);

	//return pm_get_wakeup_count(&val, false) ?
	//	sprintf(buf, "%u\n", val) : -EINTR;
...
static struct kobj_attribute wakeup_count_attr = {
         .attr   = {
                 .name = "wakeup_count",
                 .mode = 0644,
         },
         .show   = wakeup_count_show,
         .store  = wakeup_count_store,
};
