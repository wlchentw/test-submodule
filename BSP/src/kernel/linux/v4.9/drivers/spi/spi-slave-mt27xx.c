...
static const struct mtk_spi_compatible mt8512_compat = {
	.max_fifo_size = 128,
	.must_rx = true,
	.need_axi_clk = true,
};
...
static int mtk_spi_slave_probe(struct platform_device *pdev)
{
	...
	if (mdata->dev_comp->need_axi_clk) {
		...
		if (IS_ERR(mdata->axi_clk)) {
			ret = PTR_ERR(mdata->axi_clk);
			dev_err(&pdev->dev, "failed to get axi-clk: %d\n", ret);
