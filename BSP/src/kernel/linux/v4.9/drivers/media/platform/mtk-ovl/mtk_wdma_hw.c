...
static const unsigned int coef[10][5][3] = {
	/* YUV to RGB SD */
...
static unsigned int mtk_wdma_hw_output_format_bpp(
	enum MTK_WDMA_HW_FORMAT outputFormat);
static enum WDMA_COLOR_SPACE mtk_wdma_hw_output_format_color_space(
	enum MTK_WDMA_HW_FORMAT outputFormat);
static unsigned int mtk_wdma_hw_output_fmt_reg_value(
	enum MTK_WDMA_HW_FORMAT outputFormat);
static int mtk_wdma_hw_enable(
	unsigned long reg_base);
static int mtk_wdma_hw_disable(
	unsigned long reg_base);
static int mtk_wdma_hw_reset(
	unsigned long reg_base);
static int mtk_wdma_hw_config_uv(
	unsigned long reg_base, struct MTK_WDMA_HW_PARAM *pParam);
static int mtk_wdma_hw_config(
	unsigned long reg_base, struct MTK_WDMA_HW_PARAM *pParam);
...
static int mtk_wdma_hw_enable(unsigned long reg_base)
{
...
static int mtk_wdma_hw_disable(unsigned long reg_base)
{
...
static int mtk_wdma_hw_reset(unsigned long reg_base)
{
	...
	unsigned long reg_base, struct MTK_WDMA_HW_PARAM *pParam)
{
	...
	DISP_REG_SET(
		reg_base + DISP_REG_WDMA_DST_ADDR1,
		(unsigned int)(pParam->addr_2nd_plane));
	DISP_REG_SET(
		reg_base + DISP_REG_WDMA_DST_ADDR2,
		(unsigned int)(pParam->addr_3rd_plane));
	DISP_REG_SET_FIELD(
		DST_W_IN_BYTE_FLD_DST_W_IN_BYTE,
		reg_base + DISP_REG_WDMA_DST_UV_PITCH,
		pParam->src_width * bpp / 2u);
	...
	unsigned long reg_base, struct MTK_WDMA_HW_PARAM *pParam)
{
	...
	DISP_REG_SET(
		...
		(unsigned int)(pParam->addr_1st_plane));
	DISP_REG_SET(
...
void mtk_wdma_hw_irq_clear(void *reg_base)
{
	unsigned int val = 0;
