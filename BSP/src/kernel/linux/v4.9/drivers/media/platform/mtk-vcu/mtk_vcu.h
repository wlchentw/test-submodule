...
/**
 * Please note that the values of IDs are consistent with the definitions in
 * the daemon which uses the IDs.
 * The values of different IDs may be same if they are used for different
 * daemons. For example, if a new ID for MDP is being added, the value can be
 * set to 1. There is no confliction because IPI_VDEC_H264 is used fro VPU
 * daemon and the new ID is for MDP daemon.
 */
enum ipi_id {
	IPI_VCU_INIT = 0,
	IPI_VDEC_H264 = 1,
