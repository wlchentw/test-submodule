	    ...
	    struct mem_obj *mem_buff_data)
{
	...
	if (mem_buff_data->len > DEC_ALLOCATE_MAX_BUFFER_SIZE ||
		...
		pr_info("Get buffer fail: buffer len = %ld num_buffers = %d !!\n",
	    ...
	    struct mem_obj *mem_buff_data)
{
	...
	if (num_buffers != 0U) {
		for (buffer = 0; buffer < num_buffers; buffer++) {
			...
			cook = vcu_queue->mem_ops->vaddr(vcu_buffer->mem_priv);
			dma_addr =
			vcu_queue->mem_ops->cookie(vcu_buffer->mem_priv);
			...
			if (mem_buff_data->va == (unsigned long)cook &&
				...
				pr_debug(
					"Free buff = %d pa = %lx va = %llx, queue_num = %d\n",
	...
	if (ret != 0)
		pr_info("Can not find free memory !\n");
