...
#define MTK_OVL_CLK_CNT                 1u
#define MTK_OVL_REG_BASE_COUNT          1u

#define PROBE_ST_DEV_ALLOC              (1u << 0)
#define PROBE_ST_MUTEX_INIT             (1u << 1)
#define PROBE_ST_DEV_LARB               (1u << 2)
#if SUPPORT_IOMMU_ATTACH
#define PROBE_ST_IOMMU_ATTACH           (1u << 3)
#endif
#define PROBE_ST_CLK_GET                (1u << 4)
#define PROBE_ST_CLK_ON                 (1u << 5)
#define PROBE_ST_WORKQUEUE_CREATE       (1u << 6)
#define PROBE_ST_IRQ_REQ                (1u << 7)
#define PROBE_ST_REG_MAP                (1u << 8)
#define PROBE_ST_V4L2_DEV_REG           (1u << 9)
#define PROBE_ST_VIDEO_DEV_REG          (1u << 10)
#define PROBE_ST_DMA_PARAM_SET          (1u << 11)
#define PROBE_ST_DMA_CONTIG_INIT        (1u << 12)
#define PROBE_ST_PM_ENABLE              (1u << 13)
#define PROBE_ST_ALL                    (0xffffffffu)
...
static const struct mtk_ovl_fmt *mtk_ovl_get_format(unsigned int index)
{
	if (index >= ARRAY_SIZE(mtk_ovl_formats))
		return NULL;

	return (&(mtk_ovl_formats[index]));
}
...
static void mtk_ovl_set_frame_size(
	struct mtk_ovl_frame *frame, unsigned int width, unsigned int height)
{
	frame->f_width = width;
...
static int mtk_ovl_param_store(struct mtk_ovl_ctx *ctx, struct vb2_buffer *vb)
{
	...
	/*lq-check*/
	hw->dst_w = ctx->s_frame.roi.width;
	hw->dst_h = ctx->s_frame.roi.height;
	...
	layer->src_x = (unsigned int)ctx->s_frame.crop.left;
	layer->src_y = (unsigned int)ctx->s_frame.crop.top;
	layer->src_pitch = ctx->s_frame.pitch[0];
	layer->dst_x = (unsigned int)ctx->s_frame.roi.left;
	layer->dst_y = (unsigned int)ctx->s_frame.roi.top;
	layer->dst_w = ctx->s_frame.crop.width;
	layer->dst_h = ctx->s_frame.crop.height;
	layer->keyEn = 0;
...
static int mtk_ovl_buf_prepare(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < frame->fmt->num_planes; i++)
		vb2_set_plane_payload(vb, (unsigned int)i, frame->payload[i]);
	...
	ret = mtk_ovl_param_store(ctx, vb);
...
static void mtk_ovl_stop_streaming(struct vb2_queue *q)
{
	...
	if (ctx->ovl_dev->id == MTK_OVL_MAIN_ID)
		ret = pm_runtime_put_sync(&ctx->ovl_dev->pdev->dev);
}
	...
	struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	void_ret = memset(str_dev_name, 0, sizeof(str_dev_name));
	void_ret = memcpy(
		str_dev_name, MTK_OVL_MODULE_NAME, sizeof(MTK_OVL_MODULE_NAME));
	void_ret = strncat(str_dev_name, "-",
		(sizeof(str_dev_name) - strlen(str_dev_name) - 1));
	ret = sprintf(str_dev_id, "%d", ctx->ovl_dev->id);
	void_ret = strncat(str_dev_name, str_dev_id,
		(sizeof(str_dev_name) - strlen(str_dev_name) - 1));
	...
	struct file *file, void *fh, struct v4l2_format *f)
{
	...
	log_dbg("[ovl%d] %s end, WH[%d, %d], col[%d], p[%d, %d][%d, %d]",
		ctx->ovl_dev->id, __func__,
	...
	struct file *file, void *fh, struct v4l2_requestbuffers *reqbufs)
{
	...
	log_dbg("[ovl%d] %s start, cnt[%d] mem[%d] type[%d, %d] point[0x%p]",
		ctx->ovl_dev->id, __func__,
		reqbufs->count,
		reqbufs->memory,
		reqbufs->type,
	...
	if (reqbufs->count != 0) {
		vfd->queue = &ctx->vb2_q;
		log_dbg("[ovl%d] save vb2_q [0x%p, 0x%p, 0x%p]",
			ctx->ovl_dev->id, (&(ctx->vb2_q)),
			vfd, (vfd->queue));
	}
	...
	if (reqbufs->count == 0) {
		vfd->queue = NULL;
		log_dbg("[ovl%d] remove vb2_q", ctx->ovl_dev->id);
	}
	...
	struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	log_dbg("[ovl%d] save ovl ctx[0x%p] to id[%d]",
	...
	struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	log_dbg("[ovl%d] remove ovl ctx[0x%p]",
		ctx->ovl_dev->id,
		_mtk_ovl_ctx[ctx->ovl_dev->id]);
	...
	struct file *file, void *fh, struct v4l2_selection *s)
{
	...
	case V4L2_SEL_TGT_COMPOSE_DEFAULT:
	case V4L2_SEL_TGT_COMPOSE:
		frame->roi.left = s->r.left;
		frame->roi.top = s->r.top;
		frame->roi.width = s->r.width;
		frame->roi.height = s->r.height;
		break;
	case V4L2_SEL_TGT_CROP_BOUNDS:
	case V4L2_SEL_TGT_CROP:
	case V4L2_SEL_TGT_CROP_DEFAULT:
	...
	log_dbg("[ovl%d] %s end, target[%d], [%d, %d, %d, %d]",
		ctx->ovl_dev->id, __func__,
		s->target,
		s->r.left, s->r.top,
...
static int mtk_ovl_open(struct file *file)
{
	...
	if (mutex_lock_interruptible(&ovl_dev->lock) != 0)
		return RET_ERR_EXCEPTION;
	...
	void_ret = memset(&(ctx->vb2_q), 0, sizeof(ctx->vb2_q));
	...
	for (i = 0; i < MTK_OVL_CLK_CNT; i++) {
		...
		if (ret != 0) {
			...
			for (; reduce > 0U; reduce--)
				clk_disable_unprepare(ovl_dev->clks[reduce-1]);
		} else
	...
	struct mtk_ovl_dev *ovl_dev, unsigned int st)
{
	...
	if (ovl_dev->id == MTK_OVL_MAIN_ID) {
		if ((st & PROBE_ST_PM_ENABLE) != 0U)
			pm_runtime_disable(dev);
	}

	#if SUPPORT_ALLOC_CTX
	if ((st & PROBE_ST_DMA_CONTIG_INIT) != 0U)
		vb2_dma_contig_cleanup_ctx(ovl_dev->alloc_ctx);
	#endif

	if ((st & PROBE_ST_VIDEO_DEV_REG) != 0U)
		video_unregister_device(&ovl_dev->vdev);

	if ((st & PROBE_ST_V4L2_DEV_REG) != 0U)
		v4l2_device_unregister(&ovl_dev->v4l2_dev);

	if (ovl_dev->id == MTK_OVL_MAIN_ID) {
		if ((st & PROBE_ST_REG_MAP) != 0U) {
	...
	if ((st & PROBE_ST_WORKQUEUE_CREATE) != 0U) {
		flush_workqueue(ovl_dev->workqueue);
	...
	if ((st & PROBE_ST_CLK_ON) != 0U)
		mtk_ovl_clock_off(ovl_dev);
	#endif
#endif

	#if SUPPORT_IOMMU_ATTACH
	if ((st & PROBE_ST_IOMMU_ATTACH) != 0U)
		arm_iommu_detach_device(dev);
	#endif

	#if 0
	if ((ovl_dev->id == MTK_OVL_MAIN_ID) != 0U)
		if ((st & PROBE_ST_DEV_LARB)
			log_dbg("maybe need to do something", ovl_dev);
	#endif

	if ((st & PROBE_ST_MUTEX_INIT) != 0U)
		mutex_destroy(&ovl_dev->lock);

	if ((st & PROBE_ST_DEV_ALLOC) != 0U) {
		log_dbg("will to do devm_kfree for addr[0x%p]\n",
			ovl_dev);
...
static int mtk_ovl_register_video_device(struct mtk_ovl_dev *ovl_dev)
{
	...
	memset(&ovl_dev->vdev, 0, sizeof(ovl_dev->vdev));
	...
	ret = snprintf(ovl_dev->vdev.name, sizeof(ovl_dev->vdev.name),
	...
	if (ovl_dev->id == MTK_OVL_MAIN_ID) {
		for (i = 0; i < MTK_OVL_CLK_CNT; i++) {
			...
			if (IS_ERR(ovl_dev->clks[i])) {
				...
				dev_err(dev,
					"fail to get dev[0x%p] clk[%s] as 0x%p\n",
					dev,
					_ap_mtk_ovl_clk_name[i],
					ovl_dev->clks[i]);
				ret = -ENXIO;
				for (; reduce > 0U; reduce--) {
					unsigned int temp = reduce - 1;

					log_err("will put clk[%d][0x%p]\n",
					temp, ovl_dev->clks[temp]);
					devm_clk_put(
						dev, ovl_dev->clks[temp]);
				}
				goto err_handle;
			} else
...
int mtk_ovl_prepare_hw(void)
{
	...
	for (i = 0; i < OVL_LAYER_NUM; i++) {
		if (_mtk_ovl_hw_param.ovl_config[i].layer_en != 0U)
			break;
	}
	...
	ret = mtk_ovl_clock_on(_mtk_ovl_ctx[i]->ovl_dev);
	ret = mtk_ovl_hw_set(
		_mtk_ovl_ctx[i]->ovl_dev->reg_base[0], &_mtk_ovl_hw_param);
...
int mtk_ovl_unprepare_hw(void)
{
	...
	ret = mtk_ovl_hw_unset(_mtk_ovl_ctx[i]->ovl_dev->reg_base[0]);
	mtk_ovl_clock_off(_mtk_ovl_ctx[i]->ovl_dev);
	...
	for (i = 0; i < OVL_LAYER_NUM; i++) {
		if (_mtk_ovl_hw_param.ovl_config[i].layer_en != 0U) {
			...
			#if LIST_OLD
			vb2_buffer_done(&ctx->curr_buf->vb.vb2_buf,
				VB2_BUF_STATE_DONE);
			#else
			vb2_buffer_done(
			...
			#else
			/* lq-check */
			/* spin_lock_irqsave(&ctx->vb2_q.lock, flags);*/
			list_del(&ctx->vb2_q.queued_list);
			/* spin_unlock_irqrestore(&ctx->vb2_q.lock, flags);*/
			#endif
