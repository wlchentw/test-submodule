 ...
 * @pq_cfg	: for store/get pq config
 * @pq_cfg_ack	: ack for get pq_config
...
struct mtk_mdp_vpu {
	...
	int32_t			failure;
	int32_t			ipi_id;
	struct mdp_pq_cfg	pq_cfg;
	struct mdp_pq_cfg_ack	pq_cfg_ack;
	...
	uint64_t                tuning_buf_va;
	uint64_t                tuning_buf_pa;
#endif
};
