...
void mtk_mdp_hw_set_out_size(struct mtk_mdp_ctx *ctx)
{
	...
	config->crop_x = 0;//frame->crop.left;
	config->crop_y = 0;//frame->crop.top;
	config->crop_w = frame->crop.width;
	config->crop_h = frame->crop.height;
	config->x = 0;
	config->y = 0;
...
void mtk_mdp_hw_set_rotation(struct mtk_mdp_ctx *ctx)
{
	...
	misc->orientation = ctx->rotation;
	misc->hflip = ctx->hflip;
	misc->vflip = ctx->vflip;
}
...
void mtk_mdp_hw_set_pq_info(struct mtk_mdp_ctx *ctx)
{
	...
	vsi_pq->gamma_enable = ctx_pq->gamma_enable;
	vsi_pq->gamma_type = ctx_pq->gamma_type;
	memcpy(vsi_pq->gamma_table, ctx_pq->gamma_table,
			vsi_pq->gamma_type == 0 ? 256 : 16);
	vsi_pq->invert = ctx_pq->invert;
	vsi_pq->dth_enable = ctx_pq->dth_enable;
	vsi_pq->dth_algo = ctx_pq->dth_algo;
	vsi_pq->rsz_algo = ctx_pq->rsz_algo;
}
