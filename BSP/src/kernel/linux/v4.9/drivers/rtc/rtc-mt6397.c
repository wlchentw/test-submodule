...
#include "../misc/mediatek/include/mt-plat/mt8518/include/mach/upmu_hw.h"
#include "../misc/mediatek/include/mt-plat/mt8518/include/mach/mt_rtc_hw.h"
...
static void RTC_Write(u16 addr, u16 data)
{
	...
	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + (uintptr_t)addr,
			   data);
	if (ret < 0)
...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	RTC_Config_PMIC(FQMTR_CON0, 0x0000, 0xFFFF, 0);
	//TOP_RST_CON[8]=1, FQMTR reset
	RTC_Config_PMIC(TOP_RST_CON, 0x1, 0x1, PMIC_RG_FQMTR_RST_SHIFT);
	while (!(RTC_Read_PMIC(FQMTR_CON2) == 0)
		&& (FQMTR_BUSY & RTC_Read_PMIC(FQMTR_CON0)) == FQMTR_BUSY)
		;
	//TOP_RST_CON[8]=0, FQMTR normal
	RTC_Config_PMIC(TOP_RST_CON, 0x0, 0x1, PMIC_RG_FQMTR_RST_SHIFT);
	//set freq. meter window value (0=1X32K(fix clock))
	RTC_Config_PMIC(FQMTR_CON1, window_size, 0xFFFF, 0);
	//enable freq. meter, set measure clock to 26Mhz
	RTC_Config_PMIC(FQMTR_CON0, FQMTR_EN | measureSrc, 0xFFFF, 0);

	mdelay(2);
#if 1
	while ((FQMTR_BUSY & RTC_Read_PMIC(FQMTR_CON0)) == FQMTR_BUSY) {
		if (time_after(jiffies, timeout)) {
			pr_info("get frequency time out\n");
			break;
		}
	}; // FQMTR read until ready
#endif
	//read data should be closed to 26M/32k = 812.5
	ret = RTC_Read_PMIC(FQMTR_CON2);

	pr_info("[RTC] get_frequency_meter: input=0x%x, output=%d\n", val, ret);
	return ret;
}
...
static void rtc_measure_four_clock(U16 *result)
{
	...
	RTC_Config_PMIC(TOP_CKCON2, 0x0, 0x1, 15);
	window_size = 4;
	mdelay(1);
	...
	RTC_Config_PMIC(TOP_CKTST2, 0x0, 0x3, 6);
	RTC_Config_PMIC(TOP_CKCON2, 0x1, 0x1, 15);
	window_size = 4;
	mdelay(1);
	//select 26M as target clock
	...
	RTC_Config_PMIC(TOP_CKTST2, 0x2, 0x3, 6);
	RTC_Config_PMIC(TOP_CKCON2, 0x1, 0x1, 15);
	window_size = 4;
	mdelay(1);
	//select 26M as target clock
...
static void rtc_switch_mode(bool XOSC, bool recovery)
{
	if (XOSC) {
		if (recovery) {
			...
			/* RTC_Read_PMIC((CHRSTATUS) | RTC_HW_DET_BYPASS) &
			 * RTC_HW_XOSC_MODE
			 */
			RTC_Config_PMIC(CHRSTATUS, 0x1, 0x1,
					3 + PMIC_RTC_XTAL_DET_RSV_SHIFT);
			RTC_Config_PMIC(CHRSTATUS, 0x0, 0x1,
					1 + PMIC_RTC_XTAL_DET_RSV_SHIFT);
		}
...
static bool rtc_init_after_recovery(void)
{
	...
	RTC_Config_PMIC(CHRSTATUS, 0x0, 0x1, 3 + PMIC_RTC_XTAL_DET_RSV_SHIFT);
...
static bool rtc_android_init(void)
{
	...
	RTC_Write(RTC_PDN2,
		  ((1970 - RTC_MIN_YEAR) << RTC_PDN2_PWRON_YEA_SHIFT) | 1);
...
static bool rtc_gpio_init(void)
{
	...
	con = RTC_Read(RTC_CON) &
		(RTC_CON_LPSTA_RAW | RTC_CON_LPRST | RTC_CON_LPEN);
	con &= ~RTC_CON_GPU;
...
static U16 eosc_cali(void)
{
	...
	//fix clock = eosc 32K
	RTC_Config_PMIC(TOP_CKCON2, 0x1, 0x1, 15);
	pr_info("[RTC] EOSC_Cali: TOP_CKCON1=0x%x\n",
		RTC_Read_PMIC(TOP_CKCON1));
	while (left <= (right)) {
		middle = (right + left) / 2;
		if (middle == left)
			break;
...
static void rtc_osc_init(void)
{
	...
	if (rtc_get_xosc_mode()) {
		...
		con = RTC_Read(RTC_OSC32CON);
		/* check XOSCCALI */
		if ((con & RTC_OSC32CON_XOSCCALI_MASK) != 0x0) {
		...
		pr_info("[RTC] EOSC cali val = 0x%x\n", val);
		//EMB_HW_Mode
		val = (val & RTC_OSC32CON_XOSCCALI_MASK) |
			RTC_OSC32CON_XOSC32_ENB | RTC_OSC32CON_EOSC32_CHOP_EN;
		rtc_xosc_write(val);
	}
...
static bool rtc_check_lpd(void)
{
	...
	if (con & RTC_CON_LPSTA_RAW ||
	    ((con & RTC_CON_LPEN) != RTC_CON_LPEN) || con & RTC_CON_LPRST) {
		return true;
	} else {
		return false;
	}
...
void rtc_bbpu_power_on(void)
{
	...
	RTC_Write(RTC_BBPU, bbpu);
	Write_trigger();
	pr_info("[RTC] rtc_bbpu_power_on done and bbpu = 0x%x\n",
		RTC_Read(RTC_BBPU));
...
U16 rtc_rdwr_uart_bits(U16 *val)
{
	...
	if (val) {
		...
		pdn2 |= (*val & (RTC_PDN2_UART_MASK >> RTC_PDN2_UART_SHIFT)) <<
			 RTC_PDN2_UART_SHIFT;
		RTC_Write(RTC_PDN2, pdn2);
		Write_trigger();
	}
...
void rtc_boot_check(void)
{
	...
	RTC_Config_PMIC(TOP_CKPDN, 0x0, 0x1, 8);
	RTC_Config_PMIC(TOP_CKPDN2, 0x00, 0x3, 0);
	RTC_Config_PMIC(TOP_CKPDN2, 0x0, 0x1, 3);

	// check clock source are match with 32K exist
	rtc_measure_four_clock(result);
	if (!rtc_xosc_check_clock(result) && !rtc_eosc_check_clock(result)) {
		pr_info("[RTC] RTC 32K mode wrong. Enter first boot/recov.\n");
		check_mode_flag = true;
	}

	pr_info("[RTC] bbpu = 0x%x, con = 0x%x\n",
	...
	/* normally HW reload is done in BROM but check again here */
		pr_info("[RTC] powerkey1 = 0x%x, powerkey2 = 0x%x\n",
	...
	//RTC_Config_PMIC(TOP_CKTST2, 0x1, 0x1, 4);
	//pr_info("[RTC] TOP_CKTST2 = 0x%x\n", RTC_Read_PMIC(TOP_CKTST2));
	...
	pr_info("[RTC] irqsta=0x%x,pdn1=0x%x,pdn2=0x%x,spar0=0x%x,spar1=0x%x\n",
		irqsta, pdn1, pdn2, spar0, spar1);
	pr_info("[RTC] new_spare0 = 0x%x, new_spare1 = 0x%x\n",
		RTC_Read(RTC_AL_HOU), RTC_Read(RTC_AL_DOM));
	pr_info("[RTC] new_spare2 = 0x%x, new_spare3 = 0x%x\n",
		RTC_Read(RTC_AL_DOW), RTC_Read(RTC_AL_MTH));
	pr_info("[RTC] bbpu = 0x%x, con = 0x%x, cali = 0x%x\n",
		RTC_Read(RTC_BBPU), RTC_Read(RTC_CON), RTC_Read(RTC_CALI));
	RTC_Write(RTC_SPAR0, spar0 & ~RTC_SPAR0_ALARM_BOOT);
	Write_trigger();
	pr_info("[RTC] spar0 = 0x%x\n", RTC_Read(RTC_SPAR0));
}

void pl_power_off(void)
{
	pr_info("[RTC] pl_power_off\n");
...
exit:
	dev_err(mt_rtc->dev, "_mtk_rtc_save_pwron_alarm regmap r/w error!!!\n");
}
...
exit:
	dev_err(mt_rtc->dev, "_mtk_rtc_get_tick_time regmap r/w error!!!\n");
}
...
exit:
	dev_err(mt_rtc->dev, "_mtk_rtc_get_pwron_alarm_time reg r/w error!\n");
}
...
exit:
	dev_err(mt_rtc->dev, "_mtk_rtc_is_pwron_alarm regmap r/w error!!!\n");
	return false;
}
...
void rtc_read_pwron_alarm(struct rtc_wkalrm *alm)
{
	...
	tm->tm_year += RTC_MIN_YEAR_OFFSET;
	tm->tm_mon--;
	dev_notice(mt_rtc->dev, "pwrOn=%04d/%02d/%02d %02d:%02d:%02d(%d)(%d)\n",
