...
int get_ntx_event0_inputdev(struct input_dev * input)
{
	if(gptNTX_EVT0_drvdata) {
		input = gptNTX_EVT0_drvdata->input;
		return 1;	
	}
	else {
		input = NULL;
...
int ntx_event0_report_event(unsigned int type, unsigned int code, int value)
{
	int iRet = -1;
	if (gptNTX_EVT0_drvdata) {
