...
static void mtk_pmic_long_press_reset_setup(struct mtk_pmic_keys *keys,
u32 pmic_rst_reg)
{
#if 0
	int ret;
	...
	if (!ret && long_press_mode == LP_ONEKEY) {
		regmap_update_bits(keys->regmap, pmic_rst_reg,
			...
			PMIC_RG_HOMEKEY_RST_EN_SHIFT, 0);
...
irqreturn_t mtk_pmic_keys_irq_handler_thread(int pressed, void *data)
{
	...
	if (pressed)
		__pm_stay_awake(pwrkey_lock);
	else
		 __pm_wakeup_event(pwrkey_lock, HZ/2);
...
static int mtk_pmic_key_setup(struct mtk_pmic_keys *keys,
		const char *propname, struct pmic_keys_info *info)
{
	int ret;
...
/*	struct mt6397_chip *pmic_chip = dev_get_drvdata(pdev->dev.parent); */
	...
	keys = devm_kzalloc(&pdev->dev, sizeof(struct mtk_pmic_keys),
	GFP_KERNEL);
	if (!keys)
		return -ENOMEM;
