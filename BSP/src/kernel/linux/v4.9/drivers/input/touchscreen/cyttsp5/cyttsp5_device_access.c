		...
		size_t buf_size, u8 *ic_buf, size_t ic_buf_size)
{
	...
	while (pbuf <= (buf + buf_size)) {
		if (i >= CY_MAX_CONFIG_BYTES) {
			dev_info(dev, "%s: %s size=%d max=%d\n", __func__,
					"Max cmd size exceeded", i,
					CY_MAX_CONFIG_BYTES);
			return -EINVAL;
		}
		if (i >= ic_buf_size) {
			dev_info(dev, "%s: %s size=%d buf_size=%zu\n", __func__,
		...
		if (ret < 0) {
			dev_info(dev, "%s: %s '%s' %s%s i=%d r=%d\n", __func__,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	length = cyttsp5_ic_parse_input(dev, buf, size, dad->ic_buf,
			CY_MAX_PRBUF_SIZE);
	if (length <= 0) {
		dev_info(dev, "%s: %s Group Data store\n", __func__,
				"Malformed input for");
		goto exit;
	}
	...
	if (rc) {
		dad->response_length = 0;
		dev_info(dev, "%s: Failed to store command\n", __func__);
	} else {
		dad->status = 1;
	}
	...
	struct configuration *configuration)
{
	...
	if (tx_num != dad->si->sensing_conf_data.tx_num) {
		dev_info(dev, "%s: TX number mismatch!\n", __func__);
		ret = -EINVAL;
	}

	if (rx_num != dad->si->sensing_conf_data.rx_num) {
		dev_info(dev, "%s: RX number mismatch!\n", __func__);
		ret = -EINVAL;
	}

	if (button_num != dad->si->num_btns) {
		dev_info(dev, "%s: Button number mismatch!\n", __func__);
		ret = -EINVAL;
	}
	...
	struct result *result, bool *pass, int test_item)
{
	...
	for (i = 0; i < sensor_num; i++) {
		...
		|| (cm_sensor_data[i] > cm_sensor_max)) {
			dev_info(dev, "%s: Sensor[%d,%d]:%d (%d,%d)\n",
	...
	struct cmcp_data *cmcp_info)
{
	...
	if (rc) {
		dev_info(dev, "short test not supported");
		goto exit;
	}
	if (dad->ic_buf[1] != 0)
	...
	if (rc) {
		dev_info(dev, "Get CM Panel not supported");
		goto exit;
	}
	...
	if (rc) {
		dev_info(dev, "Get CP Panel not supported");
		goto exit;
	}
	...
	if (rc) {
		dev_info(dev, "Get CM BTN not supported");
		goto exit;
	}
	if (cm_btn_data != NULL) {
		for (i = 0; i < btn_num; i++) {
	...
	if (rc) {
		dev_info(dev, "Get CP BTN not supported");
		goto exit;
	}
	if (cp_btn_data != NULL) {
		for (i = 0; i < btn_num; i++) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (dad->builtin_cmcp_threshold_status < 0) {
		dev_info(dev, "%s: No cmcp threshold file.\n", __func__);
		no_builtin_file = 1;
		mutex_unlock(&dad->sysfs_lock);
		goto start_testing;
	}
...
start_testing:
	...
	if (rc)
		dev_info(dev, "stop watchdog failed");
	/*force single tx*/
	rc = cmd->nonhid_cmd->set_param(dev, 0, 0x1F, 1, 1);
	if (rc)
		dev_info(dev, "force single tx failed");
	/*suspend_scanning */
	rc = cmd->nonhid_cmd->suspend_scanning(dev, 0);
	if (rc)
		dev_info(dev, "suspend_scanning failed");
	/*do calibration*/
	if (!dad->cmcp_force_calibrate) {
		...
		if (rc < 0) {
			dev_info(dev, "%s: Error on calibrate idacs for mutual r=%d\n",
					__func__, rc);
		}
		rc = _cyttsp5_calibrate_idacs_cmd(dev, 1, &status);
		if (rc < 0) {
			dev_info(dev, "%s: Error on calibrate idacs for buttons r=%d\n",
					__func__, rc);
		}
		rc = _cyttsp5_calibrate_idacs_cmd(dev, 2, &status);
		if (rc < 0) {
			dev_info(dev, "%s: Error on calibrate idacs  for self r=%d\n",
					__func__, rc);
		}
	}
	/*resume_scanning */
	...
	if (rc)
		dev_info(dev, "start watchdog failed");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (length <= 0 || length > 3) {
		dev_info(dev, "%s: Input format error!\n", __func__);
	...
	int no_builtin_file)
{
	...
	if (configuration == NULL)
		dev_info(dev, "config is NULL");
	if (result == NULL)
		dev_info(dev, "result is NULL");
	if (cmcp_info == NULL)
		dev_info(dev, "cmcp_info is NULL");
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (test_executed)
		...
		dev_info(dev, "%s: No test result available!\n", __func__);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value == 1)
		...
		if (dad->cmcp_threshold_size == 0) {
			dev_info(dev, "%s: No cmcp threshold data\n", __func__);
			goto exit_free;
		}
...
static int cyttsp5_suspend_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->suspend_scanning(dev, 0);
	if (rc < 0)
		dev_info(dev, "%s: Suspend scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_resume_scan_cmd_(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->resume_scanning(dev, 0);
	if (rc < 0)
		dev_info(dev, "%s: Resume scan failed r=%d\n",
			__func__, rc);
	return rc;
}
...
static int cyttsp5_exec_scan_cmd_(struct device *dev)
{
	...
	if (rc < 0)
		dev_info(dev, "%s: Heatmap start scan failed r=%d\n",
			__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *return_buf)
{
	...
	rc = cmd->nonhid_cmd->retrieve_panel_scan(dev, 0, read_offset,
			...
			return_buf);
	if (rc < 0)
		dev_info(dev, "%s: Retrieve scan data failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		dev_info(dev, "%s: Get data structure failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u8 *results_available)
{
	...
	rc = cmd->nonhid_cmd->run_selftest(dev, 0, test_id,
			...
			results_available);
	if (rc < 0)
		dev_info(dev, "%s: Run self test failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		u16 *actual_read_len, u8 *data)
{
	...
	if (rc < 0)
		dev_info(dev, "%s: Get self test result failed r=%d\n",
				__func__, rc);
	return rc;
}
		...
		bool get_result_on_pass)
{
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on request exclusive r=%d\n",
				__func__, rc);
		goto put_pm_runtime;
	}
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on suspend scan r=%d\n",
				__func__, rc);
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on run self test for test_id:%d r=%d\n",
				__func__, test_id, rc);
		goto resume_scan;
	}
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on get self test result r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on execute panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on retrieve panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (length != 1) {
		dev_info(dad->dev, "%s: Malformed input\n", __func__);
		rc = -EINVAL;
		goto exit_unlock;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on get data structure r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	rc = _cyttsp5_calibrate_idacs_cmd(dev, dad->calibrate_sensing_mode,
			&dad->ic_buf[0]);
	if (rc < 0) {
		dev_info(dev, "%s: Error on calibrate idacs r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (dad->calibrate_initialize_baselines) {
		...
		rc = _cyttsp5_initialize_baselines_cmd(dev, CY_IB_SM_MUTCAP |
				...
				&dad->ic_buf[length]);
		if (rc < 0) {
			dev_info(dev, "%s: Error on initialize baselines r=%d\n",
					__func__, rc);
			goto resume_scan;
		}
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create command\n",
				__func__);
		goto exit;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create status\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create response\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->base_dentry)) {
		dev_info(dev, "%s: Error, could not create base directory\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->mfg_test_dentry)) {
		dev_info(dev, "%s: Error, could not create mfg_test directory\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("panel_scan", 0600,
		...
		dev_info(dev, "%s: Error, could not create panel_scan\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("get_idac", 0600,
		...
		dev_info(dev, "%s: Error, could not create get_idac\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("auto_shorts", 0400,
		...
		dev_info(dev, "%s: Error, could not create auto_shorts\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("opens", 0400,
		...
		dev_info(dev, "%s: Error, could not create opens\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("calibrate", 0600,
		...
		dev_info(dev, "%s: Error, could not create calibrate\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("baseline", 0600,
		...
		dev_info(dev, "%s: Error, could not create baseline\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cm_panel", 0400,
		...
		dev_info(dev, "%s: Error, could not create cm_panel\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cp_panel", 0400,
		...
		dev_info(dev, "%s: Error, could not create cp_panel\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cm_button", 0400,
		...
		dev_info(dev, "%s: Error, could not create cm_button\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cp_button", 0400,
		...
		dev_info(dev, "%s: Error, could not create cp_button\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(dad->cmcp_results_debugfs)) {
		dev_info(dev, "%s: Error, could not create cmcp_results\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->tthe_get_panel_data_debugfs)) {
		dev_info(dev, "%s: Error, could not create get_panel_data\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create cmcp_test\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create cmcp_thresold_loading\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create cmcp_thresold_data\n",
				__func__);
		...
		void *context)
{
	...
	if (!fw->data || !fw->size) {
		dev_info(dev, "%s: Invalid builtin cmcp threshold file\n",
		__func__);
		goto exit;
	}
	...
	struct work_struct *cmcp_threshold_update)
{
	...
	dad->si = cmd->request_sysinfo(dev);
	if (!dad->si) {
		dev_info(dev, "%s: Fail get sysinfo pointer from core\n",
			__func__);
		return;
	}
	...
	if (retval < 0) {
		dev_info(dev, "%s: Failed loading cmcp threshold file, attempting legacy file\n",
			__func__);
		...
		if (retval < 0) {
			dev_info(dev, "%s: Fail request cmcp threshold class file load\n",
				__func__);
			goto exit;
		}
	}
...
static int cyttsp5_device_access_probe(struct device *dev, void **data)
{
	...
	*data = dad;
	...
	if (dad->si) {
		...
		dev_info(dev, "%s: Fail get sysinfo pointer from core p=%p\n",
				__func__, dad->si);
...
static int __init cyttsp5_device_access_init(void)
{
	...
	if (rc < 0) {
		pr_info("%s: Error, failed registering module\n",
			__func__);
			return rc;
	}
