...
#define CYTTSP5_DISABLE_WD_TIMER
...
static int cyttsp5_hid_output_read_manufacture_data_(struct cyttsp5_core_data *cd,
		u8 *name)
{
	int rc;
	u8 read_buf[68]={0};
	...
	memcpy(name,read_buf+4,11);
	name[11] = '\0';

	printk(KERN_ERR"[%s] manufacture :%s \n",__FUNCTION__,name);
	return 0;
}
		...
		u8 mode, u8 *status)
{
	...
	printk(KERN_ERR"[%s_%d] mode:%x \n" ,__FUNCTION__,__LINE__,mode);
	printk(KERN_ERR"[%s_%d] type:%x ,code:%x \n" ,__FUNCTION__,__LINE__,hid_output.cmd_type,hid_output.command_code);
	write_buf[cmd_offset++] = mode;
	rc =  cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	printk(KERN_ERR"[%s_%d] rc:%d \n",__FUNCTION__,__LINE__,rc);

	if (rc)
		return rc;

	printk(KERN_ERR"[%s_%d] Buf:%x_%x_%x_%x_%x_%x \n",__FUNCTION__,__LINE__, cd->response_buf[0], cd->response_buf[1], cd->response_buf[2],
		 cd->response_buf[3], cd->response_buf[4], cd->response_buf[5]);
...
static int cyttsp5_get_manufacture_data_(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_sysinfo *si = &cd->sysinfo;
	int rc;
	u8 name[16] = {0};
	...
	rc = cyttsp5_hid_output_read_manufacture_data_(cd, name);
	if (rc)
		goto exit;

	memcpy(si->cydata.manufacture,name,16);
...
#ifndef CYTTSP5_DISABLE_WD_TIMER
...
read:
	...
	if (rc) {
		dev_info(dev, "%s: Error getting report, r=%d\n",
				__func__, rc);
		return rc;
	}
...
reset:
	...
	// Manufacture Code
	cyttsp5_get_manufacture_data_(cd);
...
static int cyttsp5_core_rt_suspend(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
	printk(KERN_ERR"[%s_%d] gSleep_Mode_Suspend:%d \n",__FUNCTION__,__LINE__,gSleep_Mode_Suspend);
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	if(gSleep_Mode_Suspend) {
		...
		if(cd->irq_wake) {
			disable_irq_wake(cd->irq);cd->irq_wake = 0;
		}

		if( 0x71==gptHWCFG->m_val.bPCB ) {// E60T04 , CFA/BW
			printk(KERN_ERR"[%s_%d] set rst low\n",__FUNCTION__,__LINE__);
			gpio_set_value(cd->cpdata->rst_gpio, 0);
		}
	}
	else {
		if(!cd->irq_wake) {
			enable_irq_wake(cd->irq);cd->irq_wake = 1;
		}
	}
#else
...
static int cyttsp5_core_resume(struct device *dev)
{
	...
	if(gSleep_Mode_Suspend) {
		if( 0x71==gptHWCFG->m_val.bPCB ) {// E60T04 , CFA/BW
			printk(KERN_ERR"[%s_%d] set rst high\n",__FUNCTION__,__LINE__);
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
	...
	printk(KERN_ERR"[%s_%d] irq:%d , irq_gpio:%d \n",__FUNCTION__,__LINE__,cd->irq,cd->cpdata->irq_gpio);
	...
	rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq, irq_flags,
		dev_name(dev), cd);
	if (rc < 0)
		dev_info(dev, "%s: Error, could not request irq\n", __func__);
...
//	INIT_WORK(&cd->watchdog_work, cyttsp5_watchdog_work);
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error, fail sysfs init\n", __func__);
