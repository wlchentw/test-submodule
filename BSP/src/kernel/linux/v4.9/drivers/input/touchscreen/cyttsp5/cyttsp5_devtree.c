		...
		const char *inp_dev_name, struct cyttsp5_virtual_keys *vkeys)
{
	...
	if (board_properties_kobj == NULL) {
		pr_info("%s: Cannot get board_properties kobject!\n", __func__);
		rc = -EINVAL;
		goto fail_free_name;
	}
...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	rc = setup_virtual_keys(dev_node, pdata->inp_dev_name,
			&ext_pdata->vkeys);
	if (rc) {
		pr_info("%s: Cannot setup virtual keys!\n", __func__);
		goto fail_free_pdata;
	}
#endif
	return pdata;
...
//	pdata->irq_gpio = value;
	...
	printk(KERN_ERR"[%s_%d] pdata->rst_gpio:%d\n",__FUNCTION__,__LINE__,pdata->rst_gpio);
//	if (!rc)
