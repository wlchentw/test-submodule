...
static int cyttsp5_xy_worker(struct cyttsp5_proximity_data *pd)
{
	...
	if (num_cur_tch > si->sensing_conf_data.max_tch) {
		dev_info(dev, "%s: Num touch err detected (n=%d)\n",
...
static int cyttsp5_proximity_attention(struct device *dev)
{
	...
	if (rc < 0)
		dev_info(dev, "%s: xy_worker error r=%d\n", __func__, rc);
...
static int _cyttsp5_proximity_enable(struct cyttsp5_proximity_data *pd)
{
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on request enable proximity scantype r=%d\n",
				__func__, rc);
		goto exit_release;
	}
		...
		bool force)
{
	...
	if (rc < 0) {
		dev_info(dev, "%s: Error on request disable proximity scan r=%d\n",
				__func__, rc);
		goto exit_release;
	}
	...
	if (value) {
		...
		if (--pd->enable_count) {
			if (pd->enable_count < 0) {
				dev_info(dev, "%s: '%s' unbalanced disable\n",
					__func__, pd->input->name);
				pd->enable_count = 0;
			}
		} else {
...
static int cyttsp5_setup_input_device_and_sysfs(struct device *dev)
{
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create enable\n",
				__func__);
		goto exit;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error, failed register input device r=%d\n",
			__func__, rc);
...
int cyttsp5_proximity_probe(struct device *dev)
{
	...
	if (!pd->input) {
		dev_info(dev, "%s: Error, failed to allocate input device\n",
			__func__);
		rc = -ENODEV;
