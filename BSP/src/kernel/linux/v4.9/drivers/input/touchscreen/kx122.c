...
struct kx122_data *g_kx122_data;
#ifdef QCOM_SENSORS
...
static irqreturn_t kx122_irq_handler(int irq, void *dev)
{
	...
	if(sdata->int_skip_one) {
		pr_info("skip first interrupt after power on \n");
		sdata->int_skip_one = 0 ;
		return IRQ_HANDLED;
	}
	...
	if(s2_status & KX122_INS2_TPS)
	{
		...
		if(cur_pos & KX122_TSCP_FU )
		{
			...
			if(80 == gptHWCFG->m_val.bPCB)
				printk(KERN_INFO"==== [Face Up] ====\n");
			else if(75 == gptHWCFG->m_val.bPCB || 0x67 == gptHWCFG->m_val.bPCB) { // E60K20
				printk(KERN_INFO"==== [Face Up] ====\n");
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_FRONT);	
			}
			else if (87 == gptHWCFG->m_val.bPCB || 101 == gptHWCFG->m_val.bPCB) {
				printk(KERN_INFO"==== [Face down] ====\n");
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_BACK);	
			}
#else
				printk(KERN_INFO"[KX122][Face down]\n");
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_BACK);	
#endif
		}
		else if(cur_pos & KX122_TSCP_FD )
...
#else
				printk(KERN_INFO"[KX122][Face up]\n");
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_FRONT);	
#endif
		}
		if(cur_pos & KX122_TSCP_UP )
		{
			...
			} else if (101 == gptHWCFG->m_val.bPCB)	{
				printk(KERN_INFO"==== [Right State] ====\n");		
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_LANDSCAPE_RIGHT);	
			}
#else
				printk(KERN_INFO"[KX122][DOWN State]\n");		
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_PORTRAIT_DOWN);	
#endif
		}
		else if(cur_pos & KX122_TSCP_DO )
...
#else
				printk(KERN_INFO"[KX122][Up State]\n");	
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_PORTRAIT_UP);
#endif
		}
		else if(cur_pos & KX122_TSCP_RI )
...
#else
				printk(KERN_INFO"[KX122][Left State]\n");
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_LANDSCAPE_LEFT);
#endif
		}
		else if(cur_pos & KX122_TSCP_LE )
...
#else
				printk(KERN_INFO"[KX122][Right State]\n");	
				ntx_report_event(EV_MSC,MSC_RAW,MSC_RAW_GSENSOR_LANDSCAPE_RIGHT);
#endif
		}
	}
...
exit_out:
	if (err < 0)
		...
		__debug("%s KX122_TILT_ANGLE_HL val %d \n",__FUNCTION__, value);
...
exit_out:
	if (err < 0)
		...
		__debug("%s KX122_TILT_ANGLE_LL val %d \n",__FUNCTION__, value);
...
exit_out:
	if (err < 0)
		...
		__debug("%s KX122_HYST_SET val %d \n",__FUNCTION__, value);
	mutex_unlock(&sdata->mutex);

	return (err < 0) ? err : value;
}
...
static int kx122_power_init(struct kx122_data *sdata, bool on)
{
	...
	if (!on) {
		if (regulator_count_voltages(sdata->vdd) > 0) {
			rc = regulator_set_voltage(sdata->vdd, 0, KX122_VDD_MAX_UV);
		}

		regulator_put(sdata->vdd);

		if (regulator_count_voltages(sdata->vio) > 0) {
			rc = regulator_set_voltage(sdata->vio, 0, KX122_VIO_MAX_UV);
		}
...
{	
...
#ifdef EN_THREAD_IRQ //[
		err = request_threaded_irq(sdata->irq1, NULL,kx122_irq_handler, /*IRQF_TRIGGER_RISING*/ IRQF_TRIGGER_FALLING | IRQF_ONESHOT,KX122_NAME, sdata);
#else //][!EN_THREAD_IRQ
...
#else
		sdata->inc1 = /*KX122_INC1_IEA1|*/  KX122_INC1_IEN1;	// active low
		//sdata->inc1 = 0 |  KX122_INC1_IEN1 | KX122_INC1_PWSEL1_2XOSA | KX122_INC1_IEL1 ;	// active low
#endif
	...
	if (sdata->pdata.use_drdy_int && (sdata->irq1 || sdata->irq2 ))
	{
		...
		printk("[%s_%d] using irq \n",__FUNCTION__,__LINE__);
	}
	else
	{
		sdata->use_poll_timer = true;
		printk("[%s_%d] using timer \n",__FUNCTION__,__LINE__);
	}
	...
	printk("[kx122-CNTL1]:%x \n",val);
	val = kx122_reg_read_byte(sdata, KX122_CNTL2);
	printk("[kx122-CNTL2]:%x \n",val);
	val = kx122_reg_read_byte(sdata, KX122_CNTL3);
	printk("[kx122-CNTL3]:%x \n",val);
	val = kx122_reg_read_byte(sdata, KX122_TILT_ANGLE_LL);
	printk("[KX122_TILT_ANGLE_LL]:%x \n",val);

	if(gptHWCFG==NULL){

	}
	else if(80==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB // E60QU4, E70K02
	|| 0x6C==gptHWCFG->m_val.bPCB || 0x6F==gptHWCFG->m_val.bPCB) // EA0T04, E70T04
		gRotate_Angle = EBRMAIN_ROTATE_R_180 ;
	else if(83==gptHWCFG->m_val.bPCB || 101==gptHWCFG->m_val.bPCB) // E60QT4 , E70K14
       	gRotate_Angle = EBRMAIN_ROTATE_R_90 ;

	printk("[gRotate_Angle]:%x \n",gRotate_Angle);
...
#if POWER_CONTROL //[
		kx122_power_on(pdata, false);
#endif //] POWER_CONTROL

		pdata->int_skip_one = 1;
	}
	else {
...
#ifdef EN_THREAD_IRQ //[
		ret = request_threaded_irq(pdata->irq1,NULL,kx122_irq_handler,/*IRQF_TRIGGER_RISING*/IRQF_TRIGGER_FALLING | IRQF_ONESHOT,KX122_NAME,pdata);
#else //][!EN_THREAD_IRQ
