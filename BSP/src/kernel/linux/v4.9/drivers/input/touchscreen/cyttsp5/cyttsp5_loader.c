	...
	struct cyttsp5_hex_image *row_image)
{
	...
	if (row_image->row_size > ARRAY_SIZE(row_image->row_data)) {
		dev_info(dev, "%s: row data buffer overflow\n", __func__);
		rc = -EOVERFLOW;
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	if (fw_size % image_rec_size != 0) {
		dev_info(dev, "%s: Firmware image is misaligned\n", __func__);
		rc = -EINVAL;
		goto _cyttsp5_load_app_infoor;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error cannot start Loader (ret=%d)\n",
			__func__, rc);
		goto _cyttsp5_load_app_exit;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error cannot init Loader (ret=%d)\n",
			__func__, rc);
		goto _cyttsp5_load_app_exit;
	}
	...
	while (p < last_row) {
		...
		if (rc) {
			dev_info(dev, "%s: Parse Row Error (a=%d r=%d ret=%d\n",
				__func__, row_image->array_id,
		...
		if (rc) {
			dev_info(dev, "%s: Program Row Error (array=%d row=%d ret=%d)\n",
				__func__, row_image->array_id,
				row_image->row_num, rc);
			goto _cyttsp5_load_app_exit;
		}
	...
	if (rc) {
		dev_info(dev, "%s: Error on exit Loader (ret=%d)\n",
			__func__, rc);

		/* verify app checksum */
		rc_tmp = cyttsp5_ldr_verify_chksum_(dev);
		if (rc_tmp)
			dev_info(dev, "%s: ldr_verify_chksum fail r=%d\n",
				__func__, rc_tmp);
		else
		...
		int fw_size)
{
	...
	while (retry--) {
		...
		if (rc < 0)
			dev_info(dev, "%s: Firmware update failed rc=%d, retry:%d\n",
				__func__, rc, retry);
		else
			break;
		msleep(20);
	...
	if (rc < 0) {
		dev_info(dev, "%s: Firmware update failed with error code %d\n",
			__func__, rc);
		...
		if (rc) {
			dev_info(dev, "%s: Failed adding callback for calibration\n",
				__func__);
			dev_info(dev, "%s: No calibration will be performed\n",
				__func__);
			rc = 0;
		} else
		...
		struct device *dev)
{
	...
	if (!fws) {
		dev_info(dev, "%s: No firmwares provided\n", __func__);
		return NULL;
	}
		...
		bool forced)
{
	...
	if (!fw || !fw->img || !fw->size) {
		dev_info(dev, "%s: No platform firmware\n", __func__);
		return rc;
	}

	if (!fw->ver || !fw->vsize) {
		dev_info(dev, "%s: No platform firmware version\n",
			__func__);
		return rc;
	}
		...
		void *context)
{
	...
	if (!fw->data || !fw->size) {
		dev_info(dev, "%s: Invalid builtin firmware\n", __func__);
		goto _cyttsp5_firmware_cont_builtin_exit;
	}
...
static int upgrade_firmware_from_class(struct device *dev)
{
	...
	if (retval < 0) {
		dev_info(dev, "%s: Fail request firmware class file load\n",
			__func__);
		return retval;
	}
		...
		u16 row_number, u16 row_size, u8 *data)
{
	...
	rc = cmd->nonhid_cmd->write_conf_block(dev, 0, row_number,
			...
			&actual_write_len);
	if (rc) {
		dev_info(dev, "%s: Fail Put EBID=%d row=%d cmd fail r=%d\n",
			__func__, ebid, row_number, rc);
		return rc;
	}

	if (actual_write_len != row_size) {
		dev_info(dev, "%s: Fail Put EBID=%d row=%d wrong write size=%d\n",
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	for (i = 0; i < row_count; i++) {
		...
		rc = cyttsp5_write_config_row_(dev, ebid, i, row_size,
				row_buf);
		if (rc) {
			dev_info(dev, "%s: Fail put row=%d r=%d\n",
				__func__, i, rc);
			break;
		}
		row_buf += row_size;
	...
	if (rc || verify_crc_status)
		dev_info(dev, "%s: CRC Failed, ebid=%d, status=%d, scrc=%X ccrc=%X\n",
		...
		struct cyttsp5_touch_config *ttconfig)
{
	...
	if (cyttsp5_check_firmware_version(dev, fw_ver_config,
			fw_revctrl_config)) {
		dev_info(dev, "%s: FW versions mismatch\n", __func__);
		return 0;
	}
		...
		u8 *bin_config_data, int bin_config_size, u8 **start, int *len)
{
	...
	if (header_size < 11 || header_size >= bin_config_size - 6) {
		dev_info(dev, "%s: Invalid header size %d\n", __func__,
			header_size);
		return -EINVAL;
	}
	...
	if (config_size != bin_config_size - header_size - 2) {
		dev_info(dev, "%s: Config size invalid\n", __func__);
		return -EINVAL;
	}
...
static int cyttsp5_loader_probe(struct device *dev, void **data)
{
	...
	if (!pdata || !pdata->loader_pdata) {
		dev_info(dev, "%s: Missing platform data\n", __func__);
		rc = -ENODEV;
		goto error_no_pdata;
	}
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create forced_upgrade\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create manual_upgrade\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create config_loading\n",
				__func__);
	...
	if (rc) {
		dev_info(dev, "%s: Error, could not create config_data\n",
				__func__);
