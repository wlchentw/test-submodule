...
#define HID_I2C_FINGER_HEADER_V2 	0x12
...
struct elants_data {
	...
	struct work_struct ts_pwron_work;

	struct pinctrl *pctrl;
	struct pinctrl_state *pin_state_active;
	struct pinctrl_state *pin_state_inactive;

	int tip_max , tip_min , tip_hist_max , tip_hist_min;
	uint32_t dwStylusID;

	uint32_t dwPtCnt; // touch down counter .
	uint32_t dwActPtCnt; // active touch down counter .
	int iLastTdX[MAX_CONTACT_NUM],iLastTdY[MAX_CONTACT_NUM],iLastTdP[MAX_CONTACT_NUM]; // last touch down x,y and pressure .
	int iLastPdX,iLastPdY,iLastPdP; // last pen down x,y and pressure .
	unsigned long dwTPdownJiffies,dwTPupJiffies;
	atomic_t tp_int_cnt; // touch interrupt counter .
	u8 header_size; // size changes to 19 byte after firmware 6436
};
...
static int elants_i2c_hw_reset(struct i2c_client *client){
	...
	pr_info("[elan] enter elants_i2c_hw_reset....\n");
#ifdef USE_IN_SUNXI
...
static int elan_iap_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("[elan] into elan_iap_open\n");
...
static ssize_t elan_iap_read(struct file *filp, char *buff, size_t count, loff_t *offp) 
{
    ...
    pr_info("[elan] into elan_iap_read\n");
...
static long elan_iap_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    pr_info("[elan] into elan_iap_ioctl, cmd value %x\n", cmd);
...
static int elants_i2c_query_test_version(struct elants_data *ts)
{
	...
	for (retry_cnt = 0; retry_cnt < MAX_RETRIES; retry_cnt++) {
		...
		if (!error) {
			...
			ts->test_version = major ;
			ts->solution_version = version & 0xff;

			pr_info("[elan] %s: (test version 0x%x) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", 
					__func__, ts->test_version,
...
static int elants_i2c_query_hw_version(struct elants_data *ts)
{
	...
	for (retry_cnt = 0; retry_cnt < MAX_RETRIES; retry_cnt++) {
		...
		pr_info("[elan] %s: (Firmware ID) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], 
			resp[2], resp[3] , resp[4],resp[5], resp[6], resp[7], resp[8], resp[8]);
		...
		if (!error) {
			...
			major = ((resp[6] & 0x0f) << 4) | ((resp[7] & 0xf0) >> 4);
			minor = ((resp[7] & 0x0f) << 4) | ((resp[8] & 0xf0) >> 4);
			ts->hw_version = major << 8 | minor;
...
static int elants_i2c_initialize(struct elants_data *ts)
{
	...
	u8 buf[HEADER_SIZE];
#endif
	pr_info("[elan] enter elants_i2c_initialize....\n");
	
	for (retry_cnt = 0; retry_cnt < MAX_RETRIES; retry_cnt++) {
		error = elants_i2c_sw_reset(client);
//		error = elants_i2c_hw_reset(client);
		if (error) {
			pr_err("[elan] reset failed.\n");
		...
		if (error) {
			pr_err("[elan] error elants_i2c_fastboot.\n");
			/* Continue initializing if it's the last try */
			if (retry_cnt < MAX_RETRIES - 1)
				continue;
		}
		...
		if (error) {
			...
			dev_info(&client->dev,"ELAN IAP OPERATIONAL mode !\n");
			break;
			 ...
			 */
			error = -EIO;
			dev_err(&client->dev,"failed to read packets\n");
			break;
		} else {
			error = -EINVAL;
			dev_err(&client->dev,
	...
	#ifdef ELAN_HID_I2C
	ts->header_size =  ( (HEADER_SIZE_V2==(ts->test_version-((ts->test_version/16)*6))) || 
		ts->fw_version == 0x6436) ? HEADER_SIZE_V2 : HEADER_SIZE;
	#else
	ts->header_size = HEADER_SIZE;
	#endif
	pr_info("%s header size=%d \n",__func__,ts->header_size);
...
static int elan_ktf_hid_parse_xy_v2(u8 *data, uint16_t *x, uint16_t *y)
{
    *x = *y = 0;
...
static int elan_ts_parse_pen(u8 *data, uint16_t *x, uint16_t *y, uint16_t *p , uint16_t *tip, uint16_t *tilt_x, uint16_t *tilt_y, uint16_t *azimuth)
{

	*x = *y = *p = *tip = 0;
	*tilt_x = *tilt_y = *azimuth = 0;
	*x = data[5];
	*x <<= 8;
	*x |= data[4];
...
//	printk("[elan]:enter ELAN_HID_I2C_Finger_PKT\n");
	...
	for(i = 0; i < finger_num; i++) {
		...
		if(finger_id>=MAX_CONTACT_NUM) {
			pr_err("[elan] invalid finger id %d >= %d (max),please check !!\n",
				finger_id,MAX_CONTACT_NUM);
			continue ;
		}
		...
		if(ts->header_size == HEADER_SIZE_V2){
		    elan_ktf_hid_parse_xy_v2(&buf[idx], &x, &y);
		}
		else
		...
		*/
...
#if 1
			if(0==p) {
				pr_info("%s : touch down without pressure,last pressure=%d\n",
						__func__,(int)ts->iLastTdP[finger_id]);
				p = ts->iLastTdP[finger_id];
			}
#endif
...
#if 0
		dev_dbg(&ts->client->dev,
				"[elan] [%d] touch p=%d->%d x=%d->%d y=%d->%d\n",
				finger_id,ts->iLastTdP,p, ts->iLastTdX,x,ts->iLastTdY, y);
#endif
		dev_dbg(&ts->client->dev,
				"[elan][%d] touch p=%d x=%d y=%d\n",finger_id,p,x,y);

		if(p>0) {
			if( (ts->iLastTdX[finger_id]==(int)x) && (ts->iLastTdY[finger_id]==(int)y) ) {
				dev_dbg(&ts->client->dev,
					"[elan] fw report same coordinates %s\n",
					(ts->iLastTdP[finger_id]==p)?"(same p)":"(diff p)");
			}
			else {
				ts->dwActPtCnt++;
			}
		...
		//printk(KERN_ERR"[elan hid] DOWN i=%d finger_id=%d x=%d y=%d Finger NO.=%d , pressure:%d\n", i, finger_id, x, y, finger_num,p);
		if(!active) {
			input_mt_slot(idev, finger_id);
		...
		mTouchStatus[finger_id] = active;
		ts->iLastTdX[finger_id] = x;
		ts->iLastTdY[finger_id] = y;
		ts->iLastTdP[finger_id] = p;

		if(ts->buf[FW_HDR_TYPE] == HID_I2C_FINGER_HEADER_V2){
			if(i>=1 && finger_num >2 ){
				if ( i%2 == 1)
					idx = ((i+1)/2)*18 + 3 ;
				else
...
//		idx += (ts->header_size == HEADER_SIZE_V2 ? 7 : 11);
	}
...
static void elants_i2c_mt_event_HID_I2C_Pen(struct elants_data *ts, u8 *buf)
{
	...
	uint16_t tilt_y = 0;
	uint16_t azimuth = 0;
	static uint32_t gdwPendownCnt = 0;
	u8 bReportID;

#define REPORT_ID_PENID	0x22
	...
	if(REPORT_ID_PENID==bReportID) {
		// Pen ID .
		ts->dwStylusID = buf[6]<<24|buf[5]<<16|buf[4]<<8|buf[3] ;
		dev_dbg(&ts->client->dev,
			"StylusID=0x%08x\n",ts->dwStylusID);
		return ;
	}
...
//	printk("[elan]:enter ELAN_HID_I2C_Pen_PKT\n");
	...
	//if (pen_hover) 
	{
		elan_ts_parse_pen(&buf[0], &x, &y, &p, &tip, &tilt_x, &tilt_y, &azimuth);
		...
		input_report_abs(idev, ABS_MT_POSITION_X, x);
		input_report_abs(idev, ABS_MT_POSITION_Y, y);
		input_report_abs(idev, ABS_TILT_X, tiltX);
		input_report_abs(idev, ABS_TILT_Y, tiltY);
		input_report_abs(idev, ABS_AZIMUTH, azimuth);
...
#endif
		dev_dbg(&ts->client->dev,
            "[elan pen] %x:%x:%x:%x:%x:%x:%x:%x:%x:%x\n",
            buf[0], buf[1], buf[2], buf[3],
            buf[4], buf[5], buf[6], buf[7],
			buf[17],buf[18]);
		dev_dbg(&ts->client->dev,
            "[elan] x=%d y=%d p=%d tip=%d tiltX=%d tiltY=%d azimuth=%d\n", x, y, p, tip, tiltX, tiltY, azimuth);


	if(p>0) {
		if( (ts->iLastPdX==x) && (ts->iLastPdY==y) ) {
			// dev_info(&ts->client->dev,
			// 	"[elan] fw report same coordinates %s\n",(ts->iLastPdP==p)?"(same p)":"(diff p)");
		}
		else {
			ts->dwActPtCnt++;
		}
	...
	*/
	if (pen_down){
		...
		if(1==++gdwPendownCnt) {
			dev_dbg(&ts->client->dev, "1st pendown : report event battery = %d\n",battery);
			input_report_key(idev, KEY_BATTERY, !battery);	
		...
		if(0!=tip) {
			if( (-1==ts->tip_max) || (0==ts->tip_max) || ((int)tip > ts->tip_max) ) {
				ts->tip_max = (int)tip;
			}
			if( (-1==ts->tip_min) || (0==ts->tip_max) || ((int)tip < ts->tip_min) ) {
				ts->tip_min = (int)tip;
			}
		}
		...
		if(gdwPendownCnt>0) {
			dev_dbg(&ts->client->dev, "penup : report event battery = %d\n",battery);
			input_report_key(idev, KEY_BATTERY, !battery);	
			input_report_key(idev, KEY_BATTERY, battery);	//#define KEY_BATTERY		236
		}
		...
		input_report_key(idev, BTN_STYLUS2, 0);	//btn2: eraser
#endif
		input_report_abs(idev, ABS_TIP_MAX, ts->tip_max);	//
		input_report_abs(idev, ABS_TIP_MIN, ts->tip_min);	//
		if( ts->tip_max > ts->tip_hist_max) {
			ts->tip_hist_max = ts->tip_max;
			input_report_abs(idev, ABS_TIP_HIST_MAX, ts->tip_hist_max);	//
		}
		if( ts->tip_min < ts->tip_hist_min) {
			ts->tip_hist_min = ts->tip_min;
			input_report_abs(idev, ABS_TIP_HIST_MIN, ts->tip_hist_min);	//
		}
...
static irqreturn_t elants_i2c_irq(int irq, void *_dev)
{
	...
	atomic_inc_return(&ts->tp_int_cnt);
	queue_work(ts->elan_wq, &ts->ts_work);
...
static int elants_register_interrupt(struct i2c_client *client)
{
	...
	if (client->irq) {
		...
		if (err < 0) {
			pr_err("%s(%s): Can't allocate irq %d\n", __FILE__, __func__, client->irq);
		}
...
static void  elan_ts_pwron_work_func(struct work_struct *work)
{
...
static void  elan_ts_work_func(struct work_struct *work)
{
	...
	do {
		...
		if(0==atomic_read(&ts->tp_int_cnt)) {
			pr_debug("%s : no pending data\n",__func__);
			break;
		}

		atomic_set(&ts->tp_int_cnt,0);
		...
		len = i2c_master_recv(client, ts->buf, ts->header_size/*sizeof(ts->buf)*/);
		if (len < 0) {
	...
	//             __func__, ts->header_size, ts->buf);

		if((ts->buf[3]&0x03) == 3){
			if(time_after_eq(ts->dwTPupJiffies,ts->dwTPdownJiffies)) {
				// first pendown .
				ts->dwTPdownJiffies = jiffies;
				ts->dwPtCnt = 1;
				ts->dwActPtCnt = 0;
				dev_dbg(&client->dev,"%s() : first tp down\n",__func__);
			}
			else {
				ts->dwPtCnt++;
			}
		...
		case ELAN_STATE_NORMAL:
			...
			case QUEUE_HEADER_SINGLE:  //no buffer mode = 0x62
				elants_i2c_event(ts, &ts->buf[ts->header_size]);
				break;
			...
			case HID_I2C_FINGER_HEADER: //HID_I2C header = 0x3F
				...
				break;
			case HID_I2C_FINGER_HEADER_V2:
				ts->buf[1] = ts->buf[17];  //store finger number

				if(ts->buf[1] > 2)  //more than 5 finger
				{
					int nRevSize = 0 ;
					if (ts->buf[1] > 10) {
						dev_err(&client->dev, "%s: finger number is larger than 10: %d\n",__func__, len);
						 goto out;
					}
					nRevSize = (((ts->buf[1]-2)+1)/2) *18 ;

					len = i2c_master_recv(client, buf2, nRevSize );
					if (len < 0) {
						dev_err(&client->dev, "%s: failed to read data: %d\n",__func__, len);
						 goto out;
					}
				
					for (i=0; i< nRevSize ; i++) {
						ts->buf[18+i] = buf2[i];
					}
				}
		...
		if( (3 != (ts->buf[3]&0x03)  && ts->dwPtCnt) ) { //if hover -> clean
			dev_dbg(&client->dev,"%s() : tp up\n",__func__);
			ts->dwTPupJiffies = jiffies;
		}
	} while(1);
 out:
...
static ssize_t elan_power_on_store(struct device *dev,
                                struct device_attribute *attr,
                                const char *buf, size_t count)
{
	struct elants_data *ts = private_ts;
...
static ssize_t get_tip_min(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int err = 0;
...
static ssize_t set_tip_min(struct device *dev, struct device_attribute *attr,
      const char *buf, size_t count)
{
	struct elants_data *ts = private_ts;
	long val;
...
static ssize_t get_tip_max(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int err = 0;
...
static ssize_t set_tip_max(struct device *dev, struct device_attribute *attr,
      const char *buf, size_t count)
{
	struct elants_data *ts = private_ts;
	long val;
...
static ssize_t get_tip_hist_min(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int err = 0;
...
static ssize_t set_tip_hist_min(struct device *dev, struct device_attribute *attr,
      const char *buf, size_t count)
{
	struct elants_data *ts = private_ts;
	long val;
	...
	ts->tip_hist_min = (int)val;

	return count;
}
static ssize_t get_tip_hist_max(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int err = 0;
...
static ssize_t set_tip_hist_max(struct device *dev, struct device_attribute *attr,
      const char *buf, size_t count)
{
	struct elants_data *ts = private_ts;
	long val;
	...
	ts->tip_hist_max = (int)val;

	return count;
}
static ssize_t get_stylus_id(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int err = 0;
...
static ssize_t get_tp_report_info(struct device *dev,
     struct device_attribute *attr, char *buf)
{
	struct elants_data *ts = private_ts;
	int len ,iTempLen;
	char *pcTemp;
	unsigned long dwDiffTicks;

	dwDiffTicks = ts->dwTPupJiffies - ts->dwTPdownJiffies;

	pcTemp = buf;
	len = 0;

	iTempLen = sprintf(pcTemp, "tp_down_cont_ms : %d\n",jiffies_to_msecs(dwDiffTicks));
	pcTemp+=iTempLen;
	len+=iTempLen;

	iTempLen = sprintf(pcTemp, "tp_down_count : %d\n",(int)ts->dwPtCnt);
	pcTemp+=iTempLen;
	len+=iTempLen;

	iTempLen = sprintf(pcTemp, "tp_down_report_rate : %d\n",ts->dwPtCnt*1000/jiffies_to_msecs(dwDiffTicks));
	pcTemp+=iTempLen;
	len+=iTempLen;
	
	iTempLen = sprintf(pcTemp, "Act_tp_down_count : %d\n",(int)ts->dwActPtCnt);
	pcTemp+=iTempLen;
	len+=iTempLen;

	iTempLen = sprintf(pcTemp, "Act_tp_down_report_rate : %d\n",ts->dwActPtCnt*1000/jiffies_to_msecs(dwDiffTicks));
	pcTemp+=iTempLen;
	len+=iTempLen;
...
static DEVICE_ATTR(power_on, S_IWUSR, NULL,elan_power_on_store);
static DEVICE_ATTR(tip_min, 0644, get_tip_min, set_tip_min);
static DEVICE_ATTR(tip_max, 0644, get_tip_max, set_tip_max);
static DEVICE_ATTR(tip_hist_max, 0644, get_tip_hist_max, set_tip_hist_max);
static DEVICE_ATTR(tip_hist_min, 0644, get_tip_hist_min, set_tip_hist_min);
static DEVICE_ATTR(stylus_id, S_IRUGO, get_stylus_id,NULL);
static DEVICE_ATTR(tp_report_info, S_IRUGO, get_tp_report_info, NULL);
...
static struct attribute *elants_attributes[] = {
	...
	&dev_attr_tip_min.attr,
	&dev_attr_tip_max.attr,
	&dev_attr_tip_hist_min.attr,
	&dev_attr_tip_hist_max.attr,
	&dev_attr_stylus_id.attr,
	&dev_attr_tp_report_info.attr,
...
static int elants_i2c_power_on(struct elants_data *ts)
{
	...
	if (IS_ERR_OR_NULL(ts->vcc33)||IS_ERR_OR_NULL(ts->vccio)) {
		dev_info(&ts->client->dev, "%s(%d):skipped since vcc33 or vccio not availible\n",__func__,__LINE__);
		return 0;
	}
	...
	if (IS_ERR_OR_NULL(ts->reset_gpio)) {
		dev_info(&ts->client->dev, "%s(%d):skipped since reset gpiod not availible\n",__func__,__LINE__);
		return 0;
	}
	...
	if (!gpio_is_valid(ts->reset_gpio)) {
		dev_info(&ts->client->dev, "%s(%d):skipped since reset gpio not availible\n",__func__,__LINE__);
		return 0;
	}
...
#if 0	// do not control regulator in touch driver
	if (!IS_ERR_OR_NULL(ts->vcc33)) {
		regulator_disable(ts->vcc33);
	}

	if(!IS_ERR_OR_NULL(ts->vccio)) {
		regulator_disable(ts->vccio);
	}
#endif
}

static void elan_pinstate_init(struct elants_data *ts)
{
	pr_info("%s(%d)\n",__func__,__LINE__);
	ts->pctrl = devm_pinctrl_get(&ts->client->dev);
	if (IS_ERR(ts->pctrl)) {
		pr_info("%s devm_pinctrl_get error! %p \n",__func__,ts->pctrl);
		return ;
	}

	ts->pin_state_active = pinctrl_lookup_state(ts->pctrl, "active");
	if (IS_ERR(ts->pin_state_active))
		pr_err("%s pinctrl_lookup_state active error!\n",__func__);

	ts->pin_state_inactive =
		pinctrl_lookup_state(ts->pctrl, "inactive");
	if (IS_ERR(ts->pin_state_inactive))
		pr_debug("%s pinctrl_lookup_state inactive error!\n",__func__);
}
static void elan_pinstate_active(struct elants_data *ts)
{
	pr_debug("%s\n",__func__);
	if (IS_ERR(ts->pctrl)) {
		return ;
	...
	if(pinctrl_select_state(ts->pctrl, ts->pin_state_active)<0) {
		dev_err(&ts->client->dev, "select active pin state failed \n");
	}
}
static void elan_pinstate_inactive(struct elants_data *ts)
{
	pr_debug("%s\n",__func__);
	if (IS_ERR(ts->pctrl)) {
		return ;
	...
	if(pinctrl_select_state(ts->pctrl, ts->pin_state_inactive)<0) {
		dev_err(&ts->client->dev, "select inactive pin state failed \n");
	}
}
                            ...
                            const struct i2c_device_id *id)
{
	...
	ts->tip_min=-1;
	ts->tip_max=-1;
	ts->tip_hist_min=0xffff;
	ts->tip_hist_max=0;
	ts->dwTPdownJiffies = ts->dwTPupJiffies = jiffies;
    ...
    pr_info("get max_x=%d, max_y=%d, revert_x=%d, revert_y=%d, exchange_x_y=%d\n", screen_max_x, screen_max_y, revert_x_flag, revert_y_flag, exchange_x_y_flag);
#endif

	elan_pinstate_init(ts);
	elan_pinstate_active(ts);
	...
	__set_bit(ABS_TIP_MAX, ts->input->keybit);
	__set_bit(ABS_TIP_MIN, ts->input->keybit);
	...
	input_set_abs_params(ts->input, ABS_TIP_MAX, 0, 0xffff, 0, 0);
	input_set_abs_params(ts->input, ABS_TIP_MIN, 0, 0xffff, 0, 0);
  
	input_set_abs_params(ts->input, ABS_TILT_X, -60, 60, 0, 0);
	input_set_abs_params(ts->input, ABS_TILT_Y, -60, 60, 0, 0);
	input_set_abs_params(ts->input, ABS_AZIMUTH, 0, 360, 0, 0);
	...
	INIT_WORK(&ts->ts_pwron_work, elan_ts_pwron_work_func);
...
static int __maybe_unused elants_i2c_suspend(struct device *dev)
{
	...
	if(work_pending(&ts->ts_work)) {
		pr_info("%s(%d) irq work is pending !!\n",__func__,__LINE__);
		return -EBUSY;
	}
	if(work_pending(&ts->ts_pwron_work)) {
		pr_info("%s(%d) pwron work is pending !!\n",__func__,__LINE__);
		return -EBUSY;
	}
...
static int __maybe_unused elants_i2c_resume(struct device *dev)
{
    ...
    if (gSleep_Mode_Suspend) {
		...
		queue_work(ts->elan_wq, &ts->ts_pwron_work);

	} else {
		if (!elan_i2c_detect_int_level(ts)) {
			elan_i2c_ts_triggered(ts);
		}
...
static const struct dev_pm_ops elants_i2c_pm_ops = {
	.suspend_late = elants_i2c_suspend,
	.resume_early = elants_i2c_resume,
};
