...
static unsigned char CTPM_FW[] = {
	/*#include "SDR10F0127A_V11_D01_20180730_app.i"*/
	//#include "mtk_s102_10.3_FT5626_V17_D01_20190618_app.i"
	//#include "FT5822_lx_v24.i"
	//#include "FT5822_Ref_V26_D01_20190717_app.i"
	//#include "FT5822_Ref_V29_D01_20190718_app.i"
	//#include "FT5822_Ref_V32_D01_20190820_app.i"
	//#include "5x46_Ref_V02_D01_20200429_app.i"
	//#include "5x46_Ref_V04_D01_20200515_app.i"
};
...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	dev_info(&client->dev, "fw_vendor_id=0x%x\n", data->fw_vendor_id);
}
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1]
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] != 0) {
			*ucPVendorID = 0;
FTS_DBG("Vender ID Mismatch,REG1=0x%x, REG2=0x%x, Definition:0x%x, i_ret=%d\n",
 ...
 * Return: no
 ***********************************************************************/
void fts_get_upgrade_array(struct fts_ts_data *data)
{
	u8 chip_id;
	u32 i;
	int ret = 0;

	ret = fts_read_reg(data->client, FTS_REG_ID, &chip_id);
	if (ret < 0)
 ...
 * Return: fail <0
 ***********************************************************************/
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client,
 ...
 * Return: fail <0
 ***********************************************************************/
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client,
		u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
 ...
 * Return: fail <0
 ***********************************************************************/
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client,
		u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[4] = {0};
	u32 i = 0;
 ...
 * Return: fail <0
 ***********************************************************************/
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client,
			u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		auc_i2c_write_buf[0] = FTS_READ_ID_REG;
		auc_i2c_write_buf[1] =
			auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
		    && reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			dev_dbg(&client->dev,
			...
			u8 *pbt_buf, u32 dw_length)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		if (bt_ecc != bt_ecc_check)
			pr_err("[FTS][%s] Host checksum error bt_ecc_check = %x\n",
 ...
 * Return: fail <0
 ***********************************************************************/
int fts_5x06_ctpm_fw_upgrade(struct i2c_client *client,
		u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
 ...
 * Return: fail <0
 ***********************************************************************/
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client *client,
			u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[4] = {0};
	u32 i = 0;
 ...
 * Input: no
 * Output: no
 * Return: fw version  //1001  88(0x58)       6805 84(0x54)
 ***********************************************************************/
int fts_ctpm_get_i_file_ver(void)
{
	...
	if (ui_sz > 2) {
		...
		else if (fts_updateinfo_curr.CHIP_ID == 0x58)//88
			return CTPM_FW[0x010A];//[0x1D0A];
		else
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	dev_dbg(&client->dev, "[FTS] old setting: uc_i2c_addr = 0x%x, uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
			buf[0], buf[2], buf[4]);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if ((fts_updateinfo_curr.CHIP_ID == 0x11)
		  ...
		  && (CTPM_FW[fw_len - 3] ^ CTPM_FW[fw_len - 4]) == 0xFF) {
			...
			if ((fts_updateinfo_curr.CHIP_ID == 0x55)
				...
				i_ret = fts_5x06_ctpm_fw_upgrade(client,
				 ...
				 || (fts_updateinfo_curr.CHIP_ID == 0x13)
				 || (fts_updateinfo_curr.CHIP_ID == 0x14)) {
				i_ret = fts_5x36_ctpm_fw_upgrade(client,
					pbt_buf, sizeof(CTPM_FW));
			} else if (fts_updateinfo_curr.CHIP_ID == 0x06) {
				i_ret = fts_6x06_ctpm_fw_upgrade(client,
					pbt_buf, sizeof(CTPM_FW));
			}
			if (i_ret != 0) {
				dev_err(&client->dev,
					"%s:upgrade failed. err.\n", __func__);
			} else if (fts_updateinfo_curr.AUTO_CLB
						== AUTO_CLB_NEED) {
				fts_ctpm_auto_clb(client);
			}
		} else {
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36_ctpm_fw_upgrade(client,
			pbt_buf, sizeof(CTPM_FW));
		if (i_ret != 0)
			dev_err(&client->dev,
				"%s:upgrade failed. err.\n", __func__);
