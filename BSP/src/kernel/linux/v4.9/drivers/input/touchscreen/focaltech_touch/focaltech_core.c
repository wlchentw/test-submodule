...
/*****************************************************************************
*  Name: fts_switch_charger
*  Brief: value : 1 -> enable charger 
*         value : 0 -> disable charger 
*        
*  Input:
*  Output:
*  Return: return 0 if successful
*****************************************************************************/
int fts_switch_charger(u8 value){
	int ret;
	
    ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, value);
	
    FTS_DEBUG("MODE CHARGER switch value :%d", value);
...
EXPORT_SYMBOL(fts_switch_charger);
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if(0==gptHWCFG->m_val.bUIStyle ){ // Ebrmain
                ...
                if(0x71==gptHWCFG->m_val.bPCB )
                    y = events[i].x ;				
                else
                    y = data->pdata->y_max- events[i].x ;
				x = events[i].y ;
...
static int fts_irq_read_report(void)
{
	int ret = -1;
	struct fts_ts_data *ts_data = fts_data;
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    printk(KERN_ERR"[%s_%d] irq :%d \n",__FUNCTION__,__LINE__,ts_data->irq);
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    if (gpio_is_valid(data->pdata->irq_gpio)) {
        ...
        printk(KERN_ERR"[%s_%d] request reset_gpio:%d\n",__FUNCTION__,__LINE__,data->pdata->irq_gpio);
        ...
        FTS_ERROR("[GPIO] irq gpio  is invalid !!");
    }


    /* request reset gpio */
    if (gpio_is_valid(data->pdata->reset_gpio)) {
        printk(KERN_ERR"[%s_%d] request reset_gpio:%d\n",__FUNCTION__,__LINE__,data->pdata->reset_gpio);
		...
		FTS_ERROR("[GPIO] reset gpio  is invalid !!");
	}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    printk(KERN_ERR"[%s_%d] irq_gpio:%d\n",__FUNCTION__,__LINE__,pdata->irq_gpio);
    ...
    FTS_INFO("[FT5426] max touch number:%d, irq gpio:%d, reset gpio:%d",
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if (gSleep_Mode_Suspend) {
		...
		//if (fts_irq_read_report()==0)
		//{
		//	printk(KERN_ERR"[%s_%d] has touch event !!\n",__FUNCTION__,__LINE__);
		//	return 0;
		//}
		enable_irq_wake (fts_data->irq);
        printk("[%s_%d] enable irq wake :%d \n",__FUNCTION__,__LINE__,fts_data->irq);
	}
