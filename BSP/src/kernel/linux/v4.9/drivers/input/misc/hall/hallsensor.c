...
static void hall_work_func(struct work_struct *work)
{
	...
	pr_info("%s: cover state: %s\n",
		__func__, priv->cover == COVER_OPENED ?
		"OPENED" : "CLOSED");
	...
	snprintf(metrics_log, METRICS_STR_LEN,
		"HALLSENSOR:TIMEOUT:COVER=%d;CT;1,DISPLAY=%d;CT;1:NR",
		priv->cover, 0);
...
static int hall_probe(struct platform_device *pdev)
{
	...
	if (error) {
		pr_info("Unable to read hall_sensor_num\n");
		return error;
	}
	...
	for (i = 0; i < hall_sensors->hall_sensor_num; i++) {
		...
		if (!gpio_is_valid(
			hall_sensors->hall_sensor_data[i].gpio_pin)) {
			pr_info("Unable to read int-gpio[%d]\n", i);
			goto fail0;
		}
		...
		if (error) {
			pr_info("Unable to read sensor-name[%d]\n", i);
			goto fail0;
		}
		...
		if (hall_sensors->hall_sensor_data[i].irq <= 0) {
			pr_info("Unable to get interrupt number[%d]\n", i);
			error = -EINVAL;
			goto fail0;
		}
	...
	if (!input) {
		pr_info("Failed to allocate input device\n");
		error = -ENOMEM;
		goto fail1;
	}
	...
	if (!priv->workqueue) {
		pr_info("Unable to create workqueue\n");
		goto fail2;
	}
	...
	if (error) {
		pr_info("Failed to configure interupts, err=%d\n", error);
		goto fail3;
	}
...
static int hall_remove(struct platform_device *pdev)
{
	...
	wakeup_source_drop(&priv->hall_wake_lock);
#ifdef CONFIG_OF
	kfree(sensor_data);
#endif
	kfree(priv);
