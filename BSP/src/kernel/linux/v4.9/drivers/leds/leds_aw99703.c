...
#include <linux/ntx_firmware.h>
#include <linux/ntx_hwconfig.h>
			   ...
			   int writelen, char *readbuf, int readlen)
{
	...
	for(;;) {
		if( time_is_after_jiffies(gdwJiffies_flen+msecs_to_jiffies(10)) ) {
			dev_info(&client->dev,"%s() waiting for poweron stable\n",__func__);
			msleep(10);
			break;
		}
		else {
...
static void aw99703_hwen_pin_ctrl(struct aw99703_data *drvdata, int en)
{
	...
	if (gpio_is_valid(drvdata->fl_gpio)) {
		if (en) {
			pr_info("hwen pin is going to be high!---<%d>\n", en);
			gpio_set_value(drvdata->fl_gpio, true);
			usleep_range(3500, 4000);
		} else {
			pr_info("hwen pin is going to be low!---<%d>\n", en);
			gpio_set_value(drvdata->fl_gpio, false);
			usleep_range(1000, 2000);
		}
	}
}
...
static int aw99703_gpio_init(struct aw99703_data *drvdata)
{
	...
	if (gpio_is_valid(drvdata->fl_gpio)) {
		ret = gpio_request(drvdata->fl_gpio, "fl_gpio");
		if (ret < 0) {
			pr_err("failed to request gpio\n");
			return -1;
		}
		ret = gpio_direction_output(drvdata->fl_gpio, 0);
		pr_info(" request gpio init\n");
		if (ret < 0) {
			pr_err("failed to set output");
			gpio_free(drvdata->fl_gpio);
			return ret;
		}
		pr_info("gpio is valid!\n");
		aw99703_hwen_pin_ctrl(drvdata, 1);
...
static int _aw99703_set_brightness_to_reg(struct aw99703_data *drvdata,int brt_val)
{
	if(drvdata->brightness != brt_val) {
		pr_info("%s [%d] brt=%d->%d\n", __func__,
			drvdata->adapter->nr,drvdata->brightness,brt_val);
		...
		drvdata->led_dev.brightness = brt_val;
		return 1;
	}
	return 0;
}
static int _aw99703_set_current_to_reg(struct aw99703_data *drvdata,unsigned char led_current)
{
	if (led_current > 0x1f) {
		pr_err("[%s_%d] led_current is out of range !!",__FUNCTION__,__LINE__);
		return -1 ;
	}

	if(drvdata->last_current != led_current) {
		pr_info("%s [%d] cur=%d->%d\n", __func__,
			drvdata->adapter->nr,drvdata->last_current,(int)led_current);
		/* set backlight current */
		aw99703_i2c_write_bit(drvdata->client,
			AW99703_REG_LEDCUR,
			AW99703_LEDCUR_BLFS_MASK,
			led_current << 3);
		drvdata->last_current = led_current;
		drvdata->full_scale_led = led_current;
		return 1;
	}
	return 0;
}
static int aw99703_set_cur_brightness(struct aw99703_data *drvdata, unsigned char led_current, int brt_val)
{
	brt_val = aw99703_brightness_map(drvdata->bl_map, brt_val);

#if 0
	pr_info("%s brt=%d->%d,cur=%d->%d\n", __func__,
		   drvdata->brightness,brt_val,drvdata->last_current,(int)led_current);
#endif
	...
	if( (led_current > drvdata->last_current) && (brt_val < drvdata->brightness) )
	{
		_aw99703_set_brightness_to_reg(drvdata,brt_val);
		_aw99703_set_current_to_reg(drvdata,led_current);
	}
	else {
		_aw99703_set_current_to_reg(drvdata,led_current);
		_aw99703_set_brightness_to_reg(drvdata,brt_val);
...
static int aw99703_set_cur_brightness_bulk(struct aw99703_data *drvdata, unsigned char led_current, int brt_val)
{
	u8 bRegA[6] = {AW99703_REG_LEDCUR,};// regadd, reg 0x03 ~ 0x07 .
	...
	platform_read_i2c_block(drvdata->client, &bRegA[0], 1, &bRegA[1], 5);
	// LEDCUR .
	bRegA[1] &= AW99703_LEDCUR_BLFS_MASK;
	bRegA[1] |= (led_current << 3) ;
	// LEDLSB
	bRegA[4] =brt_val&0x0007;
	// LEDMSB
	bRegA[5] =(brt_val >> 3)&0xff;
	platform_write_i2c_block(drvdata->client, bRegA, 6);
...
{
	return aw99703_set_cur_brightness(drvdata,drvdata->last_current,brt_val);
}
...
int fl_aw99703_current(struct aw99703_data *drvdata, unsigned char led_current)
{
	...
	return aw99703_set_cur_brightness(drvdata,led_current,drvdata->brightness);
}
...
int fl_aw99703_x2_percentage (int iFL_Percentage)
{
	...
	if(gptHWCFG==NULL){
		printk(KERN_ERR"[%s_%d] gptHWCFG is NULL \n",__FUNCTION__,__LINE__);
		return -1;
	}

	if( iFL_Percentage > 100 || iFL_Percentage < 0)
	{
		printk(KERN_ERR"[%s_%d] iFL_Percentage is out of range !!", __func__, __LINE__);

		return -1 ;
	}

	if( (0x0c == gptHWCFG->m_val.bFL_PWM) && gpled[0] ) /* FL PWM is AW99703 */
	{
		iColor = gpled[0]->frontlight_table;
		...
		if(gptAw99703fl_dualcolor_percent_tab){
			...
			if( 0 == iFL_Percentage){
				aw99703_set_cur_brightness_bulk(gpled[0], 0, 0);
				aw99703_set_cur_brightness_bulk(gpled[1], 0, 0);
			}
			else{
				if( gptHWCFG->m_val.bPCB == 0x62 || gptHWCFG->m_val.bPCB == 0x65 ) // E80P04 / E70K14
				{
					aw99703_set_cur_brightness_bulk(gpled[0], led_A_current, gptAw99703fl_dualcolor_percent_tab[iColor].bC1_BrightnessA[iFL_Percentage-1]);
					aw99703_set_cur_brightness_bulk(gpled[1], led_B_current, gptAw99703fl_dualcolor_percent_tab[iColor].bC2_BrightnessA[iFL_Percentage-1]);
				}
				else{
					aw99703_set_cur_brightness_bulk(gpled[0], led_A_current, gptAw99703fl_dualcolor_percent_tab[iColor].bC1_BrightnessA[iFL_Percentage-1]);
					aw99703_set_cur_brightness_bulk(gpled[1], led_B_current, gptAw99703fl_dualcolor_percent_tab[iColor].bC2_BrightnessA[iFL_Percentage-1]);
				}
				pr_info("[AW99703] iColor:%d Percentage:%d , A:(%d)%d , B:(%d)%d \n",iColor,iFL_Percentage,
			...
			printk(KERN_ERR"[%s_%d] gptAw99703fl_dualcolor_percent_tab NULL\n",__FUNCTION__,__LINE__);
		}
	}
...
static int aw99703_suspend(struct device *dev)
{
	...
	printk("[%s_%d] gSleep_Mode_Suspend:%d \n", __FUNCTION__, __LINE__, gSleep_Mode_Suspend);

	mutex_lock(&drvdata->lock);
	if (gSleep_Mode_Suspend) {

		aw99703_set_cur_brightness_bulk(drvdata, 0, 0);

		if(--giFL_PWR_state <= 0) {
			printk("[%s_%d] giFL_PWR_state:%d , fl_en_status:%d\n", __FUNCTION__, __LINE__, giFL_PWR_state,fl_en_status);
			if(giFL_PWR_state < 0) {
				giFL_PWR_state = 0;
			}
			if (0 != fl_en_status) {
				...
				if (gpio_is_valid(drvdata->fl_gpio)) {
					gpio_direction_output(drvdata->fl_gpio,0);
				}
				fl_en_status = 0;
			}
		}
...
static int aw99703_resume(struct device *dev)
{
	...
	printk(KERN_INFO"[%s_%d]i2c%d gSleep_Mode_Suspend:%d \n",
		__FUNCTION__,__LINE__,client->adapter->nr,gSleep_Mode_Suspend);
	...
	if (gSleep_Mode_Suspend) {
		...
		if (giFL_PWR_state == 1) {
			printk("%s():FL PWR[ON] \n",__FUNCTION__);
			if (gpio_is_valid(drvdata->fl_gpio)) {
				gpio_direction_output(drvdata->fl_gpio,1);
			}
			if(gpio_is_valid(drvdata->hwen_gpio)) {
...
static void aw99703_get_dt_data(struct device *dev, struct aw99703_data *drvdata)
{
	...
	drvdata->fl_gpio = of_get_named_gpio(np, "aw99703,fl_gpio", 0);
	pr_info("%s drvdata->fl_gpio --<%d>\n", __func__, drvdata->fl_gpio);
			...
			const struct i2c_device_id *id)
{
	...
	drvdata->brightness = -1;
	drvdata->last_current = 0xff;
	drvdata->enable = true;
