...
extern void max20342_DisableUSBSWC(void);
extern void max20342_EnableUSBSWC(void);
struct delayed_work	switch_usbmode;
		...
		const char *page, size_t len)
{
	...
	pr_info("%s write %s\n", __func__, name);
#ifdef CONFIG_SENSORS_MAX20342 //[
	//disable the USB switch.
	max20342_DisableUSBSWC();
#endif //]CONFIG_SENSORS_MAX20342
...
#ifdef CONFIG_MTPROF
	{
		...
		schedule_delayed_work(&switch_usbmode, msecs_to_jiffies(1000));
	}
#endif
#endif //] CONFIG_SENSORS_MAX20342
...
static void switch_usbstaus(struct work_struct *work)
{
	//enable the USB switch.
	max20342_EnableUSBSWC();
}
...
static int android_device_create(struct gadget_info *gi)
{
	...
	usb_state_monitor_work();
#endif
	INIT_DELAYED_WORK(&switch_usbmode, switch_usbstaus);
	return 0;
}
