...
#define UAC_CLOCK_SELECTOR_DES_SIZE(ch) \
	(sizeof(struct uac_clock_selector_descriptor) + (ch))
...
#define UAC2_FEATURE_UNIT_DES_SIZE(ch) \
	(sizeof(struct uac2_feature_unit_descriptor) + (ch))
...
static int uac2_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if ((audio_dev->as_in_alt == 0) &&
		...
		pr_info("UAC2 open fail: there is no avild alt ");
		pr_info("please Check Host default output and input\n");
		return -EPERM;
	}
...
/* Output Terminal for I/O-Out CANCEL */
...
afunc_alt_match_params(struct f_uac2_alt *value, u8 alt, u8 dir)
{
	if (dir == ALT_OUT) {
		...
		case 2:
			...
			break;
		default:
			pr_info("uac2 alt error %d, dir %d!\n", alt, dir);
			break;
		}
	} else if (dir == ALT_IN) {
		switch (alt) {
		...
		pr_info("uac2 alt %d, dir error %d!\n", alt, dir);
	}
}
...
in_rq_range(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	...
	if (control_selector == UAC2_CS_CONTROL_SAM_FREQ) {
		if (agdev->gspeed == USB_SPEED_HIGH) {
			value = min_t(unsigned int, w_length, sizeof(r_high));
			memcpy(req->buf, &r_high, value);
		} else {
			value = min_t(unsigned int, w_length, sizeof(r_full));
	...
	if (value != -EOPNOTSUPP) {
		...
		pr_info("%s:%d control_selector=%d TODO!\n",
			__func__, __LINE__, control_selector);
	}
...
out_rq_cur(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	...
	if (((cr->bRequestType & (0x01 << 7)) == USB_DIR_OUT) &&
		((en_id == USB_OUT_CLK_ID) || (en_id == USB_IN_CLK_ID))) {
		pr_info("uac2 host %s sample rate:",
			(en_id == USB_OUT_CLK_ID) ? "playback" : "capture");
		req->context =
			kzalloc(sizeof(struct usb_ctrlrequest), GFP_ATOMIC);
		if (!req->context)
			return -ENOMEM;
