...
extern int get_ep0_status(void);
extern void set_ep0_status(void);
#endif //]
...
int check_usb_state_by_ep(void)
{
	#if defined(USB_GADGET_DETECT) //[
	int usb_state = -1;
	int iTimeout_ms=3000;
	unsigned long dwCheckTimeoutJiffies = jiffies+msecs_to_jiffies(iTimeout_ms);
	int iChk = 0;

	pr_debug("%s() mt_usb_connect \n",__func__);
	mt_usb_connect();
	set_ep0_status();
	mdelay(50); // avoid "unexpected unclosed PLL: univpll2"
	pr_debug("%s() usb_composite_probe ... \n",__func__);
	iChk = usb_composite_probe(&chargedetect_driver);
	if(iChk!=0) {
		printk(KERN_ERR"%s() usb_composite loading failed !!\n",__func__);
		return -2;
	}

	for(;;)
	{
		if(time_is_after_jiffies(dwCheckTimeoutJiffies)) {
			if(get_ep0_status())
			{
				pr_debug("%s()  usb_state = 1 !!\n",__func__);
				usb_state = 1;
				break;
			}
		}
		else {
			pr_err("%s() check usb ep %d ms times out !!\n",__func__,iTimeout_ms);
			usb_state = 0;
			break;
		}
		msleep(100);
	...
	pr_debug("%s() usb_composite_unregister ... \n",__func__);
	mdelay(50);
	iChk = usb_composite_unregister_ex(&chargedetect_driver);
	if(0!=iChk) {
		pr_err("[ERR] unregister usb composite failed (err=%d) !!!\n",iChk);
	}
	pr_debug("%s() mt_usb_disconnect \n",__func__);
	mt_usb_disconnect();
	return usb_state;
	#else //][!
	printk("%s() check usb ep skipped !!\n",__func__);
	return -1;
	#endif //]
}
...
int usb_composite_unregister_ex(struct usb_composite_driver *driver)
{
	return usb_gadget_unregister_driver(&driver->gadget_driver);
}
EXPORT_SYMBOL_GPL(usb_composite_unregister_ex);
