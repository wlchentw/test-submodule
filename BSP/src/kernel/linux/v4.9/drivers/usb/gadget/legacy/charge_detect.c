...
static struct fsg_module_parameters chargedetect_mod_data = { .stall = 1, .removable = { 1, }, };
...
FSG_MODULE_PARAMETERS(/* no prefix */, chargedetect_mod_data);

static int chargedetect_do_config(struct usb_configuration *c)
{
...
static struct usb_configuration chargedetect_config_driver = {
...
static int chargedetect_bind(struct usb_composite_dev *cdev)
{
	...
	fsg_config_from_params(&config, &chargedetect_mod_data, fsg_num_buffers);
	opts = fsg_opts_from_func_inst(fi_msg);
	...
	status = usb_add_config(cdev, &chargedetect_config_driver, chargedetect_do_config);
	if (status < 0)
		goto fail_otg_desc;
