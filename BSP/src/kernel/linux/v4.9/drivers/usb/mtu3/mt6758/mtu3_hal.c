...
int get_ssusb_ext_rscs(struct ssusb_mtk *ssusb)
{
	...
	priv->vusb10 = devm_regulator_get(dev, "vusb10");
	if (IS_ERR(priv->vusb10)) {
		dev_err(dev, "failed to get vusb10\n");
		return PTR_ERR(priv->vusb10);
	}

	priv->clk_mux = devm_clk_get(dev, "clk_mux");
	if (IS_ERR(priv->clk_mux)) {
		dev_err(dev, "failed to get clk_mux\n");
		return PTR_ERR(priv->clk_mux);
	}

	priv->clk_mux_default = devm_clk_get(dev, "clk_mux_default");
	if (IS_ERR(priv->clk_mux_default)) {
		dev_err(dev, "failed to get clk_mux_default\n");
		return PTR_ERR(priv->clk_mux_default);
	}

	priv->clk_mux_host = devm_clk_get(dev, "clk_mux_host");
	if (IS_ERR(priv->clk_mux_host)) {
		dev_err(dev, "failed to get clk_mux_host\n");
		return PTR_ERR(priv->clk_mux_host);
	}

	priv->clk_refpll = devm_clk_get(dev, "clk_refpll");
	if (IS_ERR(priv->clk_refpll)) {
		dev_err(dev, "failed to get clk_refpll\n");
		return PTR_ERR(priv->clk_refpll);
	}
...
static int ssusb_host_clk_on(struct ssusb_mtk *ssusb)
{
	...
	struct ssusb_priv *priv = ssusb->priv_data;

	ret = clk_prepare_enable(priv->clk_refpll);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable clk_refpll\n");
		goto refpll_err;
	}

	ret = clk_prepare_enable(priv->clk_mux);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable clk_mux\n");
		goto clk_mux_err;
	}

	ret = clk_set_parent(priv->clk_mux, priv->clk_mux_host);
	if (ret) {
		dev_err(ssusb->dev, "failed to enable clk_mux\n");
		goto clk_parent_err;
	}
...
clk_mux_err:
	clk_disable_unprepare(priv->clk_refpll);
refpll_err:
	return ret;
}
...
static int ssusb_host_clk_off(struct ssusb_mtk *ssusb)
{
	...
	if (clk_set_parent(priv->clk_mux, priv->clk_mux_default))
		dev_err(ssusb->dev, "ssusb_clk_mux switch fail\n");
	clk_disable_unprepare(priv->clk_mux);
	clk_disable_unprepare(priv->clk_refpll);
				...
				void *arg)
{
	...
	case NOTIFY_SOIDLE_ENTER:
		if (dpidle_status == USB_DPIDLE_AUDIO) {
			writel(readl(infra_ao_base + 0x78) |
				(0x1<<1), infra_ao_base + 0x78);
			writel(readl(infra_ao_base + 0x78) &
				~(0x1<<0), infra_ao_base + 0x78);
			writel(readl(infra_ao_base + 0x78) |
				(0x1<<6), infra_ao_base + 0x78);
			writel(readl(infra_ao_base + 0x78) |
				(0x1<<0), infra_ao_base + 0x78);
			writel(readl(infra_ao_base + 0x78) &
				~(0x1<<1), infra_ao_base + 0x78);
