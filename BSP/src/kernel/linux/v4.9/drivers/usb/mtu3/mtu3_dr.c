...
int ssusb_set_vbus(struct otg_switch_mtk *otg_sx, int is_on)
{
	...
	if (is_on) {
		...
		if(regulator_is_enabled(vbus)) {
			...
			dev_warn(ssusb->dev, "vbus regulator already disabled \n");
		}
	}
...
static bool devicemode = 1;
ssize_t ssusb_device_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{

	sprintf(buf, "device mode is: %s\n",
		devicemode ? "enable" : "disable");
	return 0;
}

ssize_t ssusb_device_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "enable", 6) && !devicemode) {
		if (!ssusb->keep_ao) {
			ret = ssusb_clks_enable(ssusb);
			if (ret) {
				dev_info(ssusb->dev, "failed to enable clock\n");
		...
		mtu3_start(mtu);
		devicemode = 1;
	} else if (!strncmp(buf, "disable", 7) && devicemode) {
		mtu3_stop(mtu);
		pm_relax(ssusb->dev);
		spin_lock_irqsave(&mtu->lock, flags);
		mtu3_gadget_disconnect(mtu);
		...
		dev_info(ssusb->dev, "wrong or duplicated setting\n");
		return -EINVAL;
	}
	return count;
}
