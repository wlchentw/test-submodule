...
DEVICE_ATTR(device, 0664, ssusb_device_show, ssusb_device_store);

static struct attribute *mtu3_attributes[] = {
	&dev_attr_device.attr,
	NULL
};

static const struct attribute_group mtu3_attr_group = {
...
static int mtu3_probe(struct platform_device *pdev)
{
	...
	case USB_DR_MODE_PERIPHERAL:
		...
		ret = sysfs_create_group(&ssusb->dev->kobj, &mtu3_attr_group);
		break;
...
static int mtu3_remove(struct platform_device *pdev)
{
	...
	switch (ssusb->dr_mode) {
	case USB_DR_MODE_PERIPHERAL:
		sysfs_remove_group(&ssusb->dev->kobj, &mtu3_attr_group);
		ssusb_gadget_exit(ssusb);
		break;
	case USB_DR_MODE_HOST:
