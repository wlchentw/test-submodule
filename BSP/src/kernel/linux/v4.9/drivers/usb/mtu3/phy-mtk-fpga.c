...
u8 phy_readb(void __iomem *port, u8 i2c_addr,  u8 addr)
{
	...
	if (ret == PHY_FALSE) {
		pr_debug("Read failed(i2c_addr: %d, addr: 0x%x)\n",
				i2c_addr, addr);
		return ret;
	}
		...
		struct fpga_phy_instance *instance)
{
	...
	phy_writelmsk(i2c_port_base, 0x68, 0x18, 4, 0xf<<4, 0xc);
	return 0;
}
		...
		struct of_phandle_args *args)
{
	...
	if (args->args_count != 1) {
		dev_dbg(dev, "invalid number of cells in 'phy' property\n");
	...
	if (!instance) {
		dev_dbg(dev, "failed to find appropriate phy\n");
		return ERR_PTR(-EINVAL);
	}
	...
	if (!(instance->type == PHY_TYPE_USB2 ||
		...
		dev_dbg(dev, "unsupported device type: %d\n", instance->type);
		return ERR_PTR(-EINVAL);
	}
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	case 'p':
		...
		break;
	default:
		dev_dbg(instance->dev, "No such cmd\n");
	}
		...
		struct fpga_phy_instance *instance)
{
	...
	if (!root) {
		dev_dbg(u3phy->dev, "create debugfs root failed\n");
		return;
	}
	instance->dbg = root;
...
static int fpga_u3phy_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_dbg(dev, "Failed to parse ippc value\n");
		return ret;
	}
	u3phy->ippc_base = ioremap(ippc, SSUSB_IPPC_LEN);
	if (!u3phy->ippc_base) {
		dev_dbg(dev, "could not ioremap ippc regs\n");
		return -ENOMEM;
	}
	...
	for_each_child_of_node(np, child_np) {
		...
		instance->i2c_base = u3phy->ippc_base + 0xA8;

		instance->phy = phy;
		version = ssusb_read_phy_version(u3phy, instance);
		instance->chip_version = version;
		if (instance->chip_version == PHY_TEST_CHIP_A60810) {
			phy_writeb(instance->i2c_base, 0x60, 0xFF, 0x00);
			phy_writeb(instance->i2c_base, 0x60, 0x05, 0x55);
			phy_writeb(instance->i2c_base, 0x60, 0x18, 0x84);

			phy_writeb(instance->i2c_base, 0x60, 0xFF, 0x10);
			phy_writeb(instance->i2c_base, 0x60, 0x0A, 0x84);

			phy_writeb(instance->i2c_base, 0x60, 0xFF, 0x40);
			phy_writeb(instance->i2c_base, 0x60, 0x38, 0x46);
			phy_writeb(instance->i2c_base, 0x60, 0x42, 0x40);
			phy_writeb(instance->i2c_base, 0x60, 0x08, 0xAB);
			phy_writeb(instance->i2c_base, 0x60, 0x09, 0x0C);
			phy_writeb(instance->i2c_base, 0x60, 0x0C, 0x71);
			phy_writeb(instance->i2c_base, 0x60, 0x0E, 0x4F);
			phy_writeb(instance->i2c_base, 0x60, 0x10, 0xE1);
			phy_writeb(instance->i2c_base, 0x60, 0x14, 0x5F);

			phy_writeb(instance->i2c_base, 0x60, 0xFF, 0x60);
			phy_writeb(instance->i2c_base, 0x60, 0x14, 0x03);

			phy_writeb(instance->i2c_base, 0x60, 0xFF, 0x00);
			phy_writeb(instance->i2c_base, 0x60, 0x6A, 0x04);
			phy_writeb(instance->i2c_base, 0x60, 0x68, 0x08);
			phy_writeb(instance->i2c_base, 0x60, 0x6C, 0x26);
			phy_writeb(instance->i2c_base, 0x60, 0x6D, 0x36);
		}
		if (instance->chip_version) {
			...
			dev_dbg(dev, "ERROR: maybe phy DTB not connected or powered?\n");
		}
