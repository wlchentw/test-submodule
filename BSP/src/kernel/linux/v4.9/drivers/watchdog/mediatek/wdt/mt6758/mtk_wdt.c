...
void wdt_arch_reset(char mode)
{
	...
	if (mode & WD_SW_RESET_BYPASS_PWR_KEY) {
		/* Bypass power key reboot, We using auto_restart
...
int mtk_rgu_status_is_sysrst(void)
{
	...
	re = (__raw_readl(MTK_WDT_STATUS) & MTK_WDT_STATUS_SYSRST_RST) ? 1 : 0;
	return re;
}
	...
	if (mark_bit == MTK_WDT_REQ_MODE_SPM_SCPSYS) {
		...
		if (en == WD_REQ_EN) {
			if (ext_debugkey_io_eint != -1) {
				pr_notice("RGU ext_debugkey_io_eint is %d\n",
...
void mtk_wdt_set_c2k_sysrst(unsigned int flag, unsigned int shift)
{
	...
	if (!toprgu_base) {
		toprgu_base = of_iomap(np_rgu, 0);
		if (!toprgu_base)
			pr_notice("mtk_wdt_set_c2k_sysrst RGU iomap failed\n");
...
get_wd_api(&wd_api);
	...
	wd_api->wd_get_kick_bit(), wd_api->wd_get_check_bit(), wdt_mode_val);
	aee_wdt_dump_reg();
    #endif
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	pr_notice("******** MTK WDT driver probe!! ********\n");
#ifdef CONFIG_OF
	if (!toprgu_base) {
	...
	if (!wdt_irq_id) {
		...
		if (!wdt_irq_id) {
			pr_notice("RGU get IRQ ID failed\n");
			return -ENODEV;
		}
	}
	pr_debug("RGU base: 0x%p  RGU irq: %d\n", toprgu_base, wdt_irq_id);
	...
	if (ret != 0) {
		pr_notice("mtk_wdt_probe : failed to request irq (%d)\n", ret);
		return ret;
	}
	 ...
	 * value is 0x7FF.
	 * We set Interval[1:0] to different value to distinguish
	 * different stage.
	 * Enter pre-loader, we will set it to 0x0
	 * Enter u-boot, we will set it to 0x1
	 * Enter kernel, we will set it to 0x2
	 * And the default value is 0x3 which means reset from a power
	 * off and power on reset
	 */
...
static void spm_wdt_init(void)
{
	...
	/* pr_notice("WDT REQ_MODE=0x%x,  WDT REQ_EN=0x%x\n",*/
	...
	/* Bit 4: WDT_Auto_restart, this is a reserved bit, we use it as bypass
	 * powerkey flag.
	 */
	/* Because HW reboot always need reboot to kernel, we set it always. */
	tmp |= MTK_WDT_MODE_AUTO_RESTART;
...
static int __init mtk_wdt_init(void)
{
	...
	if (ret) {
		pr_notice("****[mtk_wdt_driver] Unable to device register(%d)\n",
			ret);
		return ret;
	}
#endif
	...
	if (ret) {
		pr_notice("****[mtk_wdt_driver] Unable to register driver (%d)\n",
			ret);
		return ret;
	}
