...
static int toprgu_reset_dev(struct reset_controller_dev *rcdev,
			      unsigned long id)
{
	int ret;
 ...
 */
	...
	if ((cmd != NULL) && (strcmp(cmd, "rpmbpk") == 0)) {
		iowrite32(ioread32(wdt_base + WDT_NONRST_REG2) | (1U << 0),
				wdt_base + WDT_NONRST_REG2);
	} else if ((cmd != NULL) && (strcmp(cmd, "recovery") == 0)) {
		iowrite32(ioread32(wdt_base + WDT_NONRST_REG2) | (1U << 1),
				wdt_base + WDT_NONRST_REG2);
		#ifdef CONFIG_MT6397_MISC
...
static int mtk_wdt_start(struct watchdog_device *wdt_dev)
{
	...
	reg &= ~(WDT_MODE_IRQ_EN | WDT_MODE_IRQ_LVL |
		WDT_MODE_EXT_POL_HIGH | WDT_MODE_DUAL_EN);
	#endif
...
static int mtk_wdt_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node,
				"registered-in-atf", &tmp);
	pr_info("watchdog interrupt registered in atf: %d\n", tmp);
	if (tmp == 0) {
		mtk_wdt->wdt_irq_id =
			irq_of_parse_and_map(pdev->dev.of_node, 0);
		if (mtk_wdt->wdt_irq_id == 0U) {
			pr_err("RGU get IRQ ID failed\n");
			return -ENODEV;
		}

		#ifndef CONFIG_FIQ_GLUE
		err = request_irq(mtk_wdt->wdt_irq_id,
			mtk_wdt_isr, IRQF_TRIGGER_NONE,
			 DRV_NAME, mtk_wdt);
		#else
		...
		#endif
		if (err != 0) {
			pr_err("watchdog failed to request irq (%d)\n", err);
			return err;
		}
	}

	toprgu_base = mtk_wdt->wdt_base;
	wdt_device = &mtk_wdt->wdt_dev;
...
static int wk_proc_cmd_read(struct seq_file *s, void *v)
{
	...
	if (0U == (ioread32(toprgu_base + WDT_MODE) & WDT_MODE_EN))
		enabled = 0;

	seq_printf(s, "enabled timeout\n%-4d %-8d\n",
			enabled, wdt_device->timeout);
...
static ssize_t wk_proc_cmd_write(struct file *file,
				const char *buf, size_t count, loff_t *data)
{
	...
	if (timeout > 20U && timeout <= WDT_MAX_TIMEOUT) {
		wdt_device->timeout = timeout;
		ret = mtk_wdt_set_timeout(wdt_device, wdt_device->timeout);
		if (ret < 0)
			return ret;
	} else {
	...
	if (enable == 1) {
		ret = mtk_wdt_start(wdt_device);
		if (ret < 0)
			return ret;
		set_bit(WDOG_ACTIVE, &wdt_device->status);
		pr_err("[WDK] enable wdt\n");
	} else if (enable == 0) {
		ret = mtk_wdt_stop(wdt_device);
		if (ret < 0)
			return ret;
		clear_bit(WDOG_ACTIVE, &wdt_device->status);
...
{
	struct proc_dir_entry *de = proc_create("wdk",
					0660, NULL, &wk_proc_cmd_fops);
