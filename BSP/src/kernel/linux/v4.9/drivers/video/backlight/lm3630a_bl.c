...
//static int lm3630a_fl_en_gpio = -1 ;

extern NTX_FW_LM3630FL_dualcolor_hdr *gptLm3630fl_dualcolor_tab_hdr; 
extern NTX_FW_LM3630FL_dualcolor_hdr *gptLm3630fl_dualcolor_x2_tab_hdr; 
extern NTX_FW_LM3630FL_dualcolor_percent_tab *gptLm3630fl_dualcolor_percent_tab; 
extern NTX_FW_LM3630FL_dualcolor_percent_tab *gptLm3630fl_dualcolor_set1_percent_tab; 
extern NTX_FW_LM3630FL_dualcolor_percent_tab *gptLm3630fl_dualcolor_set2_percent_tab; 
...
static int lm3630a_write_ex(struct lm3630a_chip *pchip,
			 unsigned int reg, unsigned int data,int iTimeout_ms)
{
	int iRet,iTryCnt=0;
	unsigned long ulTimeout_jiffies = jiffies+msecs_to_jiffies(iTimeout_ms);
	do {
		iTryCnt++;
		iRet = regmap_write(pchip->regmap, reg, data);
		if(iRet>=0) {
			break;
		}
		if(time_is_before_eq_jiffies(ulTimeout_jiffies)) {
			printk(KERN_ERR"%s(),retry %d times in %d ms timeout !!\n",__func__,iTryCnt,iTimeout_ms);
			break;
		}
		udelay(100);
		if(1==iTryCnt) {
			printk(KERN_WARNING"%s(),reg0x%x failed ! retrying in %d ms\n",
__func__,reg,iTimeout_ms);
		}
	} while(1);
	return iRet;
}
...
{
	return lm3630a_write_ex(pchip, reg, data,DEFAULT_TIMEOUT_MS);
}

static int lm3630a_update_ex(struct lm3630a_chip *pchip,
			  unsigned int reg, unsigned int mask,
			  unsigned int data,int iTimeout_ms)
{
	int iRet,iTryCnt=0;
	unsigned long ulTimeout_jiffies = jiffies+msecs_to_jiffies(iTimeout_ms);
	do {
		iTryCnt++;
		iRet = regmap_update_bits(pchip->regmap, reg, mask, data);
		if(iRet>=0) {
			break;
		}
		if(time_is_before_eq_jiffies(ulTimeout_jiffies)) {
...
static int lm3630a_update(struct lm3630a_chip *pchip,
			  ...
			  unsigned int data)
{
	return lm3630a_update_ex(pchip,reg,mask,data,DEFAULT_TIMEOUT_MS);
}
...
static int lm3630a_chips_init(void)
{
	int i;
	for(i=0;i<gilm3630a_chips;i++) {
		lm3630a_chip_init(gpchip[i]);
	}
	return 0;
}
...
static int lm3630a_bank_a_update_status(struct backlight_device *bl)
{
	...
	else if (giSuspendingState && (1!=fl_en_status)) {
		fl_en_status = 1;
		if(gpio_is_valid(pchip->hwen_gpio)) {
			gpio_direction_output(pchip->hwen_gpio, 1);
			msleep (50);
...
int lm3630a_get_FL_current(void)
{
	...
	case 5:// 4 color FL .
	{
		if(gptLm3630fl_dualcolor_set1_percent_tab != NULL && gptLm3630fl_dualcolor_set2_percent_tab !=NULL)
		{
			int iColor=gpchip[0]->frontlight_table;
			int iPercentIdx=gpchip[0]->bled->props.brightness-1;

			if(gptLm3630fl_dualcolor_set1_percent_tab) {
				if(gpchip[0]->bled->props.brightness>0) {
					iRet = (int)gptLm3630fl_dualcolor_set1_percent_tab[iColor].dwCurrentA[iPercentIdx];
					printk("set1:dualcolor[%d][%d] curr=%d\n",iColor,iPercentIdx+1,iRet);
				}
				else {
					iRet = 0;
				}
			}
			if(gptLm3630fl_dualcolor_set2_percent_tab) {
				if(gpchip[0]->bled->props.brightness>0) {
					iRet = (int)gptLm3630fl_dualcolor_set2_percent_tab[iColor].dwCurrentA[iPercentIdx];
					printk("set2: dualcolor[%d][%d] curr=%d\n",iColor,iPercentIdx+1,iRet);
				}
				else {
					iRet = 0;
				}
		...
		else 
		{
			...
			dwCurrent=0;
			lm3630a_get_FL_RGBW_RicohCurrTab(&dwRGBW_CurrTabItems,(void **)&ptRGBW_CurrTab);

			if(ptRGBW_CurrTab) {
				// binary search the RGBW current .
				int iFirst,iLast,iMiddle=0,iEnd,iTmp;
	...
	case 6:// 1 color FL @ channel 0(A).
		if(gpchip[0]->bleda->props.brightness) {
			...
			// mtk compile error , so disable it
			//iRet;
		}
		break;
		...
		unsigned char led_B_current, unsigned char led_B_brightness)
{
	...
	//if (gpio_is_valid(pchip->hwen_gpio)) 
	{
		if ((led_A_current | led_B_current) && (1!=fl_en_status)) {
		...
		else if (!led_A_current && !led_B_current) {
			if (gpio_is_valid(pchip->hwen_gpio)) {
				if(0!=fl_en_status) {
					printk("%s():FL PWR[OFF] \n",__FUNCTION__);
					gpio_direction_output(pchip->hwen_gpio,0);
					...
					printk("%s():skipped set brightness 0,FL PWR OFF!\n",__FUNCTION__);
				}
				return ;
			}
		}
...
// dual color fl percentage control .
static int _fl_lm3630_set1_percentage (struct lm3630a_chip *pchip,int iFL_Percentage)
{
	int iFL_temp = pchip->frontlight_table;
	...
	else if(gptLm3630fl_dualcolor_set1_percent_tab) {
		unsigned char led_A_current=0,led_B_current=0;
		
		if ((int)gptLm3630fl_dualcolor_x2_tab_hdr->dwTotalColors <= iFL_temp) {
			printk ("[%s-%d] Front light color index %d >= %d\n", __func__,__LINE__, 
					(int)gptLm3630fl_dualcolor_x2_tab_hdr->dwTotalColors,iFL_temp);
			return -1;
		}
		
		if(gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC1_CurrentA[iFL_Percentage-1]!=0) {
			led_A_current = gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC1_CurrentA[iFL_Percentage-1];
		}
		else if(gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bDefaultC1_Current!=0) {
			led_A_current = gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bDefaultC1_Current;
		}
		else if(gptLm3630fl_dualcolor_set1_percent_tab->bDefaultC1_Current!=0) {
			led_A_current = gptLm3630fl_dualcolor_set1_percent_tab->bDefaultC1_Current;
		}

		if(gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC2_CurrentA[iFL_Percentage-1]!=0) {
			led_B_current = gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC2_CurrentA[iFL_Percentage-1];
		}
		else if(gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bDefaultC2_Current!=0) {
			led_B_current = gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bDefaultC2_Current;
		}
		else if(gptLm3630fl_dualcolor_set1_percent_tab->bDefaultC2_Current!=0) {
			led_B_current = gptLm3630fl_dualcolor_set1_percent_tab->bDefaultC2_Current;
		}

		_lm3630a_set_FL (pchip ,
			led_A_current, 
			gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC1_BrightnessA[iFL_Percentage-1],
			led_B_current,
			gptLm3630fl_dualcolor_set1_percent_tab[iFL_temp].bC2_BrightnessA[iFL_Percentage-1]);
	}
	else {
		printk (KERN_ERR"[%s-%d] Front light table not exist !!\n", __func__,__LINE__);
		return -1;
...
static int _fl_lm3630_set2_percentage (struct lm3630a_chip *pchip,int iFL_Percentage)
{
	int iFL_temp = pchip->frontlight_table;
	...
	if (0 == iFL_Percentage) {
		...
		if(gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC1_CurrentA[iFL_Percentage-1]!=0) {
			led_A_current = gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC1_CurrentA[iFL_Percentage-1];
		}
		else if(gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bDefaultC1_Current!=0) {
			led_A_current = gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bDefaultC1_Current;
		}
		else if(gptLm3630fl_dualcolor_set2_percent_tab->bDefaultC1_Current!=0) {
			led_A_current = gptLm3630fl_dualcolor_set2_percent_tab->bDefaultC1_Current;
		}

		if(gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC2_CurrentA[iFL_Percentage-1]!=0) {
			led_B_current = gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC2_CurrentA[iFL_Percentage-1];
		}
		else if(gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bDefaultC2_Current!=0) {
			led_B_current = gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bDefaultC2_Current;
		}
		else if(gptLm3630fl_dualcolor_set2_percent_tab->bDefaultC2_Current!=0) {
			led_B_current = gptLm3630fl_dualcolor_set2_percent_tab->bDefaultC2_Current;
		}

		_lm3630a_set_FL (pchip ,
			led_A_current, 
			gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC1_BrightnessA[iFL_Percentage-1],
			led_B_current,
			gptLm3630fl_dualcolor_set2_percent_tab[iFL_temp].bC2_BrightnessA[iFL_Percentage-1]);
	}
	else {
		printk (KERN_ERR"[%s-%d] Front light table not exist !!\n", __func__,__LINE__);
		return -1;
...
int fl_lm3630a_percentageEx (int iChipIdx,int iFL_Percentage)
{
	if(gilm3630a_chips>iChipIdx) {
		if(5==gptHWCFG->m_val.bFL_PWM||6==gptHWCFG->m_val.bFL_PWM||7==gptHWCFG->m_val.bFL_PWM) {
			...
			else if(gptLm3630fl_dualcolor_x2_tab_hdr) {
				if(5==gptHWCFG->m_val.bFL_PWM){
					if(iChipIdx==0)
						_fl_lm3630_set1_percentage(gpchip[iChipIdx],iFL_Percentage);
					if(iChipIdx==1)
						_fl_lm3630_set2_percentage(gpchip[iChipIdx],iFL_Percentage);
				}
			}
			else {
...
static void lm3630a_bl_parse_dt(struct lm3630a_chip *pchip)
{
	...
	pchip->hwen_gpio = of_get_named_gpio(lm3630a_bl_pmic_np,"flen",0);
	if (!gpio_is_valid(pchip->hwen_gpio)) {
		printk("invalid gpio: %d\n",  pchip->hwen_gpio);
	}
	printk ("%s pchip=%p,flen_gpio %d\n",__func__,pchip,pchip->hwen_gpio);

	pchip->pwr_on_gpio = of_get_named_gpio(lm3630a_bl_pmic_np,"flpwron",0);
	if (!gpio_is_valid(pchip->pwr_on_gpio)) {
		printk("invalid gpio: %d\n",  pchip->pwr_on_gpio);
	}
	printk ("%s pchip=%p,pwr_on_gpio %d \n",__func__,pchip,pchip->pwr_on_gpio);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (gpio_request(pchip->hwen_gpio, "lm3630a_bl_hwen") < 0) {
		...
		if (gpio_is_valid(pchip->pwr_on_gpio)) {
			gpio_direction_output(pchip->pwr_on_gpio,1);
		}
		gpio_direction_output(pchip->hwen_gpio,1);
		fl_en_status = 1;
		msleep (50);
	...
	rval = lm3630a_read(pchip,0x1f); // read revision register .
	if(rval<0) {
		dev_err(&client->dev, "chip not exist !\n");
		rval = -ENXIO;
		goto err_exit;
	}
	else {
		pchip->bChipRev = (unsigned char)rval;
	}
	...
	dev_info(&client->dev, "LM3630A[%d] chip rev=0x%02x.\n",gilm3630a_chips,pchip->bChipRev);
	return 0;
...
err_exit:
	...
	if(pdata) {
		devm_kfree(&client->dev,pdata);pdata=0;
	}
	if(pchip) {
		devm_kfree(&client->dev,pchip);pchip=0;
	}
	return rval;
}
