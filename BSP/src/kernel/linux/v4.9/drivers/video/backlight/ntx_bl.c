...
#include <linux/backlight.h>
	...
	#include "../../../arch/arm/mach-sunxi/ntx_firmware_parser.h"
#else //][!CONFIG_ARCH_SUN8IW15P1
	#ifdef CONFIG_ARCH_MTK_PROJECT
		#include "../../../arch/arm/mach-mediatek/ntx_hwconfig.h"
		#include "../../../arch/arm/mach-mediatek/ntx_firmware.h"
		#include "../../../arch/arm/mach-mediatek/ntx_firmware_parser.h"
...
extern int fl_lm36922_x2_percentage(int iFL_Percentage);
...
static ssize_t fw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (0==gptNTXFW) {
		...
		len = sprintf(buf+tot_len,"size=%ld\n",gdwNTXFW_size);tot_len += len;
...
int ntx_set_fl_percent(int percent)
{
	...
	if(gptHWCFG && gptHWCFG->m_val.bFrontLight) 
	{
		...
		else if (0x0a == gptHWCFG->m_val.bFL_PWM){ // LM3922*2
			fl_lm36922_x2_percentage(percent);
