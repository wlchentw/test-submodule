...
void mtk_ecc_disable(struct mtk_ecc *ecc)
{
	...
	if (op == ECC_DECODE)
		 ...
		 * decode IRQ.
		 */
		readw(ECC_REG(ecc, ECC_DECIRQ_STA));
...
int mtk_ecc_wait_done(struct mtk_ecc *ecc, enum mtk_ecc_operation op)
{
	...
	if (op == ECC_DECODE) {
		ret = readl_poll_timeout_atomic(ECC_REG(ecc, ECC_DECDONE), dec,
					dec & ecc->sectors, 1, ECC_TIMEOUT);
		/*read clear irq stat*/
		readw(ECC_REG(ecc, ECC_DECIRQ_STA));
		ecc->sectors = 0;
	} else {
		ret = readl_poll_timeout_atomic(ECC_REG(ecc, ECC_ENCIRQ_STA),
			enc, enc, 1, ECC_TIMEOUT);
	}
...
static const struct mtk_ecc_comp ecc_mt8518 = {
	.ecc_regs = mt2712_ecc_regs,
	.type = MTK_ECC_MT8518,
	.mode_shift = 5,
	.max_ecc_str = 24,
