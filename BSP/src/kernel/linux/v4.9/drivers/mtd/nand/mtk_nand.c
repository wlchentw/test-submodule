					...
					bool check_only)
{
	...
	dev_info(nfc->dev, "nand access timing control: 0x%x\n",
...
static void mt8518_nfc_adjust_ecc(struct mtd_info *mtd)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	u32 spare;
...
#else
	ret = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_INTR_STA), reg,
		reg & INTR_AHB_DONE_EN, 10, MTK_TIMEOUT);
#endif
	if (ret) {
...
#endif
	rc = readl_poll_timeout_atomic(NFC_REG(nfc, NFI_INTR_STA), reg,
		reg & INTR_AHB_DONE_EN, 10, MTK_TIMEOUT);
	if (rc)
		dev_warn(nfc->dev, "read ahb/dma done timeout\n");
...
static int mtk_nfc_enable_clk(struct device *dev, struct mtk_nfc_clk *clk)
{
	...
	ret = clk_prepare_enable(clk->nfi_top_clk);
	if (ret) {
		dev_info(dev, "failed to enable top clk\n");
		clk_disable_unprepare(clk->nfi_clk);
...
static void mtk_nfc_disable_clk(struct mtk_nfc_clk *clk)
{
	clk_disable_unprepare(clk->nfi_clk);
	clk_disable_unprepare(clk->pad_clk);
	clk_disable_unprepare(clk->nfi_top_clk);
}
...
static const struct mtk_nand_type nfc_mt8518 = {
	.nfi_regs = mt2712_nfi_regs,
	.type = MTK_NAND_MT8518,
	.fdm_ecc_size = 1,
	.parity_bit = 14,
	.max_sector = 16,
	.max_sector_spare = 16,
	...
	.PAGEFMT_SPARE_64 = 0x10,
	.PAGEFMT_SPARE_SHIFT = 16,
	.nfc_adjust_ecc = mt8518_nfc_adjust_ecc,
	.nfi_clk_div = 2,
};
...
static int mtk_nfc_probe(struct platform_device *pdev)
{
	...
	nfc->clk.nfi_top_clk = devm_clk_get(dev, "nfi_top_clk");
	if (IS_ERR(nfc->clk.nfi_top_clk)) {
		dev_info(dev, "no top clk\n");
		ret = PTR_ERR(nfc->clk.nfi_top_clk);
		goto release_ecc;
	}

	nfc->clk.nfi_clk_sel = devm_clk_get(dev, "nfi_clk_sel");
	if (IS_ERR(nfc->clk.nfi_clk_sel)) {
		ret = PTR_ERR(nfc->clk.nfi_clk_sel);
		goto release_ecc;
	}

	nfc->clk.nfi_clk_parent = devm_clk_get(dev, "nfi_clk_parent");
	if (IS_ERR(nfc->clk.nfi_clk_parent)) {
		ret = PTR_ERR(nfc->clk.nfi_clk_parent);
		goto release_ecc;
	}
...
#ifdef NAND_USE_IRQ /*don't useit, due to nfi's irq not friendly*/
...
static int nfc_runtime_suspend(struct device *dev)
{
...
static int nfc_runtime_resume(struct device *dev)
{
	struct mtk_nfc *nfc = dev_get_drvdata(dev);
	struct mtk_nfc_nand_chip *chip;
	struct nand_chip *nand;
...
static const struct dev_pm_ops nfc_dev_pm_ops = {
	...
	SET_RUNTIME_PM_OPS(nfc_runtime_suspend, nfc_runtime_resume, NULL)
};
#endif
