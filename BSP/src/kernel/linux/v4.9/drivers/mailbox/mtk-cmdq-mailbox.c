				    ...
				    struct cmdq_thread *thread)
{
	...
	if (err < 0)
		cmdq_err("pc:0x%08x end:0x%08x err:%d index:%d",
			curr_pa, task_end_pa, err, thread->idx);

	cmdq_log("task status %p~%p err:%d index:%d",
		(void *)(unsigned long)curr_pa,
		(void *)(unsigned long)task_end_pa, err, thread->idx);
