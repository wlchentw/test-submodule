 ...
 * Author: gtk_yangyang <gtk_yangyang@mediatek.com>
 *
...
static int alloc_rx_skbs(star_dev *star_dev)
{
	...
	do {
		...
		STAR_PR_DEBUG("desc idx(%d) for skb(%p)\n", retval, skb);
		if (retval < 0) {
...
static int star_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8518-ethernet");
	if (!np) {
