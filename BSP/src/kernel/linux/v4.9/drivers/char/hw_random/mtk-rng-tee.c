...
static int mtk_rng_read(struct hwrng *rng, void *buf, size_t max, bool wait)
{
	...
	if (ret != TZ_RESULT_SUCCESS) {
		dev_info((struct device *)priv->rng.priv,
			"Error: KREE_CreateSession failed(0x%x)\n", ret);
		return ret;
	}
	...
	for (i = 0; i < loop; i++) { /* once read 4 TRNG_DATA */
		...
		if (ret != TZ_RESULT_SUCCESS) {
			dev_info((struct device *)priv->rng.priv,
			"Error: KREE_TeeServiceCall failed(0x%x)\n", ret);
			return ret;
		}

		/* for example: max is 14, remain_rng_len is 3 */
		cpy_len = min(4, remain_rng_len);
		for (j = 0; j < cpy_len; j++)
			*(u32 *)(buf + j * sizeof(u32)) = param[j].value.a;

		dev_info((struct device *)priv->rng.priv,
			"DEBUG: rng_mtee_read:[0]0x%x, [1]0x%x, [2]0x%x, [3]0x%x\n",
	...
	if (ret != TZ_RESULT_SUCCESS) {
		dev_info((struct device *)priv->rng.priv,
			"Error: KREE_CloseSession failed(0x%x)\n", ret);
		return ret;
	}
...
static int mtk_rng_probe(struct platform_device *pdev)
{
	...
	/* per mill: set to 100 to dominate 10% of /dev/random */
	priv->rng.quality = 100;

	ret = devm_hwrng_register(&pdev->dev, &priv->rng);
	if (ret) {
		dev_info(&pdev->dev, "failed to register rng device: %d\n",
			ret);
		return ret;
	}
