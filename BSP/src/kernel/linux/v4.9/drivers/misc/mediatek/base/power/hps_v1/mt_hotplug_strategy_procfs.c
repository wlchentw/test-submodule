...
static ssize_t hps_proc_uint_write_with_lock(struct file *file,
					     ...
					     size_t count, loff_t *pos)
{
	return hps_proc_uint_write(file, buffer, count, pos, lock_hps_ctxt,
				   unlock_hps_ctxt);
}
...
#define PROC_FOPS_RW_UINT(name, var, _write)                                   \
	static int hps_##name##_proc_open(struct inode *inode,                 \
					  struct file *file)                   \
	{                                                                      \
		return single_open(file, hps_proc_uint_show, &(var));          \
	}                                                                      \
	RPOC_FOPS_OPEN_WRITE(hps_##name##_proc_fops, hps_##name##_proc_open,   \
			     _write)
...
#define PROC_ENTRY(name)                                                       \
	{                                                                      \
		__stringify(name), &hps_##name##_proc_fops                     \
	}
...
PROC_FOPS_RW_UINT(log_mask, hps_ctxt.log_mask, hps_proc_uint_write_with_lock);
...
PROC_FOPS_RW_UINT(rush_boost_times, hps_ctxt.rush_boost_times,
		  hps_proc_uint_write_with_lock_reset);
PROC_FOPS_RW_UINT(quick_landing_enabled, hps_ctxt.quick_landing_enabled,
		  hps_proc_uint_write_with_lock);
...
static ssize_t hps_num_base_proc_write(unsigned int *pnbl, unsigned int *pnbb,
				       struct file *file,
				       ...
				       loff_t *pos)
{
	...
	if (hps_ctxt.is_hmp && (sscanf(desc, "%u %u", &nbl, &nbb) == 2)) {
		...
		*pnbb = nbb;
		...
		return count;
	} else if (!hps_ctxt.is_hmp && !kstrtouint(desc, 0, &nbl)) {
		if (nbl > num_possible_little_cpus() || nbl < 1) {
...
{
	return hps_num_base_proc_show(hps_ctxt.little_num_base_perf_serv,
				      hps_ctxt.big_num_base_perf_serv, m, v);
}
...
static ssize_t hps_num_base_perf_serv_proc_write(struct file *file,
						 ...
						 size_t count, loff_t *pos)
{
	return hps_num_base_proc_write(&hps_ctxt.little_num_base_perf_serv,
				       &hps_ctxt.big_num_base_perf_serv, file,
				       buffer, count, pos);
}

static int hps_num_base_custom1_proc_show(struct seq_file *m, void *v)
{
	return hps_num_base_proc_show(hps_ctxt.little_num_base_custom1,
				      hps_ctxt.big_num_base_custom1, m, v);
}

static ssize_t hps_num_base_custom1_proc_write(struct file *file,
					       const char __user *buffer,
					       size_t count, loff_t *pos)
{
	return hps_num_base_proc_write(&hps_ctxt.little_num_base_custom1,
				       &hps_ctxt.big_num_base_custom1, file,
				       buffer, count, pos);
}

static int hps_num_base_custom2_proc_show(struct seq_file *m, void *v)
{
	return hps_num_base_proc_show(hps_ctxt.little_num_base_custom2,
				      hps_ctxt.big_num_base_custom2, m, v);
}

static ssize_t hps_num_base_custom2_proc_write(struct file *file,
					       const char __user *buffer,
					       size_t count, loff_t *pos)
{
	return hps_num_base_proc_write(&hps_ctxt.little_num_base_custom2,
				       &hps_ctxt.big_num_base_custom2, file,
				       buffer, count, pos);
}
...
static ssize_t hps_num_limit_proc_write(unsigned int *pnll, unsigned int *pnlb,
					struct file *file,
					const char __user *buffer, size_t count,
					loff_t *pos)
{
	...
	if (hps_ctxt.is_hmp && (sscanf(desc, "%u %u", &nll, &nlb) == 2)) {
		...
		*pnlb = nlb;
		...
		return count;
	} else if (!hps_ctxt.is_hmp && !kstrtouint(desc, 0, &nll)) {
		if (nll > num_possible_little_cpus() || nll < 1) {
			hps_warn("%s(): bad argument(%u)\n", __func__, nll);
			return -EINVAL;
		}
...
{
	return hps_num_limit_proc_show(hps_ctxt.little_num_limit_thermal,
				       hps_ctxt.big_num_limit_thermal, m, v);
}
...
static ssize_t hps_num_limit_thermal_proc_write(struct file *file,
						...
						size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(&hps_ctxt.little_num_limit_thermal,
					&hps_ctxt.big_num_limit_thermal, file,
					buffer, count, pos);
}
...
{
	return hps_num_limit_proc_show(hps_ctxt.little_num_limit_low_battery,
				       hps_ctxt.big_num_limit_low_battery, m,
				       v);
}
...
static ssize_t hps_num_limit_low_battery_proc_write(struct file *file,
						    ...
						    size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(&hps_ctxt.little_num_limit_low_battery,
						      ...
						      void *v)
{
	return hps_num_limit_proc_show(
		...
		hps_ctxt.big_num_limit_ultra_power_saving, m, v);
}
	...
	struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(
		&hps_ctxt.little_num_limit_ultra_power_saving,
		&hps_ctxt.big_num_limit_ultra_power_saving, file, buffer, count,
		pos);
}
...
{
	return hps_num_limit_proc_show(hps_ctxt.little_num_limit_power_serv,
				       hps_ctxt.big_num_limit_power_serv, m, v);
}
...
static ssize_t hps_num_limit_power_serv_proc_write(struct file *file,
						   ...
						   size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(&hps_ctxt.little_num_limit_power_serv,
...
{
	return hps_num_limit_proc_show(hps_ctxt.little_num_limit_custom1,
				       hps_ctxt.big_num_limit_custom1, m, v);
}

static ssize_t hps_num_limit_custom1_proc_write(struct file *file,
						const char __user *buffer,
						size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(&hps_ctxt.little_num_limit_custom1,
					&hps_ctxt.big_num_limit_custom1, file,
					buffer, count, pos);
}

static int hps_num_limit_custom2_proc_show(struct seq_file *m, void *v)
{
	return hps_num_limit_proc_show(hps_ctxt.little_num_limit_custom2,
				       hps_ctxt.big_num_limit_custom2, m, v);
}

static ssize_t hps_num_limit_custom2_proc_write(struct file *file,
						const char __user *buffer,
						size_t count, loff_t *pos)
{
	return hps_num_limit_proc_write(&hps_ctxt.little_num_limit_custom2,
					&hps_ctxt.big_num_limit_custom2, file,
					buffer, count, pos);
}
...
int hps_procfs_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create(entries[i].name, 0664, hps_dir,
				 entries[i].fops))
