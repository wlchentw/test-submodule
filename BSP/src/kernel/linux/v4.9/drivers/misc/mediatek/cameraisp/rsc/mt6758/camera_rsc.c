 ...
 */
#define RSC_WR32(addr, data)                                                   \
	mt_reg_sync_writel(                                                    \
		data, addr) /* For 89 Only.   // NEED_TUNING_BY_PROJECT */
...
do {                                                                          \
	...
	if (avaLen > 1) {                                                     \
		...
		if (pSrc->_cnt[ppb][logT] != 0) {                             \
			if (logT == _LOG_DBG) {                               \
				for (logi = 0; logi < DBG_PAGE;               \
				     logi++) {                                \
					if (ptr[NORMAL_STR_LEN *              \
							(logi + 1) -          \
						1] != '\0') {                 \
						ptr[NORMAL_STR_LEN *          \
							    (logi +           \
...
#else
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...)                               \
	xlog_printk(ANDROID_LOG_DEBUG, "KEEPER", "[%s] " fmt, __func__,        \
		    ##__VA_ARGS__)
#endif
...
static bool ConfigRSCRequest(signed int ReqIdx)
{
	...
	if (g_RSC_ReqRing.RSCReq_Struct[ReqIdx].State ==
		...
		LOG_ERR("ConfigRSCRequest FSM error!,ReqId:%d,Stat:%d\n",
...
static bool ConfigRSC(void)
{
	...
	for (k = 0; k < _SUPPORT_MAX_RSC_REQUEST_RING_SIZE_; k++) {
		...
		if (j != _SUPPORT_MAX_RSC_FRAME_REQUEST_) {
			LOG_ERR("CfgEr idj(%d),HWProc(%d),Stat(%d)\n",
	...
	for (k = 0; k < _SUPPORT_MAX_RSC_REQUEST_RING_SIZE_; k++) {
		...
		if (g_RSC_ReqRing.RSCReq_Struct[i].State ==
			...
			/*else {*/
			LOG_ERR("Cfg State Err! HWProc(%d),State(%d)\n",
...
static bool UpdateRSC(pid_t *ProcessID)
{
	...
	for (i = g_RSC_ReqRing.HWProcessIdx;
		...
		if (g_RSC_ReqRing.RSCReq_Struct[i].State ==
			...
			/*else {*/
			IRQ_LOG_KEEPER(
				RSC_IRQ_TYPE_INT_RSC_ST, m_CurrentPPB, _LOG_ERR,
				"FSM Err HWProc(%d), Stat(%d)\n",
				g_RSC_ReqRing.HWProcessIdx,
				g_RSC_ReqRing.RSCReq_Struct[i].State);
			g_RSC_ReqRing.RSCReq_Struct[i].State =
				RSC_REQUEST_STATE_FINISHED;
...
#if !BYPASS_REG
{
	...
	cmdqRecWrite(handle, RSC_INT_CTL_HW, 0x1,
		     CMDQ_REG_MASK); /* RSC Interrupt read-clear mode */
	...
	cmdqRecWrite(handle, RSC_MVI_STRIDE_HW, pRscConfig->RSC_MVI_STRIDE,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, RSC_MVO_BASE_ADDR_HW,
		     pRscConfig->RSC_MVO_BASE_ADDR, CMDQ_REG_MASK);
	cmdqRecWrite(handle, RSC_MVO_STRIDE_HW, pRscConfig->RSC_MVO_STRIDE,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, RSC_BVO_BASE_ADDR_HW,
		     pRscConfig->RSC_BVO_BASE_ADDR, CMDQ_REG_MASK);
	cmdqRecWrite(handle, RSC_BVO_STRIDE_HW, pRscConfig->RSC_BVO_STRIDE,
		     CMDQ_REG_MASK);
#ifdef RSC_TUNABLE
	cmdqRecWrite(handle, RSC_MV_OFFSET_HW, pRscConfig->RSC_MV_OFFSET,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, RSC_GMV_OFFSET_HW, pRscConfig->RSC_GMV_OFFSET,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, RSC_CAND_NUM_HW, pRscConfig->RSC_CAND_NUM,
		     CMDQ_REG_MASK);
	...
	cmdqRecWrite(handle, RSC_SAD_CTRL_HW, pRscConfig->RSC_SAD_CTRL,
		     CMDQ_REG_MASK);
...
#endif
	cmdqRecWrite(handle, RSC_START_HW, 0x1,
		     CMDQ_REG_MASK); /* RSC Interrupt read-clear mode */

	cmdqRecWait(handle, CMDQ_EVENT_RSC_EOF);
	cmdqRecWrite(handle, RSC_START_HW, 0x0,
		     CMDQ_REG_MASK); /* RSC Interrupt read-clear mode */

	/* non-blocking API, Please  use cmdqRecFlushAsync() */
	cmdqRecFlushAsync(handle);
	cmdqRecReset(
		handle); /* if you want to re-use the handle, please reset the
			  * handle
			  */
	...
	RSC_WR32(RSC_INT_CTL_REG,
		 0x1); /* RSC Interrupt enabled in read-clear mode */
...
#else
{
	...
	for (i = 0; i < _SUPPORT_MAX_RSC_REQUEST_RING_SIZE_; i++) {
		LOG_INF("Stt:%d,pID:0x%0x,cID:0x%0x,enqReq:%d,FrmW:%d,Rrm:%d\n",
			g_RSC_ReqRing.RSCReq_Struct[i].State,
		...
		for (j = 0; j < _SUPPORT_MAX_RSC_FRAME_REQUEST_;) {
			LOG_INF("Frm[%d]:%d,Frm[%d]:%d,Frm[%d]:%d,Frm[%d]:%d\n",
				j, g_RSC_ReqRing.RSCReq_Struct[i]
					   .RscFrameStatus[j],
				j + 1, g_RSC_ReqRing.RSCReq_Struct[i]
					       .RscFrameStatus[j + 1],
				j + 2, g_RSC_ReqRing.RSCReq_Struct[i]
					       .RscFrameStatus[j + 2],
				j + 3, g_RSC_ReqRing.RSCReq_Struct[i]
					       .RscFrameStatus[j + 3]);
			j = j + 4;
		}
	}
...
{
	int ret;
/* must keep this clk open order: CG_SCP_SYS_MM0-> CG_MM_SMI_COMMON ->
 * CG_SCP_SYS_ISP -> RSC clk
 */
#ifndef SMI_CLK
	ret = clk_prepare_enable(rsc_clk.CG_SCP_SYS_MM0);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_SCP_SYS_MM0\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_2X);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_2X\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_GALS_M0_2X);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_GALS_M0_2X\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_GALS_M1_2X);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_GALS_M1_2X\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_UPSZ0);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_UPSZ0\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_UPSZ1);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_UPSZ1\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_FIFO0);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_FIFO0\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_SMI_COMMON_FIFO1);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_SMI_COMMON_FIFO1\n");

	ret = clk_prepare_enable(rsc_clk.CG_MM_LARB5);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_MM_LARB5\n");

	ret = clk_prepare_enable(rsc_clk.CG_SCP_SYS_ISP);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_SCP_SYS_ISP\n");

	ret = clk_prepare_enable(rsc_clk.CG_IMGSYS_LARB);
	if (ret)
		LOG_ERR("cannot prepare_enable CG_IMGSYS_LARB\n");
#else

#ifdef CONFIG_MTK_SMI_EXT
	smi_bus_prepare_enable(SMI_LARB5_REG_INDX, "camera_rsc", true);
#endif
#endif
	ret = clk_prepare_enable(rsc_clk.CG_IMGSYS_RSC);
	if (ret)
		LOG_ERR("cannot  prepare_enable CG_IMGSYS_RSC clock\n");
}

static inline void RSC_Disable_Unprepare_ccf_clock(void)
{
	/* must keep this clk close order: RSC clk -> CG_SCP_SYS_ISP ->
...
#else
#ifdef CONFIG_MTK_SMI_EXT
	smi_bus_disable_unprepare(SMI_LARB5_REG_INDX, "camera_rsc", true);
#endif
#endif
}
#endif
				   ...
				   unsigned int Count)
{
	...
	/* Use local variable to store RSCInfo.DebugMask & RSC_DBG_WRITE_REG for
	 * saving lock time
	 */
	spin_lock(&(RSCInfo.SpinLockRSC));
...
static signed int RSC_WriteReg(struct RSC_REG_IO_STRUCT *pRegIo)
{
	 ...
	 * RSC_REG_STRUCT), GFP_ATOMIC);
	 */
	pData = kmalloc((pRegIo->Count) * sizeof(struct RSC_REG_STRUCT),
			GFP_ATOMIC);
	if (pData == NULL) {
		LOG_DBG("kmalloc failed, (process, pid, tgid)=(%s, %d, %d)\n",
	...
	if (copy_from_user(pData, (void __user *)(pRegIo->pData),
			   pRegIo->Count * sizeof(struct RSC_REG_STRUCT)) !=
	    0) {
...
static signed int RSC_WaitIrq(struct RSC_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if ((RSCInfo.DebugMask & RSC_DBG_INT) &&
		(WaitIrq->Status & RSCInfo.IrqInfo.Mask[WaitIrq->Type]) &&
		(WaitIrq->UserKey > 0)) {
		LOG_DBG("WIrq clr(%d)Typ(%d)Stt(0x%X)Tout(%d)usr(%d)PID(%d)\n",
	...
	if (WaitIrq->Status & RSC_INT_ST) {
		...
		LOG_ERR("No Such Stats!Type/User/Sts/Pid(0x%x/%d/0x%x/%d)\n",
	...
	if ((Timeout != 0) &&
	    (!RSC_GetIRQState(WaitIrq->Type, WaitIrq->UserKey, WaitIrq->Status,
			      whichReq, WaitIrq->ProcessID))) {
		LOG_DBG("system intterrupt\n");
	...
	if (Timeout == 0) {
		...
		LOG_ERR("WIrq Tout(%d)clr(%d)Typ(%d)IrqStt(%d)WStt(%d)usr(%d)",
			WaitIrq->Timeout, WaitIrq->Clear, WaitIrq->Type,
			irqStatus, WaitIrq->Status, WaitIrq->UserKey);
		LOG_ERR("whichReq(%d)PID(%d)IrqCnt(%d)WReq(%d)RdReq(%d)\n",
...
/* Store irqinfo status in here to redeuce time of spin_lock_irqsave */
		...
		if (WaitIrq->Clear == RSC_IRQ_WAIT_CLEAR) {
			...
			if (WaitIrq->Status & RSC_INT_ST) {
				...
				LOG_ERR("-EIRQ_WAIT_CLEAR Typ(%d)WaitStt(%d)",
...
static long RSC_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case RSC_WAIT_IRQ: {
		if (copy_from_user(&IrqInfo, (void *)Param,
			...
			if ((IrqInfo.UserKey >= IRQ_USER_NUM_MAX) ||
			    (IrqInfo.UserKey < 0)) {
				LOG_ERR("invalid uKey(%d),max(%d)\n",
					IrqInfo.UserKey, IRQ_USER_NUM_MAX);
				IrqInfo.UserKey = 0;
			}

			LOG_INF("IRQ clr(%d)typ(%d)uKey(%d)Tout(%d)stt(%d)\n",
	...
	case RSC_CLEAR_IRQ: {
		if (copy_from_user(&ClearIrq, (void *)Param,
			...
			LOG_DBG("RSC_CLEAR_IRQ:Typ(%d)Stat(%d)IrqStatus(%d)\n",
	...
	case RSC_ENQNUE_NUM: {
		/* enqueNum */
		if (copy_from_user(&enqueNum, (void *)Param, sizeof(int)) ==
		    0) {
			if (RSC_REQUEST_STATE_EMPTY ==
				...
				if (enqueNum >
				    _SUPPORT_MAX_RSC_FRAME_REQUEST_) {
					LOG_ERR("Enque Num >  enqueNum:%d\n",
						enqueNum);
				}
				LOG_DBG("RSC_ENQNUE_NUM:%d\n", enqueNum);
			} else {
				LOG_ERR("Non-empty Enque req:%d wrId:%d\n",
					g_RSC_ReqRing
						.RSCReq_Struct
	...
	case RSC_ENQUE: {
		if (copy_from_user(&rsc_RscConfig, (void *)Param,
			...
			if ((RSC_REQUEST_STATE_EMPTY ==
				...
				LOG_ERR("NoBuf!WrId(%d)Stt(%d)FW(%d)RqN(%d)\n",
					g_RSC_ReqRing.WriteIdx,
					g_RSC_ReqRing
						.RSCReq_Struct
							[g_RSC_ReqRing.WriteIdx]
	...
	case RSC_ENQUE_REQ: {
		if (copy_from_user(&rsc_RscReq, (void *)Param,
			...
			if (rsc_RscReq.m_ReqNum >
			    _SUPPORT_MAX_RSC_FRAME_REQUEST_) {
				LOG_ERR("RSC Enque Num > enqueNum:%d\n",
					rsc_RscReq.m_ReqNum);
				Ret = -EFAULT;
				goto EXIT;
			}
			if (copy_from_user(g_RscEnqueReq_Struct.RscFrameConfig,
					   ...
					   rsc_RscReq.m_ReqNum *
						   sizeof(struct RSC_Config)) !=
			    0) {
				LOG_ERR("copy RSCConfig from req failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
			...
			if (RSC_REQUEST_STATE_EMPTY ==
				...
				g_RSC_ReqRing
					.RSCReq_Struct[g_RSC_ReqRing.WriteIdx]
					.enqueReqNum = rsc_RscReq.m_ReqNum;

				for (idx = 0; idx < rsc_RscReq.m_ReqNum;
				     idx++) {
					g_RSC_ReqRing
						.RSCReq_Struct
							[g_RSC_ReqRing.WriteIdx]
				...
				LOG_ERR("EnqueNG:Wr(%d)Stt(%d)FW(%d)eqR(%d)\n",
					g_RSC_ReqRing.WriteIdx,
					g_RSC_ReqRing
						.RSCReq_Struct
						[g_RSC_ReqRing.WriteIdx]
	...
	case RSC_DEQUE_NUM: {
		if (RSC_REQUEST_STATE_FINISHED ==
			...
			dequeNum = g_RSC_ReqRing
					   .RSCReq_Struct[g_RSC_ReqRing.ReadIdx]
					   .enqueReqNum;
			LOG_DBG("RSC_DEQUE_NUM(%d)\n", dequeNum);
		} else {
			dequeNum = 0;
			LOG_ERR("DEQ_NUM:NoBuf Rd(%d)Stt(%d)Rrm(%d)eqRq(%d)\n",
				g_RSC_ReqRing.ReadIdx,
				g_RSC_ReqRing
					.RSCReq_Struct[g_RSC_ReqRing.ReadIdx]
					.State,
	...
	case RSC_DEQUE: {
		...
		if ((RSC_REQUEST_STATE_FINISHED ==
			...
			spin_unlock_irqrestore(
				&(RSCInfo.SpinLockIrq[RSC_IRQ_TYPE_INT_RSC_ST]),
				flags);
			LOG_ERR("DEQ NoBuf:RdIdx(%d)Stt(%d)Rrm(%d)eqRqN(%d)\n",
				g_RSC_ReqRing.ReadIdx,
				g_RSC_ReqRing
					.RSCReq_Struct[g_RSC_ReqRing.ReadIdx]
					.State,
	...
	case RSC_DEQUE_REQ: {
		if (copy_from_user(&rsc_RscReq, (void *)Param,
				   sizeof(struct RSC_Request)) == 0) {
			mutex_lock(&gRscDequeMutex); /* Protect the Multi
						      * Process
						      */
			...
			if (RSC_REQUEST_STATE_FINISHED ==
				...
				dequeNum = 0;
				LOG_ERR("DEQ_REQ:R(%d)St(%d)Rrm(%d)eqRq(%d)\n",
					g_RSC_ReqRing.ReadIdx,
					g_RSC_ReqRing
						.RSCReq_Struct[g_RSC_ReqRing
								       .ReadIdx]
			...
			for (idx = 0; idx < dequeNum; idx++) {
				if (RSC_FRAME_STATUS_FINISHED ==
					...
					LOG_ERR("deq err(%d)dqNum(%d)Rd(%d)\n",
						idx, dequeNum,
						g_RSC_ReqRing.ReadIdx);
				}
			}
			...
			if (copy_to_user(
				    ...
				    dequeNum * sizeof(struct RSC_Config)) !=
			    0) {
				LOG_ERR("DEQUE_REQ copy_to_user config NG\n");
				Ret = -EFAULT;
			}
			...
			LOG_ERR("RSC_CMD_RSC_DEQUE_REQ copy_from_user NG\n");
			Ret = -EFAULT;
		}
...
static int
compat_get_RSC_enque_req_data(struct compat_RSC_Request __user *data32,
...
static int
compat_put_RSC_enque_req_data(struct compat_RSC_Request __user *data32,
			      struct RSC_Request __user *data)
{
	compat_uint_t count;
	/*compat_uptr_t uptr;*/
...
static int
compat_get_RSC_deque_req_data(struct compat_RSC_Request __user *data32,
			      struct RSC_Request __user *data)
{
	compat_uint_t count;
	compat_uptr_t uptr;
...
static int
compat_put_RSC_deque_req_data(struct compat_RSC_Request __user *data32,
			      struct RSC_Request __user *data)
{
	compat_uint_t count;
	/*compat_uptr_t uptr;*/
			     ...
			     unsigned long arg)
{
	...
	case COMPAT_RSC_READ_REGISTER: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, RSC_READ_REGISTER,
						 (unsigned long)data);
	...
	case COMPAT_RSC_WRITE_REGISTER: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, RSC_WRITE_REGISTER,
						 (unsigned long)data);
		return ret;
	}
	case COMPAT_RSC_ENQUE_REQ: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, RSC_ENQUE_REQ,
						 (unsigned long)data);
	...
	case COMPAT_RSC_DEQUE_REQ: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, RSC_DEQUE_REQ,
						 (unsigned long)data);
...
static signed int RSC_open(struct inode *pInode, struct file *pFile)
{
	...
	pFile->private_data =
		kmalloc(sizeof(struct RSC_USER_INFO_STRUCT), GFP_ATOMIC);
	if (pFile->private_data == NULL) {
	...
	if (RSCInfo.UserCount > 0) {
		RSCInfo.UserCount++;
		spin_unlock(&(RSCInfo.SpinLockRSCRef));
		LOG_DBG("Curr UsrCnt(%d),(process,pid,tgid)=(%s,%d,%d),users",
		...
		RSCInfo.UserCount++;
		spin_unlock(&(RSCInfo.SpinLockRSCRef));
		LOG_DBG("Curr UsrCnt(%d),(process,pid,tgid)=(%s,%d,%d),1st usr",
			RSCInfo.UserCount, current->comm, current->pid,
			current->tgid);
	}
...
static signed int RSC_release(struct inode *pInode, struct file *pFile)
{
	...
	if (RSCInfo.UserCount > 0) {
		spin_unlock(&(RSCInfo.SpinLockRSCRef));
		LOG_DBG("Curr UsrCnt(%d),(process,pid,tgid)=(%s,%d,%d),usrs",
			RSCInfo.UserCount, current->comm, current->pid,
			current->tgid);
		goto EXIT;
	} else
		spin_unlock(&(RSCInfo.SpinLockRSCRef));
	/*  */
	LOG_DBG("Curr UsrCnt(%d),(process,pid,tgid)=(%s,%d,%d),last user",
...
static signed int RSC_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	LOG_INF("mmap:pgoff(0x%lx)pfn(0x%x)phy(0x%lx)vstart(0x%lx)vend(0x%lx)",
	...
	case RSC_BASE_HW:
		if (length > RSC_REG_RANGE) {
			LOG_ERR("mmap range err :module:0x%x length(0x%lx)",
				pfn, length);
			return -EAGAIN;
		}
		break;
	...
	if (RSC_dev->irq > 0) {
		...
		for (i = 0; i < RSC_IRQ_TYPE_AMOUNT; i++) {
			if (strcmp(pDev->dev.of_node->name,
				...
				LOG_INF("nr_devs=%d,devnode(%s)irq=%dISR:%s\n",
					nr_RSC_devs, pDev->dev.of_node->name,
					RSC_dev->irq,
					RSC_IRQ_CB_TBL[i].device_name);
				break;
...
#ifdef CONFIG_OF
/*
 * Note!!! The order and member of .compatible must be the same with
 * RSC_DEV_NODE_IDX
 */
 ...
 *
 ******************************************************************************/
static struct platform_driver RSCDriver = {.probe = RSC_probe,
					   .remove = RSC_remove,
					   .suspend = RSC_suspend,
					   .resume = RSC_resume,
					   .driver = {
...
static const struct file_operations rsc_dump_proc_fops = {
	.owner = THIS_MODULE, .open = proc_rsc_dump_open, .read = seq_read,
};
...
/*static int rsc_reg_write(struct file *file, const char __user *buffer, size_t
 * count, loff_t *data)
 */
			     ...
			     size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%23s %23s", addrSzBuf, valSzBuf) == 2) {
		...
		if (pszTmp == NULL) {
			...
			if (strlen(addrSzBuf) > 2) {
				if (sscanf(addrSzBuf + 2, "%x", &addr) != 1)
					LOG_ERR("Err scan hexadecimal addr:%s",
						addrSzBuf);
			} else {
		...
		if (pszTmp == NULL) {
			...
			if (strlen(valSzBuf) > 2) {
				if (sscanf(valSzBuf + 2, "%x", &val) != 1)
					LOG_ERR("Err scan hexadecimal val:%s",
						valSzBuf);
			} else {
		...
		if ((addr >= RSC_BASE_HW) && (addr <= RSC_DMA_RDY_STATUS_HW)) {
			...
			LOG_INF("Write-Addr Out-of-range addr:0x%x,val:0x%x\n",
				addr, val);
		}
		...
		if ((addr >= RSC_BASE_HW) && (addr <= RSC_DMA_RDY_STATUS_HW)) {
			...
			LOG_INF("Read-Addr Out-of-range addr:0x%x, val:0x%x\n",
				addr, val);
		}
	}
...
static signed int __init RSC_Init(void)
{
	...
	/* proc_entry = proc_create("pll_test", S_IRUGO | S_IWUSR, isp_rsc_dir,
	 * &pll_test_proc_fops);
	 */

	proc_entry =
		proc_create("rsc_dump", 0444, isp_rsc_dir, &rsc_dump_proc_fops);

	proc_entry =
		proc_create("rsc_reg", 0644, isp_rsc_dir, &rsc_reg_proc_fops);
	...
	for (i = 0; i < LOG_PPNUM; i++) {
		 ...
		 */
		tmp = (void *)((char *)tmp + NORMAL_STR_LEN); /* log buffer
							       * ,in case of
							       *  overflow
							       */
	}
...
/* Update the frame status. */
	...
	IRQ_LOG_KEEPER(
		...
		RSCInfo.IrqInfo.RscIrqCnt, RSCInfo.WriteReqIdx,
		RSCInfo.ReadReqIdx);
	/* IRQ_LOG_KEEPER(RSC_IRQ_TYPE_INT_RSC_ST, m_CurrentPPB, _LOG_INF,
	 * "RscHWSta:0x%x, RscHWSta:0x%x,
