...
static long
__mt_sched_addaffinity(struct task_struct *p, const struct cpumask *new_mask)
{
...
static long
__mt_sched_getaffinity(pid_t pid, struct cpumask *mask, struct cpumask *mt_mask)
{
	struct task_struct *p;
	struct mt_task *tmp;
	unsigned long irq_flags;
	...
	if (retval) {
		pr_debug(
			"MT_SCHED: getaffinity security_task_getscheduler fail, status: %d\n",
		     retval);
		goto out_unlock;
	}
...
long mt_sched_setaffinity(pid_t pid, const struct cpumask *in_mask)
{
	return __mt_sched_setaffinity(pid, in_mask);
}
EXPORT_SYMBOL(mt_sched_setaffinity);

long
mt_sched_addaffinity(struct task_struct *p, const struct cpumask *new_mask)
{
	return __mt_sched_addaffinity(p, new_mask);
}
EXPORT_SYMBOL(mt_sched_addaffinity);

long
mt_sched_getaffinity(pid_t pid, struct cpumask *mask, struct cpumask *mt_mask)
{
	return __mt_sched_getaffinity(pid, mask, mt_mask);
}
EXPORT_SYMBOL(mt_sched_getaffinity);

long mt_sched_exitaffinity(pid_t pid)
{
	return __mt_sched_exitaffinity(pid);
}
EXPORT_SYMBOL(mt_sched_exitaffinity);

static long
sched_ioctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int retval;
	pid_t pid;
	...
	case IOCTL_GETAFFINITY:
		...
		if (retval == 0) {
			...
			if (copy_to_user((int __user *)data.mask,
				mask, retlen)) {
				retval = -EFAULT;
			...
			if (copy_to_user((int __user *)data.mt_mask,
				mt_mask, retlen))
				retval = -EFAULT;
			else
				retval = retlen;
...
static int __init sched_ioctl_init(void)
{
	...
	class_dev = device_create(sched_class, NULL, sched_dev_num,
			NULL, "mtk_sched");
	if (IS_ERR(class_dev)) {
	...
	pr_info("MT_SCHED: Init complete, device major number = %d\n",
			MAJOR(sched_dev_num));
...
static int __init sched_proc_init(void)
{
	...
	pe = proc_create("mtk_sched/affinity_status", 0444,
				NULL, &sched_status_fops);
	if (!pe)
		return -ENOMEM;
 ...
 */
static int
sched_cpu_notify(struct notifier_block *self, unsigned long action, void *hcpu)
{
	long cpu = (long)hcpu;
	struct mt_task *tmp, *tmp2;
	unsigned long irq_flags;
...
static void sched_module_exit(void)
{
	...
	cdev_del(sched_ioctl_cdev);
	unregister_chrdev_region(sched_dev_num, 1);
	pr_info("MT_SCHED: driver removed.\n");
}
