...
/* This function pointer is for GPU LKM to register a function to get GPU
 * loading.
 */
...
#define THRML_LOG(fmt, args...)                                                \
	do {                                                                   \
		if (enable_ThermalMonitorXlog) {                               \
			pr_info("THERMAL/PLATFORM" fmt, ##args);              \
		}                                                              \
	} while (0)

#define THRML_ERROR_LOG(fmt, args...) pr_debug("THERMAL/PLATFORM" fmt, ##args)
...
static struct cpu_index_st
	cpu_index_list[NO_CPU_CORES]; /* /< 4-Core is maximum */
...
static int get_sys_cpu_usage_info_ex(void)
{
	...
	for_each_online_cpu(nCoreIndex) {
		...
		cpu_index_list[nCoreIndex].i[CPU_USAGE_FRAME_FIELD] = TRIMz_ex(
			cpu_index_list[nCoreIndex].tz,
			(cpu_index_list[nCoreIndex].i[CPU_USAGE_CURRENT_FIELD] -
		...
		for (i = 0; i < 3; i++) {
			THRML_LOG(
				...
				i, (int)cpu_index_list[nCoreIndex].u[i],
...
int mtk_thermal_get_extra_info(int *no_extra_attr, char ***attr_names,
			       int **attr_values, char ***attr_units)
{
	...
	extra_attr_values[i] =
		get_sys_wifi_throughput("/proc/driver/thermal/wifi_tx_thro", 3);
...
unsigned int mtk_thermal_set_user_scenarios(unsigned int mask)
{
	if ((mask & MTK_THERMAL_SCEN_CALL)) {
		...
		set_taklking_flag(
			true); /* make mtk_ts_cpu.c aware of call scenario */
...
unsigned int mtk_thermal_clear_user_scenarios(unsigned int mask)
{
	if ((mask & MTK_THERMAL_SCEN_CALL)) {
		/* only one scen is handled now... */
		set_taklking_flag(
			false); /* make mtk_ts_cpu.c aware of call scenario */
		_thermal_scen &= ~((unsigned int)MTK_THERMAL_SCEN_CALL);
	}
	return _thermal_scen;
}
