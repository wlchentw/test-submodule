...
#define _kernel_trace_begin(name) do {\
...
static inline void TSF_Prepare_Enable_ccf_clock(void)
{
	...
	/* CG_MM_SMI_COMMON -> CG_SCP_SYS_ISP -> TSF clk */
	/* smi_bus_enable(SMI_LARB_CAMSYS1, TSF_DEV_NAME); */
	smi_bus_prepare_enable(SMI_LARB5_REG_INDX, TSF_DEV_NAME, true);
...
{
/* must keep this clk close order: TSF clk -> */
/* CG_SCP_SYS_ISP -> CG_MM_SMI_COMMON -> CG_SCP_SYS_DIS */
	clk_disable_unprepare(TSF_clk.CG_IMGSYS_TSF);
	/* smi_bus_disable(SMI_LARB_CAMSYS1, TSF_DEV_NAME); */
	smi_bus_disable_unprepare(SMI_LARB5_REG_INDX, TSF_DEV_NAME, true);
}
#endif
...
static void TSF_EnableClock(bool En)
{
	if (En) {		/* Enable clock. */
/* LOG_DBG */
/* ("TSF clock enbled. g_u4EnableClockCount: %d.", g_u4EnableClockCount); */
...
/* LOG_DBG */
/* ("TSF clock disabled. g_u4EnableClockCount: %d.", g_u4EnableClockCount); */
		spin_lock(&(TSFInfo.SpinLockTSF));
		g_u4EnableClockCount--;
		spin_unlock(&(TSFInfo.SpinLockTSF));
		switch (g_u4EnableClockCount) {
...
static signed int TSF_ReadReg(struct TSF_REG_IO_STRUCT *pRegIo)
{
	...
	if ((pRegIo->pData == NULL) || (pRegIo->Count == 0) ||
		...
		LOG_INF("TSF_ReadReg pRegIo->pData is NULL, Count:%d!!",
		pRegIo->Count);
		Ret = -EFAULT;
		goto EXIT;
	}
	pData = kmalloc((pRegIo->Count) *
	sizeof(struct TSF_REG_STRUCT), GFP_KERNEL);
	if (pData == NULL) {
	...
	pTmpData = pData;
	if (copy_from_user(pData, (void *)pRegIo->pData, (pRegIo->Count) *
	sizeof(struct TSF_REG_STRUCT)) == 0) {
	...
	unsigned int Count)
{
	...
	/* Use local variable to store TSFInfo.DebugMask & */
	/*	TSF_DBG_WRITE_REG for saving lock time */
	spin_lock(&(TSFInfo.SpinLockTSF));
...
static signed int TSF_WriteReg(struct TSF_REG_IO_STRUCT *pRegIo)
{
	...
	/* sizeof(TSF_REG_STRUCT), GFP_ATOMIC); */
	pData = kmalloc((pRegIo->Count) *
	sizeof(struct TSF_REG_STRUCT), GFP_KERNEL);
	if (pData == NULL) {
		LOG_DBG
...
("No Such Stats can be waited!! irq Type/User/Sts/Pid(0x%x/%d/0x%x/%d)\n",
	...
	LOG_INF
	("bef wait_event: Timeout(%d),Clr(%d),Type(%d)\n",
	WaitIrq->Timeout, WaitIrq->Clear, WaitIrq->Type);
	LOG_INF
	("bef wait_event: IrqStatus(0x%08X),WaitStatus(0x%08X)\n",
	irqStatus, WaitIrq->Status);
	LOG_INF
	("bef wait_event: userKey(%d), ProcessID(%d)\n",
...
/* 2nd layer behavoir of copy from user is implemented in TSF_WriteReg(...) */
		if ((RegIo.pData != NULL) && (RegIo.Count <=
		(TSF_REG_RANGE >> 2)))
	...
	case TSF_WAIT_IRQ:
		...
		sizeof(struct TSF_WAIT_IRQ_STRUCT)) ==
		    0) {
			/*  */
	...
	case TSF_CLEAR_IRQ:
		...
		if (copy_from_user(&ClearIrq, (void *)Param,
			sizeof(struct TSF_CLEAR_IRQ_STRUCT))
			== 0) {
...
static long TSF_ioctl_compat(struct file *filp,
...
static signed int TSF_open(struct inode *pInode, struct file *pFile)
{
	...
	pFile->private_data =
		kmalloc(sizeof(struct TSF_USER_INFO_STRUCT), GFP_ATOMIC);
	if (pFile->private_data == NULL) {
...
static signed int TSF_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	LOG_INF("TSF_mmap: pfn(0x%x),phy(0x%lx)",
		pfn, pVma->vm_pgoff << PAGE_SHIFT);
...
static signed int TSF_probe(struct platform_device *pDev)
{
	...
	nr_TSF_devs += 1;
	_tsfdev = krealloc(TSF_devs, sizeof(struct TSF_device) *
		nr_TSF_devs, GFP_KERNEL);
	if (!_tsfdev) {
...
if (i > TSF_IRQ_TYPE_AMOUNT) {
	...
	if (nr_TSF_devs == 1) {
		...
		TSF_clk.CG_IMGSYS_TSF = devm_clk_get(&pDev->dev,
...
static signed int __init TSF_Init(void)
{
	...
	/*			isp_TSF_dir, &pll_test_proc_fops); */

	proc_entry = proc_create("TSF_dump", 0444,
				isp_TSF_dir, &TSF_dump_proc_fops);

	proc_entry = proc_create("TSF_reg", 0644,
				isp_TSF_dir, &TSF_reg_proc_fops);
