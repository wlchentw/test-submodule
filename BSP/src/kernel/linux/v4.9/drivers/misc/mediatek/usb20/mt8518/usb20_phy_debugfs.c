...
#include "mtk_musb.h"
#define MYDBG(fmt, args...) pr_notice("MTK_ICUSB [DBG], <%s(), %d> " fmt, \
...
int usb20_phy_init_debugfs(void)
{
	...
	file = debugfs_create_file(FILE_USB_DRIVING_CAPABILITY,
				   0644, root, NULL,
				   &usb_driving_capability_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
	file = debugfs_create_file(FILE_RG_USB20_TERM_VREF_SEL,
				   0644, root, NULL,
				   &rg_usb20_term_vref_sel_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
	...
	file = debugfs_create_file(FILE_RG_USB20_VRT_VREF_SEL,
				   0644, root, NULL,
				   &rg_usb20_vrt_vref_sel_fops);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
