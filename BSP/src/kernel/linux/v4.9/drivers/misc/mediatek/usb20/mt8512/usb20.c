...
#endif
#if defined(CONFIG_CHARGER_MTK) /* Only for mt8518 */
...
static struct musb_fifo_cfg fifo_cfg[] = {
	...
	{.hw_ep_num = 8, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_SINGLE},
#else
#ifdef CONFIG_USB_CONFIGFS_F_UAC2
	{.hw_ep_num = 1, .style = MUSB_FIFO_TX,	.maxpacket = 1024, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 1, .style = MUSB_FIFO_RX,	.maxpacket = 1024, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 2, .style = MUSB_FIFO_RX,	.maxpacket = 4, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_DOUBLE},
#else
	{.hw_ep_num = 1, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
...
void do_connection_work(struct work_struct *data)
{
	...
	if (!mtk_musb->is_ready) {
		...
		DBG_LIMIT(3,
		  ...
		  mtk_musb->is_host,
		  mtk_musb->power);
		queue_delayed_work(mtk_musb->st_wq, &connection_work,
	...
	usb_in = musb_force_cable_in ? true : usb_cable_connected();
...
#if defined(CONFIG_USB_VBUSDET_IN_GPIO) && \
	!defined(CONFIG_MTK_MUSB_SW_WITCH_MODE)
enum CHARGER_TYPE mt_get_extconn_type(void)
{
#ifdef CONFIG_USB_PHYCHK_EXTCONN
	return usb_extconntype;
#endif
	if (gpio_get_vbus_status())
		return STANDARD_HOST;
	else
...
#if defined(CONFIG_USB_VBUSDET_IN_GPIO) && \
	!defined(CONFIG_MTK_MUSB_SW_WITCH_MODE)
	charge_type = mt_get_extconn_type();
#else
...
static ssize_t mt_usb_show_swmode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	if (!dev) {
		DBG(0, "dev is null!!\n");
	...
	return scnprintf(buf, PAGE_SIZE, "In %s mode.\n",
			host_mode ? "host" : (device_mode ? "device" : "idle"));
}

static ssize_t mt_usb_store_swmode(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (count >= 4 && !strncmp(buf, "idle", 4)) {
		if (device_mode) {
			DBG(0, "switch device to idle mode\n");
			musb_force_cable_in = false;
			mt_usb20_sw_disconnect();
			device_mode = false;
		} else if (host_mode) {
			DBG(0, "switch host to idle mode\n");
...
DEVICE_ATTR(swmode, 0664, mt_usb_show_swmode, mt_usb_store_swmode);

static struct delayed_work sw_default_mode_work;
static u32 sw_default_mode;
static void musb_sw_default_mode_work(struct work_struct *data)
{
	if (!mtk_musb->is_ready) {
		/* dealy 1 sec if usb function is not ready */
		schedule_delayed_work(&sw_default_mode_work, HZ);
		return;
	}

	/* switch mode back to idle */
	if (device_mode) {
		DBG(0, "switch device to idle mode\n");
		musb_force_cable_in = false;
		mt_usb20_sw_disconnect();
	...
	/* default switch mode: 0-idle, 1-device, 2-host */
	if (sw_default_mode == 1) {
		DBG(0, "default switch to device mode\n");
		musb_force_cable_in = true;
		mt_usb20_sw_connect();
		device_mode = true;
	} else if (sw_default_mode == 2) {
		DBG(0, "default switch to host mode\n");
		musb_id_pin_sw_work(true);
		host_mode = true;
	}
}
...
static int mt_usb_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_MTK_MUSB_SW_WITCH_MODE)
	/* Set default mode: 0-idle, 1-device, 2-host */
	ret = of_property_read_u32(np, "sw_default_mode", &sw_default_mode);
	if (ret == 0) {
		INIT_DELAYED_WORK(&sw_default_mode_work,
				  musb_sw_default_mode_work);
		schedule_delayed_work(&sw_default_mode_work,
				      5000 * HZ / 1000);
	} else {
		DBG(0, "fail to get sw default_mode from dts: %d\n", ret);
		sw_default_mode = 0;
		ret = 0;
	}
#endif
	...
	ret =
	    platform_device_add_resources(musb, pdev->resource,
					  pdev->num_resources);
	if (ret) {
		DBG(0, "failed to add resources\n");
		goto err2;
	}

	ret = platform_device_add_data(musb, pdata, sizeof(*pdata));
	if (ret) {
		DBG(0, "failed to add platform_data\n");
		goto err2;
	}
	...
	if (ret) {
		DBG(0, "failed to register musb device\n");
		goto err2;
	}
	...
	if (ret) {
		DBG(0, "failed to create musb device\n");
		goto err2;
	}
#ifdef CONFIG_OF
