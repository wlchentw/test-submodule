...
static struct cdev *ir_learn_cdev;
static dev_t ir_learn_devno;
static struct class *ir_learn_class;
#define IR_LEARN_DEVNAME "ir_learn"
wait_queue_head_t wait_head_ir_learn;
#endif
...
static const struct of_device_id mtk_ir_of_match[] = {
	...
	{.compatible = "mediatek,mt8518-irrx",},
...
static int mtk_ir_core_enable_clock(int enable)
{
	...
	if (enable) {
		res = clk_prepare_enable(mtk_ir_context_obj->hw->irrx_sw_clk);
		res = clk_prepare_enable(mtk_ir_context_obj->hw->irrx_clk);
	} else {
		clk_disable_unprepare(mtk_ir_context_obj->hw->irrx_clk);
		clk_disable_unprepare(mtk_ir_context_obj->hw->irrx_sw_clk);
	}
	return res;
}
...
#if CONFIG_IR_LEARN_SUPPORT
#define IRRX_PULSE_COUNT (32)
u8 ir_pulse_data[IRRX_PULSE_COUNT * 4] = {0};
struct _IRRX_IO_T ir_learned_data[MAX_LEARNED_KEY];

u32 ir_learned_key_match(void)
{
	u32 count = 0;
	u32 learned_key = 0;

	MTK_IR_FUN();
	for (learned_key = 0; learned_key < MAX_LEARNED_KEY;
		learned_key++) {
		count = 0;
		while (count < IRRX_PULSE_COUNT * 4) {
			if (abs(ir_pulse_data[count] -
				ir_learned_data[learned_key].raw_data[count])
				> 3) {
				MTK_IR_LOG("Print matched fail data:\n");
				MTK_IR_LOG("ir_pulse[%d]=%d,learn[%d][%d]=%d\n",
				count, ir_pulse_data[count],
				learned_key, count,
				ir_learned_data[learned_key].raw_data[count]);
				break;
			}
			count++;
			if (IRRX_PULSE_COUNT * 4 == count) {
				MTK_IR_LOG("ir match ok\n");
				MTK_IR_LOG("ir_learned_data[%d].key:0x%x\n",
					learned_key,
					ir_learned_data[learned_key].key);
				return ir_learned_data[learned_key].key;
			}
		}
		MTK_IR_LOG("key_unmacth:%d\n", learned_key);
	}
	return BTN_NONE;
}

u8 *get_ir_pluse_data(void)
{
	return ir_pulse_data;
}

u8 *pulse_width_check(void)
{
	u32 addr = IRRX_CHKDATA0;
	u32 count = 0;
	u32 ir_data = 0;

	memset(ir_pulse_data, 0, sizeof(ir_pulse_data));

	MTK_IR_LOG("pluse_width_get: addr; pulse_data: time\n");
	while (count < (IRRX_PULSE_COUNT * 4)) {
		int i = 0;

		ir_data = IR_READ32(addr);
		for (i = 0; i < 4; i++) {
			ir_pulse_data[count+i] = (ir_data >> (8 * i)) & 0xFF;
			MTK_IR_LOG("addr=%x data[%d]=%x\n", addr+i, count+i,
				ir_pulse_data[count+i]);
		}
		count += 4;
		addr += 4;
	}
...
static int ir_learn_close(struct inode *inode, struct file *file)
{
	MTK_IR_FUN();
	return 0;
}

static int ir_learn_open(struct inode *inode, struct file *file)
{
	MTK_IR_FUN();
	init_waitqueue_head(&wait_head_ir_learn);
	return 0;
}

static ssize_t ir_learn_read(struct file *file, char *buffer,
...
void show_all_learned_data(void)
{
	u32 count = 0;
	u32 count_learned_key = 0;

	MTK_IR_FUN();
	for (count_learned_key = 0; count_learned_key < MAX_LEARNED_KEY;
		count_learned_key++) {
		count = 0;
		MTK_IR_LOG("learned_data[%d].key:0x%x\n", count_learned_key,
			ir_learned_data[count_learned_key].key);
		while (count < IRRX_PULSE_COUNT * 4) {
			MTK_IR_LOG("%d,",
			ir_learned_data[count_learned_key].raw_data[count]);
			count++;
			if (count % 10 == 0)
			MTK_IR_LOG("\n");
		}
...
static long ir_learn_ioctl(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	struct _IRRX_IO_T ir_set_data;
	struct _IRRX_IO_T ir_get_data;
	u8 *p_pluse_data = NULL;
	int ret = 0;
	static unsigned long timeout = HZ * 3;//default 3s to timeout
	DECLARE_WAITQUEUE(ir_learn_wait, current);

	memset(&ir_set_data, 0, sizeof(ir_set_data));
	memset(&ir_get_data, 0, sizeof(ir_get_data));

	MTK_IR_FUN();
	switch (cmd) {
	case CMD_IRRX_SET_LEARN_DATA:
		MTK_IR_LOG("ir_ioctl: CMD_IRRX_SET_LEARN_DATA\n");
		if (copy_from_user(&ir_set_data,
			(struct _IRRX_IO_T __user *)arg,
			sizeof(struct _IRRX_IO_T))) {
			MTK_IR_LOG("copy_from_user failed!!!\n");
			return -1;
		}
		if (ir_set_data.key_index >= MAX_LEARNED_KEY) {
			MTK_IR_LOG("Invalid index. MAX_LEARNED_KEY is %d\n",
				MAX_LEARNED_KEY);
			return -1;
		}
		memcpy(&ir_learned_data[ir_set_data.key_index],
			&ir_set_data, sizeof(struct _IRRX_IO_T));

		MTK_IR_LOG("ir_learned_data:key=%x, key_index=%x\n",
			ir_set_data.key, ir_set_data.key_index);
		show_all_learned_data();
		break;

	case CMD_IRRX_GET_LEARN_DATA:
		MTK_IR_LOG("ir_ioctl: CMD_IRRX_GET_LEARN_DATA\n");
		add_wait_queue(&wait_head_ir_learn, &ir_learn_wait);
		set_current_state(TASK_INTERRUPTIBLE);
		MTK_IR_LOG("before schedule\n");
		if (schedule_timeout(timeout) == 0) {
			MTK_IR_LOG("ir_learn time out return\n");
			ret = -ETIME;
			goto out;
		}
		if (signal_pending(current)) {
			MTK_IR_LOG("wakeup by signal\n");
			ret = -ERESTARTSYS;
			return ret;
		}
		MTK_IR_LOG("after wakeup\n");
		p_pluse_data = get_ir_pluse_data();
		memcpy(ir_get_data.raw_data, p_pluse_data,
			sizeof(ir_get_data.raw_data));
		ir_get_data.key = BTN_NONE;
		ir_get_data.key_index = 0;

		if (copy_to_user((struct _IRRX_IO_T __user *)arg,
			&ir_get_data, sizeof(struct _IRRX_IO_T))) {
			MTK_IR_LOG("copy to user error\n");
			return -1;
		}
out:
		remove_wait_queue(&wait_head_ir_learn, &ir_learn_wait);
		set_current_state(TASK_RUNNING);
	break;

	case CMD_IRRX_SET_LEARN_TIMEOUT:
		MTK_IR_LOG("ir_ioctl: CMD_IRRX_SET_LEARN_TIMEOUT\n");
		timeout = arg * HZ / 1000;
		MTK_IR_LOG("ir_learned_timeout_set! %ld ms\n", timeout);
		break;

	default:
		MTK_IR_LOG("ir_ioctl:ERROR CMD\n");
		return -ENOTTY;
	}
	return ret;
}

static const struct file_operations ir_learn_fops = {
	.owner		= THIS_MODULE,
	.open		= ir_learn_open,
	.read		= ir_learn_read,
	.release	= ir_learn_close,
	.unlocked_ioctl	= ir_learn_ioctl
};
#endif
...
static void mtk_ir_irq_function(struct work_struct *work)
{
	...
	if (scancode != BTN_NONE) {
		...
		pulse_width_check();
		wake_up_interruptible(&wait_head_ir_learn);
		MTK_IR_LOG("scancode = 0x%x, start matching keys\n", scancode);
		scancode = ir_learned_key_match();
		MTK_IR_LOG("ir_learn match done, scancode:0x%x\n", scancode);
		if (scancode != BTN_NONE)
			rc_keydown(rcdev, cxt->protocol, scancode, 0);
	}
#endif
...
static int mtk_ir_get_hw_info(struct platform_device *pdev)
{
	...
	const char *sw_clkname = "irrx_sw_clock";
	...
	cxt->hw->irrx_sw_clk = devm_clk_get(&pdev->dev, sw_clkname);
	if (IS_ERR(cxt->hw->irrx_sw_clk)) {
		MTK_IR_ERR(" Cannot get irrx_sw clock!\n");
		ret = PTR_ERR(cxt->hw->irrx_sw_clk);
		MTK_IR_ERR(" ret=%d!\n", ret);
		iounmap(cxt->hw->irrx_base_addr);
		return ret;
	}
	...
	if (IS_ERR(irrx_pinctrl1)) {
		...
		iounmap(cxt->hw->irrx_base_addr);
		clk_disable_unprepare(cxt->hw->irrx_clk);
		clk_disable_unprepare(cxt->hw->irrx_sw_clk);
		return ret;
	}
...
static int mtk_ir_core_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		MTK_IR_ERR(" get ir hw info fail!\n");
		goto err_probe;
	}
...
#if CONFIG_IR_LEARN_SUPPORT
	/* create char device */
	ret = alloc_chrdev_region(&ir_learn_devno, 0, 1, IR_LEARN_DEVNAME);
	if (ret) {
		pr_notice("alloc_chrdev_region failed! ret=%d\n", ret);
		goto err_alloc_ir_learn;
	}

	ir_learn_cdev = cdev_alloc();
	ir_learn_cdev->owner = THIS_MODULE;
	ir_learn_cdev->ops = &ir_learn_fops;
	ret = cdev_add(ir_learn_cdev, ir_learn_devno, 1);
	if (ret != 0) {
		MTK_IR_LOG("ir learn cdev_add Failed! ret=%d\n", ret);
		goto err_alloc_ir_learn;
	}

	ir_learn_class = class_create(THIS_MODULE, IR_LEARN_DEVNAME);
	/* mknod /dev/ir_learn */
	class_dev =
		(struct class_device *)device_create(ir_learn_class, NULL,
			ir_learn_devno, NULL, IR_LEARN_DEVNAME);
	if (IS_ERR(class_dev)) {
		MTK_IR_LOG("Create ir_learn class failed!\n");
		goto err_alloc_ir_learn;
	}
	memset(&ir_learned_data, 0,
		sizeof(struct _IRRX_IO_T) * MAX_LEARNED_KEY);
	init_waitqueue_head(&wait_head_ir_learn);
#endif
