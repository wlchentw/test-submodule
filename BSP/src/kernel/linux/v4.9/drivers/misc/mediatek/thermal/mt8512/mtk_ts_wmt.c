...
#define wmt_tm_printk(fmt, args...) pr_debug("[thermal/wmt]" fmt, ##args)

#define wmt_tm_info(fmt, args...) pr_debug("[thermal/wmt]" fmt, ##args)
...
static unsigned int g_trip_temp[COOLER_NUM] = {
	125000, 115000, 105000, 85000, 0, 0, 0, 0, 0, 0};

/* static int g_thro[COOLER_THRO_NUM] = {10 * ONE_MBITS_PER_SEC, 5 *
 * ONE_MBITS_PER_SEC, 1 * ONE_MBITS_PER_SEC};
 */
...
/* initial value: assume 1 degreeC for temp. <=> 1 unit for
 * wifi_tput_ratio(0~100)
 */
...
static unsigned long set_adaptive_wifi_tput_limit(unsigned int cur_tput,
						  unsigned int limit_ratio)
{
	...
	wmt_tm_dprintk("%s: Curr Tput=%lu, ratio=%u, limit_tput=%d", __func__,
		       tx_throughput, limit_ratio, limit_tput);
...
static int adaptive_tput_ratio(long prev_temp, long curr_temp)
{
	...
	wmt_tm_dprintk("%s: active=%d tirgger=%d\n", __func__,
		       cl_dev_adp_cpu_state_active, triggered);
	...
	if (cl_dev_adp_cpu_state_active == 1) {
		...
		wifi_tput_ratio = (wifi_tput_ratio > min_wifi_tput_ratio)
					  ? wifi_tput_ratio
					  : min_wifi_tput_ratio;
		wifi_tput_ratio = (wifi_tput_ratio < max_wifi_tput_ratio)
					  ? wifi_tput_ratio
					  : max_wifi_tput_ratio;
		wmt_tm_dprintk(
			...
			struct thermal_cooling_device *cool_dev)
{
	...
	if (mtk_thermal_zone_bind_cooling_device(thz_dev, table_val,
						 cool_dev)) {
				...
				unsigned long v)
{
	...
	if (cl_dev_state == 1) {
		...
		*(unsigned int *)0x0 = 0xdead;
		/* To trigger data abort to reset the
		 * system for thermal protection.
		 */
	}
...
static int wmt_judge_throttling(int index, int is_on, int interval)
{
	...
	/*
	 * If Wifi-display is on, go to WFD_STAT state, and reset the
	 * throttling.
	 */
	if (tm_wfd_stat == 2)
...
/*At this moment, current throughput is none. Use
 ...
 */
			...
			wmt_send_signal(
				thro_constraint); /* [star] unit : Kbytes */
	...
	/* current throughput is large than constraint. WHAT!!! */
					thro_constraint = thro(thro_constraint,
							       up_numerator,
							       up_denominator);
			...
			if ((below_low_time * interval) >= low_duration) {
				if (low_rst_time >= low_rst_max && !is_reset) {
					...
					if (cur_thro < 5000 && cur_thro > 0) {
						...
						thro_constraint = thro(
							cur_thro, low_numerator,
							low_denominator);
						low_rst_time++;
					} else {
						thro_constraint =
			    ...
			    size_t len, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d/%d %d %d/%d %d", &tmp_up_dur, &tmp_up_num,
		   &tmp_up_den, &tmp_low_dur, &tmp_low_num, &tmp_low_den,
		   &tmp_low_rst_max) == 7) {
			      ...
			      size_t len, loff_t *data)
{
	...
	ret = sscanf(tmp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		     &wifi_throttle_version, &sensor_select,
		     &resource_allocator_policy, &cpu_wifi_target_tj_offset,
		     &wmt_wifi_target_tj, &wmt_wifi_target_offset,
		     &tj_stable_range, &min_wifi_tput_ratio,
		     &max_wifi_tput_ratio, &min_wifi_tput, &tt_wifi_high,
		     &tt_wifi_low, &tp_wifi_rise, &tp_wifi_fall);

	wmt_tm_printk(
		"qqq %s ret :%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		__func__, ret, wifi_throttle_version, sensor_select,
		resource_allocator_policy, cpu_wifi_target_tj_offset,
		wmt_wifi_target_tj, wmt_wifi_target_offset, tj_stable_range,
		min_wifi_tput_ratio, max_wifi_tput_ratio, min_wifi_tput,
	...
	/* wmt_wifi_in_soc_write ret 13 1 3 3 0 70000 3000 1000 50 200 1000 50
	 * 50 10000
	 */
	return len;
}
...
#define DEFAULT_POLL_TIME 0 /*Default disable, turn on by thermal policy */
	...
	p_linux_if->thz_dev = mtk_thermal_zone_device_register(
		"mtktswmt", g_num_trip, NULL, &wmt_thz_dev_ops, 0, 0, 0,
		p_linux_if->interval);
			    ...
			    size_t count, loff_t *data)
{
	...
	if (sscanf(ptr_tm_data->desc,
		   ...
		   &g_num_trip, &ptr_tm_data->trip_temp[0],
		   &ptr_tm_data->thermal_trip[0], ptr_tm_data->bind0,
		   &ptr_tm_data->trip_temp[1], &ptr_tm_data->thermal_trip[1],
		   ptr_tm_data->bind1, &ptr_tm_data->trip_temp[2],
		   &ptr_tm_data->thermal_trip[2], ptr_tm_data->bind2,
		   &ptr_tm_data->trip_temp[3], &ptr_tm_data->thermal_trip[3],
		   ptr_tm_data->bind3, &ptr_tm_data->trip_temp[4],
		   &ptr_tm_data->thermal_trip[4], ptr_tm_data->bind4,
		   &ptr_tm_data->trip_temp[5], &ptr_tm_data->thermal_trip[5],
		   ptr_tm_data->bind5, &ptr_tm_data->trip_temp[6],
		   &ptr_tm_data->thermal_trip[6], ptr_tm_data->bind6,
		   &ptr_tm_data->trip_temp[7], &ptr_tm_data->thermal_trip[7],
		   ptr_tm_data->bind7, &ptr_tm_data->trip_temp[8],
		   &ptr_tm_data->thermal_trip[8], ptr_tm_data->bind8,
		   &ptr_tm_data->trip_temp[9], &ptr_tm_data->thermal_trip[9],
		   ptr_tm_data->bind9, &ptr_tm_data->time_msec) == 32) {
...
static int wmt_tm_proc_register(void)
{
	...
	if (!wmt_tm_proc_dir) {
		...
		entry = proc_create("clwmt_val", 0660, wmt_tm_proc_dir,
				    &_wmt_val_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("clwmt_wfdstat", 0660, wmt_tm_proc_dir,
				    &_wfd_stat_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("wifi_in_soc", 0660, wmt_tm_proc_dir,
				    &_wifi_in_soc_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
	}
