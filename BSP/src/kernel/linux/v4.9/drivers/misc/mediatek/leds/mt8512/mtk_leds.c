...
int mt_led_get_cust_led_gpio_dtsi(struct device_node *led_node, int num)
{
	int err = -1;
	int count = -1;
	...
	if (num < 0)
		return -1;
	count = num;
	if (strncmp(leds_name[count], "red", strlen(leds_name[count])) == 0)
		pled_dtsi[count].data = of_get_named_gpio(led_node,
							"red_gpios", 0);
	else if (strncmp(leds_name[count], "green",
						strlen(leds_name[count])) == 0)
		pled_dtsi[count].data = of_get_named_gpio(led_node,
							"green_gpios", 0);

	if (gpio_is_valid(pled_dtsi[count].data)) {
		err = gpio_request_one(pled_dtsi[count].data,
				GPIOF_OUT_INIT_LOW, "led1 gpio");
		if (err < 0) {
			pr_info("[%s,%d],cannot request gpio:%ld as high\n",
			__func__, __LINE__, pled_dtsi[count].data);
			return err;
		}
	}
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (!led_node) {
			...
			case MT65XX_LED_MODE_GPIO:
				mt_led_get_cust_led_gpio_dtsi(led_node, i);
				LEDS_DEBUG("kernel:the led hw mode is gpio.\n");
				break;
...
int mt_led_set_gpio(struct cust_mt65xx_led *cust, int level)
{
	int err = -1;
	int led_gpio = cust->data;

	LEDS_DEBUG("level=%d, led_gpio=%d\n", level, led_gpio);
	if (level > 255)
		level = 255;
	...
	if (level > 0) {
		if (gpio_is_valid(led_gpio)) {
			gpio_free(led_gpio);
			err = gpio_request_one(led_gpio,
				GPIOF_OUT_INIT_HIGH, "led gpio");
			if (err < 0) {
				pr_info("[%s,%d], cannot request gpio:%d as high\n",
						__func__, __LINE__, led_gpio);
				return err;
			}
		}
	} else if (level == 0) {
		if (gpio_is_valid(led_gpio)) {
			gpio_free(led_gpio);
			err = gpio_request_one(led_gpio,
				GPIOF_OUT_INIT_LOW, "led gpio");
			if (err < 0) {
				pr_info("[%s,%d], cannot request gpio:%d as low\n",
						__func__, __LINE__, led_gpio);
				return err;
			}
		}
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_GPIO:
		...
		return mt_led_set_gpio(cust, level);
