...
enum pwr_ctrl_enum {
	...
	PW_PCM_FLAGS2,
	PW_PCM_FLAGS2_CUST,
	PW_PCM_FLAGS2_CUST_SET,
	PW_PCM_FLAGS2_CUST_CLR,
	PW_TIMER_VAL,
	PW_TIMER_VAL_CUST,
...
struct pwr_ctrl {
	...
	u32 pcm_flags2;
	u32 pcm_flags2_cust;
	u32 pcm_flags2_cust_set;
	u32 pcm_flags2_cust_clr;
	u32 timer_val;
	u32 timer_val_cust;
...
static char *pwr_ctrl_str[PW_MAX_COUNT] = {
	...
	[PW_PCM_FLAGS2] = "pcm_flags2",
	[PW_PCM_FLAGS2_CUST] = "pcm_flags2_cust",
	[PW_PCM_FLAGS2_CUST_SET] = "pcm_flags2_cust_set",
	[PW_PCM_FLAGS2_CUST_CLR] = "pcm_flags2_cust_clr",
...
static ssize_t show_pwr_ctrl(int id, const struct pwr_ctrl *pwrctrl, char *buf)
{
	...
	p += sprintf(p, "pcm_flags2 = 0x%zx\n",
		     SMC_CALL(GET_PWR_CTRL_ARGS, id, PW_PCM_FLAGS2, 0));
	p += sprintf(p, "pcm_flags2_cust = 0x%zx\n",
		     SMC_CALL(GET_PWR_CTRL_ARGS, id, PW_PCM_FLAGS2_CUST, 0));
	p += sprintf(p, "pcm_flags2_cust_set = 0x%zx\n",
		     SMC_CALL(GET_PWR_CTRL_ARGS,
			      id, PW_PCM_FLAGS2_CUST_SET, 0));
	p += sprintf(p, "pcm_flags2_cust_clr = 0x%zx\n",
		     SMC_CALL(GET_PWR_CTRL_ARGS,
			      id, PW_PCM_FLAGS2_CUST_CLR, 0));
			      ...
			      const char *buf, size_t count)
{
	...
	} else if (!strcmp(cmd,	pwr_ctrl_str[PW_PCM_FLAGS2])) {
		pwrctrl->pcm_flags2 = val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2, val);
	} else if (!strcmp(cmd,	pwr_ctrl_str[PW_PCM_FLAGS2_CUST])) {
		pwrctrl->pcm_flags2_cust = val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2_CUST, val);
	} else if (!strcmp(cmd,	pwr_ctrl_str[PW_PCM_FLAGS2_CUST_SET])) {
		pwrctrl->pcm_flags2_cust_set = val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2_CUST_SET, val);
	} else if (!strcmp(cmd,	pwr_ctrl_str[PW_PCM_FLAGS2_CUST_CLR])) {
		pwrctrl->pcm_flags2_cust_clr = val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2_CUST_CLR, val);
...
static ssize_t dpidle_show_count(int id, const struct pwr_ctrl *pwrctrl,
						char *buf)
{
	int cpu = 0;
	char *p = buf;
...
static ssize_t dpidle_show_blk(int id, const struct pwr_ctrl *pwrctrl,
								char *buf)
{
	...
	p += sprintf(p, "dpidle_block_cnt[by_swt ] = %zu\n",
			SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_BY_SWT, 0));
	p += sprintf(p, "dpidle_block_cnt[by_boot] = %zu\n",
			SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_BY_BOOT, 0));
	p += sprintf(p, "dpidle_block_cnt[by_cpu ] = %zu\n",
			SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_BY_CPU, 0));
	p += sprintf(p, "dpidle_block_cnt[by_clk ] = %zu\n",
			SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_BY_CLK, 0));
	p += sprintf(p, "dpidle_block_cnt[by_gpt ] = %zu\n\n",
			SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_BY_GPT, 0));

	for (i = 0; i < NR_GRPS; i++)
		p += sprintf(p, "[%-8s] = 0x%08x, block = 0x%08x\n",
...
static ssize_t dpidle_count_store(struct kobject *kobj,
				     struct kobj_attribute *attr,
				     const char *buf, size_t count)
{
	return 1;
...
static ssize_t dpidle_count_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)

{
	return dpidle_show_count(SPM_PWR_CTRL_DPIDLE,
		&pwrctrl_dp, buf);
}

static ssize_t dpidle_blk_store(struct kobject *kobj,
				     struct kobj_attribute *attr,
				     const char *buf, size_t count)
{
	return 1;
...
static ssize_t dpidle_blk_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)

{
	return dpidle_show_blk(SPM_PWR_CTRL_DPIDLE,
		&pwrctrl_dp, buf);
}
...
DEFINE_ATTR_RW(dpidle_count);
DEFINE_ATTR_RW(dpidle_blk);
...
static struct attribute *spm_attrs[] = {
	...
	__ATTR_OF(dpidle_count),
	__ATTR_OF(dpidle_blk),
...
{
	struct pwr_ctrl *pwrctrl = &pwrctrl_sleep;

	if (req == APSRC_REQ) {
		pwrctrl->spm_apsrc_req = val;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
				PW_SPM_APSRC_REQ, val);
	} else if (req == F26M_REQ) {
		pwrctrl->spm_f26m_req = val;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
				PW_SPM_F26M_REQ, val);
	} else if (req == INFRA_REQ) {
		pwrctrl->spm_infra_req = val;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
				PW_SPM_INFRA_REQ, val);
	} else if (req == VRF18_REQ) {
		pwrctrl->spm_vrf18_req = val;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
				PW_SPM_VRF18_REQ, val);
	} else if (req == DDREN_REQ) {
		pwrctrl->spm_ddren_req = val;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
				PW_SPM_DDREN_REQ, val);
	}

	return 0;
}
EXPORT_SYMBOL(sloa_suspend_src_req);

int sloa_dpidle_src_req(enum SRC_REQ req, unsigned int val)
{
	struct pwr_ctrl *pwrctrl = &pwrctrl_dp;
...
EXPORT_SYMBOL(sloa_dpidle_src_req);

int dpidle_i2c0_clk_req(bool req)
{
	unsigned int idle_mask = SMC_CALL(GET_PWR_CTRL_ARGS,
		SPM_PWR_CTRL_DPIDLE, PW_INFRA3_IDLE_MASK, 0);

	if (req) {
		idle_mask &= ~(0x1 << 24);
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_DPIDLE,
			PW_INFRA3_IDLE_MASK, idle_mask);
	} else {
		idle_mask |= (0x1 << 24);
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_DPIDLE,
			PW_INFRA3_IDLE_MASK, idle_mask);
...
EXPORT_SYMBOL(dpidle_i2c0_clk_req);

int sloa_vcore_req(bool on)
{
	struct pwr_ctrl *pwrctrl_s = &pwrctrl_sleep;
	struct pwr_ctrl *pwrctrl_d = &pwrctrl_dp;

	if (on) {
		pwrctrl_s->pcm_flags2_cust_set = 0xC0;
		pwrctrl_d->pcm_flags2_cust_set = 0xC0;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
			PW_PCM_FLAGS2_CUST_SET, 0xC0);
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_DPIDLE,
			PW_PCM_FLAGS2_CUST_SET, 0xC0);
	} else {
		pwrctrl_s->pcm_flags2_cust_clr = 0xC0;
		pwrctrl_d->pcm_flags2_cust_clr = 0xC0;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
			PW_PCM_FLAGS2_CUST_CLR, 0xC0);
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_DPIDLE,
			PW_PCM_FLAGS2_CUST_CLR, 0xC0);
	}
...
EXPORT_SYMBOL(sloa_vcore_req);

int sloa_flag_req(enum FLAG_REQ req, enum POWER_OPT option, unsigned int val)
{
	struct pwr_ctrl *pwrctrl_s = &pwrctrl_sleep;
	int id = SPM_PWR_CTRL_SUSPEND;

	if (option == DPIDLE_OPT) {
		id =  SPM_PWR_CTRL_DPIDLE;
		pwrctrl_s = &pwrctrl_dp;
	}

	if (req == PCM_FLAG_SET) {
		pwrctrl_s->pcm_flags_cust_set |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS_CUST_SET, val);
	} else if (req == PCM_FLAG_CLR) {
		pwrctrl_s->pcm_flags_cust_clr |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS_CUST_CLR, val);
	} else if (req == PCM_FLAG1_SET) {
		pwrctrl_s->pcm_flags1_cust_set |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS1_CUST_SET, val);
	} else if (req == PCM_FLAG1_CLR) {
		pwrctrl_s->pcm_flags1_cust_clr |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS1_CUST_CLR, val);
	} else if (req == PCM_FLAG2_SET) {
		pwrctrl_s->pcm_flags2_cust_set |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2_CUST_SET, val);
	} else if (req == PCM_FLAG2_CLR) {
		pwrctrl_s->pcm_flags2_cust_clr |= val;
		SMC_CALL(PWR_CTRL_ARGS,	id, PW_PCM_FLAGS2_CUST_SET, val);
	}

	return 0;
}
EXPORT_SYMBOL(sloa_flag_req);
...
int sloa_26m_on(bool on)
{
	...
	if (on) {
		pwrctrl_s->pcm_flags1_cust_set = 0x140000;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
			PW_PCM_FLAGS1_CUST_SET, 0x140000);
	} else {
		pwrctrl_s->pcm_flags1_cust_clr = 0x140000;
		SMC_CALL(PWR_CTRL_ARGS,	SPM_PWR_CTRL_SUSPEND,
			PW_PCM_FLAGS1_CUST_CLR, 0x140000);
	}

	return 0;
}
EXPORT_SYMBOL(sloa_26m_on);
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	len += snprintf(buf + len, BUF_SIZE - len,
			"infra1_mask: echo infra1_mask 0xx > %s\n", d);
				  ...
				  size_t count, loff_t *f_pos)
{
	...
	if (sscanf(buf, "%31s %d", cmd, &param) == 2) {
		if (!strcmp(cmd, "dpidle"))
			...
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_LOG_EN, param);
		else if (!strcmp(cmd, "infra1_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_INFRA1_IDLE_MASK, param);
		else if (!strcmp(cmd, "infra0_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_INFRA0_IDLE_MASK, param);
		else if (!strcmp(cmd, "infra2_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_INFRA2_IDLE_MASK, param);
		else if (!strcmp(cmd, "infra3_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_INFRA3_IDLE_MASK, param);
		else if (!strcmp(cmd, "infra4_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_INFRA4_IDLE_MASK, param);
		else if (!strcmp(cmd, "mmsys0_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_MMSYS0_IDLE_MASK, param);
		else if (!strcmp(cmd, "mmsys_oth_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_MMSYS_OTH_IDLE_MASK, param);
		else if (!strcmp(cmd, "image0_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_IMAGE0_IDLE_MASK, param);
		else if (!strcmp(cmd, "image1_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_IMAGE1_IDLE_MASK, param);
		else if (!strcmp(cmd, "image_oth_mask"))
			SMC_CALL(PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				 PW_IMAGE_OTH_IDLE_MASK, param);
