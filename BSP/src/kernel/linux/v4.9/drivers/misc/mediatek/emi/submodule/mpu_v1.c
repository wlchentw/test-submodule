...
{
	unsigned int mpus, mput;

	/* clear violation status */
	emi_mpu_smc_write(EMI_MPUP_OFSET, 0x00FF03FF);
	emi_mpu_smc_write(EMI_MPUQ_OFSET, 0x00FF03FF);
	emi_mpu_smc_write(EMI_MPUR_OFSET, 0x00FF03FF);
	emi_mpu_smc_write(EMI_MPUY_OFSET, 0x00FF03FF);

	/* clear debug info */
	emi_mpu_smc_write(EMI_MPUS_OFSET, 0x80000000);

	mpus = (unsigned long long)emi_mpu_smc_read(EMI_MPUS_OFSET);
	mput = (unsigned long long)emi_mpu_smc_read(EMI_MPUT_OFSET);
	if (mpus) {
...
static void check_violation(void)
{
	...
	unsigned int region;
	unsigned int w_vio, r_vio;
	unsigned long long vio_addr;
	const char *master_name;
	mpus = (unsigned long long)emi_mpu_smc_read(EMI_MPUS_OFSET);
	mput = (unsigned long long)emi_mpu_smc_read(EMI_MPUT_OFSET);
	vio_addr = mput + EMI_MPU_ADDR_OFFSET;

	/* decode EMI_MPUS */
	master_id = mpus & 0x1FFF;
	domain_id = (mpus >> 21) & 0x3;
	region = (mpus >> 16) & 0x7;
	w_vio = (mpus >> 28) & 0x1;
	r_vio = (mpus >> 29) & 0x1;
	port_id = master_id & 0x7;
	axi_id = (master_id >> 3) & 0x3FF;
	master_name = id2name(axi_id, port_id);

	pr_info("[MPU] EMI MPU violation\n");
	pr_info("[MPU] MPUS: %x, MPUT: %x\n", mpus, mput);
	 ...
	 */
	pr_info("[MPU] violation domain 0x%x\n", domain_id);
...
int emi_mpu_set_protection(struct emi_region_info_t *region_info)
{
	if (region_info->region >= EMI_MPU_REGION_NUM) {
		...
		emi_mpu_smc_protect(region_info->start, region_info->end,
