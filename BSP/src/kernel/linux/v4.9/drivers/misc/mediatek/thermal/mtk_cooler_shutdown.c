...
static ssize_t _mtk_cl_sd_rst_write(struct file *filp,
...
static ssize_t _mtk_cl_sd_pid_write(struct file *filp,
	const char __user *buf, size_t len, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
...
static ssize_t _mtk_cl_sd_debouncet_write(struct file *filp,
	const char __user *buf, size_t len, loff_t *data)
{
	char desc[MAX_LEN] = { 0 };
	int tmp_dbt = -1;
...
static int mtk_cl_shutdown_get_max_state(struct thermal_cooling_device *cdev,
	unsigned long *state)
{
	*state = 1;
	return 0;
...
static int mtk_cl_shutdown_get_cur_state(struct thermal_cooling_device *cdev,
	unsigned long *state)
{
...
static int mtk_cl_shutdown_set_cur_state(struct thermal_cooling_device *cdev,
	unsigned long state)
{
	struct sd_state *cl_state = (struct sd_state *) cdev->devdata;
#if defined(MTK_COOLER_SHUTDOWN_SIGNAL)
...
static int __init mtk_cooler_shutdown_init(void)
{
...
#if defined(MTK_COOLER_SHUTDOWN_SIGNAL)
	{
		...
		entry =
		    proc_create("clsd_pid", 0664, dir_entry,
				&_cl_sd_pid_fops);
		if (!entry)
			mtk_cooler_shutdown_dprintk(
		...
		entry =
			proc_create("clsd_rst", 0664, dir_entry,
			&_cl_sd_rst_fops);
		if (!entry)
			mtk_cooler_shutdown_dprintk(
		...
		entry =
			proc_create("clsd_dbt", 0664, dir_entry,
