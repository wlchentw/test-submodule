...
#define mtkTTimer_dprintk(fmt, args...)              \
	do {                                             \
		if (mtkTTimer_debug_log)                     \
			pr_info("[Thermal timer] " fmt, ##args); \
	} while (0)
...
int mtkTTimer_register(const char *name, void (*start_timer)(void),
		       void (*cancel_timer)(void))
{
	int index;
	...
	if (name) {
		if (strlen(name) >= NAME_LEN) {
			...
			aee_kernel_warning_api(
				...
				"mtkTTimer_register", "Name is too long");
#endif
...
#ifdef CONFIG_MTK_AEE_FEATURE
		aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
				       "mtkTTimer_register", "Array is full");
#endif
...
int mtkTTimer_unregister(const char *name)
{
	...
	if (name) {
		if (strlen(name) >= NAME_LEN) {
			...
			aee_kernel_warning_api(
				__FILE__, __LINE__, DB_OPT_DEFAULT,
				"mtkTTimer_unregister", "Name is too long");
#endif
			mtkTTimer_dprintk("[%s] Name is too long\n", __func__);
			return -1;
		}
