...
int __attribute__((weak)) set_bat_charging_current_limit(int current_limit)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}
...
int __attribute__((weak)) mtk_chr_get_soc(unsigned int *soc)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_ui_soc(unsigned int *ui_soc)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_vbat(unsigned int *vbat)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_ibat(unsigned int *ibat)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_vbus(unsigned int *vbus)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_aicr(unsigned int *aicr)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}

int __attribute__((weak)) mtk_chr_get_tchr(int *min_temp, int *max_temp)
{
	pr_info("E_WF: %s doesn't exist\n", __func__);
	return 0;
}
...
#define MTK_CL_BCCT_SET_LIMIT(limit, state)                                    \
	{                                                                      \
		(state) = ((((unsigned long)(state)) & 0xFFFF) |               \
			   ((short)limit << 16));                              \
	}
...
static struct chrlmt_handle *chrlmt_registered_users[CHR_LMT_MAX_USER_COUNT] = {
	0};
...
static int chrlmt_set_limit(struct chrlmt_handle *handle,
			    int chr_input_curr_limit, int bat_char_curr_limit)
{
	int i;
...
static int mtk_cooler_bcct_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_BCCT; i-- > 0;) {
		...
		cl_bcct_dev[i] = mtk_thermal_cooling_device_register(
			temp, (void *)&cl_bcct_state[i], &mtk_cl_bcct_ops);
	}
...
static int mtk_cl_abcct_get_max_state(struct thermal_cooling_device *cdev,
				      unsigned long *state)
{
	*state = 1;
	mtk_cooler_bcct_dprintk("%s %s %lu\n", __func__, cdev->type, *state);
...
static int mtk_cl_abcct_get_cur_state(struct thermal_cooling_device *cdev,
				      unsigned long *state)
{
...
static int mtk_cl_abcct_set_cur_state(struct thermal_cooling_device *cdev,
				      unsigned long state)
{
	int ret = 0;
	...
	if (cl_bcct_klog_on == 1) {
		...
		if (ret)
			mtk_cooler_bcct_dprintk("bat_info_aicr: %d err: %d\n",
						bat_info_aicr, ret);
		/*
		 *	ret = mtk_chr_get_tchr(&bat_info_mintchr,
		 *&bat_info_maxtchr);
		 *	if (ret)
		 *		mtk_cooler_bcct_dprintk("mtk_chr_get_tchr: %d %d
		 *err: %d\n",
...
static int mtk_cl_abcct_set_cur_temp(struct thermal_cooling_device *cdev,
				     unsigned long temp)
{
	...
	mtk_cooler_bcct_dprintk("%s %ld %ld %ld %ld %ld %d %d\n", __func__,
				abcct_curr_temp, pterm, abcct_iterm, dterm,
...
static int
mtk_cl_abcct_lcmoff_get_max_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
	*state = 1;
	mtk_cooler_bcct_dprintk("%s %s %lu\n", __func__, cdev->type, *state);
...
static int
mtk_cl_abcct_lcmoff_get_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
...
static int
mtk_cl_abcct_lcmoff_set_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long state)
{
...
static int mtk_cl_abcct_lcmoff_set_cur_temp(struct thermal_cooling_device *cdev,
					    unsigned long temp)
{
	long delta, pterm, dterm;
	int limit;
	...
	if (!cl_abcct_dev)
		cl_abcct_dev =
			mtk_thermal_cooling_device_register_wrapper_extra(
				"abcct", (void *)NULL, &mtk_cl_abcct_ops,
...
static ssize_t _cl_bcct_write(struct file *filp, const char __user *buf,
			      size_t len, loff_t *data)
{
	 ...
	 * <mtk-cl-bcct00 limit> can only be positive integer or -1 to denote no
	 * limit
	 */
...
static ssize_t _cl_abcct_write(struct file *filp, const char __user *buf,
			       size_t len, loff_t *data)
{
	/* int ret = 0; */
	char tmp[128] = {0};
...
static int _cl_abcct_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%d %ld %ld %ld %ld %d %d\n",
		   abcct_cur_bat_chr_curr_limit, abcct_target_temp, abcct_kp,
		   abcct_ki, abcct_kd, abcct_max_bat_chr_curr_limit,
...
static ssize_t _cl_abcct_lcmoff_write(struct file *filp, const char __user *buf,
				      size_t len, loff_t *data)
{
	/* int ret = 0; */
	char tmp[128] = {0};
	int _lcmoff_policy_enable;
	long _abcct_lcmoff_target_temp, _abcct_lcmoff_kp, _abcct_lcmoff_ki,
		_abcct_lcmoff_kd;
	...
	if (sscanf(tmp, "%d %ld %ld %ld %ld %d %d", &_lcmoff_policy_enable,
		   &_abcct_lcmoff_target_temp, &_abcct_lcmoff_kp,
		   &_abcct_lcmoff_ki, &_abcct_lcmoff_kd, &_max_cur,
		   &_min_cur) >= 7) {
...
static int bcct_lcmoff_fb_notifier_callback(struct notifier_block *self,
...
static int _cl_battery_status_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%d,%d,%d,%d,%d,%d,%d,%d\n", bat_info_soc, bat_info_uisoc,
		   bat_info_vbat, bat_info_ibat, bat_info_mintchr,
		   bat_info_maxtchr, bat_info_vbus, bat_info_aicr);
...
static int __init mtk_cooler_bcct_init(void)
{
	...
	/* create a proc file */
	{
		...
		entry = proc_create("clabcct_lcmoff", 0664, dir_entry,
				    &_cl_abcct_lcmoff_fops);
		if (!entry)
			mtk_cooler_bcct_dprintk_always(
	...
	bcct_chrlmt_queue =
		alloc_workqueue("bcct_chrlmt_work",
