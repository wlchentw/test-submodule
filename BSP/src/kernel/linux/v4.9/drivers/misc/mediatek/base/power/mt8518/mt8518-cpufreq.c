...
unsigned int _mt_cpufreq_get_cpu_level(void)
{
	...
	val = get_devinfo_with_index(3) & 0x7;
...
static int mtk_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	/* 1.5G  is default opp table in dts
	 * 1.4G: [1404, 1209, 1040, 806, 598]MHz
	 * there are variant opp tables need to be updated
	 */
	if (lv == 0 || lv == 3) {
		/* efuse = 0, default 1.5G */
		/* efuse = 3, 1.5G */
	} else if (lv == 6) {
		/* efuse = 6, 1.4G */
		dev_pm_opp_remove(info->cpu_dev, 1508000000);
		dev_pm_opp_add(info->cpu_dev, 1404000000, 1125000);
	} else {
		/* will not appear in mt8518 now */
		/* efuse = 1, 1.8G */
		/* efuse = 2, 1.6G */
		/* efuse = 4, 1.3G */
		/* efuse = 5, 1.0G */
		pr_debug("%s: invalid efuse value: %d\n", __func__, lv);
		ret = -EINVAL;
...
static struct cpufreq_driver mt8518_cpufreq_driver = {
...
static int mt8518_cpufreq_probe(struct platform_device *pdev)
{
	struct mtk_cpu_dvfs_info *info;
	...
	ret = cpufreq_register_driver(&mt8518_cpufreq_driver);
	if (ret) {
...
static struct platform_driver mt8518_cpufreq_platdrv = {
	.driver = {
		.name	= "mt8518-cpufreq",
	},
	.probe		= mt8518_cpufreq_probe,
};

static int mt8518_cpufreq_driver_init(void)
{
	...
	err = platform_driver_register(&mt8518_cpufreq_platdrv);
	if (err)
		return err;
	 ...
	 * probing.
	 */
	pdev = platform_device_register_simple("mt8518-cpufreq", -1, NULL, 0);
	if (IS_ERR(pdev)) {
...
device_initcall(mt8518_cpufreq_driver_init);
