...
static void musb_vbus_detect_work(struct work_struct *data)
{
	...
	if (val) {
		pr_info("isr happenned since VBUS DET GPIO: %i, HIGH\n", val);

#ifdef CONFIG_USB_PHYCHK_EXTCONN
		usb_extconntype = mt_usb_phychk_extconn();
		if (usb_extconntype == STANDARD_HOST)
			mt_usb_connect();
#else
		mt_usb_connect();
#endif
		...
		pr_info("isr happenned since VBUS DET GPIO: %i, LOW\n", val);
		vbus_deboun_time = 500;

		mt_usb_disconnect();
#ifdef CONFIG_USB_PHYCHK_EXTCONN
		usb_extconntype = CHARGER_UNKNOWN;
#endif
		irq_set_irq_type(usb_vbus_detect_number, IRQF_TRIGGER_HIGH);
	}
	enable_irq(usb_vbus_detect_number);
...
static irqreturn_t mt_usb_vbus_detect_int(int irq, void *dev_id)
{
	...
	if (!mtk_musb->is_ready)
		...
		schedule_delayed_work(&mtk_musb->vd_pin_work,
				      vbus_deboun_time*HZ/1000);

	DBG(0, "vbus pin interrupt assert\n");
...
static void otg_int_init(void)
{
...
#if defined(CONFIG_USB_VBUSDET_IN_GPIO) && \
	!defined(CONFIG_MTK_MUSB_SW_WITCH_MODE)
	pinctrl_vbus_detect = pinctrl_lookup_state(pinctrl,
	...
	pr_info("====> usb_vbus_detect_number IRQF_TRIGGER_HIGH!!!\n");

	ret = request_irq(usb_vbus_detect_number, mt_usb_vbus_detect_int,
			  IRQF_TRIGGER_HIGH, "USB_VBUSD", NULL);
	if (ret > 0)
		DBG(0, "USB IDDIG IRQ LINE not available!!\n");
	else
		DBG(0, "USB IDDIG IRQ LINE available!!\n");
...
#ifdef CONFIG_OF
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8518-usb20");
	if (node == NULL) {
...
#endif
	pr_info("====> iddig_gpio virtual Number: %u !!!\n", iddig_pin);

#if defined(CONFIG_USB_VBUSDET_IN_GPIO) && \
	!defined(CONFIG_MTK_MUSB_SW_WITCH_MODE)
	usb_vbus_detect_pin = of_get_named_gpio(node, "vbusdetect_gpio", 0);
	if (!gpio_is_valid(usb_vbus_detect_pin))
		pr_info("get dtsi vbusdetect_gpio fail since %i\n",
			usb_vbus_detect_pin);

	pr_info("====> vbusdetect_gpio virtual Number %u!!!\n",
		usb_vbus_detect_pin);
#endif
	}
#if !defined(CONFIG_MTK_LEGACY)
