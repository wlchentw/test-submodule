...
static const struct of_device_id emi_of_ids[] = {
	{.compatible = "mediatek,mt8518-emi",},
	{}
};
#endif
...
static int emi_probe(struct platform_device *pdev)
{
	...
	i = get_ddr_type();
	if (i == 1)
		pr_info("[EMI] get get_ddr_type @ LPDDR3\n");
	else if (i == 2)
		pr_info("[EMI] get get_ddr_type @ LPDDR4\n");
	else if (i == 3)
		pr_info("[EMI] get get_ddr_type @ DDR3\n");
	else if (i == 4)
		pr_info("[EMI] get get_ddr_type @ DDR4\n");
	else
		pr_info("[EMI] get get_ddr_type @ LPDDR3\n");
#if ENABLE_MPU
...
static int __init emi_ctrl_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8518-emi");
		if (!node)
		return -ENODEV;
#if 0
	if (node) {
