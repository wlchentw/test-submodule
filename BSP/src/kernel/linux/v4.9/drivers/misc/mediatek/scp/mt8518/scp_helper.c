...
char *core_ids = {"SCP A"};
DEFINE_SPINLOCK(scp_sem_spinlock);
 ...
 *        -1 :get sema timeout
 */
int get_scp_semaphore(int index)
{
	int read_back;
	int count = 0;
	int ret = -1;
	void __iomem *reg;
	unsigned long spin_flags, mask;

	/* return 1 to prevent from access when driver not ready */
	if (!scpreg.probed || index > SEMAPHORE_MAX_CNT)
		return -1;

	/* spinlock context safe*/
	spin_lock_irqsave(&scp_sem_spinlock, spin_flags);

	mask = index % 32;
	reg = SCP_SEMAPHORE + (index / 32) * 4;
	read_back = (readl(reg) >> mask) & 0x1;
	if (read_back == 0) {
		writel((1 << mask), reg);
	...
	spin_unlock_irqrestore(&scp_sem_spinlock, spin_flags);
 ...
 *        -1 :release sema fail
 */
int release_scp_semaphore(int index)
{
	int read_back;
	int ret = -1;
	void __iomem *reg;
 ...
 * NOTE: this function will never be blocked
 *
 * @param reset:    none
 * @return:         0
 */
int reset_scp(void)
{
	...
	reg = readl(SCP_RESET_CTL);
	reg &= ~CPU_RST_SW;
	writel(reg, SCP_RESET_CTL);

	reg = readl(SCP_CLKGATE_CTL);
	reg &= ~CPU_CLK_EN;
	writel(reg, SCP_CLKGATE_CTL);
...
static int scp_device_probe(struct platform_device *pdev)
{
	...
	scpreg.clk_sel = devm_clk_get(dev, "cmsyssel");
	if (IS_ERR(scpreg.clk_sel))
		return PTR_ERR(scpreg.clk_sel);

	ret = clk_prepare_enable(scpreg.clk_sel);
	if (ret) {
		pr_err("Failed to enable cmsys sel clk (%d)\n", ret);
		return ret;
	}

	scpreg.clk_bus = devm_clk_get(dev, "cmsysbus");
	if (IS_ERR(scpreg.clk_bus))
		return PTR_ERR(scpreg.clk_bus);

	ret = clk_prepare_enable(scpreg.clk_bus);
	if (ret) {
		pr_err("Failed to enable cmsys bus clk (%d)\n", ret);
		return ret;
	}

	scpreg.clk_sys = devm_clk_get(dev, "cmsys");
	if (IS_ERR(scpreg.clk_sys))
		return PTR_ERR(scpreg.clk_sys);

	ret = clk_prepare_enable(scpreg.clk_sys);
	if (ret) {
		pr_err("Failed to enable cmsys clk (%d)\n", ret);
		return ret;
	}
...
static int __init scp_init(void)
{
	...
	if (scp_mem_size == 0) {
		...
		pr_err("[SCP] continue to go........\n");
		//return -1;
	}

	scp_workqueue = create_workqueue("SCP_WQ");
	if (!scp_workqueue)
		return PTR_ERR(scp_workqueue);
