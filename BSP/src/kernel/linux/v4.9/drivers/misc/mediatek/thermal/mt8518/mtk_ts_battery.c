...
/* Weak functions */
/* ************************************ */
int __attribute__((weak)) mtk_charger_manager_register_notifier(
...
struct charger_consumer *pthermal_consumer;
...
static int get_hw_battery_temp(void)
{
	...
	ret = mtk_charger_manager_get_tbat(pthermal_consumer, &val);
	ret = val * 10;
#endif
...
/* static int mtktsbattery_read(char *buf, char **start, off_t off, int count,
 * int *eof, void *data)
 */
{
...
static ssize_t mtktsbattery_write(struct file *file, const char __user *buffer,
				  size_t count, loff_t *data)
/* static ssize_t mtktsbattery_write(struct file *file, const char *buffer, int
 * count, void *data)
 */
{
	int len = 0, i;
	...
	struct mtktsbattery_data *ptr_mtktsbattery_data =
		kmalloc(sizeof(*ptr_mtktsbattery_data), GFP_KERNEL);
	...
	len = (count < (sizeof(ptr_mtktsbattery_data->desc) - 1))
		      ? count
		      : (sizeof(ptr_mtktsbattery_data->desc) - 1);
	...
	if (sscanf(ptr_mtktsbattery_data->desc,
		   "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		   &num_trip, &ptr_mtktsbattery_data->trip[0],
...
int mtktsbattery_register_cooler(void)
{
	...
	cl_dev_sysrst = mtk_thermal_cooling_device_register(
		"mtktsbattery-sysrst", NULL, &mtktsbattery_cooling_sysrst_ops);
	return 0;
}
...
#if 0
	mtkTTimer_register("mtktsbattery",
		mtkts_battery_start_thermal_timer,
		mtkts_battery_cancel_thermal_timer);
#endif

	pthermal_consumer = mtk_charger_manager_get_consumer(&thz_dev->device);
