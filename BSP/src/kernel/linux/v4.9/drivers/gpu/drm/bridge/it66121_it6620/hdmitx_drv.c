...
RegSetEntry Pattern_Generator_Table[] = {
	{0x0F, 0x01, 0x00}, {0x90, 0xFF, 0x90}, {0x91, 0xFF, 0x35},
	{0x92, 0xFF, 0x7A}, {0x93, 0xFF, 0x4A}, {0x94, 0xFF, 0x30},
	{0x95, 0xFF, 0x00}, {0x96, 0xFF, 0x3E}, {0x97, 0xFF, 0x00},
	{0x98, 0xFF, 0x0C}, {0x99, 0xFF, 0x02}, {0x9A, 0xFF, 0x23},
	{0x9B, 0xFF, 0x03}, {0x9C, 0xFF, 0x20}, {0x9D, 0xFF, 0xFF},
	{0x9E, 0xFF, 0xFF}, {0x9F, 0xFF, 0xFF}, {0xA0, 0xFF, 0x00},
	{0xA1, 0xFF, 0x60}, {0xA2, 0xFF, 0xFF}, {0xA3, 0xFF, 0xFF},
	{0xA4, 0xFF, 0xAD}, {0xA5, 0xFF, 0x01}, {0xA6, 0xFF, 0xF0},
	{0xB1, 0xFF, 0x00}, {0xB2, 0xFF, 0x00}, {0xA8, 0xFF, 0x01},
	{0, 0, 0} };
...
/* //////////////////////////////////////////////////////////////// */
	...
	/* [7:6]=REGAudSWL, [5]=REGSPDIFTC,*/
	/*[4]=REGAudSel, [3:0]=REGAudioEn */
	HDMITX_WriteI2C_Byte(0xE1, 0x01);
	HDMITX_WriteI2C_Byte(0xE2, 0x00);
	/* SRC0/1/2/3 maps to SRC0 */
	...
	/* AVI Packet */
	HDMITX_WriteI2C_Byte(0x58, 0x20);
	/* D[6:5] color mode */
	HDMITX_WriteI2C_Byte(0x59, 0x08);
	HDMITX_WriteI2C_Byte(0x5A, 0x00);
	HDMITX_WriteI2C_Byte(0x5B, 0x04);
	/* Video Format Code */
	HDMITX_WriteI2C_Byte(0x5C, 0x00);
	HDMITX_WriteI2C_Byte(0x5D, 0x43);
	/* CheckSum */
	...
	/* AUDIO Info Frame */
	HDMITX_WriteI2C_Byte(0x68, 0x07);
/* should be 00, temp assign for solving MulCh problem */
...
bool getHDMITX_LinkStatus(void)
{
	...
	if (B_TX_RXSENDETECT & reg1) {
		...
		if (reg2 == 0) {
			IT66121_LOG(
				"getHDMITX_LinkStatus(reg[0E]=%x, reg[61]=%x) OK!!\n",
				reg1, reg2);
			return TRUE;
		}
	}
...
/* %X\n",(int)REG_TX_INT_STAT2,(int)intdata2); */
		...
		HDMITX_WriteI2C_Byte(REG_TX_SYS_STATUS,
				     intclr3); /* clear interrupt. */
		intclr3 &= ~(B_TX_INTACTDONE);
		HDMITX_WriteI2C_Byte(REG_TX_SYS_STATUS,
				     intclr3); /* INTACTDONE reset to zero. */
	}
	/*  */
	/* else */
	/* { */
...
/*	(int)table[i].offset,(int)table[i].invAndMask,(int)table[i].OrMask); */
			HDMITX_SetI2C_Byte(table[i].offset, table[i].invAndMask,
					   table[i].OrMask);
		}
	}
}
...
#if SOFTWARE_I2C
	IT66121_LOG("software I2C\n");
	if (EDIDBlockID < 2) {
		if (fgDDC2DataRead(0x50, (EDIDBlockID % 2) * 128,
			128, pEDIDData) != TRUE)
			return FALSE;
	} else {
		if (fgDDC2DataRead(0x60, (EDIDBlockID % 2) * 128,
			128, pEDIDData) != TRUE)
			return FALSE;
	}
#else
			       ...
			       unsigned char offset, short Count)
{
	...
	while (RemainedCount > 0) {
		...
		ReqCount = (RemainedCount > DDC_FIFO_MAXREQ) ? DDC_FIFO_MAXREQ
							     : RemainedCount;
		/* HDMITX_DEBUG_PRINTF("getHDMITX_EDIDBytes(): */
...
/* HDMITX_DEBUG_PRINTF("start getting EDID data via DDC..\n");*/
		...
		HDMITX_WriteI2C_Byte(
			REG_TX_DDC_HEADER,
			DDC_EDID_ADDRESS); /* for EDID ucdata get */
...
/* HDMITX_AndReg_Byte(REG_TX_INT_CTRL,~(1<<1));*/
		...
		if (TimeOut == 0) {
			HDMITX_DEBUG_PRINTF(
				"getHDMITX_EDIDBytes(): DDC TimeOut.\n");
			/* HDMITX_AndReg_Byte(REG_TX_INT_CTRL,~(1<<1)); */
			return ER_FAIL;
		}
		do {
...
void hdmitx_AbortDDC(void)
{
	...
	HDMITX_WriteI2C_Byte(
		REG_TX_HDCP_DESIRE,
		CPDesire & (~B_TX_CPDESIRE)); /* @emily change order */
	HDMITX_WriteI2C_Byte(
		REG_TX_SW_RST,
		SWReset | B_TX_HDCP_RST_HDMITX); /* @emily change order */
	HDMITX_WriteI2C_Byte(REG_TX_DDC_MASTER_CTRL,
			     B_TX_MASTERDDC | B_TX_MASTERHOST);
...
void HDMITX_DisableVideoOutput(void)
{
	...
	unsigned char uc =
		HDMITX_ReadI2C_Byte(REG_TX_SW_RST) | B_HDMITX_VID_RST;
...
/* VIDEOPCLKLEVEL level ; */
	...
	HDMITX_WriteI2C_Byte(REG_TX_SW_RST, B_HDMITX_AUD_RST | B_TX_AREF_RST |
...
bool setHDMITX_SyncEmbeddedByVIC(unsigned char VIC, unsigned char bInputType)
{
	...
	/* if( bInputSignalType & T_MODE_SYNCEMB ) */
	{
		...
		HDMITX_WriteI2C_Byte(0xa6,
				     (VRE2nd & 0xF0) + ((VRE & 0xF0) >> 4));
			 ...
			 unsigned char bInputSignalType)
{
	...
	ucData &= ~(M_TX_INCOLMOD | B_TX_2X656CLK | B_TX_SYNCEMB | B_TX_INDDR |
		    B_TX_PCLKDIV2);
...
void hdmitx_SetCSCScale(unsigned char bInputMode, unsigned char bOutputMode)
{
	...
	unsigned char i;
	unsigned char filter = 0; /* filter is for Video CTRL*/
				     /*DN_FREE_GO,EN_DITHER,and ENUDFILT */
...
/* (2) YUV444/422  in,RGB output (CSC enable,and output is not YUV422) */
	...
	HDMITX_DEBUG_PRINTF(
		"hdmitx_SetCSCScale(unsigned char bInputMode = %x,unsigned char bOutputMode = %x)\n",
	...
	ucData = HDMITX_ReadI2C_Byte(REG_TX_CSC_CTRL) &
		 ~(M_TX_CSC_SEL | B_TX_DNFREE_GO | B_TX_EN_DITHER |
		   B_TX_EN_UDFILTER);
	ucData |= filter | csc;
...
void setHDMITX_ChStat(unsigned char ucIEC60958ChStat[])
{
	...
	HDMITX_WriteI2C_Byte(REG_TX_AUDCHST_CAT,
			     ucIEC60958ChStat[1]); /* 192, audio CATEGORY */
	...
	HDMITX_WriteI2C_Byte(REG_TX_AUDCHST_CA_FS,
			     ucIEC60958ChStat[3]); /* choose clock */
...
void setHDMITX_NLPCMAudio(bool bSPDIF) /* no Source Num, no I2S. */
{
	...
	HDMITX_WriteI2C_Byte(REG_TX_AUDIO_CTRL1,
			     0x01);
	/* regE1 bOutputAudioMode should be loaded from ROM image. */
	HDMITX_WriteI2C_Byte(REG_TX_AUDIO_FIFOMAP, 0xE4); /* default mapping. */
			      ...
			      unsigned char *pIEC60958ChStat, ULONG TMDSClock)
{
	...
	HDMITX_DEBUG_PRINTF1(
		"HDMITX_EnableAudioOutput(%02X, %s, %ld, %d, %p, %ld);\n",
	...
	case T_AUDIO_LPCM:
		...
		setHDMITX_ChStat(pIEC60958ChStat);
		setHDMITX_LPCMAudio((ChNum + 1) / 2, SUPPORT_AUDI_AudSWL,
				    bSPDIF);
		/* can add auto adjust */
		break;
	}
...
void hdmitx_AutoAdjustAudio(void)
{
	...
	if (bForceCTS) {
		...
		HDMITX_AndReg_Byte(
			...
			~B_TX_SW_CTS); /* D[1] = 0, HW auto count CTS */
...
int hdmiTx_pattern_interface(void)
{
	IT66121_LOG(">>> %s\n", __func__);
	...
	HDMITX_PowerOn();
	mdelay(10);
	hdmitx_LoadRegSetting(Pattern_Generator_Table);
	mdelay(10);
	//HDMITX_SetOutput();
/**********trigger HDMI mode*********************************/
	Switch_HDMITX_Bank(0);
	HDMITX_WriteI2C_Byte(REG_TX_HDMI_MODE, B_TX_HDMI_MODE);
/**********trigger send avi infoframe*********************************/
	Switch_HDMITX_Bank(0);
	hdmitx_ENABLE_AVI_INFOFRM_PKT();
/*******************************************/
	setHDMITX_AVMute(FALSE);
