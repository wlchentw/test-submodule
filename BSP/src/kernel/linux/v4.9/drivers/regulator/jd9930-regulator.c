...
#define JD9930_VPOS_NEG_MIN_UV	7040000
#define JD9930_VPOS_NEG_MAX_UV	15060000
static const int giJD9930_vpos_neg_uVA[] = {
	JD9930_VPOS_NEG_MIN_UV, // <=04H ,
	7260000, // 05H ,
	7490000, // 06H ,
	7710000, // 07H ,
	7930000, // 08H ,
	...
	14610000, // 26H ,
	14830000, // 27H ,
	JD9930_VPOS_NEG_MAX_UV, // 28H ,
};
#define JD9930_VPOSNEG_REG_TO_uV(reg) \
({\
	int iuV;\
	if(reg<=0x04) {\
		iuV = JD9930_VPOS_NEG_MIN_UV;\
	}\
	else if(reg>=0x28) {\
		iuV = JD9930_VPOS_NEG_MAX_UV;\
	}\
	else {\
...
#define JD9933_VPOS_NEG_MIN_UV	7030000
#define JD9933_VPOS_NEG_MAX_UV	21001000
static const int giJD9933_vpos_neg_uVA[] = {
	JD9933_VPOS_NEG_MIN_UV, // <=04H ,
	7421000, // 05H ,
	7809000, // 06H ,
	8197000, // 07H ,
	8585000, // 08H ,
	...
	20225000, // 26H ,
	20613000, // 27H ,
	JD9933_VPOS_NEG_MAX_UV, // 28H ,
};
#define JD9933_VPOSNEG_REG_TO_uV(reg) \
({\
	int iuV;\
	if(reg<=0x04) {\
		iuV = JD9933_VPOS_NEG_MIN_UV;\
	}\
	else if(reg>=0x28) {\
		iuV = JD9933_VPOS_NEG_MAX_UV;\
	}\
	else {\
		iuV = giJD9933_vpos_neg_uVA[(reg-4)];\
	}\
	iuV;\
})	

#define JD9933_VPOSNEG_uV_TO_REG(_uV) \
({\
	int _iRegIdx,_iFoundRegIdx=4;\
	for(_iRegIdx=0;\
	 _iRegIdx<sizeof(giJD9933_vpos_neg_uVA)/sizeof(giJD9933_vpos_neg_uVA[0]);\
	 _iRegIdx++) \
	 {\
		if(giJD9933_vpos_neg_uVA[_iRegIdx]>(_uV)) {\
			break;\
		}\
		else {\
			_iFoundRegIdx = _iRegIdx+4;\
					...
					int minuV, int uV, unsigned *selector)
{
	...
	if(3==jd9930->chip_rev) {
		iReg = JD9933_VPOSNEG_uV_TO_REG(uV);
	}
	else {
		iReg = JD9930_VPOSNEG_uV_TO_REG(uV);
	}	
	JD9930_REG_WRITE_EX(jd9930,VPOS_VNEG_SET,iReg);
	DBG_MSG("%s() :chip_rev=%d, iReg=%d,minuV=%d,uV=%d\n",__func__,
			jd9930->chip_rev,iReg,minuV,uV);
		...
		struct regulator_dev *reg)
{
	...
	if(3==jd9930->chip_rev) {
		iuV = JD9933_VPOSNEG_REG_TO_uV(iRegVPosNeg);
	}
	else {
		iuV = JD9930_VPOSNEG_REG_TO_uV(iRegVPosNeg);
	}

	DBG_MSG("%s() : chip_rev=%d,vpos/vneg =%d uV\n",
			__func__,jd9930->chip_rev,iuV);
...
static void jd9930_int_func(struct work_struct *work)
{
	...
	if(iChk<0) {
		pr_err("%s(),get power status failed !\n",__FUNCTION__);
		return ;
	}

	if(bFaults) {
		pr_err("JD9930 faults occured !!,faults=0x%x\n",bFaults);
...
static int jd9930_wait_power_good(struct jd9930 *jd9930,int iWaitON)
{
	...
	if(iRet<0) {
		pr_err("%s():waiting(%d) for PG=%d (%d) timeout\n",
...
static int _jd9930_rals_onoff(struct jd9930 *jd9930,int iIsON)
{
	...
	if(!gpiod_pwrup) {
		pr_err("%s(%d) get pmic_powerup gpio desc failed !\n",__func__,__LINE__);
	}
...
static int jd9930_display_enable(struct regulator_dev *reg)
{
...
#ifdef TURNOFF_PWR_EPD_UPDATED //[
	{
		...
		if(!gpiod_pwrall) {
			pr_debug("%s(%d) get pmic_powerup gpio desc failed !\n",__func__,__LINE__);
		}
		else {
	...
	if(mutex_is_locked(&jd9930->enpwr_mutex)) {
		DBG_MSG("%s() i2c accessing ? \n",__func__);
	}

	mutex_lock(&jd9930->enpwr_mutex);
	...
	do {
		...
		if(iRet<0) {
			pr_warning("%s power on failed %d,retry ...\n",__FUNCTION__,iTryCnt);
#if 1
			pr_warning("reseting JD9930 chip ...\n");
			msleep(JD9930_SLEEP0_MS);
			mutex_unlock(&jd9930->enpwr_mutex);
...
static int jd9930_display_is_enabled(struct regulator_dev *reg)
{
	...
	gpiod_pwrall = gpio_to_desc(jd9930->gpio_pmic_pwrall);
	if(!gpiod_pwrall) {
		pr_warning("%s(%d) get pmic_powerup gpio desc failed !\n",__func__,__LINE__);
	}
	else {
		if(0==gpiod_get_value(gpiod_pwrall)) {
			pr_warning("%s(%d) PG=0 when chip power off\n",__func__,__LINE__);
			return 0;
		}
	}
	...
	if(iChk>=0) {
		...
		pr_err("%s(%d) get power status failed !\n",__func__,__LINE__);
		return 0;
	}
}
...
static void jd9930_setup_timings(struct jd9930 *jd9930)
{
	...
	do {
		...
		{
			// nothing to do .
			pr_debug("%s(),skipped on_delay settings !\n",__FUNCTION__);
			break ;
		}
		...
		if(cur_reg!=new_reg_val) {
			pr_debug("%s(),set poweron delay=0x%x\n",__FUNCTION__,new_reg_val);
			//JD9930_REG_WRITE_EX(jd9930,PWRON_DLY,new_reg_val);
					...
					struct jd9930_platform_data *pdata)
{
	...
	if(of_property_read_u32(pmic_np,"chip_rev",&dwTemp)) {
		jd9930->chip_rev=0;
		//dev_info(&pdev->dev, "failed to get chip_rev property,default will be 0x0\n");
	}
	else {
		jd9930->chip_rev=(unsigned char)dwTemp;
		dev_info(&pdev->dev, "chip_rev=0x%x\n",jd9930->chip_rev);
	}
...
struct regulator * jd9930_get_tmst_regulator(void)
{
	return jd9930_get_regulator_by_id(JD9930_TMST) ;
}
