...
int jd9930_reg_read(struct jd9930 *jd9930,int reg_num, unsigned int *reg_val)
{
	...
	if(mutex_is_locked(&jd9930->enpwr_mutex)) {
		dev_warn(&jd9930_client->dev,"%s() rails powering on ? \n",__func__);
	}


	if(time_is_after_jiffies(jd9930->jiffies_chip_wake)) {
		int iWait_ms_for_chip = jiffies_to_msecs(jd9930->jiffies_chip_wake-jiffies);
		dev_warn(&jd9930_client->dev,"%s() wait %dms for chip ready ... ",
				__func__,iWait_ms_for_chip);
		msleep(iWait_ms_for_chip);
		dev_warn(&jd9930_client->dev,"done\n");
	}
...
int jd9930_reg_write(struct jd9930 *jd9930,int reg_num, const unsigned int reg_val)
{
	...
	do {
		...
		if (result < 0) {
			...
			dev_dbg(&jd9930_client->dev,"%s():reg%d<=0x%x\n",__FUNCTION__,reg_num,reg_val);
		}
		break;
		mdelay(1);
	}while(1);
...
int jd9930_chip_power(struct jd9930 *jd9930,int iIsON)
{
	...
	struct gpio_desc *gpiod_pwrall,*gpiod_xon = NULL,*gpiod_rail;

	if(!jd9930) {
		pr_err("%s(): object error !! \n",__FUNCTION__);
		return -1;
	}

	if (!gpio_is_valid(jd9930->gpio_pmic_pwrall)) {
		pr_err("%s(): no epdc pmic pwrall pin available\n",__FUNCTION__);
		return -2;
	}
	...
	if (gpio_is_valid(jd9930->gpio_pmic_xon_ctrl)) {
		...
		pr_err("%s(%d) gpio_pmic_xon_ctrl (%d) is invalid  !\n",__func__,__LINE__,jd9930->gpio_pmic_xon_ctrl);
	}

	gpiod_rail = gpio_to_desc(jd9930->gpio_pmic_powerup);
	if(!gpiod_rail) {
		pr_err("%s(%d) get pmic_rail gpio desc failed !\n",__func__,__LINE__);
	}
...
static void jd9930_gpio_free(struct device *dev)
{

	struct jd9930 *jd9930 = dev_get_drvdata(dev);

	if (gpio_is_valid(jd9930->gpio_pmic_pwrall)) {
		devm_gpio_free(jd9930->dev,jd9930->gpio_pmic_pwrall);
		jd9930->gpio_pmic_pwrall = ARCH_NR_GPIOS;
	}

	if ( gpio_is_valid(jd9930->gpio_pmic_powerup) ) {
		devm_gpio_free(jd9930->dev,jd9930->gpio_pmic_powerup);
		jd9930->gpio_pmic_powerup = ARCH_NR_GPIOS;
	}

	if ( gpio_is_valid(jd9930->gpio_pmic_xon_ctrl) ) {
		devm_gpio_free(jd9930->dev,jd9930->gpio_pmic_xon_ctrl);
		jd9930->gpio_pmic_xon_ctrl = ARCH_NR_GPIOS;
	}

	if ( gpio_is_valid(jd9930->gpio_pmic_ts_en) ) {
		devm_gpio_free(jd9930->dev,jd9930->gpio_pmic_ts_en);
		jd9930->gpio_pmic_ts_en = ARCH_NR_GPIOS;
	}
			  ...
			  struct i2c_board_info *info)
{
	...
	if(time_is_after_jiffies(jd9930->jiffies_chip_wake)) {
		int iWait_ms_for_chip = jiffies_to_msecs(jd9930->jiffies_chip_wake-jiffies);
		dev_warn(&adapter->dev,"%s() wait %dms for chip ready ... ",
				__func__,iWait_ms_for_chip);
		msleep(iWait_ms_for_chip);
		dev_warn(&adapter->dev,"done\n");
	}
