...
static int mmc_sleep(struct mmc_host *host)
{
	...
	/* The sleep/awake timeout is too long, use checking card_busy()
	 * instead of delay sleep/awake timeout.
	 */
	...
	if (host->ops->card_busy) {
		do {
			...
			if (expired && busy) {
				pr_notice("%s: Card stuck being busy! %s\n",
					mmc_hostname(host), __func__);
				return err;
			}

			/* Throttle the polling rate to
