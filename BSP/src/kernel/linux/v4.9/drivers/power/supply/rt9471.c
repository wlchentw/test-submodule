...
static void rt9471_buck_dwork_handler(struct work_struct *work)
{
	...
	if (ret < 0)
		return;
	dev_info(chip->dev,
		"%s chg_done = %d, chg_done_once = %d, sys_min = %d\n",
		__func__, chg_done, chip->chg_done_once, sys_min);
	if (sys_min)
		reg_vals[1] = 0x2D;
	else
		reg_vals[1] = 0x2F;
...
static int rt9471_parse_dt(struct device *dev, struct rt9471_chip *chip)
{
	...
	if (of_property_read_u32(np, "vac-ovp", &desc->vac_ovp) < 0)
...
static int __rt9471_dump_registers(struct rt9471_chip *chip)
{
	...
	if (stats[RT9471_STATIDX_STAT2] & RT9471_ST_JEITACOLD_MASK)
		batt_val.intval = -10;
	else if (stats[RT9471_STATIDX_STAT2] & RT9471_ST_JEITACOOL_MASK)
		batt_val.intval = 5;
	else if (stats[RT9471_STATIDX_STAT2] & RT9471_ST_JEITAWARM_MASK)
		batt_val.intval = 50;
	else if (stats[RT9471_STATIDX_STAT2] & RT9471_ST_JEITAHOT_MASK)
		batt_val.intval = 65;
	else
		batt_val.intval = 25;
	ret = power_supply_set_property(batt_psy, POWER_SUPPLY_PROP_TEMP,
					&batt_val);
	ret = power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_CAPACITY,
					&batt_val);
