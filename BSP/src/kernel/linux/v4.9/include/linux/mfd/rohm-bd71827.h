...
/*
 * When GPIO2_MODE bits are set to 2'b10 (LDO5_VSEL), LDO5 output voltage is
 * controlled by GPIO2 pin.
 ...
 * When GPIO2_MODE bits are not set to 2'b10, LDO5 output voltage corresponds
 * to the setting of LDO5_L bits.
 */
...
#define LDO5VSEL_EQ_H		0
	...
	#error define LDO5VSEL_EQ_H to 1 when connect to High, \
		to 0 when connect to Low
#else
	#if LDO5VSEL_EQ_H == 1
...
#define bd71827_debug(debug, fmt, arg...) \
	do { \
		if (debug & bd71827_debug_mask) \
			pr_notice("BD7181x:" fmt, ##arg); \
	} while (0)
