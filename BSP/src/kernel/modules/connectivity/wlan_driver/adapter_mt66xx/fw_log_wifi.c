...
#if 0
	//willcai
	typedef int (*connsys_log_read_to_user_fp) (int conn_type, char __user *buf, size_t count);
	connsys_log_read_to_user_fp connsys_log_read_to_user_function =
			(connsys_log_read_to_user_fp)kallsyms_lookup_name("connsys_log_read_to_user");
	if (connsys_log_read_to_user_function == 0)
		{
		WIFI_ERR_FUNC("Fail to get mtk_wcn_wmt_func1_on \n");
		return ret ;
	    }
	//end
#endif
