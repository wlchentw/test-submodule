 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_writeb(UINT32 ctx, UINT32 offset, UINT8 vb)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
#else
	INT32 ret = -(INT32)HIF_SDIO_ERR_FAIL;
	INT32 probe_index = -1;
	struct sdio_func *func = 0;
#endif
	INT32 retVal = -1;
	...
	/* 4 <2> */
	retVal = wmt_tra_sdio_update();
	HIF_SDIO_LOUD_FUNC("wmt_tra_sdio_update ret(%d)", retVal);
	osal_ftrace_print("%s|S\n", __func__);
	sdio_claim_host(func);
	sdio_writeb(func, vb, offset, &ret);
	sdio_release_host(func);
 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_readl(UINT32 ctx, UINT32 offset, PUINT32 pvl)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_writel(UINT32 ctx, UINT32 offset, UINT32 vl)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_read_buf(UINT32 ctx,
				UINT32 offset, PUINT32 pbuf, UINT32 len)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	...
	osal_ftrace_print("%s|S|L|%d\n", __func__, len);
	sdio_claim_host(func);
	ret = sdio_readsb(func, pbuf, offset, (INT32)len);
	sdio_release_host(func);
	osal_ftrace_print("%s|E|L|%d\n", __func__, len);
...
EXPORT_SYMBOL(hif_sdio_read_buf);
 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_write_buf(UINT32 ctx,
				 UINT32 offset, PUINT32 pbuf, UINT32 len)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	...
	sdio_claim_host(func);
	ret = sdio_writesb(func, offset, pbuf, (INT32)len);
	sdio_release_host(func);
...
EXPORT_SYMBOL(hif_sdio_write_buf);
 ...
 * \retval < 0 list error code here
 */
INT32 hif_sdio_clt_reg(const MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	INT32 ret = -(INT32)HIF_SDIO_ERR_FAIL;
	INT32 clt_index = -1;
	UINT32 i = 0;
	...
	for (i = 0; i < pinfo->func_tbl_size; i++) {
		...
		for (j = 0; j < CFG_CLIENT_COUNT; j++) {
			/* probed spin lock */
			spin_lock_bh(&g_hif_sdio_lock_info.probed_list_lock);
			if (g_hif_sdio_probed_func_list[j].func == NULL) {
				/* probed spin unlock */
				spin_unlock_bh(&g_hif_sdio_lock_info.probed_list_lock);
				continue;
			}
			...
			if ((g_hif_sdio_clt_drv_list[clt_index].func_info->manf_id ==
				...
				/* probed spin unlock */
				spin_unlock_bh(&g_hif_sdio_lock_info.probed_list_lock);
				if (g_hif_sdio_probed_func_list[j]
						.func->num != 1
					|| (g_hif_sdio_probed_func_list[j]
						.on_by_wmt == MTK_WCN_BOOL_TRUE
					&& g_hif_sdio_probed_func_list[j]
						.func->num == 1)) {

				/* use worker thread to perform the client's .hif_clt_probe() */
				clt_probe_worker_info =
			    vmalloc((UINT32)sizeof(MTK_WCN_HIF_SDIO_CLT_PROBE_WORKERINFO));
				if (clt_probe_worker_info != NULL) {
					INIT_WORK(&clt_probe_worker_info->probe_work,
						  hif_sdio_clt_probe_worker);
					clt_probe_worker_info->registinfo_p =
					    &g_hif_sdio_clt_drv_list[clt_index];
					clt_probe_worker_info->probe_idx
						= (INT8)j;
					retVal = (INT32)schedule_work(
						&clt_probe_worker_info
						->probe_work);
					if (retVal == 0)
						HIF_SDIO_WARN_FUNC(
	...
	ret = (INT32)HIF_SDIO_ERR_SUCCESS;
	gRefCount++;
...
EXPORT_SYMBOL(hif_sdio_clt_reg);
 ...
 * \retval < 0  list error code here
 */
INT32 hif_sdio_clt_unreg(const MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	INT32 ret = -(INT32)HIF_SDIO_ERR_FAIL;
	INT32 clt_list_index = 0;
	UINT32 i = 0;
...
EXPORT_SYMBOL(hif_sdio_clt_unreg);
