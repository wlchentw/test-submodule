...
static _osal_inline_ INT32 stp_sdio_host_info_op(INT32 opId)
{
	...
	if (p_tx_buffer == NULL) {
		while (i < 5) {
			p_tx_buffer = kmalloc(
			    STP_SDIO_TX_BUF_CNT * STP_SDIO_TX_ENTRY_SIZE,
			     GFP_ATOMIC);
			if (p_tx_buffer == NULL) {
				STPSDIO_PR_ERR
				 ("mem alloc for tx_buf fail times=%d\n", i);
				i++;
				msleep(20);
				continue;
			} else {
				STPSDIO_PR_DBG
				    ("memory alloc for tx_buf succeed!\n");
				iRet = 0;
				break;
			}
		}
	...
	if (p_rx_buffer == NULL) {
		while (i < 5) {
			...
			if (p_rx_buffer == NULL) {
				STPSDIO_PR_ERR
				   ("mem alloc for rx_buf fail times=%d\n", i);
				i++;
				msleep(20);
				continue;
			} else {
				STPSDIO_PR_DBG
				    ("memory alloc for rx_buf succeed!\n");
				iRet = 0;
				break;
			}
		}
		if (i == 5) {
			STPSDIO_PR_ERR
			("memory allocate for rx_buf fail!\n");
			iRet = -1;
		}
	} else {
		STPSDIO_PR_DBG
		...
		if ((pInfo->sleep_flag != 0)
			...
			if (pInfo->firmware_info.tx_packet_num == 0) {
				...
				osal_ftrace_print("|E|set F own, sleep_flag(%d), wakeup_flag(%d), awake_falg(%d)",
						pInfo->sleep_flag, pInfo->wakeup_flag, pInfo->awake_flag);
#else
				STPSDIO_PR_INFO("PS not supported, skip F own\n");
#endif
			} else {
#if 0
				/* debug code */
...
void __attribute__((weak)) sdio_writeb(struct sdio_func *func, u8 b, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_writeb\n");
}
u8 __attribute__((weak)) sdio_readb(struct sdio_func *func, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_readb\n");
	return 0;
}
u32 __attribute__((weak)) sdio_readl(struct sdio_func *func, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_readl\n");
	return 0;
}
void __attribute__((weak)) sdio_writel(struct sdio_func *func, u32 b, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_writel\n");
}
int __attribute__((weak)) sdio_readsb(struct sdio_func *func, void *dst, unsigned int addr, int count)
{
	STPSDIO_PR_INFO("no impl. sdio_readsb\n");
	return 0;
}
int __attribute__((weak)) sdio_writesb(struct sdio_func *func, unsigned int addr, void *src, int count)
{
	STPSDIO_PR_INFO("no impl. sdio_writesb\n");
	return 0;
}
int __attribute__((weak)) sdio_enable_func(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_enable_func\n");
	return 0;
}
int __attribute__((weak)) sdio_disable_func(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_disable_func\n");
	return 0;
}
void __attribute__((weak)) sdio_claim_host(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_claim_host\n");
}
void __attribute__((weak)) sdio_release_host(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_release_host\n");
}
unsigned char __attribute__((weak)) sdio_f0_readb(struct sdio_func *func, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_f0_readb\n");
	return 0;
}
void __attribute__((weak)) sdio_f0_writeb(struct sdio_func *func, unsigned char b, unsigned int addr, int *err_ret)
{
	STPSDIO_PR_INFO("no impl. sdio_f0_writeb\n");
}
int __attribute__((weak)) sdio_set_block_size(struct sdio_func *func, unsigned blksz)
{
	STPSDIO_PR_INFO("no impl. sdio_set_block_size\n");
	return 0;
}
mmc_pm_flag_t __attribute__((weak)) sdio_get_host_pm_caps(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_get_host_pm_caps\n");
	return 0;
}
int __attribute__((weak)) sdio_set_host_pm_flags(struct sdio_func *func, mmc_pm_flag_t flags)
{
	STPSDIO_PR_INFO("no impl. sdio_set_host_pm_flags\n");
	return 0;
}
int __attribute__((weak)) sdio_release_irq(struct sdio_func *func)
{
	STPSDIO_PR_INFO("no impl. sdio_release_irq\n");
	return 0;
}
int __attribute__((weak)) sdio_claim_irq(struct sdio_func *func, sdio_irq_handler_t *handler)
{
	STPSDIO_PR_INFO("no impl. sdio_claim_irq\n");
	return 0;
}
void __attribute__((weak)) sdio_unregister_driver(struct sdio_driver *drv)
{
	STPSDIO_PR_INFO("no impl. sdio_unregister_driver\n");
}
int __attribute__((weak)) sdio_register_driver(struct sdio_driver *drv)
{
	STPSDIO_PR_INFO("no impl. sdio_register_driver\n");
	return 0;
}
#endif
