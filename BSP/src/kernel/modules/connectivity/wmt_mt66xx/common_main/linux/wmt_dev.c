...
LONG WMT_unlocked_ioctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	...
	case WMT_IOCTL_LPBK_POWER_CTRL:
		do {
			...
			WMT_DBG_FUNC("ZQ enter cmd WMT_IOCTL_LPBK_POWER_CTRL %d\n", arg);
			WMT_DBG_FUNC("ZQ always_pwr_on_flag = %d\n", always_pwr_on_flag);
			//WMT_DBG_FUNC("ZQ always_pwr_on_flag skip\n");
		...
		if (mtk_wcn_stp_coredump_start_get()) {
			...
			if (wmt_detect_get_chip_type() == WMT_CHIP_TYPE_SOC)
				...
				if (wmt_detect_get_chip_type() == WMT_CHIP_TYPE_COMBO ||
					...
					osal_memcpy(pBuffer, "MT662x f/w coredump start-",osal_strlen("MT662x f/w coredump start-"));
