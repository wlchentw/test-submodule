...
static INT32 mt6630_gps_1pss_ctrl(WMT_IC_PIN_STATE state, UINT32 flag)
{
	INT32 iRet = -1;
	UINT32 uVal = 0;
	if (state == WMT_IC_PIN_MUX) {
		/* For OCS_EN to support 1pps outpur */
		/* 0x81023010[1:0] set to be 0, 0x80025068[3:0] = h’4 */
		WMT_INFO_FUNC("Enable GPS 1 pps ctrl\n");
		uVal = 0;
		iRet = wmt_core_reg_rw_raw(1, 0x81023010, &uVal, 0x3);
		if (iRet != 0)
			WMT_ERR_FUNC("(0x81023010) failed, iRet(%d)\n", iRet);
		uVal = 4;
		iRet = wmt_core_reg_rw_raw(1, 0x80025068, &uVal, 0xf);

		if (iRet != 0)
			WMT_ERR_FUNC("(0x80025068) failed, iRet(%d)\n", iRet);
	}
	/* anyway, we return 0 */
	return 0;
}
...
static INT32 mt6630_pin_ctrl(WMT_IC_PIN_ID id, WMT_IC_PIN_STATE state, UINT32 flag)
{
	...
	case WMT_IC_PIN_1PPS:
		ret = mt6630_gps_1pss_ctrl(state, flag);
		break;
