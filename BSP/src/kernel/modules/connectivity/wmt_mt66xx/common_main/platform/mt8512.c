...
#define JTAG_GPIO_BASE        0x10005000
#define JTAG_INFRACFG_AO_BASE 0x10001000
#endif
...
INT32 mtk_wcn_consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		remap_addr1 = ioremap(JTAG_GPIO_BASE, 0x1000);
		if (remap_addr1 == 0) {
			WMT_PLAT_PR_ERR("remap JTAG_GPIO_BASE fail!\n");
			ret = -1;
			goto error;
		}

		remap_addr2 = ioremap(JTAG_INFRACFG_AO_BASE, 0x1000);
		if (remap_addr2 == 0) {
			WMT_PLAT_PR_ERR("remap JTAG_INFRACFG_AO_BASE fail!\n");
			ret = -1;
			goto error;
		}
		...
		case 1:
			...
			/* PAD Enable pull Control */
			addr = remap_addr1 + 0x860;
			tmp = readl(addr);
			tmp = tmp & 0xfff80fff;
			tmp = tmp | 0x7f000;
			writel(tmp, addr);
			...
			/* PAD TRST pull low, others pull high */
			addr = remap_addr1 + 0x900;
			tmp = readl(addr);
			tmp = tmp & 0xfff80fff;
			tmp = tmp | 0x6f000;
			writel(tmp, addr);
		...
		case 2:
			...
			/* 2-wire jtag PAD AUX Function Selection */
			addr = remap_addr1 + 0x250;
			tmp = readl(addr);
			tmp = tmp & 0xff03ffff;
			tmp = tmp | 0xb40000;
			writel(tmp, addr);
			...
			/* 2-wire jtag PAD Driving Selection */
			addr = remap_addr1 + 0x740;
			tmp = readl(addr);
			tmp = tmp & 0xfff0ffff;
			tmp = tmp | 0x70000;
			writel(tmp, addr);
			...
			/* 2-wire jtag PAD PULL Selection */
			addr = remap_addr1 + 0x410;
			tmp = readl(addr);
			tmp = tmp & 0xcfffffff;
			tmp = tmp | 0x30000000;
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	...
	UINT8 *topckgen_reg_base = NULL;
	struct pinctrl *consys_pinctrl = NULL;
	struct pinctrl_state *pinctrl_tcxo_en = NULL;
	struct pinctrl_state *pinctrl_tcxo_dis = NULL;

	consys_pinctrl = devm_pinctrl_get(&my_pdev->dev);
	if (IS_ERR(consys_pinctrl)) {
		WMT_PLAT_PR_WARN("Cannot find consys pinctrl.\n");
		consys_pinctrl = NULL;
	} else {
		pinctrl_tcxo_en = pinctrl_lookup_state(consys_pinctrl, "consys_pins_tcxo_en");
		if (IS_ERR(pinctrl_tcxo_en)) {
			WMT_PLAT_PR_WARN("Cannot find consys_pins_tcxo_en!\n");
			pinctrl_tcxo_en = NULL;
		}

		pinctrl_tcxo_dis = pinctrl_lookup_state(consys_pinctrl, "consys_pins_tcxo_dis");
		if (IS_ERR(pinctrl_tcxo_dis)) {
			WMT_PLAT_PR_WARN("Cannot find consys_pins_tcxo_dis!\n");
			pinctrl_tcxo_dis = NULL;
		}
	}
	...
	/* topckgen remap */
	topckgen_reg_base = ioremap_nocache(TOPCKGEN_BASE, 0x500);
	if (!topckgen_reg_base) {
		WMT_PLAT_PR_ERR("topckgen_reg_base(%x) ioremap fail\n", TOPCKGEN_BASE);
		return;
	}
#if 0
	CONSYS_REG_WRITE(topckgen_reg_base + 0x220,
					CONSYS_REG_READ(topckgen_reg_base + 0x220) | (0x1 << 30));
	WMT_PLAT_PR_INFO("topckgen_reg(0x220) = 0x%x\n", CONSYS_REG_READ(topckgen_reg_base + 0x220));

	CONSYS_REG_WRITE(topckgen_reg_base + 0x228,
					CONSYS_REG_READ(topckgen_reg_base + 0x228) | (0x1 << 13));
	WMT_PLAT_PR_INFO("topckgen_reg(0x228) = 0x%x\n", CONSYS_REG_READ(topckgen_reg_base + 0x228));
#endif
	if (enable) {
		...
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_03_OFFSET,
				(CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_03_OFFSET) &
				CONSYS_IF_PINMUX_03_MASK) | CONSYS_IF_PINMUX_03_VALUE);

		/* Set pinmux driving to 2mA, BT to 4mA */
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_01_OFFSET,
				(CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_01_OFFSET) &
				CONSYS_IF_DRV_PINMUX_01_MASK) | CONSYS_IF_DRV_PINMUX_01_VALUE);
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_02_OFFSET,
				(CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_02_OFFSET) &
				CONSYS_IF_DRV_PINMUX_02_MASK) | CONSYS_IF_DRV_PINMUX_02_VALUE);

		/* ZQ TODO: how to check TCXO mode (GPIO54 Aux4) */
		if (0) {
			CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_TCXO_OFFSET,
					(CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_TCXO_OFFSET) &
					CONSYS_IF_PINMUX_TCXO_MASK) | CONSYS_IF_PINMUX_TCXO_VALUE);
			WMT_PLAT_PR_INFO("Set GPIO54 pinux for TCXO mode(0x%x)\n",
				CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_TCXO_OFFSET));
		}

		if (pinctrl_tcxo_en) {
			pinctrl_select_state(consys_pinctrl, pinctrl_tcxo_en);
			WMT_PLAT_PR_INFO("Select pinctrl_tcxo_en!\n");
		} else {
			WMT_PLAT_PR_INFO("Not set pinctrl_tcxo_en!\n");
		}
	} else {
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_01_OFFSET,
				CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_01_OFFSET) &
		...
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_03_OFFSET,
				CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_03_OFFSET) &
				CONSYS_IF_PINMUX_03_MASK);

		if (pinctrl_tcxo_dis) {
			pinctrl_select_state(consys_pinctrl, pinctrl_tcxo_dis);
			WMT_PLAT_PR_INFO("Select pinctrl_tcxo_dis!\n");
		} else {
			WMT_PLAT_PR_INFO("Not set pinctrl_tcxo_dis!\n");
		}
	}
...
static VOID consys_hw_reset_bit_set(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		cnt = 0;
		while (CONSYS_REG_READ(conn_reg.mcu_conn_hif_on_base + CONSYS_CPUPCR_OFFSET) != 0xB7C) {
			if (cnt > 10)
				break;
			WMT_PLAT_PR_INFO("poll 0xB7C(%d): 0x18002600(0x%x), 0x1800216c(0x%x), 0x18007104(0x%x)\n",
					cnt,
					CONSYS_REG_READ(conn_reg.mcu_base + 0x600),
					CONSYS_REG_READ(conn_reg.mcu_base + 0x16c),
					CONSYS_REG_READ(conn_reg.mcu_conn_hif_on_base + CONSYS_CPUPCR_OFFSET));
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
#if CONSYS_PWR_ON_OFF_API_AVAILABLE
		pm_runtime_enable(&my_pdev->dev);
		WMT_PLAT_PR_INFO("pm_runtime_enable() CONSYS ok\n");
		 ...
		 */
		/* Turn off AHB Rx bus sleep protect 0x100012AC[21] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_CLR_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_CLR_OFFSET) |
				 CONSYS_AHB_RX_PROT_MASK);
		/* Check 0x10001258[21] = 1'b0 */
		value = ~CONSYS_AHB_RX_PROT_MASK &
			CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
		i = 0;
		while (value == 0 || i > 10) {
			value = ~CONSYS_AHB_RX_PROT_MASK &
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
		...
		/* Turn off AXI Rx bus sleep protect 0x100012A4[14] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_CLR_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_CLR_OFFSET) |
				 CONSYS_AXI_RX_PROT_MASK);
		/* Check 0x10001228[14] = 1'b0*/
		 ...
		 * been turn off.
		 */
		/* Turn off AXI Tx bus sleep protect 0x100012AC[18] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_CLR_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_CLR_OFFSET) |
				 CONSYS_AXI_TX_PROT_MASK);
		/* Check 0x10001258[18] = 1'b0 */
		...
		/* Turn off AHB TX bus sleep protect 0x100012A4[13] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_CLR_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_CLR_OFFSET) |
				 CONSYS_AHB_TX_PROT_MASK);
		/* Check 0x10001228[13] = 1'b0 */
		...
		pm_runtime_disable(&my_pdev->dev);
		WMT_PLAT_PR_INFO("pm_runtime_disable() CONSYS ok\n");
#else
		/* Turn on AHB Tx bus sleep protect (AP2CONN AHB Bus protect) 0x100012A0[13] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_SET_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_SET_OFFSET) |
				 CONSYS_AHB_TX_PROT_MASK);
		/* Check 0x10001228[13] = 1'b1 */
		value = CONSYS_AHB_TX_PROT_MASK &
			CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_OFFSET);
		i = 0;
		while (value != CONSYS_AHB_TX_PROT_MASK || i > 10) {
			value = CONSYS_AHB_TX_PROT_MASK &
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_OFFSET);
			i++;
		}

		/*Turn on AXI Tx bus sleep protect 0x100012A8[18] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_SET_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_SET_OFFSET) |
				 CONSYS_AXI_TX_PROT_MASK);
		/* Check 0x10001258[18] = 1'b1 */
		value = CONSYS_AXI_TX_PROT_MASK &
			CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
		i = 0;
		while (value != CONSYS_AXI_TX_PROT_MASK || i > 10) {
			value = CONSYS_AXI_TX_PROT_MASK &
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
			i++;
		}

		/*Turn on AXI Rx bus sleep protect 0x100012A0[14] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_SET_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_SET_OFFSET) |
				 CONSYS_AXI_RX_PROT_MASK);
		/* Check 0x10001228[14] = 1'b1 */
		value = CONSYS_AXI_RX_PROT_MASK &
			CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_OFFSET);
		i = 0;
		while (value != CONSYS_AXI_RX_PROT_MASK || i > 10) {
			value = CONSYS_AXI_RX_PROT_MASK &
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_OFFSET);
			i++;
		}

		/*Turn on AHB Rx bus sleep protect 0x100012A8[21] = 1'b1 */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_SET_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_EN_1_SET_OFFSET) |
				 CONSYS_AHB_RX_PROT_MASK);
		/* Check 0x10001258[21] = 1'b1 */
		value = CONSYS_AHB_RX_PROT_MASK &
			CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
		i = 0;
		while (value != CONSYS_AHB_RX_PROT_MASK || i > 10) {
			value = CONSYS_AHB_RX_PROT_MASK &
				CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AHBAXI_PROT_STA1_1_OFFSET);
			i++;
		}
...
static INT32 polling_consys_chipid(VOID)
{
	...
	UINT32 value = 0;
	UINT32 consys_slp_protect_ctrl = 0;
	UINT8 *ap_hw_version_reg_base = NULL;
	UINT32 hw_version = 0;
	...
	while (retry-- > 0) {
		...
		WMT_PLAT_PR_INFO("Read CONSYS version id(0x%08x)\n", consys_ver_id);
		msleep(20);
	}
	...
	consys_slp_protect_ctrl =
		CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + CONN_ON_SLPPROTECT_OFFSET);
	WMT_PLAT_PR_INFO("consys slp protect before set(0x%x)\n", consys_slp_protect_ctrl);
	CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_ON_SLPPROTECT_OFFSET,
				 consys_slp_protect_ctrl | CONN_ON_SLPPROTECT_BIT);
	consys_slp_protect_ctrl =
		CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + CONN_ON_SLPPROTECT_OFFSET);
	WMT_PLAT_PR_INFO("consys slp protect after set(0x%x)\n", consys_slp_protect_ctrl);

	/* ioremap */
	ap_hw_version_reg_base = ioremap_nocache(0x8000008, 0x100);
	if (ap_hw_version_reg_base) {
		/* read the HW_VERSION from AP 0x0800_0008 */
		hw_version = CONSYS_REG_READ(ap_hw_version_reg_base);
		WMT_PLAT_PR_INFO("Read AP HW_VERSION (0x%x)\n", hw_version);

		/* write the HW_VERSION to CONNSYS reserved CR 0x8002_1140 */
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_off_base + CONSYS_MCU_CFG_MISC_OFFSET,
			hw_version);

		hw_version = CONSYS_REG_READ(conn_reg.mcu_top_misc_off_base + CONSYS_MCU_CFG_MISC_OFFSET);
		WMT_PLAT_PR_INFO("Read CONNSYS CR 0x80021140 (0x%x)\n", hw_version);

		iounmap(ap_hw_version_reg_base);
	}
	...
	/* MT8512 NEW: write reserved CR for identify adie is 6635 or 6631 */
	value = CONSYS_REG_READ(conn_reg.mcu_top_misc_off_base + CONN_ON_ADIE_IC_OFFSET);
	value = (value & CONN_ON_ADIE_IC_MASK) | CONN_ON_ADIE_6631_VALUE;
	CONSYS_REG_WRITE(conn_reg.mcu_top_misc_off_base, value);

	/* MT8512 NEW: consys bus time out configure, enable AHB bus timeout */
	CONSYS_REG_WRITE(conn_reg.mcu_base + CONSYS_BUS_TIMEOUT_CONFIG_OFFSET,
		CONSYS_ENABLE_AHB_BUS_TIMEOUT);
...
static VOID consys_afe_reg_setting(VOID)
{
	...
	if (consys_afe_reg_base) {
		CONSYS_REG_WRITE(consys_afe_reg_base + CONSYS_AFE_RG_WBG_AFE_01_OFFSET,
				CONSYS_AFE_RG_WBG_AFE_01_VALUE);
		CONSYS_REG_WRITE(consys_afe_reg_base + CONSYS_AFE_RG_WBG_PLL_01_OFFSET,
				CONSYS_AFE_RG_WBG_PLL_01_VALUE);
	...
	/* write reserverd cr for identify adie is 6635 or 6631, 0x180c1130[8] = 1 for 6631 */
	CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + 0x130,
				CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + 0x130) | (0x1 << 8));
	WMT_PLAT_PR_INFO("0x180c1130(0x%x) for 6631 adie!\n",
		CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base + 0x130));
}

static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	struct device_node *node = NULL;
	INT32 gpio_bk_en1, gpio_bk_en2;

	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8512-consys");
	if (!node) {
		WMT_PLAT_PR_ERR("consys_hw_vcn18_ctrl:can't find device tree node!\n");
		return 0;
	}

	gpio_bk_en1 = of_get_named_gpio(node, "1v8_en", 0);
	if (gpio_bk_en1 < 0) {
		WMT_PLAT_PR_ERR("consys_hw_vcn18_ctrl:can't get 1v8_en gpio number!\n");
		return 0;
	}

	gpio_bk_en2 = of_get_named_gpio(node, "3v3_en", 0);
	if (gpio_bk_en2 < 0) {
		WMT_PLAT_PR_ERR("consys_hw_vcn18_ctrl:can't get 3v3_en gpio number!\n");
		return 0;
	}

	gpio_request(gpio_bk_en1, "consys 1v8_en");
	gpio_request(gpio_bk_en2, "consys 3v3_en");

	if (enable) {
		gpio_direction_output(gpio_bk_en1, 1);	// pull up
		WMT_PLAT_PR_INFO("enable VCN18 ok\n");

		udelay(10);

		gpio_direction_output(gpio_bk_en2, 1);	// pull up
		WMT_PLAT_PR_INFO("enable VCN33 ok\n");
	} else {
		gpio_direction_output(gpio_bk_en2, 0);
		WMT_PLAT_PR_INFO("disable VCN33 ok\n");

		gpio_direction_output(gpio_bk_en1, 0);
		WMT_PLAT_PR_INFO("disable VCN18 ok\n");
	}
...
static INT32 consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	buck = devm_regulator_get(&my_pdev->dev, "vcn28");
	if (IS_ERR(buck)) {
			WMT_PLAT_PR_ERR("can't get vcn28 regulator\n");
		return PTR_ERR(buck);
	}
	...
	if (enable) {
		...
		if (ret)
			WMT_PLAT_PR_ERR("unable to regulator_enable vcn28\n");
		else
			WMT_PLAT_PR_INFO("regulator_enable vcn28 OK\n");
	} else {
		ret = regulator_disable(buck);
		if (ret)
			WMT_PLAT_PR_ERR("unable to regulator_disable vcn28\n");
		else
			WMT_PLAT_PR_INFO("regulator_disable vcn28 OK\n");
