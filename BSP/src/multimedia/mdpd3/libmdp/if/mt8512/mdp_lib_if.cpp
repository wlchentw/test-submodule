...
DP_STATUS_ENUM mdp_if_mdp_process(struct mdp_param *param)
{
	...
	DP_TIMER_GET_CURRENT_TIME(tp0);

	pqParam.scenario = MEDIA_PICTURE;
	pqParam.sharpness_enable = vsi->pq.sharpness_enable;
	pqParam.sharpness_level = vsi->pq.sharpness_level;
	pqParam.gamma_enable = vsi->pq.gamma_enable;
	pqParam.gamma_type = vsi->pq.gamma_type;
	memcpy(pqParam.gamma_table, vsi->pq.gamma_table, 256);
	pqParam.invert = vsi->pq.invert;
	pqParam.dth_enable = vsi->pq.dth_enable;
	pqParam.dth_algo = vsi->pq.dth_algo;
	pqParam.rsz_algo = vsi->pq.rsz_algo;
	...
	DP_TIMER_GET_CURRENT_TIME(tp1);
	DP_TIMER_GET_DURATION_IN_MS(tp0, tp1, diff);
	if (diff > 20)
		DPLOGE("mdp_if_mdp_process total time = %d\n", diff);
