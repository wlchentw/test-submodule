...
#include "DpDriver_Android.h"

// Minimum Y pitch that is acceptable by HW
#ifdef MT8512_PQ_SUPPORT
#define DP_COLOR_GET_MIN_Y_PITCH(color, width) (width / 2)
#else
...
#ifdef MT8512_PQ_SUPPORT
    inline uint32_t getGammaStatus() const;

    inline uint32_t getInvertStatus() const;

    inline uint32_t getDthStatus() const;
#endif
...
#ifdef MT8512_PQ_SUPPORT
    const uint32_t        m_GammaStatus;
    const uint32_t 	  m_InvertStatus;
    const uint32_t 	  m_DthStatus;
#endif
...
#ifdef MT8512_PQ_SUPPORT
inline uint32_t DpPortAdapt::getGammaStatus() const
{
    return m_GammaStatus;
}

inline uint32_t DpPortAdapt::getInvertStatus() const
{
    return m_InvertStatus;
}

inline uint32_t DpPortAdapt::getDthStatus() const
{
    return m_DthStatus;
}
#endif
