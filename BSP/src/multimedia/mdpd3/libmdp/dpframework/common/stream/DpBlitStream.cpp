...
DP_STATUS_ENUM DpBlitStream::invalidate(struct timeval *endTime)
{
    ...
    uint32_t pq_cfg = (m_PqConfig.enSharp) | (m_PqConfig.enGamma << 1) |
	    (m_PqConfig.enInvert << 2) | (m_PqConfig.enDth << 3); 
    uint32_t tdshp = m_PqConfig.enSharp;
#else
    ...
    if (enableLog)
    {
        ...
        DPLOGE("DpBlit: in: (%d, %d, %d, %d, C%d%s%s%s%s, P%d), crop: (%d, %d, %d, %d, %d, %d), sec%d %s\n",
        ...
        DPLOGE("DpBlit: out: (%d, %d, %d, %d, C%d%s%s%s%s, P%d), misc: (X:%d, Y:%d, R:%d, F:%d, S:%d, D:%d), sec%d %s\n",
...
DP_STATUS_ENUM DpBlitStream::setPQParameter(const DpPqParam &pqParam)
{
...
#ifdef MT8512_PQ_SUPPORT
	pqConfig.enSharp = pqParam.sharpness_enable;
	pqConfig.enGamma = pqParam.gamma_enable;
	pqConfig.enInvert = pqParam.invert;
	pqConfig.enDth = pqParam.dth_enable;
#endif

        DPLOGI("DpBlitStream: pPQsession id %lux created\n", PQSessionID);
...
#ifdef MT8512_PQ_SUPPORT
        DPLOGI("DpBlitStream: getPQConfig enSharp %d enGamma %d enInvert %d enDth %d\n",pqConfig.enSharp, pqConfig.enGamma, pqConfig.enInvert, pqConfig.enDth);
#endif
	    ...
	    ||
            ...
            (m_PqConfig.enGamma != pqConfig.enGamma) ||
            (m_PqConfig.enInvert != pqConfig.enInvert) ||
            (m_PqConfig.enDth != pqConfig.enDth)
#endif
	    )
        {
	...
	if ((pqParam.gamma_enable != 0) ||
	    (pqParam.rsz_algo != 0) ||
	    (pqParam.sharpness_enable != 0) ||
	    (pqParam.dth_enable != 0)
	   )
	{
            m_frameChange = true;
	}
