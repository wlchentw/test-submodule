        ...
        if(0 < stream->m_jobIDsList.size())
        {
            ...
            for(int i = 0; i < curJobs.size(); i++)
            {
                ...
                if(stream->m_DREPABufferStartIndexList.empty() || (i != (curJobs.size() - 1)))
                {
                    ...
                    {
                        DPLOGE("DpIspStream: wait complete stream failed(%d) vss job(%llx) %d/%lu\n", status,_jobID, i, curJobs.size());
...
DP_STATUS_ENUM DpIspStream::dequeueSrcBuffer()
{
    ...
    if (diff[4] > 34)
    {
        DPLOGW("DpIsp: 0x%p, src %d ms, %d, %d, %d, %d\n"
                , this, diff[4], diff[0], diff[1], diff[2], diff[3]);
    }
    else
    {
        DPLOGI("DpIsp: 0x%p, src %d ms, %d, %d, %d, %d\n"
                , this, diff[4], diff[0], diff[1], diff[2], diff[3]);
    }
...
DP_STATUS_ENUM DpIspStream::waitComplete()
{
    ...
    DPLOGI("DpIspStream: Wait complete in asynchronized mode: 0x%p\n", this);
    ...
    if (0 < m_jobIDsList.size())
    {
        ...
        m_pListMutex->unlock();
        DPLOGI("DpIspStream: Job ID : %llx is in JobID List!!!\n", _jobID);
...
DP_STATUS_ENUM DpIspStream::configPQParameter(int32_t portIndex)
{
    ...
    if (0 != m_pqSupport) {
        ...
        DPLOGI("DpIspStream: pPQsession id %lux created\n", PQSessionID);
