...
DP_STATUS_ENUM DpBufferPoolBase::decorateProxy(BufferNode *pCurNode)
{
    ...
    if (DP_MEMORY_ION == pCurNode->memType)
    {
        ...
        if ((pCurNode->addr[0] == pCurNode->addr[1]) && (pCurNode->plane != 1))
        {
            ...
            DPLOGI("New ion proxy - FD:%p, size :%d", pCurNode->addr[0], size);
...
DpRingBufferPool::~DpRingBufferPool()
{
    ...
    {
        DPLOGI("DpRingBufferPool: delete active ring buffer node %p\n", &(*iterator));
        delete &(*iterator);
    }
    m_activeList.clear();
    ...
    {
        DPLOGI("DpRingBufferPool: delete pending ring buffer node %p\n", &(*iterator));
        delete &(*iterator);
    }
    m_pendingList.clear();
