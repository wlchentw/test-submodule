...
extern EngineReg GAMMAReg;
extern EngineReg DTHReg;
...
inline DP_STATUS_ENUM DpEngineBase::configFrameAal(DpConfig &config)
{
    ...
    config.pqSessionId = m_pOutDataPort[0]->getPqIdStatus();
    DPLOGI("DpEngineBase: PROC_AAL, getPqIdStatus() = %llx\n", (long long unsigned int)m_pOutDataPort[0]->getPqIdStatus());
...
#ifdef MT8512_PQ_SUPPORT
inline DP_STATUS_ENUM DpEngineBase::configFrameGamma(DpConfig &config)
{
    DP_STATUS_ENUM status;
    int32_t rotation;
...
inline DP_STATUS_ENUM DpEngineBase::configFrameDTH(DpConfig &config)
{
    DP_STATUS_ENUM status;
    int32_t rotation;
                                         ...
                                         DpConfig  &config)
{
    ...
    if (feature & eGAMMA)
    {
        status = configFrameGamma(config);
        if (DP_STATUS_RETURN_SUCCESS != status)
        {
            return status;
        }
