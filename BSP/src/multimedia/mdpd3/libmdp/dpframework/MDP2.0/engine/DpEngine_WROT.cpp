...
private:
	...
	DPLOGI("WROT: ROTFormat = %x subfmt = %d\n", ROTFormat, subfmt);
	ROTFormat |= ((subfmt & 0x0000000F) << 4);
...
DP_STATUS_ENUM DpEngine_WROT::onConfigFrame(DpCommand &command, DpConfig &config)
{
        ...
        (vSubsample == 1))  // YUV420
    {
        ...
        else if (270 == m_rotationAngle)
        {
		    ...
		    DP_COLOR_Y8 == m_targetFormat ||
		    DP_COLOR_Y4_M0 == m_targetFormat ||
		    DP_COLOR_Y4_M1 == m_targetFormat ||
		    DP_COLOR_Y4_M2 == m_targetFormat ||
		    DP_COLOR_Y2_M0 == m_targetFormat ||
		    DP_COLOR_Y2_M1 == m_targetFormat ||
		    DP_COLOR_Y2_M2 == m_targetFormat ||
		    DP_COLOR_Y2_M3 == m_targetFormat ||
		    DP_COLOR_Y1_M0 == m_targetFormat ||
		    DP_COLOR_Y1_M1 == m_targetFormat ||
		    DP_COLOR_Y1_M2 == m_targetFormat ||
		    DP_COLOR_Y1_M3 == m_targetFormat)
    {
        WROT_uv_xsel = 0;
        WROT_uv_ysel = 0;
    }
    ...
    MM_REG_WRITE(command, VIDO_CTRL, (WROT_uv_ysel    << 30) +
                                      ...
                                      (WROT_crop_en    << 12) +
                                      (WROT_swap       <<  8) +
                                      ((WROT_format & 0x0000000F) <<  0), 0xF131510F);
    command.addMetLog((m_identifier?"MDP_WROT1__VIDO_CTRL":"MDP_WROT0__VIDO_CTRL"),
                                      ...
                                      (WROT_crop_en    << 12) +
                                      (WROT_swap       <<  8) +
                                      ((WROT_format & 0x0000000F) <<  0));
#ifdef MT8512_PQ_SUPPORT
    MM_REG_WRITE(command, VIDO_Y_MODE, (WROT_format & 0x000000F0) >> 4, 0x0000000F);
#endif
