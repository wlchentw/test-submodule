...
PQSessionManager::~PQSessionManager()
{
    ...
    while (pPQSessionListHandle)
    {
        ...
        PQ_LOGD("[PQSessionManager] --delete PQSessionListHandle, id[%lux]", pPQSessionListHandle->pPQSessionInstance->getID());
...
PQSession* PQSessionManager::createPQSession(uint64_t id)
{
    ...
    PQ_LOGI("[PQSessionManager] createPQSession(%lux)", id);
...
void PQSessionManager::destroyPQSession(uint32_t id)
{
    ...
    while (pPQSessionListHandle != NULL)
    {
        ...
        {
            PQ_LOGI("[PQSessionManager] getPQSessionListHandle, find id[%lux] ", PQSessionID);
...
PQSession* PQSessionManager::getPQSession(uint64_t id)
{
    ...
    PQ_LOGI("[PQSessionManager] PQSessionManager::getPQSession id[%lux] ", id);
...
PQSessionListHandle* PQSessionManager::createPQSessionListHandle(uint64_t id)
{
    ...
    PQ_LOGI("[PQSessionManager] create PQSessionListHandle, id[%lux]", id);
    ...
    else
    {
        ...
        while(1)
        {
                ...
                (videoid > 0) && (videoid <= videonum) && (videoid == (pPQSessionListHandle->pPQSessionInstance->getID() << 32)))
            {
                ...
                {
                    PQ_LOGI("[PQSessionManager] create PQSessionListHandle, same video id[%lux]", pPQSessionListHandle->pPQSessionInstance->getID());
...
PQSessionListHandle* PQSessionManager::getPQSessionListHandle(uint64_t id)
{
    ...
    PQ_LOGI("[PQSessionManager] getPQSessionListHandle id[%lux] ", id);
    ...
    while (pPQSessionListHandle != NULL)
    {
        ...
        {
            PQ_LOGI("[PQSessionManager] getPQSessionListHandle, find id[%lux] ", id);
            return pPQSessionListHandle;
        }
    ...
    if (pPQSessionListHandle == NULL)
    {
        ...
        {
            PQ_LOGI("[PQSessionManager] getPQSession id[%lux] fail!! check createPQSession!", id);
...
bool PQSessionManager::isIdDuplicated(uint64_t id)
{
    ...
    while (pPQSessionListHandle)
    {
        ...
        if (pPQSessionListHandle->pPQSessionInstance->getID() == id)
        {
            PQ_LOGI("[PQSessionManager] Duplicated ID[%lux] found, please check if double createPQSession!", pPQSessionListHandle->pPQSessionInstance->getID());
            return true;
        }
...
uint32_t PQSessionManager::findVideoID(uint32_t videoID)
{
    ...
    else if(m_VideoIDBufferRing[0] == 0)
    {
        ...
        PQ_LOGI("[PQSessionManager] findVideoID[%x] buffer is empty. Return new ID %x", videoID, m_lastVideoIndex + 1);
    ...
    for (uint32_t i = 0; i < VIDEO_ID_NUM; i++)
    {
        ...
        if (videoID == m_VideoIDBufferRing[VideoIndex])
        {
            ...
            PQ_LOGI("[PQSessionManager] findVideoID[%x] found on index %x", videoID, VideoIndex + 1);
    ...
    m_VideoIDBufferRing[m_lastVideoIndex] = videoID;
    PQ_LOGI("[PQSessionManager] findVideoID[%x] not found. Return new ID %x", videoID, m_lastVideoIndex + 1);
    return m_lastVideoIndex + 1;
}
