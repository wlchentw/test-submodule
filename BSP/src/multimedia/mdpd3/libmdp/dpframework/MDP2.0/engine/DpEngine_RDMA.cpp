...
private:
	...
	uint32_t subfmt = 0;
        ...
        {
            case DP_COLOR_GREY:
            //case DP_COLOR_Y8: //same with GREY
                m_bitsPerPixelY    = 8;
                m_bitsPerPixelUV   = 0;
                m_horizontalShiftUV = 0;
                m_verticalShiftUV   = 0;
...
#endif
*/
	DPLOGI("RDMA: sourceFormat = %x subfmt = %d\n", sourceFormat, subfmt);
	sourceFormat |= ((subfmt & 0x0000000F) << 4);
                                            ...
                                            DpConfig  &config)
{
    ...
    if (config.inFormat == DP_COLOR_Y4_M2 || config.inFormat == DP_COLOR_Y2_M3
		    || config.inFormat == DP_COLOR_Y1_M3)
    {
	    m_sourceImagePitchY = m_sourceImagePitchY * 2;
    }
...
// Setup source frame info
    ...
    MM_REG_WRITE(command, MDP_RDMA_SRC_CON, ((sourceFormat & 0x0000000F) <<  0) +
                                            //(cosite       <<  8) +
                                            (filterMode   <<  9) +
                                            (m_enableUFO  << 11) +
                                            (fieldMode    << 12) +
...
#ifdef MT8512_PQ_SUPPORT
    MM_REG_WRITE(command, MDP_RDMA_Y4_MODE_CFG, (sourceFormat & 0x000000F0) >>  4,
		    				0x0000000F);
#endif
    if (m_enableUFO)
    {
    ...
    if ((PqParam.gamma_enable != 0) || (PqParam.invert != 0) || (PqParam.dth_enable != 0))
    {
	    uint32_t f_bpp;
	    f_bpp = DP_COLOR_BITS_PER_PIXEL(config.inFormat);
	    MM_REG_WRITE(command, MDP_RDMA_SRC_BASE_0, (config.memAddr[0] + (config.inYOffset * config.inWidth + config.inXOffset) * f_bpp / 8), 0xFFFFFFFF, NULL, m_frameConfigLabel[index++]);
    }
    else
