...
#define GRALLOC_SUPPORT    0    //must set 0 for yocto
#if GRALLOC_SUPPORT
...
bool PQDSAdaptor::calRegs(PQSession* pPQSession, DpCommand &command, DpConfig &config)
{
    ...
    shp_enable = PqParam.sharpness_enable;
    shp_level = PqParam.sharpness_level;

    pPQSession->getDSConfig(&m_dsConfig);
    memcpy(&m_dsConfig->TDSHP, (void *)tdshpindex.entry[shp_level], sizeof(m_dsConfig->TDSHP));
    m_dsConfig->ENABLE = shp_enable;
#else
    pPQSession->getDSConfig(&m_dsConfig);
