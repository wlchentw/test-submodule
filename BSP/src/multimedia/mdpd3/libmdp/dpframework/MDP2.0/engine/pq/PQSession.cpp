    ...
    : m_pPQSessionHandle(NULL)
{
    ...
    PQ_LOGI("[PQSession] PQSession() %lux... ",id);
...
PQSession::~PQSession()
{
    ...
    {
        PQ_LOGI("[PQSession] --delete PQSession handle, id[%lux]", m_pPQSessionHandle->id);
...
void PQSession::setDpPqConfig(DpPqConfig *pConfig, const DpPqParam *pParam)
{
    ...
    if (isPQEnabledOnActive)
    {
        ...
        if ((pParam->enable & PQ_DEFAULT_EN) != 0)
        {
            ...
            {
                PQ_LOGE("[PQSession] setDpPqConfig() id[%lux], unknown scenario[%d]\n", m_pPQSessionHandle->id, (int32_t)pParam->scenario);
            }
        }
        else
        {
...
bool PQSession::setPQparam(const DpPqParam *pParam)
{
    ...
    if (0)//pPQConfig->getPQServiceStatus() != PQSERVICE_READY)
    {
        PQ_LOGI("[PQSession] setPQParam() Bypassed ...\n");
        return false;
    }

    PQ_LOGI("[PQSession] setPQParam() id[%lux] enable[%d], scenario[%d]\n", m_pPQSessionHandle->id, (int32_t)pParam->enable, (int32_t)pParam->scenario);
...
void PQSession::setHistogram(uint32_t *pHist, uint32_t size)
{
    ...
    if (m_pPQSessionHandle && m_pPQSessionHandle->DCHandle)
    {
        ...
        PQ_LOGI("[PQSession] setHistogram(), id[%lux] waiting[%lu] done[%lu]", id, m_pPQSessionHandle->DCHandle->pWaitingHistList->size(), m_pPQSessionHandle->DCHandle->pDoneHistList->size());
