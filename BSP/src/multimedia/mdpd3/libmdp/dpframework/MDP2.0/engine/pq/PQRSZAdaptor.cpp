     ...
     RszInput* inParam, RszOutput* outParam)
{
    ...
    if (PQParam.rsz_algo == 10 || PQParam.rsz_algo == 11)
    {
	if (inParam->srcWidth < inParam->dstWidth)
		outParam->horTable = PQParam.rsz_algo;
	if (inParam->srcHeight < inParam->dstHeight)
		outParam->verTable = PQParam.rsz_algo;
    }
#endif
