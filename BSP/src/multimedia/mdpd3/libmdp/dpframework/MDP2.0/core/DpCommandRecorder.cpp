...
void DpCommandRecorder::setSecureMetaData(uint32_t regAddr, uint32_t memAddr)
{
    ...
    if (DP_SECURE_NONE != m_secureMode)
    {
        if (0 != memAddr)
        {
            ...
            {
                DPLOGE("secureAddr list overflow! addr %08x, index %lu", m_secureInfo[index].secureRegAddr[sub_index],  ((unsigned long)m_pCurCommand - (unsigned long)m_pBackBuffer) >> 3);
                return;
            }
            ...
            DPLOGI("index %d  m_secureAddrMD[%d] addr %08x, index %d  handle %016llx  offset %08x  size %08x  port %d type %d\n",
                ...
                (long long unsigned int)m_secureAddrMD[m_addrListLength].baseHandle,
