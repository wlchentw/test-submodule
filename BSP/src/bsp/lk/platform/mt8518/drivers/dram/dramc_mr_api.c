...
const unsigned char lpddr4_mrr_mapping_pop[CHANNEL_NUM][16] = {
	{
		16, 18, 23, 20, 22, 21, 19, 17,
		26, 27, 25, 24, 28, 30, 29, 31,
	},
	{
		16, 18, 23, 20, 22, 21, 19, 17,
		26, 27, 25, 24, 28, 30, 29, 31,
...
/* pinmux for mrr */
const unsigned char lpddr4_mrr_mapping_pop2[CHANNEL_NUM][16] = {
	{
		24, 26, 31, 28, 30, 29, 27, 25,
		18, 19, 17, 16, 20, 22, 21, 23,
	},
	{
		24, 26, 31, 28, 30, 29, 27, 25,
		18, 19, 17, 16, 20, 22, 21, 23,
...
static void mr_init_lp4_fsp1(DRAMC_CTX_T *p)
{
	...
	if (p->odt_onoff == ODT_OFF)
		dramc_mode_reg_write(p, MR14,
			dram_mr.mr14_value[p->channel][p->rank][0]);
...
static void mode_reg_init_lp4(DRAMC_CTX_T *p)
{
	...
	for (channel_idx = 0;
		...
		for (rank_idx = 0;
			 ...
			 */
		#if 0 /* cc mark for not support 4P */
			if (p->dram_type == TYPE_LPDDR4P)
				dramc_mode_reg_write(p, MR37, 0x1);
		#endif

		#if 0 /* cc mark for not support switch freq */
			 ...
			 */
			 mr_init_lp4_fsp1(p);
#if DVFS_EN
			ddr_phy_freq_sel(p, DVFS_LOW_FREQ);
#endif
...
{
	/* reset dram = low */
	io_32_write_fld_align_all(DRAMC_REG_ADDR(DRAMC_REG_DDRCONF0),
		CLEAR_FLD, DDRCONF0_GDDR3RST);
	/* cc add. RESET bit is moved to PLL2 */
	io_32_write_fld_align_all(DRAMC_REG_ADDR(DDRPHY_PLL2),
		CLEAR_FLD, PLL2_GDDR3RST);
	...
	/* reset dram = high */
	io_32_write_fld_align_all(DRAMC_REG_ADDR(DRAMC_REG_DDRCONF0),
		SET_FLD, DDRCONF0_GDDR3RST);
	io_32_write_fld_align_all(DRAMC_REG_ADDR(DDRPHY_PLL2),
		SET_FLD, PLL2_GDDR3RST);
	...
	/* CKE high */
	cke_fix_on_off(p, CKE_FIXON, CKE_WRITE_TO_ALL_CHANNEL);
	/* tINIT5=2us(min) */
	delay_us(2);
	/* u1PowerOn=1; */
	...
	unsigned char mr_fsp, unsigned char rank)
{
	...
	unsigned char ii, mr[] = {MR05, MR12, MR14, MR18, MR19};
...
void dramc_mode_reg_check(DRAMC_CTX_T *p)
{
...
#endif
	{
				...
				show_msg2((INFO, "FSP_%d, CH_%d, RK%d\n",
					mr_fsp, p->channel, p->rank));
				dramc_mode_reg_check2(p, p->channel, mr_fsp, p->rank);
...
void dramc_mr_init_ddr3(DRAMC_CTX_T *p)
{
	...
	case DDR1066_FREQ:
		...
		dram_mr.mr03_value[p->dram_fsp] = 0x0;
		dram_mr.mr01_value[p->dram_fsp] = DDR_PC3_MR1;
		dram_mr.mr00_value[p->dram_fsp] = 0x1960;
		break;
	default:
	case DDR1866_FREQ:
		...
		dram_mr.mr03_value[p->dram_fsp] = 0x0;
		dram_mr.mr01_value[p->dram_fsp] = DDR_PC3_MR1;
		dram_mr.mr00_value[p->dram_fsp] = 0x1114;
		break;
	}
...
void dramc_mr_init_ddr4(DRAMC_CTX_T *p)
{
	...
	default:
	case DDR2666_FREQ:
		dram_mr.mr02_value[p->dram_fsp] = 0xf0;
		dram_mr.mr03_value[p->dram_fsp] = 0x0;
	...
	case DDR2400_FREQ:
			dram_mr.mr02_value[p->dram_fsp] = 0xe8;
			dram_mr.mr03_value[p->dram_fsp] = 0x0;
			dram_mr.mr01_value[p->dram_fsp] = DDR_PC4_MR1;
			dram_mr.mr04_value[p->dram_fsp] = 0x800;
			dram_mr.mr05_value[p->dram_fsp] = 0x400;
			dram_mr.mr06_value[p->dram_fsp] = (0x2 << 10);
			dram_mr.mr00_value[p->dram_fsp] = 0x964;
			break;
	...
	case DDR1866_FREQ:
		...
		dram_mr.mr00_value[p->dram_fsp] = 0x520;
		break;
	}
	...
	delay_us(10);
	dramc_mode_reg_write(p, 6, 0x08A |
		dram_mr.mr06_value[p->dram_fsp]);
	delay_us(10);
	dramc_mode_reg_write(p, 6, 0x00A |
		dram_mr.mr06_value[p->dram_fsp]);
	delay_us(10);
...
void dramc_mr_init_lp3(DRAMC_CTX_T *p)
{
	...
	for (rankidx = 0; rankidx < (U32)(p->support_rank_num); rankidx++) {
		...
		case DDR1066_FREQ:
			...
			dram_mr.mr10_value[p->dram_fsp] = 0xff;
			dram_mr.mr01_value[p->dram_fsp] = 0x83;
			dram_mr.mr02_value[p->dram_fsp] = 0x16;
			break;
		}
	 ...
	 * io_32_write_fld_multi(DRAMC_REG_SHU_HWSET_MR13,
		p_fld(mr2value, SHU_HWSET_MR13_HWSET_MR13_OP) |
...
void set_mrr_pinmux_mapping(DRAMC_CTX_T *p, unsigned char option)
{
	...
	for (chIdx = CHANNEL_A;
		...
		if (p->dram_type == TYPE_LPDDR4) {
			...
			if (option)
				lpddr_mrr_mapping =
					(unsigned char *)lpddr4_mrr_mapping_pop2[chIdx];
			else
				lpddr_mrr_mapping =
