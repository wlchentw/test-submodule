...
#define GPT4_CLK4	BIT(4)
#define GPT4_FREERUN	GENMASK(5, 4)
#define GPT4_CLK_SYS	0x0
#define GPT4_CLKDIV_MSK	GENMASK(3, 0)
...
static int mtk_timer_probe(struct udevice *dev)
{
	...
	/*clear GPT4's clkdiv to set 13M clksrc*/
	val = readl(priv->base + MTK_GPT4_CLK);
	val &= ~(GPT4_CLKDIV_MSK | GPT4_CLK4);
	writel(val, priv->base + MTK_GPT4_CLK);

	/*enable gpt4 & run in free-run mode*/
	val = readl(priv->base + MTK_GPT4_CTRL);
	val |= (GPT4_ENABLE | GPT4_FREERUN);
	writel(val, priv->base + MTK_GPT4_CTRL);
