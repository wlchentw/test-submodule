		    ...
		    int states, bootm_headers_t *images, int boot_progress)
{
	 ...
	 */
#if 0
	printf("skip reading ntxfw & ntxhwconfig ...\n");
#else
	// Read hwcfg's offset
	sprintf(szCmd,"part start mmc 0 hwcfg hwcfg_block");
	run_command(szCmd,0);
	sprintf(szCmd,"echo ${hwcfg_block}");
	ret = run_command(szCmd,0);
	sprintf(szCmd,"read_hwcfg  ${hwcfg_block}");
	ret = run_command(szCmd,0);

	// Read ntxfw's offset
	sprintf(szCmd,"part start mmc 0 ntxfw ntxfw_block");
	run_command(szCmd,0);
	sprintf(szCmd,"echo ${ntxfw_block}");
	ret = run_command(szCmd,0);
	sprintf(szCmd,"read_ntxfw  ${ntxfw_block}");
	ret = run_command(szCmd,0);	
		...
		printf("%s(%d):powerkey_status=%d\,usb_status=%d\n",
				__func__,__LINE__,
				(int)gpt_boot_args->powerkey_status,
				(int)gpt_boot_args->usb_status);

		int enterRecovery;
		if(gptNtxHwCfg->m_val.bPCB == E70T04) {		
			printf("upkey_status=%d,downkey_status=%d\n",
				(int)gpt_boot_args->upkey_status,
				(int)gpt_boot_args->downkey_status);
			enterRecovery = gpt_boot_args->powerkey_status && (!gpt_boot_args->upkey_status || !gpt_boot_args->downkey_status);
		}
		else
			enterRecovery = gpt_boot_args->powerkey_status && gpt_boot_args->usb_status;
			//printf("gptNtxHwCfg->m_val.bSysPartType = %d, enterRecovery = %d\n", gptNtxHwCfg->m_val.bSysPartType, enterRecovery);
		if (enterRecovery) {
			int iBootPartNo = -1;
			switch(gptNtxHwCfg->m_val.bSysPartType) {
			case 25:// TYPE24 .
				iBootPartNo = 11;
				printf("boot to recovery partition %d...\n",iBootPartNo);
				break;
			default :
				break;
			}
			if(iBootPartNo!=-1) {
				sprintf(szCmd,"setenv rootdev /dev/mmcblk0p%d",iBootPartNo);
				run_command(szCmd,0);
			}
		}
		else if (gptNtxHwCfg->m_val.bBootPartNo>0) {
			printf("%s(%d):bBootPartNo=%d\n",__func__,__LINE__,(int)gptNtxHwCfg->m_val.bBootPartNo);
			sprintf(szCmd,"setenv rootdev /dev/mmcblk0p%d",(int)gptNtxHwCfg->m_val.bBootPartNo);
			run_command(szCmd,0);
		}

#endif

	// Append NTX Hidden Memory
	//sprintf(szCmd,"append_NTX_HiddenMem");
	//ret = run_command(szCmd,0);	
