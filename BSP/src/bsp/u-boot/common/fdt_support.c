...
int fdt_chosen(void *fdt)
{
	...
	char cBootargsA[512] = {0,};
	int iBootargsFromDtb=0;
	...
	str = fdt_getprop(fdt, nodeoffset, "bootargs", &len);

	if(str) {
		iBootargsFromDtb = 1;
	}
	else {
		// if there is no fdt bootargs defined we use bootargs in boot environment . 
	...
	if (str) {
		...
		if(strstr(str,"root=")) {
			// root= option already exist in command line .
			sprintf(cBootargsA,"%s",str);
		}
		else {
			szRootDev = env_get("rootdev");
			if(szRootDev) {
				sprintf(cBootargsA,"%s root=%s",str,szRootDev);
			}
			else {
				sprintf(cBootargsA,"%s",str);
			}
		...
		if(gptNtxHwCfg) {
			if(9==gptNtxHwCfg->m_val.bCustomer||33==gptNtxHwCfg->m_val.bCustomer) {
				strcat(cBootargsA," quiet");
			}
		}
		else {
			printf("[WARNING]: hwconfg missing !?\n");
		}

		// Append ntxfw / hwcfg
		NtxHiddenMem_append_kcmdline(512,cBootargsA);
#if 0
		env_set("bootargs",cBootargsA);
#else
		err = fdt_setprop(fdt, nodeoffset, "bootargs", cBootargsA,
				  strlen(cBootargsA) + 1);
		if (err < 0) {
...
#endif 
		printf("%s(%d),dtb=%d,appended bootargs=\"%s\"\n",__func__,__LINE__,iBootargsFromDtb,cBootargsA);
	}
	else {
