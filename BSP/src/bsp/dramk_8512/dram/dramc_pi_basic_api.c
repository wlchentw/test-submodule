...
//MRR DRAM->DRAMC
#if ENABLE_LP3_SW
#if (fcFOR_CHIP_ID != fcSchubert) //Schubert only have MCP LP3 ball
...
const U8 uiLPDDR3_DQ_Mapping_POP[DQS_NUMBER_LP3][16] =
{
    { 6, 7, 4, 5, 2, 3, 0, 1},      //B0
    { 8, 9, 10, 11, 12, 13, 14, 15} //B1
};
#endif /* ENABLE_LP3_SW */
const U8 uiLPDDR4_O1_Mapping_POP[CHANNEL_NUM][16] =
{
    ...
    {
        0, 4, 3, 6, 7, 2, 5, 1,
        13, 12, 14, 15, 8, 11, 10, 9
    },
};

const U8 uiLPDDR4_DQ_Mapping_POP[CHANNEL_NUM][16] =
{
    //CH-A
    {
        0, 7, 5, 2, 1, 6, 3, 4,
        12, 15, 14, 13, 9, 8, 10, 11
    },
};

const U8 uiLPDDR4_CAVref_Mapping_POP[CHANNEL_NUM][8] =
{
    //CH-A
    {
        0, 1, 2, 3, 4, 5, 6, 7
...
{
#if (fcFOR_CHIP_ID == fcSchubert)
    //tg removed Broadcast for Schubert(single channel/rank)
    return;
#else
#if (FOR_DV_SIMULATION_USED==0 && SW_CHANGE_FOR_SIMULATION==0)
...
#ifdef ENABLE_LP3_SW
U8 u1IsLP3Family(DRAM_DRAM_TYPE_T dram_type)
{
...
void setFreqGroup(DRAMC_CTX_T *p)
{
    ...
    if (u1IsLP4Family(p->dram_type))
    {
        ...
        else if (p->frequency <= 1400) // tg added DDR2800
        {
            p->freqGroup = 1400;
        }
...
void Set_MRR_Pinmux_Mapping(DRAMC_CTX_T *p, U8 CBTDone)
{
    ...
    for(chIdx = CHANNEL_A; chIdx < p->support_channel_num; chIdx++)
    {
        ...
        if (u1IsLP4Family(p->dram_type))
        {
            ...
            {
                uiLPDDR_MRR_Mapping = (U8 *)uiLPDDR4_O1_Mapping_POP[chIdx];
            }
        }
        #if ENABLE_LP3_SW
        else
        ...
        //Set MRR pin mux
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MRR_BIT_MUX1), P_Fld(uiLPDDR_MRR_Mapping[0], MRR_BIT_MUX1_MRR_BIT0_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[1], MRR_BIT_MUX1_MRR_BIT1_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[2], MRR_BIT_MUX1_MRR_BIT2_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[3], MRR_BIT_MUX1_MRR_BIT3_SEL));
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MRR_BIT_MUX2), P_Fld(uiLPDDR_MRR_Mapping[4], MRR_BIT_MUX2_MRR_BIT4_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[5], MRR_BIT_MUX2_MRR_BIT5_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[6], MRR_BIT_MUX2_MRR_BIT6_SEL)
                                                                 | P_Fld(uiLPDDR_MRR_Mapping[7], MRR_BIT_MUX2_MRR_BIT7_SEL));
        if (u1IsLP4Family(p->dram_type))
        {
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MRR_BIT_MUX3), P_Fld(uiLPDDR_MRR_Mapping[8], MRR_BIT_MUX3_MRR_BIT8_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[9], MRR_BIT_MUX3_MRR_BIT9_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[10], MRR_BIT_MUX3_MRR_BIT10_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[11], MRR_BIT_MUX3_MRR_BIT11_SEL));
            vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_MRR_BIT_MUX4), P_Fld(uiLPDDR_MRR_Mapping[12], MRR_BIT_MUX4_MRR_BIT12_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[13], MRR_BIT_MUX4_MRR_BIT13_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[14], MRR_BIT_MUX4_MRR_BIT14_SEL)
                                                                     | P_Fld(uiLPDDR_MRR_Mapping[15], MRR_BIT_MUX4_MRR_BIT15_SEL));
        }
    }
...
U8 get_shuffleIndex_by_Freq(DRAMC_CTX_T *p)
{
    ...
    if (pFreqTbl == NULL) {
        mcSHOW_ERR_MSG("%s get freqTbl is NULL!\n", __func__);
        while(1) 
            ;
    }
...
#if 0	//no need (fcFOR_CHIP_ID == fcSchubert)
...
#if ENABLE_CS1_CKE1_FORCEOFF && (fcFOR_CHIP_ID == fcSchubert)
    //tg add for single rank force CS1 and CKE1 off
    if(p->support_rank_num == RANK_SINGLE)
    {
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_RKCFG), 0x1, RKCFG_CS1FORCE0);
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_DRAMC_PD_CTRL), 0x1, DRAMC_PD_CTRL_CKE1FORCE0);
    }
#endif
}
...
void DVFSSettings(DRAMC_CTX_T *p)
{
    ...
    if(u1IsLP4Family(p->dram_type))
    {
        switch (p->frequency)
        {
            case 1866:
                ...
                break;
            case 1600:
                u1DLL_Idle = 0x18; //tg change for mp setting
                break;
            case 1400:
                u1DLL_Idle = 0x18; //tg added 2800
                break;
            case 1333:
                u1DLL_Idle = 0xd;
                break;
            case 1200:
                u1DLL_Idle = 0x20; //tg change for MP setting
                break;
            case 800:
                u1DLL_Idle = 0x2f; //tg change for MP setting
                break;
            case 600:
                u1DLL_Idle = 0x3f; //tg change for MP setting
                break;
            case 400:
                u1DLL_Idle = 0x30;
                break;
            default:
                mcSHOW_ERR_MSG("u1DLL_Idle err of %d!\n", p->frequency);
    ...
    else
    {
        ...
        {
            case 933:
                u1DLL_Idle = 0x15; //tg change for lp3 mp setting
                break;
            case 800:
	    case 700:
                u1DLL_Idle = 0x18; //tg change for lp3 mp setting
                break;
            case 600:
                u1DLL_Idle = 0x20; //tg change for lp3 mp setting
                break;
            case 667:
                u1DLL_Idle = 0xC;
                break;
	...
	//DRAMC-Enable ShortPulse option
	vIO32WriteFldAlign_All(DRAMC_REG_DVFSDLL, 0x1, DVFSDLL_R_BYPASS_1ST_DLL_SHU4); //Enable to fix short pulse during DVFS
...
void DDRPhyFreqSel(DRAMC_CTX_T *p, DRAM_PLL_FREQ_SEL_T sel)
{
    ...
    if ((p->frequency == 1333) || (p->frequency == 1400) || (p->frequency == 1600))
        p->odt_onoff = ODT_OFF;
#endif
...
void DDRPhyPLLSetting(DRAMC_CTX_T *p)
{
    ...
    else if(p->frequency < 1200) //tg change <=1333 -> < 1200 for mp setting
    {
        u1CAP_SEL = 0x2;
    }
    else
    ...
    if (p->frequency <= 1200)//Lewis@20161129: Fix DDR1600 S-idle Vcore power is too big problem.
    {
        if(u1IsLP4Family(p->dram_type))
        {
            u1MIDPICAP_SEL = 0x1; //tg change 0->1 for mp setting
        }
        else
        {
            u1MIDPICAP_SEL = 0x0;
    ...
    {
        u1MIDPICAP_SEL = 0x3; //tg change 2->3 for mp setting
    }
    ...
    //tg change 0x0->0xfe for mp setting
    ...
    if(u1IsLP4Family(p->dram_type))
    {
        //tg removed CHB setting for Schubert
        #if DLL_ASYNC_MODE
        u1BRPI_MCTL_EN_CA = 1;
        ...
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+((U32)CHANNEL_A<<POS_BANK_NUM), P_Fld(0, CKMUX_SEL_FMEM_CK_MUX)
        ...
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+((U32)CHANNEL_A<<POS_BANK_NUM), P_Fld(1, CKMUX_SEL_FMEM_CK_MUX)
    ...
    else//LP3 sync mode only
    {
        ...
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+((U32)CHANNEL_A<<POS_BANK_NUM), P_Fld(1, CKMUX_SEL_FMEM_CK_MUX)
                                                                          | P_Fld(1, CKMUX_SEL_FB_CK_MUX));
        vIO32WriteFldMulti(DDRPHY_CKMUX_SEL+SHIFT_TO_CHB_ADDR, P_Fld(1, CKMUX_SEL_FMEM_CK_MUX)
                                                             | P_Fld(1, CKMUX_SEL_FB_CK_MUX));
        vIO32WriteFldAlign_All(DDRPHY_SHU1_CA_CMD0, 0x1, SHU1_CA_CMD0_RG_FB_CK_MUX);
    }
...
#if (fcFOR_CHIP_ID == fcSchubert)
#if ENABLE_LP3_SW
    for(iChannel=CHANNEL_A; iChannel<=CHANNEL_B; iChannel++) //Schubert LP3 used  PHY CHB
#else
    for(iChannel=CHANNEL_A; iChannel<=CHANNEL_A; iChannel++)
#endif
#endif
    {
        if(u1CA_DLL_Mode[iChannel]==DLL_MASTER)
        {
            ...
            vIO32WriteFldMulti(DDRPHY_SHU1_CA_DLL1+((U32)iChannel<<POS_BANK_NUM), P_Fld(0x1, SHU1_CA_DLL1_RG_ARDLL_PD_CK_SEL_CA)
                                                                                | P_Fld(0x0, SHU1_CA_DLL1_RG_ARDLL_FASTPJ_CK_SEL_CA));
        ...
        else
        {
            ...
            vIO32WriteFldMulti(DDRPHY_SHU1_CA_DLL1+((U32)iChannel<<POS_BANK_NUM), P_Fld(0x0, SHU1_CA_DLL1_RG_ARDLL_PD_CK_SEL_CA)
                                                                                | P_Fld(0x1, SHU1_CA_DLL1_RG_ARDLL_FASTPJ_CK_SEL_CA));
    ...
    U32 u4RegBackupAddress[] =
    {
        ...
        (DDRPHY_B1_DQ7),
        (DDRPHY_CA_CMD7),
#if (fcFOR_CHIP_ID == fcSchubert) && ENABLE_LP3_SW
        (DDRPHY_B0_DQ7 + SHIFT_TO_CHB_ADDR),
        //(DDRPHY_B1_DQ7 + SHIFT_TO_CHB_ADDR),  //tg removed CHB CA and B1
    ...
    //if(p->vendor_id==VENDOR_SAMSUNG && p->dram_type==TYPE_LPDDR3)
    {
        ...
        vIO32WriteFldMulti_All(DDRPHY_MISC_SPM_CTRL1, P_Fld(0x1, MISC_SPM_CTRL1_RG_ARDMSUS_10)
                                                    | P_Fld(0x1, MISC_SPM_CTRL1_RG_ARDMSUS_10_B0)
                                                    | P_Fld(0x1, MISC_SPM_CTRL1_RG_ARDMSUS_10_B1)
                                                    | P_Fld(0x1, MISC_SPM_CTRL1_RG_ARDMSUS_10_CA));
#else
        ...
        vIO32WriteFldMulti_All(DDRPHY_B1_DQ7, P_Fld(0x1, B1_DQ7_RG_TX_ARDQ_PULL_DN_B1)
                                            | P_Fld(0x1, B1_DQ7_RG_TX_ARDQM0_PULL_DN_B1)
                                            | P_Fld(0x1, B1_DQ7_RG_TX_ARDQS0_PULL_DN_B1)
                                            | P_Fld(0x1, B1_DQ7_RG_TX_ARDQS0B_PULL_DN_B1));
    ...
    {
		// Note: **** if possible, change freq for desense ****
        u2SDM_PCW = 0x7b00; //tg change 0x7600->7b00 for mp setting
    }
    else if(p->frequency==1333 || p->frequency==667)
    {
    #if !__ETT__
    #if FOR_DV_SIMULATION_USED
        if (!u1IsLP4Family(p->dram_type)) //tg modify
    #else
        if (0)
    #endif
        {
    ...
    else if (p->frequency == 1400 || p->frequency == 700)
    {
        u2SDM_PCW = 0x6b00; //tg added
    }
    ...
    #if FOR_DV_SIMULATION_USED
    delay_us(20);
    mcSHOW_DBG_MSG("tg change delay 20us\n");
    #else
    mcDELAY_US(100);
    #endif
    ...
    else
    {
        ...
        vIO32WriteFldAlign(DDRPHY_B0_DLL_ARPI2, 0, B0_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B0);//tg change for Schubert Lp3
        vIO32WriteFldAlign(DDRPHY_B1_DLL_ARPI2, 0, B1_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B1);
        //tg removed CHB CA and B1
        //vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI2+SHIFT_TO_CHB_ADDR, 0, CA_DLL_ARPI2_RG_ARPI_CG_CLKIEN);
        vIO32WriteFldAlign(DDRPHY_B0_DLL_ARPI2+SHIFT_TO_CHB_ADDR, 1, B0_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B0); //tg change for Schubert Lp3
    ...
    //DLL
    vIO32WriteFldAlign_All(DDRPHY_CA_DLL_ARPI2, 0x1, CA_DLL_ARPI2_RG_ARDLL_PHDET_EN_CA); //tg change for lp3 mp setting
    mcDELAY_US(1);
...
void DramcPowerOnSequence(DRAMC_CTX_T *p)
{
    ...
    //tINIT3=2ms(min)
    mcDELAY_US(2000);   // don't use mcDELAY_MS: it's not accurate: if mcDELAY_MS(2), actually maybe 1.7ms.
        ...
        for(u4RankIdx =0; u4RankIdx < (U32)(p->support_rank_num); u4RankIdx++)
        {
           ...
           // if(p->frequency<=1200)
            ...
            //else
            {
                ...
                else if (p->freqGroup == 1400) //tg added
                {
                    u1MR02Value[u1MRFsp] = 0x2d;
                }
...
/*
    ...
    mcDELAY_MS(1);
*/
    DRV_WriteReg32(MPLL_CON3, 0x1); // power on MPLL
...
void DDRPhyReservedRGSetting(DRAMC_CTX_T *p)
{
    ...
    //if(p->frequency<=1333)
    if(1) //tg change for mp setting
        u1HYST_SEL=1;

    if(u1IsLP4Family(p->dram_type) && p->frequency<1333 && p->frequency > 800) //tg change for mp setting
        u1MIDPI_CAP_SEL2=1;
    else
        u1MIDPI_CAP_SEL2=0;
    ...
    //PI
    vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD6, P_Fld(0x1, RG_ARPI_RESERVE_BIT_00_TX_CG_EN) // RG_*RPI_RESERVE_B0[0] 1'b1 prevent leakage
                                        | P_Fld(0x0, RG_ARPI_RESERVE_BIT_01_DLL_FAST_PSJP) //tg change 1->0
                                        ...
                                        | P_Fld(0x0, RG_ARPI_RESERVE_BIT_13_CG_SYNC_ENB)
                                        | P_Fld(u1LowFreq_SEL, RG_ARPI_RESERVE_BIT_14_LP3_SEL)
                                        | P_Fld(u1PSMUX_DRV_SEL, RG_ARPI_RESERVE_BIT_15_PSMUX_DRV_SEL));

    #if 0 //tg removed CHB CA
    vIO32WriteFldMulti(DDRPHY_SHU1_CA_CMD6+SHIFT_TO_CHB_ADDR, P_Fld(0x1, RG_ARPI_RESERVE_BIT_00_TX_CG_EN) // RG_*RPI_RESERVE_B0[0] 1'b1 prevent leakage
                                        | P_Fld(0x0, RG_ARPI_RESERVE_BIT_01_DLL_FAST_PSJP)
                                        | P_Fld(u1HYST_SEL, RG_ARPI_RESERVE_BIT_02_HYST_SEL)
                                        | P_Fld(u1MIDPI_CAP_SEL2, RG_ARPI_RESERVE_BIT_03_MIDPI_CAP_SEL)
                                        | P_Fld(u1ARPI_BIT4to10, RG_ARPI_RESERVE_BIT_04_8PHASE_XLATCH_FORCE)
                                        | P_Fld(u1ARPI_BIT4to10, RG_ARPI_RESERVE_BIT_05_PSMUX_XLATCH_FORCE)
    ...
    vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_DLL1, P_Fld(0x1, RG_ARCMD_REV_BIT_00_TX_LSH_DQ_CG_EN)
                                        ...
                                        | P_Fld(0x9, RG_ARCMD_REV_BIT_1208_TX_CKE_DRVN) //tg change 4->9
                                        ...
                                        | P_Fld(0x1, RG_ARCMD_REV_BIT_23_NA)); //tg change 0->1
    ...
    vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DLL1, P_Fld(0x0, RG_ARDQ_REV_BIT_00_DQS_MCK4X_DLY_EN)
                                        ...
                                        | P_Fld(0x1, RG_ARDQ_REV_BIT_23_NA)); //tg change 0->1
...
static void UpdateTxOEN(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU1_WODT, P_Fld(1, SHU1_WODT_DQOE_OPT)
                                          | P_Fld(u1DQ_OE_CNT, SHU1_WODT_DQOE_CNT));
...
static DRAM_STATUS_T UpdateInitialSettings_LP4(DRAMC_CTX_T *p)
{
    ...
    {
        vIO32WriteFldAlign(DRAMC_REG_SHU_ODTCTRL, 0x1, SHU_ODTCTRL_ROEN);
        ...
        vIO32WriteFldMulti(DDRPHY_SHU1_CA_CMD0, P_Fld(0x1, SHU1_CA_CMD0_RG_TX_ARCMD_PRE_EN) // OE Suspend EN tg change 0->1 for mp setting
                | P_Fld(0x0, SHU1_CA_CMD0_RG_TX_ARCLK_PRE_EN)); //ODT Suspend EN tg change 1->0 for mp setting
    }
    else
    {
        vIO32WriteFldAlign(DRAMC_REG_SHU_ODTCTRL, 0x0, SHU_ODTCTRL_ROEN);
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DQ3, P_Fld(0x1, B0_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B0) //tg change 1->0 for mp setting
                        | P_Fld(0x1, B0_DQ3_RG_RX_ARDQM0_IN_BUFF_EN_B0)
                        | P_Fld(0x1, B0_DQ3_RG_RX_ARDQS0_IN_BUFF_EN_B0)); //tg change 1->0 for mp setting
    vIO32WriteFldMulti(DDRPHY_B1_DQ3, P_Fld(0x1, B1_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B1)  //tg change 1->0 for mp setting
                        | P_Fld(0x1, B1_DQ3_RG_RX_ARDQM0_IN_BUFF_EN_B1)
                        | P_Fld(0x1, B1_DQ3_RG_RX_ARDQS0_IN_BUFF_EN_B1));  //tg change 1->0 for mp setting
    vIO32WriteFldMulti(DDRPHY_CA_CMD3, P_Fld(0x1, CA_CMD3_RG_RX_ARCMD_IN_BUFF_EN) //tg change 1->0 for mp setting
                            | P_Fld(0x1, CA_CMD3_RG_RX_ARCLK_IN_BUFF_EN) //tg change 1->0 for mp setting
                            | P_Fld(0x0, CA_CMD3_RG_RX_ARCMD_STBENCMP_EN)); //tg add for mp setting
    ...
    vIO32WriteFldAlign(DDRPHY_CA_CMD5, 0x0, CA_CMD5_RG_RX_ARCLK_DVS_EN); //tg change 1->0 for mp setting
    ...
    vIO32WriteFldMulti(DDRPHY_CA_CMD6, P_Fld(0x0, CA_CMD6_RG_TX_ARCMD_DDR3_SEL)
                                    ...
                                    | P_Fld(0x0, CA_CMD6_RG_RX_ARCMD_DDR4_SEL)); //tg change 1->0 for mp setting
    ...
    vIO32WriteFldAlign(DDRPHY_B0_DQ6, 0x0, B0_DQ6_RG_RX_ARDQ_O1_SEL_B0); //tg change 1->0 for mp setting
    vIO32WriteFldAlign(DDRPHY_B1_DQ6, 0x0, B1_DQ6_RG_RX_ARDQ_O1_SEL_B1); //tg change 1->0 for mp setting
    vIO32WriteFldAlign(DDRPHY_CA_CMD6, 0x0, CA_CMD6_RG_RX_ARCMD_O1_SEL); //tg change 1->0 for mp setting
    ...
    if (p->dram_type == TYPE_LPDDR4)
    {
        if(p->odt_onoff==ODT_ON)
        {
            u2RXVrefDefault = 0x10; //tg change e->0x10 for mp setting
        }
        else
        {
    ...
    else
    {
        vIO32WriteFldMulti(DDRPHY_B0_DQ8, P_Fld(0x0, B0_DQ8_RG_TX_ARDQ_EN_LP4P_B0)
                                            | P_Fld(0x1, B0_DQ8_RG_TX_ARDQ_EN_CAP_LP4P_B0) //tg change 0->1 for mp setting
                                            | P_Fld(0x1, B0_DQ8_RG_ARDLL_RESETB_B0) //tg change 0->1 for mp setting
                                            | P_Fld(0x0, B0_DQ8_RG_TX_ARDQ_CAP_DET_B0));
                                            //| P_Fld(0x1, B0_DQ8_RG_RX_ARDQS_DQSSTB_CG_EN_B0) // Field only exists for 10nm APHY
                                            //  corresponds to B0_DQ6_RG_RX_ARDQ_RPRE_TOG_EN_B0 for 16nm APHY
        vIO32WriteFldMulti(DDRPHY_B1_DQ8, P_Fld(0x0, B1_DQ8_RG_TX_ARDQ_EN_LP4P_B1)
                                            | P_Fld(0x1, B1_DQ8_RG_TX_ARDQ_EN_CAP_LP4P_B1) //tg change 0->1 for mp setting
                                            | P_Fld(0x1, B1_DQ8_RG_ARDLL_RESETB_B1) //tg change 0->1 for mp setting
                                            | P_Fld(0x0, B1_DQ8_RG_TX_ARDQ_CAP_DET_B1));
                                            //| P_Fld(0x1, B1_DQ8_RG_RX_ARDQS_DQSSTB_CG_EN_B1) // Field only exists for 10nm APHY
                                            //  corresponds to B1_DQ6_RG_RX_ARDQ_RPRE_TOG_EN_B1 for 16nm APHY
        vIO32WriteFldMulti(DDRPHY_CA_CMD9, P_Fld(0x0, CA_CMD9_RG_TX_ARCMD_EN_LP4P)
                                            | P_Fld(0x1, CA_CMD9_RG_TX_ARCMD_EN_CAP_LP4P)  //tg change 0->1 for mp setting
                                            | P_Fld(0x1, CA_CMD9_RG_ARDLL_RESETB_CA)   //tg change 0->1 for mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU1_DQSG, P_Fld(0x2a, SHU1_DQSG_SCINTV)
           | P_Fld((p->dram_fsp == FSP_1) ? 0x1 : 0x0, SHU1_DQSG_DQSINCTL_PRE_SEL)); //tg change for mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_IMPCAL1, P_Fld(8, SHU_IMPCAL1_IMPCAL_CALICNT)
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHU_IMPCAL1, 0x7, SHU_IMPCAL1_IMPCAL_CHKCYCLE); //tg change for mp setting
    //tg add for mp setting
	vIO32WriteFldMulti(DRAMC_REG_SHU_APHY_TX_PICG_CTRL, P_Fld(0xc, SHU_APHY_TX_PICG_CTRL_APHYPI_CG_CK_SEL)
                                            | P_Fld(0x1, SHU_APHY_TX_PICG_CTRL_APHYPI_CG_CK_OPT)
                                            | P_Fld(0x5, SHU_APHY_TX_PICG_CTRL_DDRPHY_CLK_DYN_GATING_SEL)
                                            | P_Fld(0x1, SHU_APHY_TX_PICG_CTRL_DDRPHY_CLK_EN_OPT));
    ...
    //vIO32WriteFldAlign(DRAMC_REG_PRE_TDQSCK1, 0x1, PRE_TDQSCK1_TXUIPI_CAL_CGAR); //tg change 1->0 for bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_CLKAR, P_Fld(0x1, CLKAR_SELPH_CMD_CG_DIS)
    ...
    vIO32WriteFldMulti(DRAMC_REG_PERFCTL0, P_Fld(0x0, PERFCTL0_WRFIFO_OPT) //tg change 1 -> 0 for bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHURK0_DQSCAL, P_Fld(0, SHURK0_DQSCAL_R0DQSIENLLMTEN)
                                              | P_Fld(0, SHURK0_DQSCAL_R0DQSIENHLMTEN));
    vIO32WriteFldMulti(DRAMC_REG_SHURK1_DQSCAL, P_Fld(0, SHURK1_DQSCAL_R1DQSIENLLMTEN)
    ...
    //tg change 7->4 for bring up setting
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x4, B0_DQ9_R_IN_GATE_EN_LOW_OPT_B0);
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x4, B1_DQ9_R_IN_GATE_EN_LOW_OPT_B1);
#endif
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DLL1, P_Fld(1, RG_ARDQ_REV_BIT_09_TX_READ_BASE_EN)
                                          | P_Fld(1, RG_ARDQ_REV_BIT_02_TX_READ_BASE_EN_DQSB)
                                          | P_Fld(!p->odt_onoff, RG_ARDQ_REV_BIT_08_TX_ODT_DISABLE));
    vIO32WriteFldMulti(DDRPHY_SHU1_B1_DLL1, P_Fld(1, RG_ARDQ_REV_BIT_09_TX_READ_BASE_EN)
                                          | P_Fld(1, RG_ARDQ_REV_BIT_02_TX_READ_BASE_EN_DQSB)
                                        | P_Fld(!p->odt_onoff, RG_ARDQ_REV_BIT_08_TX_ODT_DISABLE));
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_B1_DLL1, P_Fld(0, RG_ARDQ_REV_BIT_09_TX_READ_BASE_EN)
                                          | P_Fld(0, RG_ARDQ_REV_BIT_02_TX_READ_BASE_EN_DQSB)
                                        | P_Fld(0, RG_ARDQ_REV_BIT_08_TX_ODT_DISABLE));
    #endif
    ...
    //vIO32WriteFldAlign(DRAMC_REG_ZQCS, 0x1, ZQCS_ZQCS_MASK_SEL_CGAR); //tg change 1->0 f or bring up setting
    vIO32WriteFldMulti(DRAMC_REG_DUMMY_RD, P_Fld(0x1,DUMMY_RD_DMYRD_REORDER_DIS)
                                         | P_Fld(0x1,DUMMY_RD_DMYRD_HPRI_DIS));
    vIO32WriteFldAlign(DRAMC_REG_SHUCTRL2, 0x0, SHUCTRL2_R_DVFS_SREF_OPT); //tg change 1->0 for mp setting
     ...
     */
    vIO32WriteFldMulti(DDRPHY_B0_DLL_ARPI1, P_Fld(0x1, B0_DLL_ARPI1_RG_ARPISM_MCK_SEL_B0_OPT)
                                        | P_Fld(0x1, B0_DLL_ARPI1_RG_ARPISM_MCK_SEL_B0));
    vIO32WriteFldMulti(DDRPHY_B1_DLL_ARPI1, P_Fld(0x1, B1_DLL_ARPI1_RG_ARPISM_MCK_SEL_B1_OPT)
                                        | P_Fld(0x1, B1_DLL_ARPI1_RG_ARPISM_MCK_SEL_B1));
    vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI1, 0x1, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA_OPT);
    ...
    vIO32WriteFldAlign(DRAMC_REG_REFCTRL1, 0x1, REFCTRL1_SREF_CG_OPT);
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL, P_Fld(0x0, SHUCTRL_DVFS_CG_OPT)
                                        | P_Fld(0x3, SHUCTRL_R_DVFS_PICG_MARGIN2)
                                        | P_Fld(0x3, SHUCTRL_R_DVFS_PICG_MARGIN3));
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL2, P_Fld(0x1, SHUCTRL2_SHORTQ_OPT)
                                         | P_Fld(0x3, SHUCTRL2_R_DVFS_PICG_MARGIN));
    vIO32WriteFldAlign(DRAMC_REG_STBCAL2, 0x0, STBCAL2_STB_DBG_EN);
    vIO32WriteFldMulti(DRAMC_REG_PRE_TDQSCK1, P_Fld(0x0, PRE_TDQSCK1_APHY_CG_OPT1)
                                            | P_Fld(0x0, PRE_TDQSCK1_SHU_PRELOAD_TX_HW));

    #ifndef FIRST_BRING_UP
    //if(u2DFSGetHighestFreq(p) >= 1866) //tg change for MP setting
    #endif
    {
    ...
    vIO32WriteFldAlign(DRAMC_REG_CKECTRL, 0x0, CKECTRL_CKEPBDIS); //tg change 1->0 for MP setting
    ...
    //tg add for mp setting
    vIO32WriteFldMulti(DDRPHY_MISC_CTRL1_PSRAM, P_Fld(0x1, MISC_CTRL1_PSRAM_R_DM_TX_ARCLK_OE_PSRAM)
                                       | P_Fld(0x1, MISC_CTRL1_PSRAM_R_DM_TX_ARCMD_OE_PSRAM)
                                       | P_Fld(0x1, MISC_CTRL1_PSRAM_R_DMDQSIENCG_EN_PSRAM));

    vIO32WriteFldMulti(DDRPHY_SHU1_CA_CMD7_PSRAM, P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CMD_NEW_PSRAM)
                                       | P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CLK_NEW_PSRAM)
                                       | P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CS_NEW_PSRAM));

    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ7_PSRAM, P_Fld(0x1, SHU1_B0_DQ7_PSRAM_MIDPI_ENABLE_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQ_NEW_B0_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQS_NEW_B0_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQM_NEW_B0_PSRAM));
...
static DRAM_STATUS_T UpdateInitialSettings_LP3(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DDRPHY_R0_B0_RXDVS2, P_Fld(0x0, R0_B0_RXDVS2_R_RK0_DVS_MODE_B0)
                | P_Fld(0x0, R0_B0_RXDVS2_R_RK0_RX_DLY_RIS_TRACK_GATE_ENA_B0)
                | P_Fld(0x0, R0_B0_RXDVS2_R_RK0_RX_DLY_FAL_TRACK_GATE_ENA_B0));
    #if 0 //tg removed CHB CA and RANK1 setting
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DQ6 + SHIFT_TO_CHB_ADDR, P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_DDR3_SEL_B0));  //tg change for lp3 mp setting
    ...
    vIO32WriteFldMulti_All(DDRPHY_CA_CMD6, P_Fld(0x1, CA_CMD6_RG_TX_ARCMD_DDR3_SEL)
                                        | P_Fld(0x0, CA_CMD6_RG_RX_ARCMD_DDR3_SEL) //tg change 1->0 for lp3 mp setting
    ...
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ8, P_Fld(0x0, B0_DQ8_RG_TX_ARDQ_EN_LP4P_B0)
                                        | P_Fld(0x1, B0_DQ8_RG_ARDLL_RESETB_B0)); //tg add for lp3 mp setting
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ8, P_Fld(0x0, B1_DQ8_RG_TX_ARDQ_EN_LP4P_B1)
                                        | P_Fld(0x1, B1_DQ8_RG_ARDLL_RESETB_B1)); //tg add for lp3 mp setting
    vIO32WriteFldMulti_All(DDRPHY_CA_CMD9, P_Fld(0x0, CA_CMD9_RG_TX_ARCMD_EN_LP4P)
                                        | P_Fld(0x1, CA_CMD9_RG_ARDLL_RESETB_CA));  //tg add for lp3 mp setting

    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_B0_DQ5), 0x0, B0_DQ5_RG_RX_ARDQ_EYE_VREF_EN_B0);
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_B1_DQ5), 0x0, B1_DQ5_RG_RX_ARDQ_EYE_VREF_EN_B1);
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_CA_CMD5), 0x0, CA_CMD5_RG_RX_ARCMD_EYE_VREF_EN);
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DDRPHY_B0_DQ5), 0x10, B0_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B0); //tg change b -> 0x10 for lp3 mp setting
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_B1_DQ5), 0x10, B1_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B1); //tg change b->0x10 for lp3 mp setting
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_CA_CMD5), 0x8, CA_CMD5_RG_RX_ARCMD_EYE_VREF_SEL); //tg change b->8 for lp3 mp setting

    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DDRPHY_SHU1_B0_DQ5), 0xb, SHU1_B0_DQ5_RG_RX_ARDQ_VREF_SEL_B0);
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_SHU1_B1_DQ5), 0xb, SHU1_B1_DQ5_RG_RX_ARDQ_VREF_SEL_B1);
    vIO32WriteFldAlign_Phy_All(DRAMC_REG_ADDR(DDRPHY_SHU1_CA_CMD5), 0x0, SHU1_CA_CMD5_RG_RX_ARCMD_VREF_SEL); //tg change b->0 for lp3 mp setting

#if (fcFOR_CHIP_ID == fcSchubert) //ChA-B1, ChB-B0/B1/CA are used as RX
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ6, P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B0)
                                        | P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_RPRE_TOG_EN_B0)); //tg change for Schubert Lp3
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ6, P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B1)
    ...
    //vIO32WriteFldAlign_All(DDRPHY_CA_CMD6, 0x1, CA_CMD6_RG_RX_ARCMD_RES_BIAS_EN); //tg removed for lp3 mp setting
    ...
    //CH_A B0/CA = 0  -> CHB B0/CA
    vIO32WriteFldMulti(DDRPHY_B0_DQ3 + SHIFT_TO_CHB_ADDR, P_Fld(0x0, B0_DQ3_RG_RX_ARDQ_IN_BUFF_EN_B0)
    ...
    #endif
    vIO32WriteFldAlign(DDRPHY_B0_DQ9 + SHIFT_TO_CHB_ADDR, 0x0, B0_DQ9_R_IN_GATE_EN_LOW_OPT_B0);
    vIO32WriteFldAlign(DDRPHY_CA_CMD10, 0x0, CA_CMD10_R_IN_GATE_EN_LOW_OPT_CA);

#ifdef XRTR2R_PERFORM_ENHANCE_DQSG_RX_DLY
    vIO32WriteFldAlign_All(DDRPHY_B1_DQ9, 0x04, B1_DQ9_R_IN_GATE_EN_LOW_OPT_B1);
    vIO32WriteFldAlign_All(DDRPHY_B0_DQ9, 0x04, B0_DQ9_R_IN_GATE_EN_LOW_OPT_B0);
...
#else
    vIO32WriteFldAlign_All(DDRPHY_B1_DQ9, 0x4, B1_DQ9_R_IN_GATE_EN_LOW_OPT_B1);
    vIO32WriteFldAlign_All(DDRPHY_B0_DQ9, 0x4, B0_DQ9_R_IN_GATE_EN_LOW_OPT_B0); //tg change 7->4 for lp3 mp setting
    ...
    //tg add for lp3 bring up setting
    vIO32WriteFldMulti(DRAMC_REG_SHU_APHY_TX_PICG_CTRL, P_Fld(0xd, SHU_APHY_TX_PICG_CTRL_APHYPI_CG_CK_SEL)
                                            | P_Fld(0x6, SHU_APHY_TX_PICG_CTRL_DDRPHY_CLK_DYN_GATING_SEL));

    //tg add for lp3 mp setting
    vIO32WriteFldMulti(DRAMC_REG_SHU_APHY_TX_PICG_CTRL, P_Fld(0x1, SHU_APHY_TX_PICG_CTRL_APHYPI_CG_CK_OPT)
                                            | P_Fld(0x1, SHU_APHY_TX_PICG_CTRL_DDRPHY_CLK_EN_OPT));
    ...
    //Rx Gating tracking settings
    vIO32WriteFldMulti_All(DRAMC_REG_SHURK0_DQSCAL, P_Fld(0x0, SHURK0_DQSCAL_R0DQSIENLLMTEN)
                                                  | P_Fld(0x0, SHURK0_DQSCAL_R0DQSIENHLMTEN));
    vIO32WriteFldMulti_All(DRAMC_REG_SHURK1_DQSCAL, P_Fld(0x0, SHURK1_DQSCAL_R1DQSIENLLMTEN)
                                                  | P_Fld(0x0, SHURK1_DQSCAL_R1DQSIENHLMTEN));
    vIO32WriteFldMulti_All(DRAMC_REG_SHU_STBCAL, P_Fld(0x1, SHU_STBCAL_DQSG_MODE)
                                               | P_Fld(0x1, SHU_STBCAL_PICGLAT));

    //Jade-like ACTiming
    #if DramcHWDQSGatingTracking_DVT_JADE_TRACKING_MODE

    vIO32WriteFldAlign_All(DRAMC_REG_SHU_PIPE, 0xFC000000,PHY_FLD_FULL);
    //tg change 0x38 -> 0x0 for lp3 mp setting
    vIO32WriteFldAlign(DDRPHY_SHU1_B1_DQ6, 0x0, SHU1_B1_DQ6_RG_ARPI_OFFSET_DQSIEN_B1);
    vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ6, 0x0, SHU1_B0_DQ6_RG_ARPI_OFFSET_DQSIEN_B0); //tg change CHB B0 -> CHA B0
    #if 0 //tg remvoed CHB CA B1
    ...
    //if(p->frequency==933)//test code
    {
        ...
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DLL1, 1, RG_ARDQ_REV_BIT_05_RX_SER_RST_MODE);
        vIO32WriteFldAlign_All(DDRPHY_SHU1_CA_DLL1, 1, RG_ARCMD_REV_BIT_05_RX_SER_RST_MODE); //tg change 0->1 for lp3 mp setting
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B0_DLL1, 1, RG_ARDQ_REV_BIT_05_RX_SER_RST_MODE);
        #if 0 //tg remvoed CHB CA and B1 setting
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_SHU1_DQSG, P_Fld(0x1, SHU1_DQSG_STB_UPDMASK_EN)
                                              | P_Fld(0x1, SHU1_DQSG_DQSINCTL_PRE_SEL)
                                              | P_Fld(0x9, SHU1_DQSG_STB_UPDMASKCYC));
    //DMSTBLAT should set as 3 for JADE-like mode
    if(p->frequency>=800)
    {
        vIO32WriteFldAlign_All(DRAMC_REG_SHU_STBCAL, 2, SHU_STBCAL_DMSTBLAT); //tg change 3->2 for mp setting
    ...
    //Need to check with, tg change 4->0 for lp3 mp setting
    vIO32WriteFldAlign_All(DDRPHY_SHU1_B0_DQ5, 0, SHU1_B0_DQ5_RG_RX_ARDQS0_DQSIEN_DLY_B0);
    vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DQ5, 0, SHU1_B1_DQ5_RG_RX_ARDQS0_DQSIEN_DLY_B1);
    vIO32WriteFldAlign_All(DDRPHY_SHU1_CA_CMD5, 0, SHU1_CA_CMD5_RG_RX_ARCLK_DQSIEN_DLY);
    ...
    vIO32WriteFldMulti(DRAMC_REG_DRAMCTRL, P_Fld(0, DRAMCTRL_FW2R)
                                        ...
                                        | P_Fld(0x0, DRAMCTRL_TMRR2WDIS) //tg change 1->0 for lp3 bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL, P_Fld(0x1, SHUCTRL_VRCG_EN) //tg change 0->1 for lp3 mp setting
                                        | P_Fld(0x3, SHUCTRL_R_DVFS_PICG_MARGIN2)
                                        | P_Fld(0x3, SHUCTRL_R_DVFS_PICG_MARGIN3));
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL2, P_Fld(0x1, SHUCTRL2_SHORTQ_OPT) //YH: short mode selection 0:original (worst case 4 commands in queue before DVFS), 1: new (worst case 2 commands)
                                        | P_Fld(0x3, SHUCTRL2_R_DVFS_PICG_MARGIN)
                                        | P_Fld(0x0, SHUCTRL2_R_DVFS_SREF_OPT)); //sw to enable dvfs + refresh tg change 1->0 for bring up setting

    vIO32WriteFldMulti(DRAMC_REG_REFCTRL0, P_Fld(0x1, REFCTRL0_REFNA_OPT)
                | P_Fld(0x4, REFCTRL0_DISBYREFNUM) //tg change 3->4 for lp3 bring up setting
    ...
    vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ7, P_Fld(0x0, SHU1_B1_DQ7_R_DMRXRANK_DQS_LAT_B1)
                                | P_Fld(0x1, SHU1_B1_DQ7_R_DMRXRANK_DQS_EN_B1)
                                | P_Fld(0x1, SHU1_B1_DQ7_R_DMRXRANK_DQ_LAT_B1)
                                | P_Fld(0x1, SHU1_B1_DQ7_R_DMRXRANK_DQ_EN_B1));
    vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ7, P_Fld(0x0, SHU1_B0_DQ7_R_DMRXRANK_DQS_LAT_B0)
                                | P_Fld(0x1, SHU1_B0_DQ7_R_DMRXRANK_DQS_EN_B0)
                                | P_Fld(0x1, SHU1_B0_DQ7_R_DMRXRANK_DQ_LAT_B0)
                                | P_Fld(0x1, SHU1_B0_DQ7_R_DMRXRANK_DQ_EN_B0));
    ...
    #endif
#else
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ6, P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_OP_BIAS_SW_EN_B0)
                                | P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_BIAS_EN_B0));
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ6, P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_OP_BIAS_SW_EN_B1)
                                | P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_BIAS_EN_B1));
    vIO32WriteFldAlign(DRAMC_REG_STBCAL2, 0x1, STBCAL2_STB_PICG_EARLY_1T_EN);
    ...
    vIO32WriteFldMulti_All(DDRPHY_CA_DLL_ARPI1, P_Fld(0x0, CA_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_CA)
                ...
                | P_Fld(0x1, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA_OPT));
    vIO32WriteFldMulti_All(DDRPHY_B0_DLL_ARPI1, P_Fld(0x0, B0_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B0)
                ...
                | P_Fld(0x1, B0_DLL_ARPI1_RG_ARPISM_MCK_SEL_B0_OPT));
    vIO32WriteFldMulti_All(DDRPHY_B1_DLL_ARPI1, P_Fld(0x0, B1_DLL_ARPI1_RG_ARPI_MCTL_JUMP_EN_B1)
                ...
                | P_Fld(0x1, B1_DLL_ARPI1_RG_ARPISM_MCK_SEL_B1_OPT));
    ...
    //vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL4, 0x11400000, MISC_CG_CTRL4_R_PHY_MCK_CG_CTRL); //tg change 0x1330 0000 -> 0x1140 0000 for lp3 bring up setting
    ...
    vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD7, 0x0, SHU1_CA_CMD7_R_DMRANKRXDVS_CA);
    ...
    //vIO32WriteFldAlign_All(DRAMC_REG_CKECTRL, 0x1, CKECTRL_CKEPBDIS); //tg change 1->0 for bring up setting
    vIO32WriteFldAlign_All(DDRPHY_CA_TX_MCK, 0x1, CA_TX_MCK_R_DMRESET_FRPHY_OPT); //tg add for lp3 bring up setting

#if (fcFOR_CHIP_ID == fcSchubert)
    {
        //tg change 3->2 for lp3 mp setting
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B0_DQ3, 0x2, SHU1_B0_DQ3_RG_TX_ARDQ_PU_PRE_B0);
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DQ3, 0x2, SHU1_B1_DQ3_RG_TX_ARDQ_PU_PRE_B1);
    ...
    //[Cervino DVT]RX FIFO debug feature, MP setting should enable debug function
    vIO32WriteFldAlign_All(DDRPHY_B0_DQ9, 0x1, B0_DQ9_R_DMRXFIFO_STBENCMP_EN_B0); //tg chane for Schubert Lp3
    vIO32WriteFldAlign(DDRPHY_B1_DQ9, 0x1, B1_DQ9_R_DMRXFIFO_STBENCMP_EN_B1);
    vIO32WriteFldAlign(DDRPHY_CA_CMD10, 0x0, CA_CMD10_R_DMRXFIFO_STBENCMP_EN_CA); //tg change 1->0 for lp3 mp setting

    //tg add for lp3 mp setting
    vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD7_PSRAM, P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CMD_NEW_PSRAM)
                                       | P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CLK_NEW_PSRAM)
                                       | P_Fld(0x1, SHU1_CA_CMD7_PSRAM_R_DMTX_ARPI_CG_CS_NEW_PSRAM));

    vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ7_PSRAM, P_Fld(0x1, SHU1_B0_DQ7_PSRAM_MIDPI_ENABLE_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQ_NEW_B0_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQS_NEW_B0_PSRAM)
                                       | P_Fld(0x1, SHU1_B0_DQ7_PSRAM_R_DMTX_ARPI_CG_DQM_NEW_B0_PSRAM));
...
static void LegacyDlyCellInitLP3(DRAMC_CTX_T *p)
{
    ...
    #if 0 //tg remvoed CHB CA setting
    ...
    #endif
    vIO32WriteFldMulti(DDRPHY_SHU1_R1_B0_DQ0, P_Fld(0xb, SHU1_R1_B0_DQ0_RK1_TX_ARDQ7_DLY_B0)
    ...
    vIO32WriteFldAlign(DDRPHY_SHU1_R1_B0_DQ1, 0x6, SHU1_R1_B0_DQ1_RK1_TX_ARDQM0_DLY_B0);
...
static void LegacyGatingDlyLP3(DRAMC_CTX_T *p)
{
    ...
    #if 0 //tg removed RANK1 and CHB setting
...
static void LegacyRxDly_LP3(DRAMC_CTX_T *p)
{
   vIO32WriteFldMulti(DDRPHY_SHU1_R0_B1_DQ2, P_Fld(0x12, SHU1_R0_B1_DQ2_RK0_RX_ARDQ1_F_DLY_B1)
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_R0_B0_DQ6, P_Fld(0x12, SHU1_R0_B0_DQ6_RK0_RX_ARDQS0_F_DLY_B0)
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_R1_B0_DQ6, P_Fld(0x13, SHU1_R1_B0_DQ6_RK1_RX_ARDQS0_F_DLY_B0)
...
static void DramcSetting_Olympus_LP3(DRAMC_CTX_T *p)
{
...
#if (fcFOR_CHIP_ID == fcSchubert)                                        //before switch clock from 26M to PHY, need to init PHY clock first
    ...
    vIO32WriteFldMulti(DDRPHY_MISC_SPM_CTRL1, P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10)
                                            ...
                                            | P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10_B1)
                                            | P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10_CA));
    vIO32WriteFldMulti(DDRPHY_MISC_SPM_CTRL1+SHIFT_TO_CHB_ADDR, P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10)
                                                              | P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10_B0)
                                                              | P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10_B1)
                                                              | P_Fld(0x0, MISC_SPM_CTRL1_RG_ARDMSUS_10_CA));
    ...
    vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL4, 0x11400000, MISC_CG_CTRL4_R_PHY_MCK_CG_CTRL); //tg change for lp3 mp setting
    ...
    //            | P_Fld(0x1, SHU1_CA_CMD7_R_DMRANKRXDVS_CA)); //tg removed CA CHB
    ...
    //tg add for lp3 mp setting
    vIO32WriteFldAlign_All(DDRPHY_B1_DQ9, 0x1, B1_DQ9_R_DMRXDVS_RDSEL_LAT_B1);
    ...
    vIO32WriteFldAlign_All(DDRPHY_PLL3, 0x0, PLL3_RG_RPHYPLL_TSTOP_EN); //tg change for lp3 mp setting

    vIO32WriteFldAlign_All(DDRPHY_MISC_VREF_CTRL, 0x1, MISC_VREF_CTRL_RG_RVREF_VREF_EN);
    vIO32WriteFldMulti_All(DDRPHY_SHU1_MISC0, P_Fld(0xb, SHU1_MISC0_RG_RVREF_SEL_CMD) //tg change 0->b for lp3 mp setting
    ...
    vIO32WriteFldMulti(DDRPHY_B0_RXDVS0, P_Fld(0x1, B0_RXDVS0_R_RX_DLY_TRACK_CG_EN_B0)
                | P_Fld(0x0, B0_RXDVS0_R_RX_RANKINCTL_B0));
    #if 0 //tg removed CHB CA and B1 to Schubert
    ...
    vIO32WriteFldMulti(DDRPHY_B0_RXDVS0, P_Fld(0x1, B0_RXDVS0_R_DMRXDVS_CNTCMP_OPT_B0)
    ...
    #endif
    //tg change CHB B0 -> CHA B0
    if(p->frequency == 933)
    {
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ5, P_Fld(0x4, SHU1_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0) //tg change 0->4 for lp3 mp setting
                | P_Fld(0x0, SHU1_B0_DQ5_RG_RX_ARDQ_VREF_SEL_B0));
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DQ5, 0x4, SHU1_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1); //tg change 0->4 for lp3 mp setting
    }
    else if(p->frequency == 800)
    {
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ5, P_Fld(0x5, SHU1_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0) //tg change 0->5 for lp3 mp setting
                | P_Fld(0x0, SHU1_B0_DQ5_RG_RX_ARDQ_VREF_SEL_B0));
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DQ5, 0x5, SHU1_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1); //tg change 0->5 for lp3 mp setting
    }
    else
    {
        vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ5, P_Fld(0x6, SHU1_B0_DQ5_RG_RX_ARDQS0_DVS_DLY_B0) //tg change 0->6 for lp3 mp setting
                | P_Fld(0x0, SHU1_B0_DQ5_RG_RX_ARDQ_VREF_SEL_B0));
        vIO32WriteFldAlign_All(DDRPHY_SHU1_B1_DQ5, 0x6, SHU1_B1_DQ5_RG_RX_ARDQS0_DVS_DLY_B1); //tg change 0->6 for lp3 mp setting
    }
    #if 0 //tg removed CHB CA and B1
    ...
    #if 0 //tg removed CHB CA RANK1 setting
    ...
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x1, B0_DQ9_RG_RX_ARDQ_STBEN_RESETB_B0); //tg change CHB B0 -> CHA B0
    ...
    #endif
    vIO32WriteFldAlign(DDRPHY_SHU1_R0_B0_DQ7 + SHIFT_TO_CHB_ADDR, 0xc, SHU1_R0_B0_DQ7_RK0_ARPI_DQ_B0); //tg change CHA B0 -> CHB B0 Macro 1 CA delay
    ...
    #if 0 //tg removed CHB and rank1 setting
    ...
    #endif
    vIO32WriteFldMulti(DDRPHY_SHU1_R0_B0_DQ7, P_Fld(0xf, SHU1_R0_B0_DQ7_RK0_ARPI_DQM_B0)
                | P_Fld(0xf, SHU1_R0_B0_DQ7_RK0_ARPI_DQ_B0));  //tg change CHB B0 -> CHA B0
    #if 0 //tg removed rank1 setting
    ...
    //tg change for lp3 mp setting
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ4, P_Fld(0x12, B1_DQ4_RG_RX_ARDQS_EYE_R_DLY_B1)
                | P_Fld(0x1a, B1_DQ4_RG_RX_ARDQS_EYE_F_DLY_B1)
                | P_Fld(0xa, B1_DQ4_RG_RX_ARDQ_EYE_R_DLY_B1)
                | P_Fld(0x12, B1_DQ4_RG_RX_ARDQ_EYE_F_DLY_B1));
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ5, P_Fld(0x0, B1_DQ5_RG_RX_ARDQ_EYE_EN_B1) //tg change 1->0 for lp3 mp setting
                | P_Fld(0x0, B1_DQ5_RG_RX_ARDQ_EYE_SEL_B1) //tg change 1->0 for lp3 mp setting
                | P_Fld(0x1, B1_DQ5_RG_RX_ARDQ_VREF_EN_B1)
                | P_Fld(0x10, B1_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B1)
                | P_Fld(0x10, B1_DQ5_B1_DQ5_RFU));
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ6, P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_EYE_DLY_DQS_BYPASS_B1)
                | P_Fld(0x0, B1_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B1)
                | P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_BIAS_EN_B1)
                | P_Fld(0x1, B1_DQ6_RG_RX_ARDQ_OP_BIAS_SW_EN_B1));
    vIO32WriteFldMulti_All(DDRPHY_B1_DQ5, P_Fld(0x1, B1_DQ5_RG_RX_ARDQ_EYE_STBEN_RESETB_B1)
    ...
    */
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DQ6, P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_DDR3_SEL_B0) //tg change for Schubert Lp3
                | P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B0)); //tg change for Schubert lp3   
    vIO32WriteFldMulti(DDRPHY_B0_DQ6 + SHIFT_TO_CHB_ADDR, P_Fld(0x0, B0_DQ6_RG_RX_ARDQ_DDR3_SEL_B0) //tg change for Schubert Lp3
                | P_Fld(0x0, B0_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B0)
                | P_Fld(0x0, B0_DQ6_RG_RX_ARDQ_RES_BIAS_EN_B0)); //tg change for Schubert lp3  
    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ1 + SHIFT_TO_CHB_ADDR, P_Fld(0x1, SHU1_B0_DQ1_RG_TX_ARDQ_DRVN_B0)
                | P_Fld(0x1, SHU1_B0_DQ1_RG_TX_ARDQ_DRVP_B0));
    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ2 + SHIFT_TO_CHB_ADDR, P_Fld(0x1, SHU1_B0_DQ2_RG_TX_ARDQS0_DRVN_B0)
    ...
    #if 0 //tg removed CHB CA setting
    ...
    #endif
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ4, P_Fld(0x12, B0_DQ4_RG_RX_ARDQS_EYE_R_DLY_B0)
                | P_Fld(0x1a, B0_DQ4_RG_RX_ARDQS_EYE_F_DLY_B0)
                | P_Fld(0xa, B0_DQ4_RG_RX_ARDQ_EYE_R_DLY_B0)
                | P_Fld(0x12, B0_DQ4_RG_RX_ARDQ_EYE_F_DLY_B0)); //tg change CHB B0 -> CHA A0
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ5 , P_Fld(0x0, B0_DQ5_RG_RX_ARDQ_EYE_EN_B0) //tg change CHB B0 -> CHA A0
                | P_Fld(0x0, B0_DQ5_RG_RX_ARDQ_EYE_SEL_B0) //tg change 1->0 for lp3 mp setting
                | P_Fld(0x1, B0_DQ5_RG_RX_ARDQ_VREF_EN_B0) //tg change for Schubert Lp3
                | P_Fld(0x10, B0_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B0)
                | P_Fld(0x10, B0_DQ5_B0_DQ5_RFU));
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ6, P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_EYE_DLY_DQS_BYPASS_B0)//tg change CHB B0 -> CHA A0
                | P_Fld(0x0, B0_DQ6_RG_RX_ARDQ_BIAS_VREF_SEL_B0)
                | P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_BIAS_EN_B0)
                | P_Fld(0x1, B0_DQ6_RG_RX_ARDQ_OP_BIAS_SW_EN_B0));
    vIO32WriteFldMulti_All(DDRPHY_B0_DQ5, P_Fld(0x1, B0_DQ5_RG_RX_ARDQ_EYE_STBEN_RESETB_B0)
    ...
    vIO32WriteFldMulti_All(DDRPHY_PLL4, P_Fld(0x0, PLL4_RG_RPHYPLL_AD_MCK8X_EN)
                | P_Fld(0x1, PLL4_PLL4_RFU)
                | P_Fld(0x1, PLL4_RG_RPHYPLL_MCK8X_SEL));


    vIO32WriteFldMulti_All(DDRPHY_CA_DLL_ARPI1, P_Fld(0x1, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA)
   ...
   // vIO32WriteFldAlign(DDRPHY_CA_DLL_ARPI1+SHIFT_TO_CHB_ADDR, 0x1, CA_DLL_ARPI1_RG_ARPI_CMD_JUMP_EN);
    vIO32WriteFldAlign(DDRPHY_B0_DLL_ARPI1, 0x1, B0_DLL_ARPI1_RG_ARPI_DQ_JUMP_EN_B0); //swap CHA/B B0

    vIO32WriteFldMulti_All(DDRPHY_CA_CMD3, P_Fld(0x0, CA_CMD3_RG_RX_ARCMD_STBENCMP_EN) //tg change 1->0 for lp3 mp setting
                | P_Fld(0x1, CA_CMD3_RG_ARCMD_RESETB)
                | P_Fld(0x1, CA_CMD3_RG_TX_ARCMD_EN)
    ...
    vIO32WriteFldAlign_All(DDRPHY_PLL3, 0x1, PLL3_RG_RPHYPLL_TST_EN); //tg change for lp3 mp setting

    vIO32WriteFldMulti_All(DDRPHY_SHU1_PLL9, P_Fld(0x0, SHU1_PLL9_RG_RPHYPLL_MONCK_EN)
                | P_Fld(0x0, SHU1_PLL9_RG_RPHYPLL_MONVC_EN)
                | P_Fld(0x1, SHU1_PLL9_RG_RPHYPLL_LVROD_EN)  //tg change 0->1 for lp3 mp setting
                | P_Fld(0x2, SHU1_PLL9_RG_RPHYPLL_RST_DLY)); //tg change 0->2 for lp3 mp setting
    vIO32WriteFldMulti_All(DDRPHY_SHU1_PLL11, P_Fld(0x0, SHU1_PLL11_RG_RCLRPLL_MONCK_EN)
                ...
                | P_Fld(0x1, SHU1_PLL11_RG_RCLRPLL_RST_DLY)); //tg chagne 0->1 for lp3 mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_DRAMCTRL, P_Fld(0x0, DRAMCTRL_MRRIOPT)
                                         | P_Fld(0x0, DRAMCTRL_TMRRIBYRK_DIS)
                                         | P_Fld(0x1, DRAMCTRL_TMRRICHKDIS));
    vIO32WriteFldAlign(DRAMC_REG_SPCMDCTRL, 0x1, SPCMDCTRL_SC_PG_UPD_OPT);
    vIO32WriteFldMulti(DRAMC_REG_SPCMDCTRL, P_Fld(0x0, SPCMDCTRL_SC_PG_MPRW_DIS)
                                          | P_Fld(0x0, SPCMDCTRL_SC_PG_STCMD_AREF_DIS)
                                          | P_Fld(0x0, SPCMDCTRL_SC_PG_OPT2_DIS)
                                          | P_Fld(0x0, SPCMDCTRL_SC_PG_MAN_DIS));
    vIO32WriteFldAlign(DRAMC_REG_RKCFG, 0, RKCFG_TXRANK);
    ...
    vIO32WriteFldMulti(DRAMC_REG_DRAMCTRL, P_Fld(0x1, DRAMCTRL_MRRIOPT)
                                         | P_Fld(0x1, DRAMCTRL_TMRRIBYRK_DIS)
                                         | P_Fld(0x0, DRAMCTRL_TMRRICHKDIS));
    vIO32WriteFldAlign(DRAMC_REG_SPCMDCTRL, 0x0, SPCMDCTRL_SC_PG_UPD_OPT);
    vIO32WriteFldMulti(DRAMC_REG_SPCMDCTRL, P_Fld(0x1, SPCMDCTRL_SC_PG_MPRW_DIS)
                                          | P_Fld(0x1, SPCMDCTRL_SC_PG_STCMD_AREF_DIS)
                                          | P_Fld(0x1, SPCMDCTRL_SC_PG_OPT2_DIS)
                                          | P_Fld(0x1, SPCMDCTRL_SC_PG_MAN_DIS));
#endif
    //LP3 don't have MPC block ALE mode (review by YH)
    vIO32WriteFldMulti(DRAMC_REG_MPC_OPTION, P_Fld(0x0, MPC_OPTION_ZQ_BLOCKALE_OPT)
                                           | P_Fld(0x0, MPC_OPTION_MPC_BLOCKALE_OPT2)
                                           | P_Fld(0x0, MPC_OPTION_MPC_BLOCKALE_OPT1)
                                           | P_Fld(0x0, MPC_OPTION_MPC_BLOCKALE_OPT));

    //tg change 1->0 for mp setting
    vIO32WriteFldAlign(DRAMC_REG_CKECTRL, 0x0, CKECTRL_RUNTIMEMRRCKEFIX);//Set Run time MRR CKE fix to 1 in tMRRI old mode to avoid no ACK from precharge all
    ...
    vIO32WriteFldAlign(DDRPHY_B0_DQ9, 0x1, B0_DQ9_RG_RX_ARDQS0_STBEN_RESETB_B0); //tg change CHB B0 -> CHA B0
    //tg remove CHB B1
    ...
    vIO32WriteFldMulti(DDRPHY_MISC_CTRL1, P_Fld(0x1, MISC_CTRL1_R_DMDA_RRESETB_E)
                | P_Fld(0x0, MISC_CTRL1_R_DMDQSIENCG_EN) //tg change 1->0 for bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x0, SHU_DQSG_RETRY_R_RDY_SEL_DLE) //tg change 1->0 for lp3 mp setting
                | P_Fld(0x6, SHU_DQSG_RETRY_R_DQSIENLAT)
                | P_Fld(0x0, SHU_DQSG_RETRY_R_RETRY_ONCE)); //tg change 1->0 for lp3 mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_DDRCONF0, P_Fld(0x1, DDRCONF0_LPDDR3EN)
                | P_Fld(0x0, DDRCONF0_DM64BITEN) //tg change for 16bit Lp3
    #if ENABLE_AP_MEMORY_LP3
                | P_Fld(0x0, DDRCONF0_BK8EN)); //tg add to support 4bank AP memory
    #else
                | P_Fld(0x1, DDRCONF0_BK8EN));
    #endif
    vIO32WriteFldMulti(DRAMC_REG_STBCAL2, P_Fld(0x0, STBCAL2_STB_GERR_B23) //STBCAL gating error by byte2/3 (LP3), tg change 1->0 for lp3 bring up setting
    ...
    vIO32WriteFldMulti_All(DDRPHY_MISC_CTRL0, P_Fld(0x1, MISC_CTRL0_R_DQS0IEN_DIV4_CK_CG_CTRL)
    ...
    vIO32WriteFldMulti(DRAMC_REG_SREFCTRL, P_Fld(0x0, SREFCTRL_SREF_HW_EN)
                ...
                | P_Fld(0x1, SREFCTRL_SREF2_OPTION)); //tg change 0->1 for lp3 bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_SCINTV, P_Fld(0x0, SHU_SCINTV_DQS2DQ_SHU_PITHRD)
                ...
                //| P_Fld(0x0, SHU_SCINTV_RDDQC_INTV) //tg change 0->1 for bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_REFCTRL1, P_Fld(0x0, REFCTRL1_SREF_PRD_OPT)
                                         | P_Fld(0x5, REFCTRL1_MPENDREF_CNT)); //tg add for lp3 bring up setting
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL4, 0xfe, SHU1_PLL4_RG_RPHYPLL_RESERVED);
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL6, 0xfe, SHU1_PLL6_RG_RCLRPLL_RESERVED);
    ...
    //if(p->frequency==933)
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x0, SHU_DQSG_RETRY_R_RETRY_USE_BURST_MDOE)
                    ...
                    #endif
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_DQSIENLAT)  //tg change for lp3 mp setting
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_DM4BYTE));
        ...
        //tg change CHB B0 -> CHA B0
        vIO32WriteFldMulti(DDRPHY_SHU1_R0_B0_DQ2, P_Fld(0xa, SHU1_R0_B0_DQ2_RK0_RX_ARDQ1_F_DLY_B0)
    ...
    //vIO32WriteFldAlign(DRAMC_REG_MPC_OPTION, 0x1, MPC_OPTION_MPCRKEN); //tg change 1->0 for lp3 bring up setting
    vIO32WriteFldMulti(DRAMC_REG_DRAMC_PD_CTRL, P_Fld(0x1, DRAMC_PD_CTRL_PHYCLKDYNGEN)
                | P_Fld(0x1, DRAMC_PD_CTRL_DCMEN));
    #if 0 //tg removed Dramc CHB
    ...
    vIO32WriteFldAlign(DRAMC_REG_TEST2_1, 0x8000, TEST2_1_TEST2_BASE); //tg change lp3 TA2 BASE from h'10000 -> h'8000
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_CKECTRL, P_Fld(0x3, SHU_CKECTRL_SREF_CK_DLY)
                                            | P_Fld(0x3, SHU_CKECTRL_TCKESRX)); //tg add for bring up setting
...
#endif
    //tg change 0175656 -> 6543321 for lp3 mp setting
    ...
    //vIO32WriteFldAlign(DRAMC_REG_HW_MRR_FUN, 0x1, HW_MRR_FUN_MRR_HW_HIPRI); //tg change 1->0 for bring up setting
...
static void DramcSetting_Olympus_LP4_ByteMode_DDR2667(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHU_DQS2DQ_TX, 0x0, SHU_DQS2DQ_TX_OE2DQ_OFFSET); //tg change 1->0 for mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x0, SHU_DQSG_RETRY_R_DQSIENLAT)
                | P_Fld(0x0, SHU_DQSG_RETRY_R_DDR1866_PLUS)); //tg change fo mp setting
...
static void DramcSetting_Olympus_LP4_ByteMode_DDR1600(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_CKECTRL, P_Fld(0x3, SHU_CKECTRL_TCKESRX)); //tg change 0->3 for bring up setting
    ...
    {
        vIO32WriteFldMulti(DRAMC_REG_SHU1_WODT, P_Fld(0x0, SHU1_WODT_WPST2T) //tg change 1->0 for bring up setting
                    | P_Fld(0x0, SHU1_WODT_DBIWR));
    }
    vIO32WriteFldAlign(DRAMC_REG_SHU_DQS2DQ_TX, 0x0, SHU_DQS2DQ_TX_OE2DQ_OFFSET); //tg change 4->0 for bring up setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x0, SHU_DQSG_RETRY_R_DQSIENLAT) //tg change 3->0 for bring up setting
...
static void DramcSetting_Olympus_LP4_ByteMode(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DQ7, P_Fld(0x1, SHU1_B0_DQ7_MIDPI_ENABLE)
                | P_Fld(0x0, SHU1_B0_DQ7_MIDPI_DIV4_ENABLE)
                | P_Fld(0x2, SHU1_B0_DQ7_R_DMRANKRXDVS_B0)); //tg change 0->2 for mp setting
    vIO32WriteFldAlign(DDRPHY_SHU1_B1_DQ7, 0x0, SHU1_B1_DQ7_R_DMRANKRXDVS_B1);
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL4, 0xfe, SHU1_PLL4_RG_RPHYPLL_RESERVED);
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DQ4, P_Fld(0x39, B0_DQ4_RG_RX_ARDQS_EYE_R_DLY_B0) //tg change 0x10->0x39 for mp setting
                | P_Fld(0x39, B0_DQ4_RG_RX_ARDQS_EYE_F_DLY_B0) //tg change 0x10->0x39 for mp setting
                | P_Fld(0x2b, B0_DQ4_RG_RX_ARDQ_EYE_R_DLY_B0) //tg add for mp setting
                | P_Fld(0x2b, B0_DQ4_RG_RX_ARDQ_EYE_F_DLY_B0));  //tg add for mp setting
    vIO32WriteFldMulti(DDRPHY_B0_DQ5, P_Fld(0x0, B0_DQ5_RG_RX_ARDQ_EYE_EN_B0)
                | P_Fld(0x0, B0_DQ5_RG_RX_ARDQ_EYE_SEL_B0) //tg change 1->0 for mp setting
                | P_Fld(0x1, B0_DQ5_RG_RX_ARDQ_VREF_EN_B0)
                | P_Fld(0x10, B0_DQ5_RG_RX_ARDQ_EYE_VREF_SEL_B0) //tg change e->0x10 for mp setting
                | P_Fld(0x10, B0_DQ5_B0_DQ5_RFU));
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DQ5, P_Fld(0x1, B0_DQ5_RG_RX_ARDQ_EYE_STBEN_RESETB_B0)
                | P_Fld(0x0, B0_DQ5_B0_DQ5_RFU));
    vIO32WriteFldMulti(DDRPHY_B1_DQ4, P_Fld(0x39, B1_DQ4_RG_RX_ARDQS_EYE_R_DLY_B1) //tg change 0x10->0x39 for mp setting
                | P_Fld(0x39, B1_DQ4_RG_RX_ARDQS_EYE_F_DLY_B1) //tg change 0x10->0x39 for mp setting
                | P_Fld(0x2b, B1_DQ4_RG_RX_ARDQ_EYE_R_DLY_B1)  //tg add for mp setting
                | P_Fld(0x2b, B1_DQ4_RG_RX_ARDQ_EYE_F_DLY_B1));  //tg add for mp setting
    vIO32WriteFldMulti(DDRPHY_B1_DQ5, P_Fld(0x0, B1_DQ5_RG_RX_ARDQ_EYE_EN_B1)
                | P_Fld(0x0, B1_DQ5_RG_RX_ARDQ_EYE_SEL_B1) //tg change 1-> 0 for mp setting
                | P_Fld(0x1, B1_DQ5_RG_RX_ARDQ_VREF_EN_B1)
    ...
    //vIO32WriteFldMulti(DDRPHY_SHU2_CA_CMD2, P_Fld(0x1, SHU2_CA_CMD2_RG_TX_ARCLK_DRVN)
    //            | P_Fld(0x1, SHU2_CA_CMD2_RG_TX_ARCLK_DRVP));
    vIO32WriteFldMulti(DDRPHY_PLL3, P_Fld(0x0, PLL3_RG_RPHYPLL_TSTOP_EN)
                                  | P_Fld(0x1, PLL3_RG_RPHYPLL_TST_EN)); //tg change for mp setting
    vIO32WriteFldAlign(DDRPHY_MISC_VREF_CTRL, 0x1, MISC_VREF_CTRL_RG_RVREF_VREF_EN); //LP3 VREF tg change 0->1 for mp setting
    ...
    vIO32WriteFldMulti(DDRPHY_SHU1_PLL9, P_Fld(0x0, SHU1_PLL9_RG_RPHYPLL_MONCK_EN)
                | P_Fld(0x0, SHU1_PLL9_RG_RPHYPLL_MONVC_EN)
                | P_Fld(0x1, SHU1_PLL9_RG_RPHYPLL_LVROD_EN) //tg change 0->1 for mp setting
                | P_Fld(0x2, SHU1_PLL9_RG_RPHYPLL_RST_DLY)); //tg change 0->2 for mp setting
    vIO32WriteFldMulti(DDRPHY_SHU1_PLL11, P_Fld(0x0, SHU1_PLL11_RG_RCLRPLL_MONCK_EN)
                | P_Fld(0x0, SHU1_PLL11_RG_RCLRPLL_MONVC_EN)
                | P_Fld(0x0, SHU1_PLL11_RG_RCLRPLL_LVROD_EN)
                | P_Fld(0x1, SHU1_PLL11_RG_RCLRPLL_RST_DLY)); //tg change 0->1 for mp setting
 ...
 //
...
#if (fcFOR_CHIP_ID == fcSchubert)
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    vIO32WriteFldMulti(DDRPHY_SHU1_CA_DLL1, P_Fld(0x1, SHU1_CA_DLL1_RG_ARDLL_PD_CK_SEL_CA)
                                          | P_Fld(0x0, SHU1_CA_DLL1_RG_ARDLL_FASTPJ_CK_SEL_CA));
    #if 0 //tg removed CHB CA
    ...
    DramcBroadcastOnOff(DRAMC_BROADCAST_ON);
#endif
    vIO32WriteFldMulti(DDRPHY_SHU1_B0_DLL1, P_Fld(0x0, SHU1_B0_DLL1_RG_ARDLL_PD_CK_SEL_B0)
                                          | P_Fld(0x1, SHU1_B0_DLL1_RG_ARDLL_FASTPJ_CK_SEL_B0));
    vIO32WriteFldMulti(DDRPHY_SHU1_B1_DLL1, P_Fld(0x0, SHU1_B1_DLL1_RG_ARDLL_PD_CK_SEL_B1)
                                          | P_Fld(0x1, SHU1_B1_DLL1_RG_ARDLL_FASTPJ_CK_SEL_B1));
    ...
    vIO32WriteFldMulti(DRAMC_REG_SPCMDCTRL, P_Fld(0x0, SPCMDCTRL_SC_PG_MPRW_DIS)
                                          ...
                                          | P_Fld(0x0, SPCMDCTRL_SC_PG_OPT2_DIS)
                                          | P_Fld(0x0, SPCMDCTRL_SC_PG_MAN_DIS));
    vIO32WriteFldMulti(DRAMC_REG_MPC_OPTION, P_Fld(0x1, MPC_OPTION_ZQ_BLOCKALE_OPT)
                                           | P_Fld(0x1, MPC_OPTION_MPC_BLOCKALE_OPT2)
                                           | P_Fld(0x1, MPC_OPTION_MPC_BLOCKALE_OPT1)
                                           | P_Fld(0x1, MPC_OPTION_MPC_BLOCKALE_OPT));
    //fix rank at 0 to trigger new TMRRI setting
    vIO32WriteFldAlign(DRAMC_REG_RKCFG, 0, RKCFG_TXRANK);
    vIO32WriteFldAlign(DRAMC_REG_RKCFG, 0, RKCFG_TXRANKFIX);
#else
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU1_DRVING1, P_Fld(0xa, SHU1_DRVING1_DQSDRVP2)
                                             | P_Fld(0xa, SHU1_DRVING1_DQSDRVN2)
                                             | P_Fld(0xa, SHU1_DRVING1_DQSDRVP1)
                                             | P_Fld(0xa, SHU1_DRVING1_DQSDRVN1)
                                             | P_Fld(0xa, SHU1_DRVING1_DQDRVP2)
                                             | P_Fld(0xa, SHU1_DRVING1_DQDRVN2));
    vIO32WriteFldMulti(DRAMC_REG_SHU1_DRVING2, P_Fld(0xa, SHU1_DRVING2_DQDRVP1)
                                             | P_Fld(0xa, SHU1_DRVING2_DQDRVN1)
                                             | P_Fld(0xa, SHU1_DRVING2_CMDDRVP2)
                                             | P_Fld(0xa, SHU1_DRVING2_CMDDRVN2)
                                             | P_Fld(0xa, SHU1_DRVING2_CMDDRVP1)
                                             | P_Fld(0xa, SHU1_DRVING2_CMDDRVN1));
    vIO32WriteFldMulti(DRAMC_REG_SHU1_DRVING3, P_Fld(0xa, SHU1_DRVING3_DQSODTP2)
                                             | P_Fld(0xa, SHU1_DRVING3_DQSODTN2)
                                             | P_Fld(0xa, SHU1_DRVING3_DQSODTP)
                                             | P_Fld(0xa, SHU1_DRVING3_DQSODTN)
                                             | P_Fld(0xa, SHU1_DRVING3_DQODTP2)
                                             | P_Fld(0xa, SHU1_DRVING3_DQODTN2));
    vIO32WriteFldMulti(DRAMC_REG_SHU1_DRVING4, P_Fld(0xa, SHU1_DRVING4_DQODTP1)
                                             | P_Fld(0xa, SHU1_DRVING4_DQODTN1)
                                             | P_Fld(0xa, SHU1_DRVING4_CMDODTP2)
                                             | P_Fld(0xa, SHU1_DRVING4_CMDODTN2)
                                             | P_Fld(0xa, SHU1_DRVING4_CMDODTP1)
                                             | P_Fld(0xa, SHU1_DRVING4_CMDODTN1));
    /*
    ...
    */
    ...
    {
        vIO32WriteFldMulti(DDRPHY_MISC_CTRL0, P_Fld(0x1, MISC_CTRL0_R_STBENCMP_DIV4CK_EN)
                    | P_Fld(0x0, MISC_CTRL0_R_DMDQSIEN_FIFO_EN) //tg change 1->0 for bring up setting
                    | P_Fld(0x1, MISC_CTRL0_R_DMSTBEN_OUTSEL)
                    | P_Fld(0xf, MISC_CTRL0_R_DMDQSIEN_SYNCOPT));
    }
    ...
    vIO32WriteFldMulti(DRAMC_REG_SHU_ODTCTRL, P_Fld(0x1, SHU_ODTCTRL_RODTE)
                | P_Fld(0x1, SHU_ODTCTRL_RODTE2)
                | P_Fld(0x0, SHU_ODTCTRL_TWODT) //tg change 1->0 for bring up setting
                //| P_Fld(0x5, SHU_ODTCTRL_RODT) //Set in UpdateACTimingReg()
                | P_Fld(0x0, SHU_ODTCTRL_WOEN) //tg change 1->0 for bring up setting
                | P_Fld(0x1, SHU_ODTCTRL_ROEN));
    vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ7, 0x1, SHU1_B0_DQ7_R_DMRODTEN_B0);
    vIO32WriteFldAlign(DDRPHY_SHU1_B1_DQ7, 0x1, SHU1_B1_DQ7_R_DMRODTEN_B1);
    vIO32WriteFldAlign(DRAMC_REG_REFCTRL0, 0x5, REFCTRL0_REF_PREGATE_CNT);
    ...
    vIO32WriteFldAlign(DRAMC_REG_SHUCTRL, 0x1, SHUCTRL_LPSM_BYPASS_B);
	vIO32WriteFldMulti(DRAMC_REG_REFCTRL1, P_Fld(0x0, REFCTRL1_SREF_PRD_OPT)
                                         | P_Fld(0x0, REFCTRL1_PSEL_OPT1)
                                         | P_Fld(0x0, REFCTRL1_PSEL_OPT2)
                                         | P_Fld(0x0, REFCTRL1_PSEL_OPT3)
                                         | P_Fld(0x5, REFCTRL1_MPENDREF_CNT)); //tg add for Bring up setting
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL4, 0xfe, SHU1_PLL4_RG_RPHYPLL_RESERVED);
    //vIO32WriteFldAlign(DDRPHY_SHU1_PLL6, 0xfe, SHU1_PLL6_RG_RCLRPLL_RESERVED);
    vIO32WriteFldMulti(DRAMC_REG_REFRATRE_FILTER, P_Fld(0x1, REFRATRE_FILTER_PB2AB_OPT)
                                                | P_Fld(0x0, REFRATRE_FILTER_PB2AB_OPT1));
    ...
    vIO32WriteFldAlign(DRAMC_REG_CLKCTRL, 0x1, CLKCTRL_CLK_EN_1);
    vIO32WriteFldMulti(DRAMC_REG_REFCTRL0, P_Fld(0x4, REFCTRL0_DISBYREFNUM)
                | P_Fld(0x1, REFCTRL0_DQDRVSWUPD) //tg add for MP setting
                | P_Fld(0x1, REFCTRL0_DRVCGWREF) //tg add for MP setting
                | P_Fld(0x1, REFCTRL0_DLLFRZ));
    ...
    vIO32WriteFldMulti(DRAMC_REG_HW_MRR_FUN, P_Fld(0x0, HW_MRR_FUN_TRPMRR_EN)
                    | P_Fld(0x0, HW_MRR_FUN_TRCDMRR_EN)
                    | P_Fld(0x0, HW_MRR_FUN_TMRR_ENA)
                    | P_Fld(0x0, HW_MRR_FUN_MANTMRR)  //tg change 6->0 for bring up setting
    ...
    else
    {
        ...
        vIO32WriteFldAlign(DRAMC_REG_SPCMDCTRL, 0x1, SPCMDCTRL_REFR_BLOCKEN);
        vIO32WriteFldAlign(DRAMC_REG_RSTMASK, 0x0, RSTMASK_GT_SYNC_MASK);
        //vIO32WriteFldAlign(DRAMC_REG_RKCFG, 0x1, RKCFG_DQSOSC2RK); //tg change 1->0 for bring up setting
        vIO32WriteFldAlign(DRAMC_REG_MPC_OPTION, 0x1, MPC_OPTION_MPCRKEN);
        vIO32WriteFldAlign(DRAMC_REG_EYESCAN, 0x0, EYESCAN_RG_RX_MIOCK_JIT_EN);
...
//Sylvia MP setting should set SHU_STBCAL_DMSTBLAT as 0x2 (review by HJ Huang)
    ...
    if (vGet_Dram_CBT_Mode(p) == CBT_BYTE_MODE1)
        ...
        vIO32WriteFldMulti(DRAMC_REG_SHU_DQSG_RETRY, P_Fld(0x0, SHU_DQSG_RETRY_R_RETRY_ROUND_NUM) //tg change 1->0 for bring up setting
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_XSR_RETRY_SPM_MODE)
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_DQSIENLAT) //tg change 5->0 for mp setting
                    | P_Fld(0x0, SHU_DQSG_RETRY_R_DDR1866_PLUS));//tg change 1->0 for mp setting
#if LEGACY_TDQSCK_PRECAL
...
///TODO: DVFS_Enable
    ...
    DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
    //CH-A
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL, P_Fld(0x0, SHUCTRL_R_DRAMC_CHA) //tg change 1->0 for bring up setting
                                        | P_Fld(0x0, SHUCTRL_SHU_PHYRST_SEL));
    vIO32WriteFldAlign(DRAMC_REG_SHUCTRL2, 0x1, SHUCTRL2_R_DVFS_DLL_CHA);
    //CH-B
    #if 0  //tg removed CHB
    vIO32WriteFldMulti(DRAMC_REG_SHUCTRL+SHIFT_TO_CHB_ADDR, P_Fld(0x0, SHUCTRL_R_DRAMC_CHA)
                                                          | P_Fld(0x1, SHUCTRL_SHU_PHYRST_SEL));
    ...
    gu4DutyCalibrationTime = u4low_tick1 - u4low_tick0;
#endif
#endif
#endif
...
void vDramcACTimingOptimize(DRAMC_CTX_T *p)
{
    ...
    enum ACTimeIdx{GRP_DDR1600_ACTIM = 0, GRP_DDR2400_ACTIM, GRP_DDR2667_ACTIM, GRP_DDR2800_ACTIM,GRP_DDR3200_ACTIM, GRP_DDR3733_ACTIM, GRP_ACTIM_NUM};
    ...
    if(u1IsLP4Family(p->dram_type))  //already read MR8 for density update
    {
        ...
        switch (p->freqGroup)
        {
            ...
            case 1333:
                ...
                break;
            case 1400:
                u1FreqGrpIdx = GRP_DDR2800_ACTIM;
                break;
            case 1600:
...
DRAM_STATUS_T DdrUpdateACTimingReg(DRAMC_CTX_T *p, const ACTime_T *ACTbl)
{
    ...
    */
    ...
    //vIO32WriteFldAlign_All(DRAMC_REG_SHURK1_DQSCTL, ACTblFinal.dqsinctl, SHURK1_DQSCTL_R1DQSINCTL);// tg removed Rank 1 DQSINCTL
...
DRAM_STATUS_T DdrUpdateACTiming_EMI(DRAMC_CTX_T *p, AC_TIMING_EXTERNAL_T *ACRegFromEmi)
{
    ...
    //Retrieve ACTimingTable's corresponding index
    u1TimingIdx = u1GetACTimingIdx(p);
    if (u1TimingIdx >= TOTAL_AC_TIMING_NUMBER)
        return DRAM_FAIL;
...
#if (fcFOR_CHIP_ID == fcSchubert) //tg removed RANK1
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_ARBCTL), (p->dram_cbt_mode[RANK_0]), ARBCTL_RSV_SA3);
    #else
    vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_ARBCTL), (p->dram_cbt_mode[RANK_0]| p->dram_cbt_mode[RANK_1]), ARBCTL_RSV_SA3);
#endif
...
void DramcDummyReadForTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    if(p->frequency >= 1600)
    {
        vIO32WriteFldMulti_All(DRAMC_REG_DUMMY_RD, P_Fld(1, DUMMY_RD_DMY_RD_RX_TRACK)
                                                 | P_Fld(1, DUMMY_RD_DUMMY_RD_EN));
...
{
    vIO32WriteFldAlign_All(DRAMC_REG_IMPCAL, 0x1, IMPCAL_IMPCAL_HWSAVE_EN);
    vIO32WriteFldAlign_All(DRAMC_REG_SHU_IMPCAL1, (p->frequency == 1600) ? 0x10 : 0x0, SHU_IMPCAL1_IMPCALCNT); //tg change 3->0x10 for mp setting
}
#endif
...
void DramcImpedanceTrackingEnable(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_IMPEDAMCE_CTRL1, P_Fld(2, IMPEDAMCE_CTRL1_DQS1_OFF)
                                                    | P_Fld(2, IMPEDAMCE_CTRL1_DOS2_OFF));
    vIO32WriteFldMulti_All(DRAMC_REG_IMPEDAMCE_CTRL2, P_Fld(2, IMPEDAMCE_CTRL2_DQ1_OFF)
                                                    | P_Fld(2, IMPEDAMCE_CTRL2_DQ2_OFF));
    #endif

    //tg change 1->0 for mp setting
    //Write (DRAMC _BASE+ 0x8B) [31:0] = 32'he4000000//enable impedance tracking
    vIO32WriteFldAlign(DDRPHY_MISC_CTRL0, 0x0, MISC_CTRL0_IMPCAL_CHAB_EN);//Set CHA this bit to enable dual channel tracking
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_IMPCAL, P_Fld(1, IMPCAL_IMPCAL_HW)
                                           | P_Fld(0, IMPCAL_IMPCAL_EN)
                                           | P_Fld(0, IMPCAL_IMPCAL_SWVALUE_EN) //tg change 1->0 for mp setting
    ...
    vIO32WriteFldMulti(DRAMC_REG_IMPCAL, P_Fld(0, IMPCAL_DIS_SUS_DRV)
                                       | P_Fld(1, IMPCAL_DIS_SUS_CH1_DRV));
    vIO32WriteFldMulti(DRAMC_REG_IMPCAL+SHIFT_TO_CHB_ADDR, P_Fld(1, IMPCAL_DIS_SUS_DRV)
...
//B3 ==========================
    ...
    //tg add for MP setting
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_PRE_TDQSCK1), P_Fld(0x0, PRE_TDQSCK1_TDQSCK_HW_SW_UP_SEL)
                                | P_Fld(0x0, PRE_TDQSCK1_TDQSCK_REG_DVFS)
                                | P_Fld(0x0, PRE_TDQSCK1_TDQSCK_PRECAL_HW));
...
void DramcGatingDebug(DRAMC_CTX_T *p, U8 u1Channel)
{
    ...
    mcSHOW_DBG_MSG("\n[GatingDebug] CH %d, DQS count (B3->B0) 0x%x, 0x%x, 0x%x, 0x%x \nError flag Rank0 (B3->B0) %d %d  %d %d  %d %d  %d %d\n", \
    ...
    if (p->support_rank_num==RANK_DUAL)
    {
        ...
        mcSHOW_DBG_MSG("Error flag Rank1 (B3->B0) %d %d  %d %d  %d %d  %d %d\n", \
                                       (u4all_result_F>>24)&0xff, (u4all_result_R>>24)&0xff, \
                                       (u4all_result_F>>16)&0xff, (u4all_result_R>>16)&0xff, \
                                       (u4all_result_F>>8)&0xff,   (u4all_result_R>>8)&0xff, \
                                       (u4all_result_F)&0xff,         (u4all_result_R)&0xff);
...
void DramcDumpDebugInfo(DRAMC_CTX_T *p)
{
    ...
    for(channelIdx=CHANNEL_A; channelIdx< p->support_channel_num; channelIdx++)
    {
        ...
        mcSHOW_DBG_MSG("[CH%d] MRR(MR4) [10:8]=%x\n", channelIdx, u1RefreshRate);
    }
#endif
...
void TransferToSPMControl(DRAMC_CTX_T *p)
{
    ...
    else if(u1IsLP3Family(p->dram_type))//LP3
    {
#if ENABLE_LP3_SW
        vIO32WriteFldAlign_All(DDRPHY_MISC_SPM_CTRL0, 0xfbffefff, MISC_SPM_CTRL0_PHY_SPM_CTL0);
...
void LP4EnableDramcPhyDCM2Channel(DRAMC_CTX_T *p, BOOL bEn)//Should refer to "vSetChannelNumber"
{
    ...
    if(bEn)//DCM on
    {
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL0, P_Fld(0x0, MISC_CG_CTRL0_RG_CG_INFRA_OFF_DISABLE)
                                                   ...
                                                   | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_PSRAMC_CHA_CK_OFF)
                                                   | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_NAO_FORCE_OFF_PSRAMC));
        for(i1ShuIdx = DRAM_DFS_SHUFFLE_1; i1ShuIdx < i1ShuffleMax; i1ShuIdx++)
        {
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_B0_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B0)
                                       ...
                                       | P_Fld(0x1, SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0)
                                       | P_Fld(0x0, SHU1_B0_DQ8_R_RMRODTEN_CG_IG_B0)
                                       | P_Fld(0x0, SHU1_B0_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B0));//19th
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_B1_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMRANK_PIPE_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_RDSEL_TOG_PIPE_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_RDSEL_PIPE_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_FLAG_PIPE_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_FLAG_SYNC_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMSTBEN_SYNC_CG_IG_B1)
                                       ...
                                       | P_Fld(0x1, SHU1_B1_DQ8_R_RMRX_TOPHY_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_RMRODTEN_CG_IG_B1)
                                       | P_Fld(0x0, SHU1_B1_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B1));//19th
            vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_CA_CMD8_R_DMRANK_CHG_PIPE_CG_IG_CA)
                                      ...
                                      | P_Fld(0x0, SHU1_CA_CMD8_R_DMSTBEN_SYNC_CG_IG_CA)
                                      | P_Fld(0x0, SHU1_CA_CMD8_R_DMRXDLY_CG_IG_CA)
                                      | P_Fld(0x0, SHU1_CA_CMD8_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_CA)
    ...
    else//DCM off
    {
        ...
        for(i1ShuIdx = DRAM_DFS_SHUFFLE_1; i1ShuIdx < i1ShuffleMax; i1ShuIdx++)
        {
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x1, SHU1_B0_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B0)
                                      ...
                                      | P_Fld(0x1, SHU1_B0_DQ8_R_DMRXDVS_RDSEL_PIPE_CG_IG_B0)
                                      | P_Fld(0x1, SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0)
                                      | P_Fld(0x1, SHU1_B0_DQ8_R_RMRODTEN_CG_IG_B0)
                                      | P_Fld(0x1, SHU1_B0_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B0));//19th
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x1, SHU1_B1_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B1)
                                      ...
                                      | P_Fld(0x1, SHU1_B1_DQ8_R_DMRXDVS_RDSEL_PIPE_CG_IG_B1)
                                      | P_Fld(0x1, SHU1_B1_DQ8_R_RMRX_TOPHY_CG_IG_B1)
                                      | P_Fld(0x1, SHU1_B1_DQ8_R_RMRODTEN_CG_IG_B1)
                                      | P_Fld(0x1, SHU1_B1_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B1));//19th
            vIO32WriteFldMulti_All(DDRPHY_SHU1_CA_CMD8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_CA_CMD8_R_DMRANK_CHG_PIPE_CG_IG_CA)
                                     | P_Fld(0x0, SHU1_CA_CMD8_R_DMRANK_PIPE_CG_IG_CA)
                                     | P_Fld(0x0, SHU1_CA_CMD8_R_DMDQSIEN_RDSEL_TOG_PIPE_CG_IG_CA)
                                     | P_Fld(0x0, SHU1_CA_CMD8_R_DMDQSIEN_RDSEL_PIPE_CG_IG_CA)
...
void LP3EnableDramcPhyDCM(DRAMC_CTX_T *p, BOOL bEn)//Should refer to "vSetChannelNumber"
{
    ...
    if(bEn)//DCM on
    {
        vIO32WriteFldMulti(DDRPHY_MISC_CG_CTRL0, P_Fld(0x0, MISC_CG_CTRL0_RG_CG_INFRA_OFF_DISABLE)
                                               ...
                                               | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_PHY_OFF_DIABLE)
                                               | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_DRAMC_OFF_DISABLE)
                                               | P_Fld(0x1, MISC_CG_CTRL0_CLK_MEM_SEL)
                                               | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_EMI_OFF_DISABLE)
                                               | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_DRAMC_CHB_CK_OFF)      //20th
                                               | P_Fld(0x0, MISC_CG_CTRL0_RG_DBG_OUT_SEL)
                                               | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_NAO_FORCE_OFF)  //tg add
                                               | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_PSRAMC_CHA_CK_OFF)
                                               | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_NAO_FORCE_OFF_PSRAMC));
        ...
        //vIO32WriteFldAlign(SHIFT_TO_CHB_ADDR + DDRPHY_MISC_CG_CTRL4, 0x22200a00, MISC_CG_CTRL4_R_PHY_MCK_CG_CTRL);
        ...
        for(i1ShuIdx = DRAM_DFS_SHUFFLE_1; i1ShuIdx < i1ShuffleMax; i1ShuIdx++)
        {
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B0_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_B0_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B0)
                                               ...
                                               | P_Fld(0x0, SHU1_B0_DQ8_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_B0)
                                               | P_Fld(0x0, SHU1_B0_DQ8_R_DMRXDVS_RDSEL_PIPE_CG_IG_B0)
                                               | P_Fld(0x1, SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0) //tg change for SChubert LP3 SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0
                                               | P_Fld(0x0, SHU1_B0_DQ8_R_RMRODTEN_CG_IG_B0)
                                               | P_Fld(0x0, SHU1_B0_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B0));//19th

            vIO32WriteFldMulti(DDRPHY_SHU1_B1_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_B1_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B1)
                                               | P_Fld(0x0, SHU1_B1_DQ8_R_DMRANK_PIPE_CG_IG_B1)
                                               | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_RDSEL_TOG_PIPE_CG_IG_B1)
                                               | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_RDSEL_PIPE_CG_IG_B1)
                                               | P_Fld(0x0, SHU1_B1_DQ8_R_DMDQSIEN_FLAG_PIPE_CG_IG_B1)
            ...
            vIO32WriteFldMulti(SHIFT_TO_CHB_ADDR + DDRPHY_SHU1_CA_CMD8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x0, SHU1_CA_CMD8_R_DMRANK_CHG_PIPE_CG_IG_CA)
                                               ...
                                               | P_Fld(0x0, SHU1_CA_CMD8_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_CA)
                                               | P_Fld(0x0, SHU1_CA_CMD8_R_DMRXDVS_RDSEL_PIPE_CG_IG_CA)
                                               | P_Fld(0x1, SHU1_CA_CMD8_R_RMRX_TOPHY_CG_IG_CA)
                                               | P_Fld(0x0, SHU1_CA_CMD8_R_RMRODTEN_CG_IG_CA)
                                               | P_Fld(0x0, SHU1_CA_CMD8_R_DMRANK_RXDLY_PIPE_CG_IG_CA));//19th
            #endif
        }
    ...
    else//DCM off
    {
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL0, P_Fld(0x1, MISC_CG_CTRL0_RG_CG_INFRA_OFF_DISABLE)
                                                   ...
                                                   | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_DRAMC_CHB_CK_OFF) //20th
                                                   | P_Fld(0x0, MISC_CG_CTRL0_RG_DBG_OUT_SEL)
                                                   | P_Fld(0x0, MISC_CG_CTRL0_RG_CG_NAO_FORCE_OFF)  //tg add close PSRAM
                                                   | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_PSRAMC_CHA_CK_OFF)
                                                   | P_Fld(0x1, MISC_CG_CTRL0_RG_CG_NAO_FORCE_OFF_PSRAMC));
        for(i1ShuIdx = DRAM_DFS_SHUFFLE_1; i1ShuIdx < i1ShuffleMax; i1ShuIdx++)
        {
            ...
            vIO32WriteFldMulti(SHIFT_TO_CHB_ADDR + DDRPHY_SHU1_B0_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x1, SHU1_B0_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B0)
                                                   ...
                                                   | P_Fld(0x1, SHU1_B0_DQ8_R_DMRXDVS_RDSEL_TOG_PIPE_CG_IG_B0)
                                                   | P_Fld(0x1, SHU1_B0_DQ8_R_DMRXDVS_RDSEL_PIPE_CG_IG_B0)
                                                   | P_Fld(0x0, SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0) //tg change for SChubert LP3 SHU1_B0_DQ8_R_RMRX_TOPHY_CG_IG_B0
                                                   | P_Fld(0x1, SHU1_B0_DQ8_R_RMRODTEN_CG_IG_B0)
                                                   | P_Fld(0x1, SHU1_B0_DQ8_R_DMRANK_RXDLY_PIPE_CG_IG_B0));//19th
            vIO32WriteFldMulti_All(DDRPHY_SHU1_B1_DQ8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x1, SHU1_B1_DQ8_R_DMRANK_CHG_PIPE_CG_IG_B1)
                                                   | P_Fld(0x1, SHU1_B1_DQ8_R_DMRANK_PIPE_CG_IG_B1)
            ...
            vIO32WriteFldMulti(SHIFT_TO_CHB_ADDR + DDRPHY_SHU1_CA_CMD8 + SHU_GRP_DDRPHY_OFFSET * i1ShuIdx, P_Fld(0x1, SHU1_CA_CMD8_R_DMRANK_CHG_PIPE_CG_IG_CA)
                                                  ...
                                                  | P_Fld(0x1, SHU1_CA_CMD8_R_DMRXDVS_RDSEL_PIPE_CG_IG_CA)
                                                  | P_Fld(0x1, SHU1_CA_CMD8_R_RMRX_TOPHY_CG_IG_CA)
                                                  | P_Fld(0x1, SHU1_CA_CMD8_R_RMRODTEN_CG_IG_CA)
                                                  | P_Fld(0x1, SHU1_CA_CMD8_R_DMRANK_RXDLY_PIPE_CG_IG_CA));//19th
            #endif
        }
    ...
    vIO32WriteFldMulti(DDRPHY_B0_DLL_ARPI2, P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_MCK_B0)
                                          ...
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_DQ_B0)
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B0)
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_MPDIV_CG_B0)); //tg change B0_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B0 for Schubert Lp3
    vIO32WriteFldMulti(DDRPHY_B0_DLL_ARPI2 + SHIFT_TO_CHB_ADDR, P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_MCK_B0)
                                          ...
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_DQM_B0)
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_DQ_B0)
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_CG_DQSIEN_B0) //tg change 1->0
                                          | P_Fld(0x0, B0_DLL_ARPI2_RG_ARPI_MPDIV_CG_B0)); //tg change for Schubert Lp3
...
void EnableDramcPhyDCM(DRAMC_CTX_T *p, BOOL bEn)//Should refer to "vSetChannelNumber"
{
    ...
    vIO32WriteFldAlign_All(DDRPHY_B1_DLL_ARPI1, 0x0, B1_DLL_ARPI1_RG_ARPISM_MCK_SEL_B1_OPT);
    vIO32WriteFldAlign_All(DDRPHY_CA_DLL_ARPI1, 0x0, CA_DLL_ARPI1_RG_ARPISM_MCK_SEL_CA_OPT);
    ...
    if(bEn)//DCM on
    {
        ...
        vIO32Write4B_All(DDRPHY_MISC_CG_CTRL2, 0x806003BE);//divided freq change to 1/4
#else
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL2, P_Fld(0x0, MISC_CG_CTRL2_RG_MEM_DCM_APB_TOG)
    ...
    else//DCM off
    {
        ...
        //mem_dcm
        //tg change [5:1] from 0x1f -> 0x17 for bring up setting
#if 0
        vIO32Write4B_All(DDRPHY_MISC_CG_CTRL2, 0x8060036E);//divided freq change to 1/4
        vIO32Write4B_All(DDRPHY_MISC_CG_CTRL2, 0x8060036F);//divided freq change to 1/4
        vIO32Write4B_All(DDRPHY_MISC_CG_CTRL2, 0x8060036E);//divided freq change to 1/4
#else
        vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL2, P_Fld(0x0, MISC_CG_CTRL2_RG_MEM_DCM_APB_TOG)
                                               | P_Fld(0x17, MISC_CG_CTRL2_RG_MEM_DCM_APB_SEL)
...
void vCKERankCtrl(DRAMC_CTX_T *p, CKE_CTRL_MODE_T CKECtrlMode)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_SHU_CONF2, P_Fld(CKE_Mode.u1FASTWake, SHU_CONF2_FASTWAKE)
                                              | P_Fld(CKE_Mode.u1FASTWake2, SHU_CONF2_FASTWAKE2));
...
void DramcConfInfraReset(DRAMC_CTX_T *p)
{
    ...
    *(volatile unsigned *)(IO_PHYS + 0x00001140) = (0x1 << 15);
    ...
    *(volatile unsigned *)(IO_PHYS + 0x00001144) = (0x1 << 15);
    ...
    *(volatile unsigned *)(0x10007018) = 0x88000000;
    ...
    //Disable SPM control
    vIO32WriteFldMulti(SPM_POWERON_CONFIG_EN, P_Fld(0xB16, POWERON_CONFIG_EN_PROJECT_CODE)
                                            | P_Fld(0, POWERON_CONFIG_EN_BCLK_CG_EN));

    //For FMeter after dcm enable
    vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL2, 0x0, MISC_CG_CTRL2_RG_MEM_DCM_DCM_EN);
    vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL2, 0x0, MISC_CG_CTRL2_RG_MEM_DCM_FORCE_ON); //tg change 1->0 for bring up setting
#endif
}
#endif
...
void SetMr13VrcgToNormalOperation(DRAMC_CTX_T *p)
{
    ...
    u1MR13Value &= ~(0x3<<3);  //tg change 0x1 -> 0x3 for mp setting
...
#if PSRAM_SPEC
    else if (p->dram_type == TYPE_PSRAM)
    {
...
#if ENABLE_LP3_SW && ENABLE_AP_MEMORY_LP3
    mcSHOW_DBG_MSG("LP3 AP Memory PER_BANK_REFRESH: OFF\n");
#else
...
#if (fcFOR_CHIP_ID != fcSchubert) //tg removed CHB
#if ENABLE_LP3_SW
...
void DramcEngine2SetPat(DRAMC_CTX_T *p, U8 testaudpat, U8 log2loopcount, U8 Use_Len1_Flag)
{
    if (testaudpat == TEST_XTALK_PATTERN)   // xtalk
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_3), P_Fld(0, TEST2_3_TESTAUDPAT)
                                                            | P_Fld(log2loopcount,TEST2_3_TESTCNT)); //dont use audio pattern

        // set addr 0x48[16] to 1, TESTXTALKPAT = 1
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_4), P_Fld(1, TEST2_4_TESTXTALKPAT)
                                                            | P_Fld(0,TEST2_4_TESTAUDMODE)
                                                            | P_Fld(0,TEST2_4_TESTAUDBITINV));  //use XTALK pattern, dont use audio pattern

        // R_DMTESTSSOPAT=0, R_DMTESTSSOXTALKPAT=0
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_4), P_Fld(0, TEST2_4_TESTSSOPAT)
                                                            | P_Fld(0,TEST2_4_TESTSSOXTALKPAT));   //dont use sso, sso+xtalk pattern
    }
    else if (testaudpat == TEST_AUDIO_PATTERN)   // audio
    {
        ...
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_4), \
            ...
            P_Fld(0, TEST2_4_TESTXTALKPAT) | P_Fld(0,TEST2_4_TESTAUDMODE) | \
            P_Fld(1,TEST2_4_TESTAUDBITINV));

        // set addr 0x044 [7] to 1 ,select audio pattern
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_3), P_Fld(1, TEST2_3_TESTAUDPAT)
                                                            | P_Fld(log2loopcount,TEST2_3_TESTCNT));
    }
    else   // ISI
    {
        // select ISI pattern
...
U32 DramcEngine2Compare(DRAMC_CTX_T *p, DRAM_TE_OP_T wr)
{
    ...
    if (wr == TE_OP_WRITE_READ_CHECK)
    {
        ...
        // disable write
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_3), P_Fld(0, TEST2_3_TEST2W)
                                                            | P_Fld(0, TEST2_3_TEST2R)
...
//    if ((log2loopcount > 15) || (log2loopcount < 0))      // U8 >=0 always.
    ...
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_DUMMY_RD), P_Fld(0, DUMMY_RD_DQSG_DMYRD_EN)
    ...
    // 1
    vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_0), P_Fld(test2_1>>24, TEST2_0_TEST2_PAT0)
    ...
    {
        //mcSHOW_ERR_MSG("tg change lp3 TA2 base is h'8000\n");
        vIO32WriteFldAlign(DRAMC_REG_ADDR(DRAMC_REG_TEST2_1), (test2_1 + 0x8000) & 0x00ffffff, TEST2_1_TEST2_BASE);
    }
#else
...
U32 DramcEngine2Run(DRAMC_CTX_T *p, DRAM_TE_OP_T wr, U8 testaudpat)
{
    ...
    {
        // enable write
        vIO32WriteFldMulti(DRAMC_REG_ADDR(DRAMC_REG_TEST2_3), P_Fld(1, TEST2_3_TEST2W)
                                                            | P_Fld(0, TEST2_3_TEST2R)
                                                            | P_Fld(0, TEST2_3_TEST1));
    }
 ...
 */
//-------------------------------------------------------------------------
void DramcRegDumpRange(U64 u4StartAddr, U64 u4EndAddr)
{
    U32 ii;
...
#define DDRPHY_NAO_DUMP_RANGE (DDRPHY_GATING_TRACKING_STATUS0 - DDRPHY_NAO_BASE_ADDRESS)
...
#if ETT_NO_DRAM
void NoDramDramcRegDumpRange(U64 u4StartAddr, U64 u4EndAddr)
{
    U32 ii;
...
DRAM_STATUS_T NoDramDramcRegDump(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG("\n\tPHY_A_BASE\n");
    NoDramDramcRegDumpRange(Channel_A_PHY_AO_BASE_ADDRESS, Channel_A_PHY_AO_BASE_ADDRESS+0x1FC8);
    mcSHOW_DBG_MSG("\n\tPHY_B_BASE\n");
    NoDramDramcRegDumpRange(Channel_B_PHY_AO_BASE_ADDRESS, Channel_B_PHY_AO_BASE_ADDRESS+0x1FC8);
...
{
    {DRAMC_AO_BASE_ADDRESS+(0x0800), DRAMC_AO_BASE_ADDRESS+(0x08E4) }, //tg change 0x8DC -> 0x8E4 for mp setting
...
{
#ifdef HW_GATING
    if((u1IsLP3Family(p->dram_type) || (u1IsLP4Family(p->dram_type))))
    {
...
void DramcSaveToShuffleReg(DRAMC_CTX_T *p, DRAM_DFS_SHUFFLE_TYPE_T srcRG, DRAM_DFS_SHUFFLE_TYPE_T dstRG)
{
    ...
    //PHY
#if ((fcFOR_CHIP_ID == fcSchubert) && ENABLE_LP3_SW)
    for(i=0; i<=CHANNEL_NUM; i++) //LP3 use PHY CHB
#else
    for(i=0; i<CHANNEL_NUM; i++)
#endif
    {
...
#if __ETT__ || defined(SLT) || ENABLE_HQA_DVFS
U8 gDVFSCtrlSel=0;
DRAM_PLL_FREQ_SEL_T currFreqSel = PLL_FREQ_SEL_MAX;
static U8 su4DfsCnt = 1;
...
void DramcDFSDirectJump(DRAMC_CTX_T *p, U8 shu_level)
{
    if(gDVFSCtrlSel==0)
    {
        mcSHOW_DBG_MSG("%s enter SPM mode\n", __FUNCTION__);
        DramcDFSDirectJump_SPMMode(p, shu_level);
    }
    else
    {
        mcSHOW_DBG_MSG("%s enter RG mode\n", __FUNCTION__);
...
void DramcDFS(DRAMC_CTX_T *p, int iDoDMA)
{
    ...
    //    return;
    mcSHOW_INFO_MSG("[DramcDFS] currFreqSel=%d, target=%d\n", currFreqSel, targetFreqSel);
    ...
    else if(targetFreqSel==LP4_LOWEST_FREQSEL)
    {
...
void SetDVSProgram(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_DBG_MSG("[V] Frequency = %d, Vcore = %d\n", p->frequency, dramc_get_vcore_voltage());
    mcDELAY_US(15); // 1us = 8mV
}
#endif

void DFSTestProgram(DRAMC_CTX_T *p, int iDoDMA)
{
    U8 updown = FALSE; //0:down, 1:up
    ...
    if(updown)
    {
        mcSHOW_DBG_MSG("Start Voltage ----------------------------------------------> Freq\n");
        SetDVSProgram(p);
    }
    else
        mcSHOW_DBG_MSG("Start Freq ----------------------------------------------> Voltage\n");
#endif
    ...
    {
        SetDVSProgram(p);
        mcSHOW_DBG_MSG("End Freq -----------------------------------------------> Voltage\n");
    }
    else
        mcSHOW_DBG_MSG("End Voltage -----------------------------------------------> Freq\n");
#endif

    su4DfsCnt++;
}

void dramc_dvfs_test(DRAMC_CTX_T *p)
{
    U32 i;
    U32 forever = 0, times;

    if (HQA_DVFS_CHANGE_CNT == 0)
        forever = 1;
    else
        times = HQA_DVFS_CHANGE_CNT;
...
#endif /* ENABLE_LP3_SW */
	return LP4_HIGHEST_FREQ;	/* default as LP4 highest freq */
}
...
void DFSInitForCalibration(DRAMC_CTX_T *p)
{
    ...
    mcSHOW_TIME_MSG("  (1) DFSInitForCalibration() take %s ms %d\n\n", 
        int_div_to_double(CPU_Cycle, 13000, 5), CPU_Cycle/1000 - gu4DutyCalibrationTime);
    mcSHOW_TIME_MSG("  (2) DramcNewDutyCalibration take %s ms\n\r",
        int_div_to_double(gu4DutyCalibrationTime, 13000, 5));
#endif
#endif
...
#ifdef DDR_INIT_TIME_PROFILING
      CPU_Cycle = TimeProfileEnd();
      mcSHOW_TIME_MSG("  (3) JMeter takes %s ms %d\n\r", 
...
if(u1IsLP4Family(p->dram_type))
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL), P_Fld(1, STBCAL_STB_DQIEN_IG)
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL1), P_Fld(1, STBCAL1_STBCAL_FILTER)
                                                            | P_Fld(1, STBCAL1_STB_FLAGCLR)
                                                            | P_Fld(1, STBCAL1_STB_SHIFT_DTCOUT_IG));

    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DDRPHY_MISC_CTRL0), P_Fld(1, MISC_CTRL0_R_DMDQSIEN_FIFO_EN)
    ...
    #else
    //REFUICHG=0, STB_SHIFT_DTCOUT_IG=0, DQSG_MODE=1, NARROW_IG=0
        vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL),  \
...
//Set LP3 as FIFO mode to gain AC timing margin (review by HJ Huang)
    ...
    //LP3 FIFO mode need to add  this setting (HJ)
        vIO32WriteFldAlign_All(DDRPHY_B1_DQ9, 0x1, B1_DQ9_R_DMDQSIEN_RDSEL_LAT_B1);
        vIO32WriteFldAlign_All(DDRPHY_B0_DQ9, 0x1, B0_DQ9_R_DMDQSIEN_RDSEL_LAT_B0);
    ...
    vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_REFCTRL0), 0, REFCTRL0_UPDBYWR);
        vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL), \
                                             P_Fld(1, STBCAL_STB_DQIEN_IG) //tg change 0->1 for lp3 mp setting
                                           | P_Fld(1, STBCAL_PICHGBLOCK_NORD) //tg change 0->1 for lp3 mp setting
                                           | P_Fld(0, STBCAL_REFUICHG) //tg change 1->0 for lp3 mp setting
                                           | P_Fld(0, STBCAL_PHYVALID_IG)
                                           | P_Fld(0, STBCAL_STBSTATE_OPT) //tg change 1->0 for lp3 mp setting
                                           | P_Fld(0, STBCAL_STBDLELAST_FILTER)
                                           | P_Fld(0, STBCAL_STBDLELAST_PULSE)
                                           | P_Fld(0, STBCAL_STBDLELAST_OPT)
                                           | P_Fld(1, STBCAL_PIMASK_RKCHG_OPT)); //tg change 0->1 for lp3 mp setting

    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL1), \
                                             P_Fld(1, STBCAL1_STBCAL_FILTER) //tg change 0->1 for mp setting
                                           | P_Fld(1, STBCAL1_STB_SHIFT_DTCOUT_IG) //tg add for mp setting
                                           | P_Fld(1, STBCAL1_STB_SHIFT_DTCOUT_IG) //tg add for mp setting
                                           | P_Fld(1, STBCAL1_STB_FLAGCLR)
                                           | P_Fld(0, STBCAL1_STB_SHIFT_DTCOUT_IG));

    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DDRPHY_MISC_CTRL0), \
                                             P_Fld(1, MISC_CTRL0_R_DMDQSIEN_FIFO_EN) //tg change 0->1 for lp3 mp setting
                                           | P_Fld(0, MISC_CTRL0_R_DMVALID_DLY) //tg change 2->0 for lp3 mp setting
                                           | P_Fld(0, MISC_CTRL0_R_DMVALID_DLY_OPT) //tg change 1->0 for lp3 mp setting
                                           | P_Fld(0, MISC_CTRL0_R_DMVALID_NARROW_IG) //tg change 1->0 for lp3 mp setting
                                           | P_Fld(1, MISC_CTRL0_IMPCAL_LP_ECO_OPT) //tg add for lp3 mp setting
                                           | P_Fld(0, MISC_CTRL0_R_DMDQSIEN_SYNCOPT)); //tg change f->0 for lp3 mp setting

        //tg change 1->0 for lp3 mp setting
        vIO32WriteFldAlign_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL1), 1, STBCAL1_STBCNT_MODESEL);
...
void DramcHWGatingInit(DRAMC_CTX_T *p)
{
    ...
    vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_STBCAL), P_Fld(0, STBCAL_STBCALEN)
                                                           | P_Fld(0, STBCAL_STBCAL2R)
                                                           | P_Fld(0, STBCAL_STB_SELPHYCALEN)
                                                           | P_Fld(0, STBCAL_STBSTATE_OPT)
                                                           | P_Fld(0, STBCAL_RKCHGMASKDIS)
                                                           | P_Fld(0, STBCAL_REFUICHG)
...
void DramcPrintHWGatingStatus(DRAMC_CTX_T *p, U8 u1Channel)
{
    ...
    for(u1RankIdx=0; u1RankIdx<u1RankMax; u1RankIdx++)
    {
        ...
        mcSHOW_DBG_MSG("[DramcHWGatingStatus] Channel=%d, Rank=%d, freq = %d\n", p->channel, u1RankIdx, p->frequency);
...
void DramcDRS(DRAMC_CTX_T *p, U8 bEnable)
{
    ...
    {
        vIO32WriteFldMulti_All(DRAMC_REG_ADDR(DRAMC_REG_DRSCTRL), P_Fld(1, DRSCTRL_DRSCLR_RK0_EN)
                                        | P_Fld(1, DRSCTRL_DRSCLR_EN)
                                        | P_Fld(8, DRSCTRL_DRSDLY));
    }
}
...
static U32 FMeter(U8 u1CLKMeterSel, U8 u1CLKMuxSel)
{
    ...
    tmp |= (0x1 << 12);
    tmp &= ~(0x1 << 8);
    DRV_WriteReg32(CLK26CALI_0, tmp); // [12]=1: [8]=0: switch in_clk/clk27m
...
void DDRPhyFreqMeter()
{
    ...
    // abist_clk50: AD_MPLL_104M_CK
    mcSHOW_DBG_MSG("AD_MPLL_104M_CK FREQ=%d\n", FMeter(ABIST_FMETER, 43));
...
void GetPhyPllFrequency(DRAMC_CTX_T *p)
{
    ...
    U32 u4DataRate = u4VCOFreq>>u4CKDIV4;
#endif
    mcSHOW_INFO_MSG("PCW=0x%X, u4PREDIV=%d, u4POSDIV=%d, CKDIV4=%d, DataRate=%d\n", u4SDM_PCW, u4PREDIV, u4POSDIV, u4CKDIV4, u4DataRate);
}
#endif
#endif
...
#ifdef DDR_INIT_TIME_PROFILING
uint64_t l_low_tick0, l_high_tick0, l_low_tick1, l_high_tick1;
static uint64_t time_begin, time_end;
void TimeProfileBegin(void)
{
#if __ETT__
    l_low_tick0 = GPT_GetTickCount(&l_high_tick0);
#else
    time_begin = read_cntpct();
#endif
}
...
#else
    time_end = read_cntpct();
    dprintf(ALWAYS, "ts:%lld, te:%lld, elapsed time:%sms %lld\n", time_begin, time_end, 
        int_div_to_double(time_end - time_begin, 13000, 5), time_end - time_begin);
    return (U32)(time_end - time_begin);    // for compatibility with old codes
#endif
}
#endif
...
const ACTime_T ACTimingTbl[TOTAL_AC_TIMING_NUMBER] =
{
    ...
    // LP4-3200, 1600MHz, RDBI_OFF, normal mode
    {
        ...
        //DQSINCTL, DATLAT aren't in ACTiming excel file
        .dqsinctl = 5,   .datlat = 15 //tg change 4->5 for mp setting(6)
    },
#endif //(ENABLE_READ_DBI)
#endif //SUPPORT_LP4_DDR3200_ACTIM

#if SUPPORT_LP4_DDR2800_ACTIM
#if (ENABLE_READ_DBI == 1)
    //AC_TIME_LP4_BYTE_DDR3200_DBI_ON
    // LP4-3200, 1600MHz, RDBI_ON, byte mode
    {
        .dramType = TYPE_LPDDR4, .freq = 1400, .cbtMode = CBT_BYTE_MODE1, .readDBI = DBI_ON,
        .readLat = 36,  .writeLat = 14,
    ...
    // LP4-3200, 1600MHz, RDBI_ON, normal mode
    {
        .dramType = TYPE_LPDDR4, .freq = 1400, .cbtMode = CBT_NORMAL_MODE, .readDBI = DBI_ON,
        .readLat = 32,  .writeLat = 14,
    ...
    // LP4-3200, 1600MHz, RDBI_OFF, byte mode
    {
        .dramType = TYPE_LPDDR4, .freq = 1400, .cbtMode = CBT_BYTE_MODE1, .readDBI = DBI_OFF,
        .readLat = 32,  .writeLat = 14,
    ...
    //AC_TIME_LP4_NORM_DDR2800_DBI_OFF
    // LP4-2800, 1400MHz, RDBI_OFF, normal mode
    {
        .dramType = TYPE_LPDDR4, .freq = 1400, .cbtMode = CBT_NORMAL_MODE, .readDBI = DBI_OFF,
        .readLat = 28,  .writeLat = 14,
    ...
    // LP4-2400, 1200MHz, RDBI_OFF, normal mode
    {
        ...
        //DQSINCTL, DATLAT aren't in ACTiming excel file
        .dqsinctl = 4,   .datlat = 13 //tg change 3->4 for mp setting
    },
#endif
    ...
    // LP4-1600, 800MHz, RDBI_OFF, normal mode
    {
        ...
        //DQSINCTL, DATLAT aren't in ACTiming excel file
        .dqsinctl = 1,   .datlat = 11 // tg change a -> b for DE posim value
    },
    ...
    // LP3-1866, 933MHz
    {
        ...
        .r_dmmrw_intv = 0xf, //Berson: LP3/4 both use this field -> Formula may change, set to 0xF for now
        .r_dmfspchg_prdcnt = 0x75, //LP3 doesn't use this field, tg change for lp setting
        .trtpd = 10,    .trtpd_05T = 0,
        ...
        //DQSINCTL, DATLAT aren't in ACTiming excel file
        .dqsinctl = 5,  .datlat = 18 //tg change 17->18 for posim value
    },
    ...
    // LP3-1600, 800MHz
    {
        ...
        .txp = 1,   .txp_05T = 1,    //tg updated from Schubert AC timing tb
        .trtp = 1,  .trtp_05T = 0,
        .trcd = 9,  .trcd_05T = 0,
        .twr = 10,  .twr_05T = 1,
        .twtr = 6,  .twtr_05T = 1,
    ...
    // LP3-1333, 667MHz
    {
        ...
        .txp = 1,   .txp_05T = 1,   //tg update from Schubert AC timing tb
        .trtp = 1,  .trtp_05T = 1,
        .trcd = 7,  .trcd_05T = 0,
        .twr = 9,   .twr_05T = 1,
        .twtr = 5,  .twtr_05T = 0,
    ...
    // LP3-1200, 600MHz
    {
        ...
        .r_dmcatrain_intv = 0, //LP3 doesn't use this field
        .r_dmmrw_intv = 0xf, //Berson: LP3/4 both use this field -> Formula may change, set to 0xF for now
        .r_dmfspchg_prdcnt = 0x4b, //LP3 doesn't use this field
        .trtpd = 7, .trtpd_05T = 0,
        .twtpd = 7, .twtpd_05T = 0,
        .tmrr2w_ODT_off = 0, //LP3 doesn't use this field
        .tmrr2w_ODT_on = 0, //LP3 doesn't use this field
...
#if (fcFOR_CHIP_ID != fcSchubert) //Schubert single rank normal mode
...
#if (FOR_DV_SIMULATION_USED == 0)
static int dramc_get_efuse_bits(int addr, int start_bits, int end_bits)
{
/* ****note that****: Todo: efuse may be not ready, verify before use! */
#define EFUSE_BASE	(IO_PHYS + 0x01C50000)

	U32 val;

	if(start_bits < 0 || end_bits < 0 || start_bits > 32 || end_bits > 32
	|| start_bits > end_bits)
    {
		return -1;
    }
	val = READ_REGISTER_UINT32(EFUSE_BASE + addr);
	return ((-1U << start_bits) & (-1U >> (31 - end_bits)) & val) >> start_bits;
}

IC_TYPE dramc_get_efuse_ic_type()
{
	return dramc_get_efuse_bits(0xCA0, 1, 2);
}

DRAM_PACKAGE_TYPE dramc_get_efuse_dram_type()
{
	return dramc_get_efuse_bits(0xCA0, 3, 4);
}

DRAM_SIZE dramc_get_efuse_dram_size()
{
	return dramc_get_efuse_bits(0xCA0, 5, 7);
}

DRAM_VENDOR dramc_get_efuse_dram_vendor()
{
	return dramc_get_efuse_bits(0xCA0, 8, 10);
}
#endif

#ifdef TRIGGER_WDT_RESERVE_MODE_BY_DDR_SELF
void cpu_mem_test_vfy(unsigned int start_addr, unsigned int end_addr)
{
	unsigned long long addr = DRAM_BASE_VIRT;
	unsigned long long temp, temp_data;
	unsigned int  result, result_ok;

	end_addr >>=1;

	result = 0;
	result_ok = 0;
	for (temp = start_addr; temp < end_addr; temp=temp+0x4)
	{
		temp_data = *(volatile unsigned int *)(addr + temp);
		if (temp != temp_data)
		{
			result++;
			if (result_ok == 0)
			{
				mcSHOW_DBG_MSG("[ch]ok->err, expect value : %x, read out : %x\n", temp, temp_data);
			}
			result_ok = 1;
		}
		else
		{
			if (result_ok == 1)
			{
				mcSHOW_DBG_MSG("[ch]err->ok, expect value : %x, read out : %x\n", temp, temp_data);
			}
			result_ok = 0;
		}
	}
	...
	if (result == 0)
	{
		mcSHOW_DBG_MSG("[ch]dram mem test 0x%x ~ 0x%x pass, result : %d\n", start_addr, end_addr, result);
	}
	else
	{
		mcSHOW_DBG_MSG("[ch]dram mem test 0x%x ~ 0x%x fail, result : %d\n", start_addr, end_addr, result);
	}
}

void cpu_mem_test_full(unsigned int len)
{
	unsigned long long addr = DRAM_BASE_VIRT;
	unsigned long long temp, temp_data;
	unsigned int result, result_ok;
	...
	mcSHOW_DBG_MSG("[ch]dram mem test write length: 0 ~ 0x%x\n", len);

	for (temp = 0; temp < len; temp=temp+0x4) {
		*(volatile unsigned int *)(addr + temp) = temp;
	}
	...
	if (result == 0)
		mcSHOW_DBG_MSG("[ch]dram mem test 0 ~ 0x%x pass, result : %d\n", len, result);
	else
		mcSHOW_DBG_MSG("[ch]dram mem test 0 ~ 0x%x fail, result : %d\n", len, result);
}

void Dramc_Trigger_Wdt_Reset()
{
	mcSHOW_DBG_MSG("[ch]+++ trigger ddr reserve mode by wdt reset\n");

	*(volatile unsigned int *)(IO_PHYS + 0x00007044) = 0x95001840;
	/* enable reserve mode and disable wdt reset */
	*(volatile unsigned int *)(IO_PHYS + 0x00007000) = 0x22000080; //bit7 : reserve mode enable; bit0: wdt reset enable
	/* reset wdt */
	*(volatile unsigned int *)(IO_PHYS + 0x00007014) = 0x1209;

	mcSHOW_DBG_MSG("[ch]--- exit trigger wdt reset\n");
//	while(1);
}
#endif

void dramc_dump_memory(void *start_addr, unsigned int size)
{
    unsigned int i, cnt, extra;
    int *pa;
    char *pc;

    pa = (int *)start_addr;
    cnt = size / sizeof(int);
    extra = size - cnt * sizeof(int);

    for(i = 0; i < cnt; ++i) {
        dprintf(ALWAYS, "0x%x ", pa[i]);

        if((i + 1) % (16 / sizeof(int)) == 0)
            dprintf(ALWAYS, "\n");
    }

    pc = (char *)(pa + cnt);
    for(i = 0; i < extra; ++i)
        dprintf(ALWAYS, "0x%x ", pc[i]);
