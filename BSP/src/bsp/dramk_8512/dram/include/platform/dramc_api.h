...
//DRAMC Chip:  TODO: remove when MP
#define fcA60501        1
#define fcElbrus        3
#define fcOlympus       4
#define fcA60838        5
...
#define fcFOR_CHIP_ID   fcSchubert
#define fcFOR_PINMUX    fcSchubert

#define VENDOR_SAMSUNG 1
#define VENDOR_NANYA   5
#define VENDOR_HYNIX 6
#define VENDOR_WINBOND 8
...
#ifdef DRAM_SLT
#define DRAM_SLT_ORIGION_OFFSET 0
#define DRAM_SLT_FULL_SIZE      0x10000000
#define DRAM_SLT_1K_LENGTH      0x400
#define DRAM_SLT_STEP_SIZE      (1024)
#endif

#define TX_WIN_CRITERIA		16  /*16PI*/
#define RX_WIN_CRITERIA		51	/*0.51UI*/
#define PERBIT_THRESHOLD_FREQ	(800)

//Feature option
#define DUMP_ALLSHU_RG       0    //dump rg for mp/bring up setting compare
#define ENABLE_NEW_SWIMP_CAL 1
#define ENABLE_AP_MEMORY_LP3 SUPPORT_TYPE_LPDDR3   //tg add for support AP memory LP3
#define ENABLE_CS1_CKE1_FORCEOFF 1  //tg add for single rank,multi rank must disable it
...
#define ENABLE_LP3_SW SUPPORT_TYPE_LPDDR3 // for LP4 only project, can set to 0 to save code size.
...
#define SPECIAL_WORKAROUND_FOR_LP4 SUPPORT_TYPE_LPDDR4///TODO: will be removed
...
#define CA_TRAINING_K_RANK1_ENABLE 0
 ...
 */
#define LP4_HIGHEST_DDR3200      ( ! LP4_MAX_2400) && (! LP4_MAX_2667)
#define LP4_HIGHEST_DDR2667      (LP4_MAX_2667)

#if LP4_HIGHEST_DDR3200
    #define LP4_HIGHEST_FREQ     (1600)
    #define LP4_MIDDLE_FREQ      (1200)
    #define LP4_LOWEST_FREQ      (600)
    #define LP4_HIGHEST_FREQSEL  (LP4_DDR3200)
    #define LP4_MIDDLE_FREQSEL   (LP4_DDR2400)
    #define LP4_LOWEST_FREQSEL   (LP4_DDR1200)
#elif LP4_HIGHEST_DDR2667
    #define LP4_HIGHEST_FREQ     (1333)
    #define LP4_MIDDLE_FREQ      (800)
    #define LP4_LOWEST_FREQ      (600)
    #define LP4_HIGHEST_FREQSEL  (LP4_DDR2667)
 ...
 */
#define SAMSUNG_LP4_NWR_WORKAROUND SUPPORT_TYPE_LPDDR4
...
#define DEFAULT_TEST2_2_CAL 0xaa000080   //tg add for lp3 AP memory, offset need < 0x100
#else
...
// common
#define DQS_NUMBER   2      //tg change for Schubert 16bit lp3/4
#define DQS_NUMBER_LP3   2 //tg change for Schubert Lp3
...
#if FOR_DV_SIMULATION_USED
#define GATING_ADJUST_TXDLY_FOR_TRACKING  0 //tg disable when run simulation
#else
...
#define TX_K_DQM_WITH_WDBI  SUPPORT_TYPE_LPDDR4
...
#define ENABLE_HQA_DVFS 0
#define HQA_DVFS_CHANGE_CNT 0 // if 0: means infinite, else, do dvfs for the HQA_DVFS_CHANGE_CNT times
...
#define ZQCS_ENABLE_LP4   SUPPORT_TYPE_LPDDR4	//Whitney E1 can't enable HQZQ,  enable SW ZQ @ kernel.
#endif
...
#endif
#define IMPEDANCE_TRACKING_ENABLE SUPPORT_TYPE_LPDDR4//Impendence tracking
#define IMPEDANCE_HW_SAVING   //mask because function fail, it lets clk swing change larger before DVFS occurs

#define MR13_RRO 0 //MR4 refresh rate option, 0:Disable codes 001 and 010 in MR4 OP[2:0]   1:Enable all codes in MR4 OP[2:0]
...
#define ENABLE_TX_TRACKING 1
    ...
    #endif
#endif
#define ENABLE_RX_TRACKING_LP4 SUPPORT_TYPE_LPDDR4
...
//#define XRTR2R_PERFORM_ENHANCE_DQSG_RX_DLY //tg marked for Schubert
...
#define CA_PER_BIT_DELAY_CELL SUPPORT_TYPE_LPDDR4 //LP4
...
//#define TRIGGER_WDT_RESERVE_MODE_BY_DDR_SELF	// for ddr reserve mode test by preloader
...
#if !__ETT__
#if (FOR_DV_SIMULATION_USED==0)
#define SUPPORT_SAVE_TIME_FOR_CALIBRATION    (DRAM_FAST_K & EMMC_PROJECT)
#else
// DV simulation, use full calibration flow
#define SUPPORT_SAVE_TIME_FOR_CALIBRATION    0
#endif
#define EMMC_READY              (DRAM_FAST_K & EMMC_PROJECT)
...
#if EMMC_READY
// **** boot para offset may be different by projects! ****
#define BOOT_PARA_OFFSET    (512 * 0x29B800)
#endif
...
#define PSRAM_RX_EYE 0
#define PSRAM_TX_EYE 0
#define PSRAM_VREF_FROM_RTN 0
#endif
...
#define SUPPORT_LP4_DDR3733_ACTIM 0  //tg change 1 -> 0 for Schubert
#define SUPPORT_LP4_DDR3200_ACTIM 1
#define SUPPORT_LP4_DDR2800_ACTIM 1
...
#if ENABLE_READ_DBI
    AC_TIME_LP4_BYTE_DDR2800_RDBI_ON,
    AC_TIME_LP4_NORM_DDR2800_RDBI_ON,
#else //(ENABLE_READ_DBI == 0)
    AC_TIME_LP4_BYTE_DDR2800_RDBI_OFF,
    AC_TIME_LP4_NORM_DDR2800_RDBI_OFF,
#endif //ENABLE_READ_DBI
#endif
...
#if SUPPORT_SAVE_TIME_FOR_CALIBRATION
#define FAST_K_LK_VERSION  0x8512002
...
typedef struct _SAVE_TIME_FOR_CALIBRATION_T
{
    ...
    U8 u1CBTCsFirstPass_Save[CHANNEL_NUM][RANK_MAX];
    U8 u1CBTCsLastPass_Save[CHANNEL_NUM][RANK_MAX];
    U16 u1CBTCmdPerbitWinMinMax_Save;
    U16 u1CBTCmdPerbitWinMinMaxIdx_Save;
    U16 u1CBTCmdWinSumMax_Save;
    PASS_WIN_DATA_T u1CBT_FinalWinPerCA[CATRAINING_NUM_LP4];
    #if CA_PER_BIT_DELAY_CELL
    U8 u1CBTCA_PerBit_DelayLine_Save[CHANNEL_NUM][RANK_MAX][CATRAINING_NUM];
    #endif
    ...
    U16 u1Txwin_size_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];
    U8 u1TX_PerBit_DelayLine_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];

    // Datlat
    U8 u1RxDatlat_Save[CHANNEL_NUM][RANK_MAX];
    U8 u1RxDatlat_ucfirst_Save[CHANNEL_NUM][RANK_MAX];
    U8 u1RxDatlat_ucsum_Save[CHANNEL_NUM][RANK_MAX];
    ...
    S16 u1Rxfirst_pass_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];
    S16 u1Rxlast_pass_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];
    S16 u1Rxwin_size_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];
    S16 u1Rxwin_center_Save[CHANNEL_NUM][RANK_MAX][DQ_DATA_WIDTH_LP4];
    
    //TX OE
    U8 u1TX_OE_DQ_MCK[CHANNEL_NUM][RANK_MAX][DQS_NUMBER_LP4];
    U8 u1TX_OE_DQ_UI[CHANNEL_NUM][RANK_MAX][DQS_NUMBER_LP4];
    U8 u1TX_OE_DQ_End[DQS_NUMBER_LP4];
    U8 u1TX_OE_DQ_Best_Step[DQS_NUMBER_LP4];
...
#ifdef DDR_INIT_TIME_PROFILING
#define TB      { dprintf(ALWAYS, "[%s@%d] begin: \n", __func__, __LINE__); TimeProfileBegin(); }
#define TE      { dprintf(ALWAYS, "[%s@%d] end: ", __func__, __LINE__); TimeProfileEnd(); }
#endif
...
extern uint64_t read_cntpct(void);
...
#ifdef DRAM_SLT
extern int dram_boundary_scan(DRAMC_CTX_T *p);
...
void dram_slt_set(DRAMC_CTX_T *p, unsigned char cal_type, unsigned char result);
