...
extern DRAMC_CTX_T DramCtx_pSRAM;
...
static int Reserve_Sync_Writel(U64 addr, unsigned int val)
{
...
static unsigned int Reserve_Reg_Readl(U64 addr)
{
	return (*(volatile unsigned int *)(addr));
}

#define RE_POWERON_CONFIG_EN               (SPM_BASE_ADDRESS)
#define RE_PCM_PWR_IO_EN                   (SPM_BASE_ADDRESS + 0x2C)
#define RE_DRAMC_DPY_CLK_SW_CON_SEL        (SPM_BASE_ADDRESS + 0x460)
#define RE_DRAMC_DPY_CLK_SW_CON_SEL2       (SPM_BASE_ADDRESS + 0x470)
#define RE_SPM_S1_MODE_CH                  (SPM_BASE_ADDRESS + 0x468)
void Set_Spm_Poweron_Config_En(DRAMC_CTX_T * p)
{
    ...
    u4value = (u1IsLP4Family(p->dram_type) || (p->dram_type == TYPE_PSRAM)) ? (0x00ffffff) : (0xaaffffff);
...
void dump_gating_error_rg(DRAMC_CTX_T * p, U32 u4DramType)
{
    ...
    mcSHOW_ERR_MSG("###DDR reserve CHA Gating value[0x%X]\n", u4value);
#if 0
    if((u4DramType == 2) || (u4DramType == 3))
    {
...
#define RESERVE_PDEF_SPM_PLL_CON (SPM_BASE_ADDRESS + 0x44C)
void Dramc_DDR_Reserved_Mode_setting(void)
{
    ...
    u4DramType = Reserve_Reg_Readl(IO_PHYS + 0x0022C000 + 0x10) >> 10 & 0x7;	// ((u4IO32Read4B(DRAMC_REG_ARBCTL) >> 10) & 0x7);
...
void Psram_DDR_Reserved_Mode_setting(void)
{
#if PSRAM_SPEC
	U32 u4WbrBackup = 0;
	U32 u4value = 0;
	...
	mcSHOW_DBG_MSG("Enter Psram_DDR_Reserved_Mode_setting \n");
	DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);

	psCurrDramCtx = &DramCtx_pSRAM;		   //CFC: please check for PSRAM part

	p = psCurrDramCtx;							   //CFC: please check for PSRAM part

	Set_Spm_Poweron_Config_En(p);				   //CFC: please check for PSRAM part

	u4DramType = ((u4IO32Read4B(DRAMC_REG_ARBCTL) >> 10) & 0x7);
	mcSHOW_DBG_MSG("Psram type =%x \n", u4DramType);

//CFC: NA for PSRAM delet	 u4value = u4IO32ReadFldAlign(DRAMC_REG_SHUSTATUS, SHUSTATUS_SHUFFLE_LEVEL);
...
#endif
	u4DramType = u4IO32ReadFldAlign(DRAMC_REG_ARBCTL, ARBCTL_RSV_DRAM_TYPE);
	mcSHOW_DBG_MSG("Psram_DDR_Reserved_Mode_setting dram type =%x \n", u4DramType);

//CFC: NA for PSRAM delet	 vIO32WriteFldAlign(DDRPHY_MISC_SPM_CTRL1, 1, MISC_SPM_CTRL1_SPM_DVFS_CONTROL_SEL);//change DVFS to RG mode
	...
	mcDELAY_US(1);
//CFC: NA for PSRAM delet	 //! toggle hsu restore
//CFC: NA for PSRAM delet	 vIO32WriteFldAlign_All(DRAMC_REG_SHUCTRL2, 1, SHUCTRL2_R_SHU_RESTORE);
//CFC: NA for PSRAM delet	 mcDELAY_US(1);
//CFC: NA for PSRAM delet	 vIO32WriteFldAlign_All(DRAMC_REG_SHUCTRL2, 0, SHUCTRL2_R_SHU_RESTORE);
//CFC: NA for PSRAM delet	 mcDELAY_US(1);
	...
	//Backup regs
	DramcBackupRegisters(p, u4ReserveRegBackupAddress, sizeof(u4ReserveRegBackupAddress)/sizeof(U32));
//CFC: NA for PSRAM delet#ifdef IMPEDANCE_TRACKING_ENABLE
//CFC: NA for PSRAM delet	 if((u4DramType == 2) || (u4DramType == 3))
//CFC: NA for PSRAM delet	 {
//CFC: NA for PSRAM delet		 //Disable IMP tracking
//CFC: NA for PSRAM delet		 vIO32WriteFldAlign_All(DRAMC_REG_IMPCAL, 0, IMPCAL_IMPCAL_HW);
//CFC: NA for PSRAM delet	 }
//CFC: NA for PSRAM delet#endif
	//Disable MR4
	vIO32WriteFldAlign_All(PSRAMC_REG_HMR4, 1, HMR4_REFRDIS);
//CFC: NA for PSRAM delet	 //Disable DQSOSC en
//CFC: NA for PSRAM delet	 vIO32WriteFldAlign_All(DRAMC_REG_SHU_SCINTV, 1, SHU_SCINTV_DQSOSCENDIS);
//CFC: NA for PSRAM delet	 //Disable DQSOSC rd
//CFC: NA for PSRAM delet	 vIO32WriteFldAlign_All(DRAMC_REG_DQSOSCR, 1, DQSOSCR_DQSOSCRDIS);
//CFC: NA for PSRAM delet	 //Disable Dummy Read
	vIO32WriteFldMulti_All(PSRAMC_REG_DUMMY_RD, P_Fld(0x0, DUMMY_RD_DQSG_DMYWR_EN)
			| P_Fld(0x0, DUMMY_RD_DQSG_DMYRD_EN)
			| P_Fld(0x0, DUMMY_RD_SREF_DMYRD_EN)
			| P_Fld(0x0, DUMMY_RD_DUMMY_RD_EN)
			| P_Fld(0x0, DUMMY_RD_DMY_RD_DBG)
			| P_Fld(0x0, DUMMY_RD_DMY_WR_DBG));
	//Disable DDRPHY dynamic clock gating
	vIO32WriteFldAlign_All(PSRAMC_REG_DRAMC_PD_CTRL, 0, PSAMC_PD_CTRL_PHYCLKDYNGEN);//disable DDRPHY dynamic clock gating

//CFC: use LP4 setting	  //To 26MHz
//CFC: use LP4 setting	  if((u4DramType == 2) || (u4DramType == 3))
//CFC: use LP4 setting	  {
		vIO32WriteFldMulti_All(DDRPHY_MISC_CG_CTRL0, P_Fld(0, MISC_CG_CTRL0_W_CHG_MEM)
					| P_Fld(0, MISC_CG_CTRL0_CLK_MEM_SEL));//[5:4] mem_ck mux: 2'b00: 26MHz, [0]: change memory clock
		vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL0, 1, MISC_CG_CTRL0_W_CHG_MEM);//change clock freq
		...
		vIO32WriteFldAlign_All(DDRPHY_MISC_CG_CTRL0, 0, MISC_CG_CTRL0_W_CHG_MEM);//disable memory clock change
//CFC: use LP4 setting	  }
//CFC: use LP4 setting	  else//LP3
//CFC: use LP4 setting	  {
//CFC: use LP4 setting#if LP3_CKE_FIX_ON_TO_LEAVE_SR_WITH_LVDRAM
//CFC: use LP4 setting#if ENABLE_LP3_SW
//CFC: use LP4 setting		  //Dram clock free run
//CFC: use LP4 setting		  vIO32WriteFldAlign_All(DRAMC_REG_DRAMC_PD_CTRL, 1, DRAMC_PD_CTRL_MIOCKCTRLOFF);//disable DDRPHY dynamic clock gating
//CFC: use LP4 setting
//CFC: use LP4 setting		  //CKE control
//CFC: use LP4 setting		  u4Cha_cke_backup = u4IO32Read4B(DRAMC_REG_CKECTRL);
//CFC: use LP4 setting		  u4Chb_cke_backup = u4IO32Read4B(DRAMC_REG_CKECTRL + SHIFT_TO_CHB_ADDR);
//CFC: use LP4 setting		  //CKE fix on
//CFC: use LP4 setting		  vIO32WriteFldMulti_All(DRAMC_REG_CKECTRL, P_Fld(0x0, CKECTRL_CKE1FIXOFF) |
//CFC: use LP4 setting			  P_Fld(0x0, CKECTRL_CKEFIXOFF) |
//CFC: use LP4 setting			  P_Fld(0x1, CKECTRL_CKE1FIXON) |
//CFC: use LP4 setting			  P_Fld(0x1, CKECTRL_CKEFIXON));
//CFC: use LP4 setting#endif
//CFC: use LP4 setting#endif
		vIO32WriteFldMulti(DDRPHY_MISC_CG_CTRL0, P_Fld(0, MISC_CG_CTRL0_W_CHG_MEM)
					| P_Fld(0, MISC_CG_CTRL0_CLK_MEM_SEL));//[5:4] mem_ck mux: 2'b00: 26MHz, [0]: change memory clock
		vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0, 1, MISC_CG_CTRL0_W_CHG_MEM);//change clock freq
...
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
...
//CFC: use LP4 setting from now to file end; ie. use (u4DramType == 2) C code
	...
	//RG_*VREF_EN=1 (Vref is only used in read, DQ(B0, B1)is use for RX)
	if((u4DramType == 2) || (u4DramType == 3) || (u4DramType == 5))
	{
		vIO32WriteFldAlign_All(DDRPHY_B0_DQ5, 0x1, B0_DQ5_RG_RX_ARDQ_VREF_EN_B0);
		vIO32WriteFldAlign_All(DDRPHY_B1_DQ5, 0x1, B1_DQ5_RG_RX_ARDQ_VREF_EN_B1);
	}
	...
	vIO32WriteFldAlign_All(PSRAMC_REG_RX_SET0, 1, RX_SET0_RDATRST);//R_DMRDATRST = 1
	vIO32WriteFldAlign_All(PSRAMC_REG_RX_SET0, 0, RX_SET0_RDATRST);//R_DMRDATRST = 0
...
static void TA2_test_run_time_HW(DRAMC_CTX_T * p)//notice: Rank number and channel number should handle
{
    ...
    //vIO32WriteFldAlign_All(DRAMC_REG_TEST2_2, 0x20, TEST2_2_TEST2_OFF); //Not change, suppose this area is preserved for memeory test
...
void Psram_DDR_Reserved_Mode_AfterSR(void)
{
#if PSRAM_SPEC
	U32 u4WbrBackup = 0;
	DRAMC_CTX_T * p = psCurrDramCtx; //done in ddr reserve mode setting
	...
	DramcBroadcastOnOff(DRAMC_BROADCAST_OFF);
//CFC: NA for PSRAM delet	 mcSHOW_DBG_MSG("Dramc_DDR_Reserved_Mode_AfterSR dram type =%x \n", u4DramType);
//CFC: NA for PSRAM delet	 if((u4DramType == 1)||(u4DramType == 5))
//CFC: NA for PSRAM delet	 {	 //LPDDR3
//CFC: NA for PSRAM delet		 vIO32WriteFldAlign(DDRPHY_MISC_CG_CTRL0 + SHIFT_TO_CHB_ADDR, 1, MISC_CG_CTRL0_RG_CG_DRAMC_CHB_CK_OFF);//Close CHB controller clk
//CFC: NA for PSRAM delet
//CFC: NA for PSRAM delet		 //Since we do MR2 with (RG of Mr13) first while DVFS, we use (RG of MR13) to apply OP of MR2
//CFC: NA for PSRAM delet		 u4LP3_MR2RLWL = u4IO32ReadFldAlign(DRAMC_REG_SHU_HWSET_MR13, SHU_HWSET_MR13_HWSET_MR13_OP);
//CFC: NA for PSRAM delet		 p->channel = CHANNEL_A;
//CFC: NA for PSRAM delet		 DramcModeRegWriteByRank(p, 0, 2, u4LP3_MR2RLWL);
//CFC: NA for PSRAM delet		 DramcModeRegWriteByRank(p, 0, 17, 0);//R1 lost data WA, set R0 R1 both for safe{On charging mode, system may issue MR17=0xFF in R1}
//CFC: NA for PSRAM delet		 if(bSupport2Rank == TRUE)//DRAM is dual rank
//CFC: NA for PSRAM delet		 {
//CFC: NA for PSRAM delet			 DramcModeRegWriteByRank(p, 1, 2, u4LP3_MR2RLWL);
//CFC: NA for PSRAM delet			 DramcModeRegWriteByRank(p, 1, 17, 0);//R1 lost data WA, set R0 R1 both for safe{On charging mode, system may issue MR17=0xFF in R1}
//CFC: NA for PSRAM delet		 }
//CFC: NA for PSRAM delet	 }
//CFC: NA for PSRAM delet	 else if((u4DramType == 2) || (u4DramType == 3))
//CFC: NA for PSRAM delet	 {	 //LPDDR4
//CFC: NA for PSRAM delet		 u4LP4_MR13FSP = u4IO32ReadFldAlign(DRAMC_REG_SHU_HWSET_MR13, SHU_HWSET_MR13_HWSET_MR13_OP);
//CFC: NA for PSRAM delet		 u4LP4_MR2RLWL = u4IO32ReadFldAlign(DRAMC_REG_SHU_HWSET_MR2, SHU_HWSET_MR2_HWSET_MR2_OP);
//CFC: NA for PSRAM delet		 for(u1channel = CHANNEL_A; u1channel < p->support_channel_num; u1channel++)
//CFC: NA for PSRAM delet		 {
//CFC: NA for PSRAM delet			 p->channel = u1channel;
//CFC: NA for PSRAM delet			 DramcModeRegWriteByRank(p, 0, 13, u4LP4_MR13FSP);
//CFC: NA for PSRAM delet			 DramcModeRegWriteByRank(p, 0, 2, u4LP4_MR2RLWL);
//CFC: NA for PSRAM delet			 DramcModeRegWriteByRank(p, 0, 17, 0);//R1 lost data WA, set R0 R1 both for safe{On charging mode, system may issue MR17=0xFF in R1}
//CFC: NA for PSRAM delet			 if(bSupport2Rank == TRUE)//DRAM is dual rank
//CFC: NA for PSRAM delet			 {
//CFC: NA for PSRAM delet				 DramcModeRegWriteByRank(p, 1, 13, u4LP4_MR13FSP);
//CFC: NA for PSRAM delet				 DramcModeRegWriteByRank(p, 1, 2, u4LP4_MR2RLWL);
//CFC: NA for PSRAM delet				 DramcModeRegWriteByRank(p, 1, 17, 0);//R1 lost data WA, set R0 R1 both for safe{On charging mode, system may issue MR17=0xFF in R1}
//CFC: NA for PSRAM delet			 }
//CFC: NA for PSRAM delet		 }
//CFC: NA for PSRAM delet	 }
...
//CFC: NA for PSRAM delet	 if((u4DramType == 2) || (u4DramType == 3))
//CFC: NA for PSRAM delet	 {
//CFC: NA for PSRAM delet		 //Enable IMP tracking,
//CFC: NA for PSRAM delet		 //SPM only handle CHA IMPCAL_IMPCAL_HW, DDR2400 into DDR reserve mode(IMPCAL_IMPCAL_HW CHA = 0 CHB = 1)
//CFC: NA for PSRAM delet		 //CHB NAO have been reset to 0, need do hand shake with CHA, but CHA IMP tracking is off --> dead lock
//CFC: NA for PSRAM delet		 vIO32WriteFldAlign_All(DRAMC_REG_IMPCAL, 1, IMPCAL_IMPCAL_HW);
//CFC: NA for PSRAM delet	 }
//CFC: NA for PSRAM delet#endif
	DramcBroadcastOnOff(backup_broadcast);
	dump_gating_error_rg(p, u4DramType);
...
void failed_assert(char *file, int line, char *expr)
{
...
#define ASSERT(expr) \
    do{ if(!(expr)){failed_assert(__FILE__, __LINE__, #expr);} }while(0)
