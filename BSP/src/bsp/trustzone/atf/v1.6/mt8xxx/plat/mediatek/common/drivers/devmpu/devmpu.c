...
#else
#define DEVMPU_ERR(fmt, ...) tf_printf("[%s][ERROR] %s: " fmt, LOG_TAG, __func__, ##__VA_ARGS__)
...
#if 0
/* uncomment if DMA support is required */
static int set_invd_tbl_by_entry(uint32_t entry, uint8_t perm_in[DEVMPU_ENT_SIZE], bool is_write)
{
	int i;
	uint32_t invd_perm;

	uintptr_t reg_invd_sram_addr = 0;
	uintptr_t reg_invd_sram_data = 0;
	...
	reg_invd_sram_addr = (is_write) ? DEVMPU_WR_INVD_SRAM_ADDR : DEVMPU_RD_INVD_SRAM_ADDR;
	reg_invd_sram_data = (is_write) ? DEVMPU_WR_INVD_SRAM_DATA : DEVMPU_RD_INVD_SRAM_DATA;
	...
	reg_write32(reg_invd_sram_addr, entry);

	/* 4 bytes per write to update 128-bit invalidate control */
	for (i = 0; i < DEVMPU_ENT_SIZE / sizeof(uint32_t); ++i) {
		invd_perm = *((uint32_t *)perm_in + i);
		reg_write32(reg_invd_sram_data, invd_perm);
	}
...
#if DEVMPU_DEBUG
	{
		...
		/* read the invalidate table entry just updated */
		if (get_invd_tbl_by_entry(entry, perm_cmp, is_write)) {
			DEVMPU_ERR("failed to get %s invalidate table entry %u\n",
					(is_write) ? "write" : "read", entry);
			return -1;
		}
		...
		for (i = 0; i < DEVMPU_ENT_SIZE; ++i) {
			if (perm_cmp[i] != perm_in[i]) {
				DEVMPU_ERR("inconsistent %s invalidate in entry[%u][%d]=0x%x, expected 0x%x\n",
						(is_write) ? "write" : "read",
						entry,
						i,
						perm_cmp[i],
		...
		DEVMPU_DBG("consistency check for invalidate entry[%u]: PASSED\n", entry);
	}
#endif
