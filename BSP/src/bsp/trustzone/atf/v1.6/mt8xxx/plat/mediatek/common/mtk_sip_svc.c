...
#include <assert.h>
#include <console.h>
#include <bl_common.h>	/* for type entry_point_info_t */
	...
	if (!ns) {
		...
		if (mtk_lk_stage == 1) {
			if (((el != MODE_EL2) && (GET_RW(spsr) == MODE_RW_64)) ||
				...
				case MTK_SIP_KERNEL_BOOT_AARCH64:
				...
				case MTK_SIP_TINYSYS_BOOT_AARCH32:
					boot_to_tinysys(x1, x2, x3, x4);
					SMC_RET0(handle);
					break;
			...
			if (((el != MODE_EL2) && (GET_RW(spsr) == MODE_RW_64)) ||
				...
				case MTK_SIP_KERNEL_WDT_AARCH64:
					wdt_kernel_cb_addr = x1;
					NOTICE("MTK_SIP_KERNEL_WDT : 0x%llx\n", wdt_kernel_cb_addr);
				...
				{
					uint64_t atf_ns = sched_clock();
					uint64_t delta_ns = 0;
					uint64_t in_ns;

					INFO("kernel time sync 0x%16lx 0x%16lx atf: 0x%16llx\n", x1, x2, atf_ns);
					/* in arch32, high 32 bits is stored in x2 and this would be 0 in arch64 */
					in_ns = x1;
					if (atf_ns >= in_ns)
						delta_ns = atf_ns - in_ns;
					sched_clock_init(delta_ns, 0);
					MT_LOG_KTIME_SET();
