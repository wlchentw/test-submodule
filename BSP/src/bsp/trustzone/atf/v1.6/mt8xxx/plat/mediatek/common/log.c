...
int mt_log_write(unsigned char c)
{
	...
	*(unsigned char *)(uintptr_t)(p_atf_log_ctrl->info.atf_buf_addr +
	...
	if (p_atf_log_ctrl->info.atf_write_pos < p_atf_log_ctrl->info.atf_buf_size) {
		...
		if (p_atf_log_ctrl->info.atf_write_pos ==
...
{
	/* p_atf_log_ctrl->info.atf_buf_addr = start + ATF_LOG_CTRL_BUF_SIZE; */
	*(unsigned char *)(uintptr_t)(p_atf_log_ctrl->info.atf_crash_log_addr +
		p_atf_log_ctrl->info.atf_crash_write_pos) = c;
	...
	if (p_atf_log_ctrl->info.atf_crash_write_pos <
		p_atf_log_ctrl->info.atf_crash_log_size)
		p_atf_log_ctrl->info.atf_crash_write_pos++;
	else
		p_atf_log_ctrl->info.atf_crash_write_pos = 0;
	return 0;
}
...
static void show_atf_wdt_footprint(void)
{
	...
	for (cpu = 0; cpu < PLATFORM_CORE_COUNT; cpu++) {
		...
		((p_atf_log_ctrl->info.irq_num[cpu] >>
		...
		NOTICE("enter(%llu)",
			p_atf_log_ctrl->info.fiq_irq_enter_timestamp[cpu]);
...
static int mt_get_unread_log_size(void)
{
...
void mt_log_setup(unsigned int start, unsigned int size,
	unsigned int aee_buf_size, unsigned int is_abnormal_boot)
{
	...
	ptr_except_write_pos_per_cpu =
		(unsigned long *) &p_atf_log_ctrl->info.except_write_pos_per_cpu[0];
	...
	for (i = 0; i < PLATFORM_CORE_COUNT; i++) {
		p_atf_log_ctrl->info.except_write_pos_per_cpu[i] =
...
void atf_enter_footprint(unsigned int irq)
{
	...
	p_atf_log_ctrl->info.irq_num[cpu] = irq;
...
void atf_exit_footprint(void)
{
	...
	if ((p_atf_log_ctrl->info.irq_num[cpu] & IAR0_EL1_INTID_MASK)
			== WDT_IRQ_BIT_ID ||
		(p_atf_log_ctrl->info.irq_num[cpu] & IAR0_EL1_INTID_MASK)
			== FIQ_SMP_CALL_SGI) {
