...
static unsigned long dfd_reserved_base_addr = 0x50000000;
static unsigned long dfd_reserved_chain_length = 0x32000;
...
int dfd_smc_dispatcher(unsigned long arg0, unsigned long arg1, unsigned long arg2)
{
	...
	case PLAT_MTK_DFD_SETUP_MAGIC:
		/* only allow access to DFD base addr ~ base addr + length */
		if ((arg1 >= dfd_reserved_base_addr)
		&& (arg1 <= (dfd_reserved_base_addr+dfd_reserved_chain_length))
		&& (arg2 > 0)
		&& (arg2 <= dfd_reserved_chain_length)
		&& ((arg1+arg2) <= (dfd_reserved_base_addr+dfd_reserved_chain_length)))
			dfd_setup(arg1, arg2);
		else
			ret = 0;
