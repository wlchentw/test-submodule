...
#define SYSTEM_BOOTING_120S	(0x173EED80)//(0x5CFBB600) /* 0x5CFBB600 / 13M = 120s */
...
const char *wakeup_src_str[32] = {
	...
	[27] = "R12_SEJ",
	[28] = "R12_MD32_WAKEUP_MD32",
	[29] = "R12_CPU_IRQOUTS_AND",
...
static uint32_t block[NR_GRPS];
static uint32_t idle_masks[NR_GRPS];
...
static const char *grp_get_name(int id)
{
	static const char * const grp_name[] = {
		...
		"0x100010D8", /*infra4*/
		"0x14000100", /*MMSYS0*/
		"0x10006374", /*MMSYS_OTH*/
		"0x15000100", /*IMAGE0*/
		"0x15000110", /*IMAGE1*/
		"0x1000638C", /*IMAGE_OTH*/
	};

	return grp_name[id];
}

static void spm_dump_clock_masks(void)
{
	int i;

	for (i = 0; i < NR_GRPS; i++)
		INFO("[%02d]condition_mask[%s]=0x%08x\t\tblock_mask[%s]=0x%08x\n",
			i, grp_get_name(i), idle_masks[i],grp_get_name(i), block[i]);
}


static void loop_spm(uint32_t t)
{
...
int __spm_is_idle_blocked_by_clk(struct pwr_ctrl *pwrctrl)
{
	...
	idle_masks[CG_MMSYS_OTH] = pwrctrl->mmsys_oth_idle_mask;
	idle_masks[CG_IMAGE0] = pwrctrl->image0_idle_mask;
	idle_masks[CG_IMAGE1] = pwrctrl->image1_idle_mask;
	idle_masks[CG_IMAGE_OTH] = pwrctrl->image_oth_idle_mask;
	...
	clks[CG_INFRA4] = ~spm_get_infra4_sta();

	if (mmio_read_32(PWR_STATUS) & PWR_STA_DISP) {
		clks[CG_MMSYS0] = ~spm_get_mmsys_con0();
		clks[CG_MMSYS_OTH] = ~spm_get_mmsys_oth();
	}

	if (mmio_read_32(PWR_STATUS) & PWR_STA_IMG) {
		clks[CG_IMAGE0] = ~spm_get_image_con0();
		clks[CG_IMAGE1] = ~spm_get_image_con1();
		clks[CG_IMAGE_OTH] = ~spm_get_image_oth();
	}
	for (i = 0; i < NR_GRPS; i++) {
		block[i] = clks[i] & idle_masks[i];
		r |= block[i];
	}
					  ...
					  const struct pcm_desc *pcmdesc)
{
	...
	INFO("r12 = 0x%x, r12_ext = 0x%x, raw_sta = 0x%x, idle_sta = 0x%x\n",
	     ...
	     wakesta->idle_sta);
	INFO("req_sta = 0x%x, event_reg = 0x%x, isr = 0x%x\n",
...
void __spm_set_pcm_flags(struct pwr_ctrl *pwrctrl)
{
	...
	if (pwrctrl->pcm_flags2_cust_clr != 0)
	    pwrctrl->pcm_flags2 &= ~pwrctrl->pcm_flags2_cust_clr;
    if (pwrctrl->pcm_flags2_cust_set != 0)
	    pwrctrl->pcm_flags2 |= pwrctrl->pcm_flags2_cust_set;

	mmio_write_32(SPM_SW_FLAG, pwrctrl->pcm_flags);
	mmio_write_32(SPM_SW_RSV_2, pwrctrl->pcm_flags1);
	mmio_write_32(SPM_SW_RSV_1, pwrctrl->pcm_flags2);
}
...
uint32_t __spm_get_xo_type(void)
{
	uint32_t xo_type;

		/*if TCXO: CLKSQ in, else XTAL in*/
	if (mmio_read_32(0x1000C33C) >> 31)
		xo_type = TYPE_TCXO;
	else
		xo_type = TYPE_XTAL;
...
void __spm_set_xo_type_ctrl(struct pwr_ctrl *pwrctrl)
{
	uint32_t xo_type;

	xo_type = __spm_get_xo_type();

	if (xo_type == TYPE_XTAL) {
		INFO("%s: xo type is XTAL\n", __func__);
		mmio_write_32(0x1000c000, mmio_read_32(0x1000c000) | 0x1 << 7);
		pwrctrl->pcm_flags2 |= SPM_FLAG2_CLKSRC_XTAL;
	}
}

void __spm_cmd_check(uint32_t cmd)
{
	spm_write(SPM_SW_RSV_6, cmd);
}
