...
void bl31_prepare_tinysys_entry(uint64_t pc,
			uint64_t r0,
			uint64_t r1,
			uint64_t k32_64)
{

	static entry_point_info_t tinysys_image_info;
	uint32_t image_type;

	/* Determine which image to execute next */
	image_type = NON_SECURE; /* bl31_get_next_image_type(); */

	SET_PARAM_HEAD(&tinysys_image_info,
				PARAM_EP,
				VERSION_1,
				0);

	SET_SECURITY_STATE(tinysys_image_info.h.attr, NON_SECURE);

	tinysys_image_info.pc = pc;
	tinysys_image_info.args.arg0 = r0;
	tinysys_image_info.args.arg1 = r1;

	if (k32_64 == 0)
		tinysys_image_info.spsr = SPSR_MODE32(MODE32_svc, SPSR_T_ARM, SPSR_E_LITTLE,
							(DAIF_FIQ_BIT | DAIF_IRQ_BIT | DAIF_ABT_BIT));
	else
		tinysys_image_info.spsr = SPSR_64(MODE_EL2, MODE_SP_ELX, DISABLE_ALL_EXCEPTIONS);

	INFO("BL3-1: Preparing for EL3 exit to %s world, Kernel\n",
		(image_type == SECURE) ? "secure" : "normal");
	INFO("BL3-1: Next image address = 0x%llx\n",
		(unsigned long long) tinysys_image_info.pc);
	INFO("BL3-1: Next image spsr = 0x%x\n", tinysys_image_info.spsr);
	cm_init_my_context(&tinysys_image_info);
	cm_prepare_el3_exit(image_type);
}

void boot_to_tinysys(uint64_t x1, uint64_t x2, uint64_t x3, uint64_t x4)
{
	console_init(gteearg.atf_log_port, UART_CLOCK, UART_BAUDRATE);

	INFO("save tinysys info\n");
	bl31_prepare_tinysys_entry(x1, x2, x3, x4);
	INFO("el3_exit\n");
	console_uninit();
}
...
void configure_el2_plat(struct boot_tag_gz_platform *gz_plat)
{
	...
	memcpy(&gz_exec_start_offset, &gz_plat->exec_start_offset,
		sizeof(uint64_t));
	INFO("GZ EXEC START OFFSET = 0x%llx\n", gz_exec_start_offset);
}
#endif
...
{
#if defined(HAVE_MTK_BL33)
	uint32_t spsr;
	INFO("Booting from EL2 for bl33!\n");
...
#ifdef MTK_FPGA_LDVT
	boot_to_kernel(KERNEL_ENTRYPOINT, 0, KERNEL_DTB_ADDR, LINUX_KERNEL_32);
