	...
	if (mtk_lk_stage == 1) {
		if (GET_RW(spsr) == MODE_RW_64) {
			...
			case MTK_SIP_KERNEL_EMIMPU_SET_AARCH64:
				rc = sip_emi_mpu_set_region_protection_domain8(x1, x2, x3, x4);
				break;
		...
		if (GET_RW(spsr) == MODE_RW_64) {
			...
			case MTK_SIP_KERNEL_SPM_SODI_ARGS_AARCH64:
				break;
			case MTK_SIP_KERNEL_SPM_MCDI_ARGS_AARCH64:
				spm_mcdi_args(x1, x2, x3);
				break;
			...
			case MTK_SIP_KERNEL_SPM_SODI_ARGS_AARCH32:
				break;
			case MTK_SIP_KERNEL_SPM_MCDI_ARGS_AARCH32:
				spm_mcdi_args(x1, x2, x3);
				break;
