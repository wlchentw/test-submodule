...
#if defined MICRO_PLATFORM_VERSION_MAJOR && MICRO_PLATFORM_VERSION_MAJOR == 6
unsigned int android_version = 6;
#elif defined MICRO_PLATFORM_VERSION_MAJOR && MICRO_PLATFORM_VERSION_MAJOR == 7
unsigned int android_version = 7;
#elif defined MICRO_PLATFORM_VERSION_MAJOR && MICRO_PLATFORM_VERSION_MAJOR == 8
unsigned int android_version = 8;
#elif defined MICRO_PLATFORM_VERSION_MAJOR && MICRO_PLATFORM_VERSION_MAJOR == 9
unsigned int android_version = 9;
#endif
...
static uintptr_t teei_fastcall_handler(uint32_t smc_fid,
							u_register_t x1,
							u_register_t x2,
							u_register_t x3,
							u_register_t x4,
							...
							u_register_t flags)
{
	...
	if (caller_security_state == SECURE) {
		...
		UART_INIT;
#if SMC_STRING_LOG
		DBG_PRINTF("ft, t to nt id[%s] cid[%d]\n\r",
				teei_smc_call_id_to_string(maskSWdRegister(smc_fid)),
				linear_id);
#else
		DBG_PRINTF("ft, t to nt id[%x] cid[%d]\n\r",
		...
		assert(handle == CM_GET_CONTEXT(SECURE));
			...
			case T_GET_RAND_SEED: {
				x1 = teei_rng();
#if MICROTRUST_LOG
				tf_printf("T_GET_RAND_SEED rand [%lx]\n\r", x1);
#endif
		...
		UART_INIT;
#if SMC_STRING_LOG
		DBG_PRINTF("ft,nt to t,id[%s]cid[%d]\n\r",
				teei_smc_call_id_to_string(smc_fid), linear_id);
#else
		DBG_PRINTF("ft,nt to t,id[%x]cid[%d]\n\r", smc_fid, linear_id);
#endif
		UART_UNINIT;
#endif
...
#if MICROTRUST_LOG
				UART_INIT;
				DBG_PRINTF("N_GET_SE_OS_STATE [0x%lx]\n\r",
						secure_os_state);
				UART_UNINIT;
#endif
...
static uintptr_t teei_standard_call_handler(uint32_t smc_fid,
								u_register_t x1,
								u_register_t x2,
								u_register_t x3,
								u_register_t x4,
								...
								u_register_t flags)
{
	...
	if (caller_security_state == SECURE) {
		...
		UART_INIT;
#if SMC_STRING_LOG
		DBG_PRINTF("st, t to nt id[%s]cid[%d]\n\r",
				teei_smc_call_id_to_string(maskSWdRegister(smc_fid)),
				linear_id);
#else
		DBG_PRINTF("st, t to nt id[%x]cid[%d]\n\r",
				maskSWdRegister(smc_fid),
				linear_id);
#endif
		UART_UNINIT;
		...
		assert(handle == CM_GET_CONTEXT(SECURE));
			...
			case T_NOTIFY_N_ERR:	{
				    ...
				    secure_os_state = x1;
				    teei_trigger_soft_intr(NSEC_CALL_INTR);
				    DBG_PRINTF("=======  Secure world error 0x%x =======\n\r",
						(uint32_t)secure_os_state);
				    teei_synchronous_sp_exit(teei_ctx, SMC_CALL_DONE, 1);
			break;
			}
		...
		UART_INIT;
#if SMC_STRING_LOG
		DBG_PRINTF("st,nt to t, id [%s]cid[%d]\n\r",
				teei_smc_call_id_to_string(smc_fid), linear_id);
#else
		DBG_PRINTF("st,nt to t, id [%x]cid[%d]\n\r",
				smc_fid, linear_id);
#endif
		UART_UNINIT;
