...
#define GET_START_CANARY(name, stack_num) *((stack_num!=0)?&name[stack_num][0]:&stack_tmp_0[0][0])
...
/* add extra buffer for stack_tmp of core0 to run complex init flow */
#define DECLARE_STACK_TMP(name, stack_num, stack_size, linkage) \
linkage uint32_t name[stack_num] \
		[ROUNDUP(stack_size + STACK_CANARY_SIZE, STACK_ALIGNMENT) / \
		sizeof (uint32_t)] \
		__attribute__((section(".nozi_stack."#name), \
			       aligned(STACK_ALIGNMENT)))

/* add the stack_tmp_0 in front of stack_tmp to provide more stack buf */
#define STACK_TMP_0_SIZE_ADDED 2048
DECLARE_STACK_TMP(stack_tmp_0, 1, STACK_TMP_0_SIZE_ADDED, static);
DECLARE_STACK_TMP(stack_tmp, CFG_TEE_CORE_NB_CORE, STACK_TMP_SIZE, static);
