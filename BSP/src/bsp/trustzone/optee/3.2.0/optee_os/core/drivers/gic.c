...
#if defined(CFG_ARM_GICV3_USE_G1S)
	/* Assign it to group1s */
	write32(read32(gd->gicd_base + GICD_IGRPMODR(idx)) | mask,
			gd->gicd_base + GICD_IGRPMODR(idx));
#endif
}
...
#if defined(CFG_ARM_GICV3_USE_G1S)
	/* Assigned to group1s */
	assert((read32(gd->gicd_base + GICD_IGRPMODR(idx)) & mask));
#endif
...
#if defined(CFG_SYSIRQ)
	uint32_t reg_index, offset, value;
	vaddr_t int_pol_ctl_base;
#endif
...
#if defined(CFG_SYSIRQ)
	if (it > 32) {
		int_pol_ctl_base = (vaddr_t)SYSIRQ_BASE;
		reg_index = (it - 32) / 32;
		offset = it & 0x1f;
		value = read32(int_pol_ctl_base + reg_index * 4);

		if (flags == IRQF_TRIGGER_LOW || flags == IRQF_TRIGGER_FALLING) {
		...
		write32(value, int_pol_ctl_base + reg_index * 4);
	}
#endif
}
