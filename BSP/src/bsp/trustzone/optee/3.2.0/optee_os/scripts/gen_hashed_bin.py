...
def write_header_v1(outf, init_size, args, paged_size):
	...
	outf.write(struct.pack('<IBBHIIIII', \
		int(magic), int(version), int(arch_id[args.arch]), int(args.flags), int(init_size), \
		int(args.init_load_addr_hi), int(args.init_load_addr_lo), \
		int(args.init_mem_usage), int(paged_size)))
...
def write_header_v2(outf, init_size, args, paged_size):
	...
	outf.write(struct.pack('<IIII', \
		int(args.init_load_addr_hi), int(args.init_load_addr_lo), \
		int(image_id['pager']), int(init_size)))
	if nb_images == 2:
		outf.write(struct.pack('<IIII', \
